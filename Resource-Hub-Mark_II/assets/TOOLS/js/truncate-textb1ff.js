window.bridges["truncate-text"] = function(){
    ...
    var bridge = function (text) {
        ...
        if (postfix.len > opts.textLen) {
            error("Invalid Truncation Length", "Postfix/prefix length is greater than truncation length.");
            return "";
        }
    ...
    function truncateTextByBytes(text, opts, postfix) {
        ...
        if (opts.direction == "right") {
            ...
            var bytesToUtf8  = truncated.slice(0, truncated.length - numberOfExtraBytes);
    ...
    function findNumberOfFirstExtraBytes(bytes, count) {
        ...
        if ((byteToCheck & 0xf0) == 0xf0 || (byteToCheck & 0xe0) == 0xe0 || (byteToCheck & 0xc0) == 0xc0) {
            return count;
        }
        else if ((byteToCheck & 0x80) == 0x80) { // byte in format "10xxxxxx"
    ...
    function findNumberOfLastExtraBytes(bytes, count) {
        ...
        if ((byteToCheck & 0xf0) == 0xf0) { // byte in format "11110xxx"
            if (count > 2) return 0;
            else return count + 1;
        }
        else if ((byteToCheck & 0xe0) == 0xe0) { // byte in format "1110xxxx"
            if (count > 1) return 0;
            else return count + 1;
        }
        else if ((byteToCheck & 0xc0) == 0xc0) { // byte in format "110xxxxx"
            if (count > 0) return 0;
            else return count + 1;
        }
        else if ((byteToCheck & 0x80) == 0x80) { // byte in format "10xxxxxx"
    ...
    function parseOptions(tool, error) {
        ...
        if (truncateBySize) {
            ...
            if (!/^[+-]?\d+$/.test(textLen)) {
                error("Invalid Truncation Length", "Length value isn't a valid number.");
                return false;
            }
