.
├── _Algorithms
│   ├── alphabeticalattributes.py
│   ├── array
│   │   ├── 215 Kth Largest Element in an Array.py
│   │   ├── array_sum_combinations.py
│   │   ├── compare_array_elements.py
│   │   ├── continuous-subarray-sum.py
│   │   ├── degree-of-an-array.py
│   │   ├── dynamic_arrays.py
│   │   ├── k-diff-pairs-in-an-array.py
│   │   ├── largest_subarray_sum.py
│   │   ├── longest_sub_array.py
│   │   ├── max_product_subarray.py
│   │   ├── max_sub_array.py
│   │   ├── max_subarray.py
│   │   ├── median_of_two_arrays.py
│   │   └── sum_array.py
│   ├── Check
│   │   ├── check_anagrams.py
│   │   ├── check_bipartite.py
│   │   ├── check_pangram.py
│   │   ├── merge_string_checker.py
│   │   └── prime_check.py
│   ├── max_by_alphabetical_order.py
│   ├── min_by_alphabetical_order.py
│   ├── Search
│   │   ├── add_and_search.py
│   │   ├── add_search_word_data_structures_trie.py
│   │   ├── bidirectional_breadth_first_search.py
│   │   ├── boyer_moore_search.py
│   │   ├── breadth_first_search_2.py
│   │   ├── breadth_first_search.py
│   │   ├── depth_first_search_2.py
│   │   ├── depth_first_search.py
│   │   ├── double_linear_search.py
│   │   ├── double_linear_search_recursion.py
│   │   ├── fibonacci_search.py
│   │   ├── interpolation_search.py
│   │   ├── jump_search.py
│   │   ├── linear_search.py
│   │   ├── maze_search.py
│   │   ├── naive_string_search.py
│   │   ├── optimal_binary_search_tree.py
│   │   ├── ordered_binary_search.py
│   │   ├── search_insert.py
│   │   ├── search_range.py
│   │   ├── search_rotate.py
│   │   ├── search_scope.py
│   │   ├── sentinel_linear_search.py
│   │   ├── similarity_search.py
│   │   ├── simple_binary_search.py
│   │   ├── tabu_search.py
│   │   └── ternary_search.py
│   └── Sort
│       ├── bitonic_sort.py
│       ├── bogo_sort.py
│       ├── bubble_sort.py
│       ├── bucket_sort.py
│       ├── cocktail_shaker_sort.py
│       ├── comb_sort.py
│       ├── counting_sort.py
│       ├── cycle_sort.py
│       ├── double_sort.py
│       ├── external_sort.py
│       ├── gnome_sort.py
│       ├── g_topological_sort.py
│       ├── heap_sort.py
│       ├── insertion_sort.py
│       ├── intro_sort.py
│       ├── is_sorted.py
│       ├── iterative_merge_sort.py
│       ├── kay_sort.py
│       ├── merge_insertion_sort.py
│       ├── merge_sorted_k_lists.py
│       ├── merge_sort.py
│       ├── modified_selection_sort.py
│       ├── natural_sort.py
│       ├── pancake_sort.py
│       ├── patience_sort.py
│       ├── pigeonhole_sort.py
│       ├── pigeon_sort.py
│       ├── pylama_isort.py
│       ├── quick_sort_3_partition.py
│       ├── quick_sort.py
│       ├── radix_sort.py
│       ├── random_pivot_quick_sort.py
│       ├── recursive_bubble_sort.py
│       ├── recursive_insertion_sort.py
│       ├── recursive_quick_sort.py
│       ├── reverse_sort.py
│       ├── searching_in_sorted_matrix.py
│       ├── search_in_sorted_matrix.py
│       ├── selection_sort.py
│       ├── shell_sort.py
│       ├── sort_colors.py
│       ├── sortingFunctions.py
│       ├── sorting.py
│       ├── sort_matrix_diagonally.py
│       ├── sort.py
│       ├── sortString.py
│       ├── stooge_sort.py
│       ├── strand_sort.py
│       ├── tim_sort.py
│       ├── topological_sort.py
│       ├── top_sort.py
│       ├── tree_sort.py
│       ├── unknown_sort.py
│       └── wiggle_sort.py
├── _data-base
│   ├── create_db.py
│   ├── createdb.py
│   ├── database.py
│   ├── db.py
│   ├── get_imdb_top_250_movies_csv.py
│   ├── get_imdbtop.py
│   ├── lldb_prepare.py
│   ├── models
│   │   └── models.py
│   ├── routes
│   │   ├── api_routes.py
│   │   ├── auth_routes.py
│   │   ├── car_routes.py
│   │   ├── cuisine_routes.py
│   │   ├── game_routes.py
│   │   ├── gas_station_routes.py
│   │   ├── generate_routes.py
│   │   ├── hotel_routes.py
│   │   ├── notebook_routes.py
│   │   ├── note_routes.py
│   │   ├── note_tag_routes.py
│   │   ├── restaurant_routes.py
│   │   ├── routes.py
│   │   ├── stop_routes.py
│   │   ├── tag_routes.py
│   │   ├── trip_routes.py
│   │   └── user_routes.py
│   ├── sandbox.py
│   └── sdbm.py
├── _Data-Structures
│   ├── binary-tree
│   │   ├── average-of-levels-in-binary-tree.py
│   │   ├── basic_binary_tree.py
│   │   ├── binary_search.py
│   │   ├── binary_search_tree.py
│   │   ├── binary_search_tree_recursive.py
│   │   ├── binary_tree_mirror.py
│   │   ├── binary_tree_paths.py
│   │   ├── binary-tree-tilt.py
│   │   └── binary_tree_traversals.py
│   ├── bst
│   │   ├── array_to_bst.py
│   │   ├── bst_alternate_starter.py
│   │   ├── bst_closest_value.py
│   │   ├── BSTIterator.py
│   │   ├── bst.py
│   │   ├── convert-bst-to-greater-tree.py
│   │   ├── count_binary_substring.py
│   │   ├── is_bst.py
│   │   ├── longest_palindromic_substring.py
│   │   ├── minimum-absolute-difference-in-bst.py
│   │   ├── repeat_substring.py
│   │   └── unique_bst.py
│   ├── graph
│   │   ├── basic_graphs.py
│   │   ├── check_bipartite_graph_bfs.py
│   │   ├── check_bipartite_graph_dfs.py
│   │   ├── check_digraph_strongly_connected.py
│   │   ├── clone_graph.py
│   │   ├── directed_and_undirected_(weighted)_graph.py
│   │   ├── eulerian_path_and_circuit_for_undirected_graph.py
│   │   ├── frequent_pattern_graph_miner.py
│   │   ├── gale_shapley_bigraph.py
│   │   ├── graph_list.py
│   │   ├── graph_matrix.py
│   │   ├── graph.py
│   │   ├── graphs_floyd_warshall.py
│   │   ├── kth_lexicographic_permutation.py
│   │   └── path_between_two_vertices_in_digraph.py
│   ├── queue
│   │   ├── circular_queue.py
│   │   ├── double_ended_queue.py
│   │   ├── linked_queue.py
│   │   ├── priority_queue.py
│   │   ├── priority_queue_using_list.py
│   │   ├── queue_on_list.py
│   │   ├── queue.py
│   │   └── reconstruct_queue.py
│   └── stack
│       ├── dijkstras_two_stack_algorithm.py
│       ├── linked_stack.py
│       ├── ordered_stack.py
│       ├── pydevd_stackless.py
│       ├── queue_on_pseudo_stack.py
│       ├── stack.py
│       └── stack_using_dll.py
├── _flask
│   ├── flask.py
│   └── hello-flask.py
├── JUPYTER_NOTEBOOKS
│   ├── 00.00-Preface.ipynb
│   ├── 00-Collections-Module-checkpoint.ipynb
│   ├── 00-Collections-Module.ipynb
│   ├── 00-Guide-to-Web-Scraping-checkpoint.ipynb
│   ├── 00-Guide-to-Web-Scraping.ipynb
│   ├── 00-Milestone-2-Warmup-Project-checkpoint.ipynb
│   ├── 00-Milestone-2-Warmup-Project.ipynb
│   ├── 00-Overview-of-Sending-Emails-checkpoint.ipynb
│   ├── 00-Overview-of-Sending-Emails.ipynb
│   ├── 00-Overview-of-Working-with-Images-checkpoint.ipynb
│   ├── 00-Overview-of-Working-with-Images.ipynb
│   ├── 00-Warm-Up-Project-Exercises-checkpoint.ipynb
│   ├── 00-Warm-Up-Project-Exercises.ipynb
│   ├── 00-Working-with-CSV-Files-checkpoint.ipynb
│   ├── 00-Working-with-CSV-Files.ipynb
│   ├── 01.00-Ipython-Beyond-Normal-python.ipynb
│   ├── 01.01-Help-And-Documentation.ipynb
│   ├── 01.02-Shell-Keyboard-Shortcuts.ipynb
│   ├── 01.03-Magic-Commands.ipynb
│   ├── 01.04-Input-Output-History.ipynb
│   ├── 01.05-Ipython-And-Shell-Commands.ipynb
│   ├── 01.06-Errors-and-Debugging.ipynb
│   ├── 01.07-Timing-and-Profiling.ipynb
│   ├── 01.08-More-Ipython-Resources.ipynb
│   ├── 01-Advanced Numbers-checkpoint.ipynb
│   ├── 01-Advanced Numbers.ipynb
│   ├── 01-Comparison Operators-checkpoint.ipynb
│   ├── 01-Comparison Operators.ipynb
│   ├── 01-Datetime-Module-checkpoint.ipynb
│   ├── 01-Decorators-checkpoint.ipynb
│   ├── 01-Decorators.ipynb
│   ├── 01-Errors and Exceptions Handling-checkpoint.ipynb
│   ├── 01-Errors and Exceptions Handling.ipynb
│   ├── 01-Final Capstone Project-checkpoint.ipynb
│   ├── 01-Final Capstone Project.ipynb
│   ├── 01-Image-Exercise-checkpoint.ipynb
│   ├── 01-Image-Exercise.ipynb
│   ├── 01-Interact-checkpoint.ipynb
│   ├── 01-Interact.ipynb
│   ├── 01_Introduction.ipynb
│   ├── 01-Introduction to python Statements-checkpoint.ipynb
│   ├── 01-Introduction to python Statements.ipynb
│   ├── 01-Iterators and Generators-checkpoint.ipynb
│   ├── 01-Iterators and Generators.ipynb
│   ├── 01-Map-checkpoint.ipynb
│   ├── 01-Map.ipynb
│   ├── 01-Methods-checkpoint.ipynb
│   ├── 01-Methods.ipynb
│   ├── 01-Milestone Project 1 - Assignment-checkpoint.ipynb
│   ├── 01-Milestone Project 1 - Assignment.ipynb
│   ├── 01-Milestone Project 2 - Assignment-checkpoint.ipynb
│   ├── 01-Milestone Project 2 - Assignment.ipynb
│   ├── 01-Modules and Packages-checkpoint.ipynb
│   ├── 01-Numbers-checkpoint.ipynb
│   ├── 01-Numbers.ipynb
│   ├── 01-Object Oriented Programming-checkpoint.ipynb
│   ├── 01-Object Oriented Programming-Copy1-checkpoint.ipynb
│   ├── 01-Object Oriented Programming.ipynb
│   ├── 01-Opening-and-Reading-Files-Folders-checkpoint.ipynb
│   ├── 01-Opening-and-Reading-Files-Folders.ipynb
│   ├── 01-Overview-of-Received-Emails-checkpoint.ipynb
│   ├── 01-Overview-of-Received-Emails.ipynb
│   ├── 01-Variable Assignment-checkpoint.ipynb
│   ├── 01-Variable Assignment.ipynb
│   ├── 01-Web-Scraping-Exercises-checkpoint.ipynb
│   ├── 01-Web-Scraping-Exercises.ipynb
│   ├── 01-Working-with-PDFs-checkpoint.ipynb
│   ├── 01-Working-with-PDFs.ipynb
│   ├── 02.00-Introduction-to-NumPy.ipynb
│   ├── 02.01-Understanding-Data-Types.ipynb
│   ├── 02.02-The-Basics-Of-NumPy-Arrays.ipynb
│   ├── 02.03-Computation-on-arrays-ufuncs.ipynb
│   ├── 02.04-Computation-on-arrays-aggregates.ipynb
│   ├── 02.05-Computation-on-arrays-broadcasting.ipynb
│   ├── 02.06-Boolean-Arrays-and-Masks.ipynb
│   ├── 02.07-Fancy-Indexing.ipynb
│   ├── 02.08-Sorting.ipynb
│   ├── 02.09-Structured-Data-NumPy.ipynb
│   ├── 02-Advanced Strings-checkpoint.ipynb
│   ├── 02-Advanced Strings.ipynb
│   ├── 02_Basicpython.ipynb
│   ├── 02-Chained Comparison Operators-checkpoint.ipynb
│   ├── 02-Chained Comparison Operators.ipynb
│   ├── 02-Datetime-Module-checkpoint.ipynb
│   ├── 02-Datetime-Module.ipynb
│   ├── 02-Decorators Homework-checkpoint.ipynb
│   ├── 02-Decorators Homework.ipynb
│   ├── 02-Errors and Exceptions Homework-checkpoint.ipynb
│   ├── 02-Errors and Exceptions Homework.ipynb
│   ├── 02-Exercise-Ideas-checkpoint.ipynb
│   ├── 02-Exercise-Ideas.ipynb
│   ├── 02-Final Capstone Project Ideas-checkpoint.ipynb
│   ├── 02-Final Capstone Project Ideas.ipynb
│   ├── 02-Functions-checkpoint.ipynb
│   ├── 02-Functions.ipynb
│   ├── 02-if, elif, and else Statements-checkpoint.ipynb
│   ├── 02-if, elif, and else Statements.ipynb
│   ├── 02-Image-Exercise-Solution-checkpoint.ipynb
│   ├── 02-Image-Exercise-Solution.ipynb
│   ├── 02-Iterators and Generators Homework-checkpoint.ipynb
│   ├── 02-Iterators and Generators Homework.ipynb
│   ├── 02-Math-and-Random-Module-checkpoint.ipynb
│   ├── 02-Milestone Project 1 - Walkthrough Steps Workbook-checkpoint.ipynb
│   ├── 02-Milestone Project 1 - Walkthrough Steps Workbook.ipynb
│   ├── 02-Milestone Project 2 - Walkthrough Steps Workbook-checkpoint.ipynb
│   ├── 02-Milestone Project 2 - Walkthrough Steps Workbook.ipynb
│   ├── 02-Object Oriented Programming Homework-checkpoint.ipynb
│   ├── 02-Object Oriented Programming Homework.ipynb
│   ├── 02-PDFs-Spreadsheets-Puzzle-checkpoint.ipynb
│   ├── 02-PDFs-Spreadsheets-Puzzle.ipynb
│   ├── 02-Reduce-checkpoint.ipynb
│   ├── 02-Reduce.ipynb
│   ├── 02-Strings-checkpoint.ipynb
│   ├── 02-Strings.ipynb
│   ├── 02-Web-Scraping-Exercise-Solutions-checkpoint.ipynb
│   ├── 02-Web-Scraping-Exercise-Solutions.ipynb
│   ├── 02-Widget Basics-checkpoint.ipynb
│   ├── 02-Widget Basics.ipynb
│   ├── 03.00-Introduction-to-Pandas.ipynb
│   ├── 03.01-Introducing-Pandas-Objects.ipynb
│   ├── 03.02-Data-Indexing-and-Selection.ipynb
│   ├── 03.03-Operations-in-Pandas.ipynb
│   ├── 03.04-Missing-Values.ipynb
│   ├── 03.05-Hierarchical-Indexing.ipynb
│   ├── 03.06-Concat-And-Append.ipynb
│   ├── 03.07-Merge-and-Join.ipynb
│   ├── 03.08-Aggregation-and-Grouping.ipynb
│   ├── 03.09-Pivot-Tables.ipynb
│   ├── 03.10-Working-With-Strings.ipynb
│   ├── 03.11-Working-with-Time-Series.ipynb
│   ├── 03.12-Performance-Eval-and-Query.ipynb
│   ├── 03.13-Further-Resources.ipynb
│   ├── 03-Advanced Sets-checkpoint.ipynb
│   ├── 03-Advanced Sets.ipynb
│   ├── 03_Containers.ipynb
│   ├── 03-Errors and Exceptions Homework - Solution-checkpoint.ipynb
│   ├── 03-Errors and Exceptions Homework - Solution.ipynb
│   ├── 03-Filter-checkpoint.ipynb
│   ├── 03-Filter.ipynb
│   ├── 03-for Loops-checkpoint.ipynb
│   ├── 03-for Loops.ipynb
│   ├── 03-Function Practice Exercises-checkpoint.ipynb
│   ├── 03-Function Practice Exercises.ipynb
│   ├── 03-Iterators and Generators Homework - Solution-checkpoint.ipynb
│   ├── 03-Iterators and Generators Homework - Solution.ipynb
│   ├── 03-Lambda expressions-checkpoint.ipynb
│   ├── 03-Math-and-Random-Module-checkpoint.ipynb
│   ├── 03-Math-and-Random-Module.ipynb
│   ├── 03-Milestone Project 1 - Complete Walkthrough Solution-checkpoint.ipynb
│   ├── 03-Milestone Project 1 - Complete Walkthrough Solution.ipynb
│   ├── 03-Milestone Project 2 - Complete Walkthrough Solution-checkpoint.ipynb
│   ├── 03-Milestone Project 2 - Complete Walkthrough Solution.ipynb
│   ├── 03-Object Oriented Programming Homework - Solution-checkpoint.ipynb
│   ├── 03-Object Oriented Programming Homework - Solution.ipynb
│   ├── 03-PDFs-Spreadsheets-Puzzle-Solution-checkpoint.ipynb
│   ├── 03-PDFs-Spreadsheets-Puzzle-Solution.ipynb
│   ├── 03-Print Formatting with Strings-checkpoint.ipynb
│   ├── 03-Print Formatting with Strings.ipynb
│   ├── 03-python Debugger (pdb)-checkpoint.ipynb
│   ├── 03-Widget List-checkpoint.ipynb
│   ├── 03-Widget List.ipynb
│   ├── 04.00-Introduction-To-Matplotlib.ipynb
│   ├── 04.01-Simple-Line-Plots.ipynb
│   ├── 04.02-Simple-Scatter-Plots.ipynb
│   ├── 04.03-Errorbars.ipynb
│   ├── 04.04-Density-and-Contour-Plots.ipynb
│   ├── 04.05-Histograms-and-Binnings.ipynb
│   ├── 04.06-Customizing-Legends.ipynb
│   ├── 04.07-Customizing-Colorbars.ipynb
│   ├── 04.08-Multiple-Subplots.ipynb
│   ├── 04.09-Text-and-Annotation.ipynb
│   ├── 04.10-Customizing-Ticks.ipynb
│   ├── 04.11-Settings-and-Stylesheets.ipynb
│   ├── 04.12-Three-Dimensional-Plotting.ipynb
│   ├── 04.13-Geographic-Data-With-Basemap.ipynb
│   ├── 04.14-Visualization-With-Seaborn.ipynb
│   ├── 04.15-Further-Resources.ipynb
│   ├── 04-Advanced Dictionaries-checkpoint.ipynb
│   ├── 04-Advanced Dictionaries.ipynb
│   ├── 04_DictionariesControl.ipynb
│   ├── 04-Function Practice Exercises - Solutions-checkpoint.ipynb
│   ├── 04-Function Practice Exercises - Solutions.ipynb
│   ├── 04-Lists-checkpoint.ipynb
│   ├── 04-Lists.ipynb
│   ├── 04-Nested Statements and Scope-checkpoint.ipynb
│   ├── 04-OOP Challenge-checkpoint.ipynb
│   ├── 04-OOP Challenge.ipynb
│   ├── 04-python Debugger (pdb)-checkpoint.ipynb
│   ├── 04-python Debugger (pdb).ipynb
│   ├── 04-Timing your code - timeit-checkpoint.ipynb
│   ├── 04-Unit Testing-checkpoint.ipynb
│   ├── 04-Unit Testing.ipynb
│   ├── 04-while Loops-checkpoint.ipynb
│   ├── 04-while Loops.ipynb
│   ├── 04-Widget Events-checkpoint.ipynb
│   ├── 04-Widget Events.ipynb
│   ├── 04-Zip-checkpoint.ipynb
│   ├── 04-Zip.ipynb
│   ├── 05.00-Machine-Learning.ipynb
│   ├── 05.01-What-Is-Machine-Learning.ipynb
│   ├── 05.02-Introducing-Scikit-Learn.ipynb
│   ├── 05.03-Hyperparameters-and-Model-Validation.ipynb
│   ├── 05.04-Feature-Engineering.ipynb
│   ├── 05.05-Naive-Bayes.ipynb
│   ├── 05.06-Linear-Regression.ipynb
│   ├── 05.07-Support-Vector-Machines.ipynb
│   ├── 05.08-Random-Forests.ipynb
│   ├── 05.09-Principal-Component-Analysis.ipynb
│   ├── 05.10-Manifold-Learning.ipynb
│   ├── 05.11-K-Means.ipynb
│   ├── 05.12-Gaussian-Mixtures.ipynb
│   ├── 05.13-Kernel-Density-Estimation.ipynb
│   ├── 05.14-Image-Features.ipynb
│   ├── 05.15-Learning-More.ipynb
│   ├── 05-Advanced Lists-checkpoint.ipynb
│   ├── 05-Advanced Lists.ipynb
│   ├── 05-Dictionaries-checkpoint.ipynb
│   ├── 05-Dictionaries.ipynb
│   ├── 05-Enumerate-checkpoint.ipynb
│   ├── 05-Enumerate.ipynb
│   ├── 05-Functions and Methods Homework-checkpoint.ipynb
│   ├── 05_Functions.ipynb
│   ├── 05-Lambda-Expressions-Map-and-Filter-checkpoint.ipynb
│   ├── 05-Lambda-Expressions-Map-and-Filter.ipynb
│   ├── 05-OOP Challenge - Solution-checkpoint.ipynb
│   ├── 05-OOP Challenge - Solution.ipynb
│   ├── 05-Overview-of-Regular-Expressions-checkpoint.ipynb
│   ├── 05-Overview-of-Regular-Expressions.ipynb
│   ├── 05-range()-checkpoint.ipynb
│   ├── 05-Useful-Operators-checkpoint.ipynb
│   ├── 05-Useful-Operators.ipynb
│   ├── 05-Widget Styling-checkpoint.ipynb
│   ├── 05-Widget Styling.ipynb
│   ├── 06.00-Figure-Code.ipynb
│   ├── 06-Advanced python Objects Test-checkpoint.ipynb
│   ├── 06-Advanced python Objects Test.ipynb
│   ├── 06-all() and any()-checkpoint.ipynb
│   ├── 06-all() and any().ipynb
│   ├── 06-Custom Widget-checkpoint.ipynb
│   ├── 06-Custom Widget.ipynb
│   ├── 06-Functions and Methods Homework - Solutions-checkpoint.ipynb
│   ├── 06_FunctionsModules.ipynb
│   ├── 06-List Comprehensions-checkpoint.ipynb
│   ├── 06-List Comprehensions.ipynb
│   ├── 06-Nested Statements and Scope-checkpoint.ipynb
│   ├── 06-Nested Statements and Scope.ipynb
│   ├── 06-Timing your code - timeit-checkpoint.ipynb
│   ├── 06-Timing your code - timeit.ipynb
│   ├── 06-Tuples-checkpoint.ipynb
│   ├── 06-Tuples.ipynb
│   ├── 06-Unzipping-and-Zipping-Files-checkpoint.ipynb
│   ├── 07-Advanced-Modules-Exercise-Puzzle-checkpoint.ipynb
│   ├── 07-Advanced-Modules-Exercise-Puzzle.ipynb
│   ├── 07-Advanced python Objects Test - Solutions-checkpoint.ipynb
│   ├── 07-Advanced python Objects Test - Solutions.ipynb
│   ├── 07-Advanced Widget List-checkpoint.ipynb
│   ├── 07-Advanced Widget List.ipynb
│   ├── 07-args and kwargs-checkpoint.ipynb
│   ├── 07-args and kwargs.ipynb
│   ├── 07-Complex-checkpoint.ipynb
│   ├── 07-Complex.ipynb
│   ├── 07_InputOutput.ipynb
│   ├── 07-OS-Module-checkpoint.ipynb
│   ├── 07-Sets and Booleans-checkpoint.ipynb
│   ├── 07-Sets and Booleans.ipynb
│   ├── 07-Statements Assessment Test-checkpoint.ipynb
│   ├── 07-Statements Assessment Test.ipynb
│   ├── 07-Unzipping-and-Zipping-Files-checkpoint.ipynb
│   ├── 07-Unzipping-and-Zipping-Files.ipynb
│   ├── 08-Advanced-Modules-Exercise-Solutions-checkpoint.ipynb
│   ├── 08-Advanced-Modules-Exercise-Solutions.ipynb
│   ├── 08-Advanced Widget Styling with Layout-checkpoint.ipynb
│   ├── 08-Advanced Widget Styling with Layout.ipynb
│   ├── 08-BONUS - With Statement Context Managers.ipynb
│   ├── 08-Built-in Functions Assessment Test-checkpoint.ipynb
│   ├── 08-Built-in Functions Assessment Test.ipynb
│   ├── 08-Files-checkpoint.ipynb
│   ├── 08-Files.ipynb
│   ├── 08-Function Practice Exercises-checkpoint.ipynb
│   ├── 08-Functions and Methods Homework-checkpoint.ipynb
│   ├── 08-Functions and Methods Homework.ipynb
│   ├── 08_NumpyArrays.ipynb
│   ├── 08-Statements Assessment Test - Solutions-checkpoint.ipynb
│   ├── 08-Statements Assessment Test - Solutions.ipynb
│   ├── 09-Advanced-Modules-Exercise-Solutions-checkpoint.ipynb
│   ├── 09-Built-in Functions Assessment Test - Solution-checkpoint.ipynb
│   ├── 09-Built-in Functions Assessment Test - Solution.ipynb
│   ├── 09-Function Practice Exercises - Solutions-checkpoint.ipynb
│   ├── 09-Functions and Methods Homework - Solutions-checkpoint.ipynb
│   ├── 09-Functions and Methods Homework - Solutions.ipynb
│   ├── 09-Guessing Game Challenge-checkpoint.ipynb
│   ├── 09-Guessing Game Challenge.ipynb
│   ├── 09_NumpyMatplotlib.ipynb
│   ├── 09-Objects and Data Structures Assessment Test-checkpoint.ipynb
│   ├── 09-Objects and Data Structures Assessment Test.ipynb
│   ├── 10-Guessing Game Challenge - Solution-checkpoint.ipynb
│   ├── 10-Guessing Game Challenge - Solution.ipynb
│   ├── 10-Objects and Data Structures Assessment Test-Solution-checkpoint.ipynb
│   ├── 10-Objects and Data Structures Assessment Test-Solution.ipynb
│   ├── 10_PlotCustomization.ipynb
│   ├── 11_PlotTypes.ipynb
│   ├── 12_RadioactiveDecay.ipynb
│   ├── 13_AirResistance.ipynb
│   ├── 14_ProjectileMotion.ipynb
│   ├── 15_Baseball.ipynb
│   ├── 16_SimpleHarmonicMotion.ipynb
│   ├── 17_ChaosPendulum.ipynb
│   ├── 18_LogisticMap.ipynb
│   ├── 19_LinearRegression.ipynb
│   ├── 20_LeastSquares.ipynb
│   ├── 21_SpectralAnalysis.ipynb
│   ├── 22_FFT.ipynb
│   ├── 23_Laplace.ipynb
│   ├── 24_RelaxationMethods.ipynb
│   ├── 25_SOR_Poisson.ipynb
│   ├── 26_Quadrature.ipynb
│   ├── 27_SimpsonsRule.ipynb
│   ├── 28_MonteCarloIntegration.ipynb
│   ├── 29_RandomNumbers.ipynb
│   ├── 30_RejectionSampling.ipynb
│   ├── 31_IsingModelMFT.ipynb
│   ├── 32_IsingModelMC.ipynb
│   ├── acknowledgements.ipynb
│   ├── actor_critic.ipynb
│   ├── Adaptive_Forgetting.ipynb
│   ├── advanced_autodiff.ipynb
│   ├── advanced.ipynb
│   ├── adversarial_fgsm.ipynb
│   ├── Algebraic_integration_from_Ludovic_Sacchelli_s_PhD_thesis.ipynb
│   ├── Algorithme de Cocke-Kasami-Younger (python3).ipynb
│   ├── Algorithme_genetique_pour_generer_des_eclairages_modelisation_agreg.ipynb
│   ├── Algorithmes_pour_l_ecriture_inclusive.ipynb
│   ├── Algorithms_to_compute_eigen_values_and_eigen_vectors_in_Julia.ipynb
│   ├── An_Identiconizer_generator_implementation_in_python.ipynb
│   ├── Animations Using clear_output.ipynb
│   ├── ANSI Test.ipynb
│   ├── apa_basic_breakdowns.ipynb
│   ├── _Appendix A - Virtual Machine Experience.ipynb
│   ├── _Appendix B - OAuth Primer.ipynb
│   ├── _Appendix C - python & Ipython Notebook Tips.ipynb
│   ├── Arithmetique_sur_les_posits.ipynb
│   ├── A_short_study_of_Renyi_entropy.ipynb
│   ├── Assignments - Dictionaries etc.ipynb
│   ├── Assignments - Hello World through Lists.ipynb
│   ├── Assignments - If Statements.ipynb
│   ├── Assignments - Real World Examples.ipynb
│   ├── A tiny regex challenge solved without another regex.ipynb
│   ├── AUC_Derivation.ipynb
│   ├── autodiff.ipynb
│   ├── autoencoder.ipynb
│   ├── autograph.ipynb
│   ├── automatic_differentiation.ipynb
│   ├── Background Jobs.ipynb
│   ├── BanachMatchbox.ipynb
│   ├── baseball.ipynb
│   ├── basic_classification.ipynb
│   ├── Basic plotting and matrix operations (Octave).ipynb
│   ├── Basic_plotting_and_matrix_operations__Octave__with_RISE_slides.ipynb
│   ├── basic_regression.ipynb
│   ├── basics.ipynb
│   ├── basic_text_classification.ipynb
│   ├── basic_training_loops.ipynb
│   ├── Bayesian-Belief-Network.ipynb
│   ├── Beat Frequencies.ipynb
│   ├── beginner.ipynb
│   ├── Benchmark_between_python_and_Julia.ipynb
│   ├── Benchmark_of_the_SHA256_hash_function__python_Cython_Numba.ipynb
│   ├── best_practices.ipynb
│   ├── Beyond Plain python.ipynb
│   ├── biking.ipynb
│   ├── Bitsets_en_OCaml.ipynb
│   ├── Blurring_a_part_of_an_image_in_python.ipynb
│   ├── boosted_trees.ipynb
│   ├── boosted_trees_model_understanding.ipynb
│   ├── Buffons_Needle_Sim.ipynb
│   ├── Calcul_d_une_paire_de_des_un_peu_particuliers.ipynb
│   ├── Capturing Output.ipynb
│   ├── Cell Magics.ipynb
│   ├── CGAN.ipynb
│   ├── ch1.ipynb
│   ├── ch2.ipynb
│   ├── ch3.ipynb
│   ├── ch4.ipynb
│   ├── ch5.ipynb
│   ├── ch6.ipynb
│   ├── ch7.ipynb
│   ├── ch8.ipynb
│   ├── ChaCha_PseudoRandom_number_generator.ipynb
│   ├── Chapeaux.ipynb
│   ├── Chapter 0 - Preface.ipynb
│   ├── __Chapter 1 - Mining Twitter (Full-Text Sampler).ipynb
│   ├── Chapter 1 - Mining Twitter.ipynb
│   ├── Chapter 2 - Mining Facebook.ipynb
│   ├── Chapter 3 - Mining LinkedIn.ipynb
│   ├── Chapter 4 - Mining Google+.ipynb
│   ├── Chapter 5 - Mining Web Pages.ipynb
│   ├── Chapter 6 - Mining Mailboxes.ipynb
│   ├── Chapter 7 - Mining GitHub.ipynb
│   ├── Chapter 8 - Mining the Semantically Marked-Up Web.ipynb
│   ├── Chapter 9 - Twitter Cookbook.ipynb
│   ├── Cheat_sheet_for_Google_Colab.ipynb
│   ├── checkpoint.ipynb
│   ├── Circuits.ipynb
│   ├── CLAHE.ipynb
│   ├── classes_exercise.ipynb
│   ├── classes.ipynb
│   ├── classification.ipynb
│   ├── classify_text_with_bert.ipynb
│   ├── cnn_adversarial_examples.ipynb
│   ├── cnn.ipynb
│   ├── Compressive_Sampling.ipynb
│   ├── Conditional_Expectation_Gaussian.ipynb
│   ├── Conditional_expectation_MSE_Ex.ipynb
│   ├── Conditional_expectation_MSE.ipynb
│   ├── Conditional_Expectation_Projection.ipynb
│   ├── conditionals_exercise.ipynb
│   ├── conditionals.ipynb
│   ├── Confidence_Intervals.ipynb
│   ├── Confined Output.ipynb
│   ├── Connected-Component-Labeling.ipynb
│   ├── Connecting with the Qt Console.ipynb
│   ├── contents.ipynb
│   ├── Contraintes_temporelles.ipynb
│   ├── Controller.ipynb
│   ├── Convolutional_Autoencoder.ipynb
│   ├── Crime_parfait.ipynb
│   ├── CSS Reference.ipynb
│   ├── csv.ipynb
│   ├── Custom Display Logic.ipynb
│   ├── Custom Keyboard Shortcuts.ipynb
│   ├── custom_layers.ipynb
│   ├── custom_training.ipynb
│   ├── custom_training_walkthrough.ipynb
│   ├── cvae.ipynb
│   ├── cyclegan.ipynb
│   ├── data_augmentation.ipynb
│   ├── Data_Challenge__Mangaki__September_2017.ipynb
│   ├── data.ipynb
│   ├── data_performance.ipynb
│   ├── Data Structures.ipynb
│   ├── Day0.ipynb
│   ├── Day1.ipynb
│   ├── Day2.ipynb
│   ├── Day3.ipynb
│   ├── Day4.ipynb
│   ├── Day5.ipynb
│   ├── dcgan.ipynb
│   ├── debugging_exercise.ipynb
│   ├── debugging.ipynb
│   ├── Decision_Tree_Regression.ipynb
│   ├── deepdream.ipynb
│   ├── deep-q-networks.ipynb
│   ├── Demonstration of numpy.polynomial.Polynomial and nice display with LaTeX and MathJax (python3).ipynb
│   ├── Demonstration_of_running_a_Jupyter_notebook_with_sudo_rights.ipynb
│   ├── Demo_of_RISE_for_slides_with_Jupyter_notebooks__Julia.ipynb
│   ├── Demo_of_RISE_for_slides_with_Jupyter_notebooks__OCaml.ipynb
│   ├── Demo_of_RISE_for_slides_with_Jupyter_notebooks__python.ipynb
│   ├── Des_dates_qui_font_des_nombres_premiers.ipynb
│   ├── Des_dates_qui_font_des_nombres_premiers__suite.ipynb
│   ├── dictionaries_exercise.ipynb
│   ├── Dictionaries.ipynb
│   ├── display-plotly.ipynb
│   ├── display-updates.ipynb
│   ├── distributed_training.ipynb
│   ├── distribute_strategy.ipynb
│   ├── Distributing Jupyter Extensions as python Packages.ipynb
│   ├── DM_Images_debut.ipynb
│   ├── Droite Discrète - public 2012 D5.ipynb
│   ├── eager_basics.ipynb
│   ├── eager.ipynb
│   ├── Eclairage_graphe.ipynb
│   ├── Ecriture inclusive.ipynb
│   ├── Efficient_sampling_from_a_Binomial_distribution.ipynb
│   ├── epilogue.ipynb
│   ├── estimator.ipynb
│   ├── Example_CSVs.ipynb
│   ├── example.ipynb
│   ├── exceptions_exercise.ipynb
│   ├── exceptions.ipynb
│   ├── Expectation_Maximization.ipynb
│   ├── Exploring Graphs.ipynb
│   ├── Exponential_Integral_python.ipynb
│   ├── Exponential_splines.ipynb
│   ├── Export As (nbconvert).ipynb
│   ├── Factoring.ipynb
│   ├── fast-fourier-transform.ipynb
│   ├── feature_columns.ipynb
│   ├── FeatureFriday.ipynb
│   ├── Février 2021 un mini challenge arithmético-algorithmique.ipynb
│   ├── file_io_exercise.ipynb
│   ├── file_io.ipynb
│   ├── Filtering.ipynb
│   ├── Filtering_Part2.ipynb
│   ├── Filtering_Part3.ipynb
│   ├── Floating_point_error_propagation_in_polynomial_multiplication_with_Fast-Fourier_Transform.ipynb
│   ├── for_loops_exercise.ipynb
│   ├── for_loops.ipynb
│   ├── Fourier_Transform.ipynb
│   ├── Frequency_Resolution.ipynb
│   ├── function.ipynb
│   ├── functions_exercise.ipynb
│   ├── Functions.ipynb
│   ├── Gauss_Markov.ipynb
│   ├── Generating_permutations_with_python.ipynb
│   ├── Generer_des_fausses_citations_latines_du_Roi_Loth.ipynb
│   ├── Getting Help.ipynb
│   ├── gilsleep.ipynb
│   ├── golf.ipynb
│   ├── gpu.ipynb
│   ├── graph_optimization.ipynb
│   ├── hello_world.ipynb
│   ├── hierarchical_clustering.ipynb
│   ├── Histogram-Equalization.ipynb
│   ├── hub_with_keras.ipynb
│   ├── Hypothesis_Testing.ipynb
│   ├── idiomatic_dicts.ipynb
│   ├── idiomatic_loops.ipynb
│   ├── idiomatic_misc1.ipynb
│   ├── idiomatic_misc2.ipynb
│   ├── idiomatic_python_exercise.ipynb
│   ├── if_statements.ipynb
│   ├── If Statements.ipynb
│   ├── Image Browser.ipynb
│   ├── image_captioning.ipynb
│   ├── Image Processing.ipynb
│   ├── images.ipynb
│   ├── imbalanced_data.ipynb
│   ├── Importing Notebooks.ipynb
│   ├── _index.ipynb
│   ├── index.ipynb
│   ├── input.ipynb
│   ├── Installing Libraries.ipynb
│   ├── integrated_gradients.ipynb
│   ├── introducing_functions.ipynb
│   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling.ipynb
│   ├── intro_to_graphs.ipynb
│   ├── intro_to_modules.ipynb
│   ├── Inverse_Projection_Constrained_Optimization.ipynb
│   ├── ipynb
│   │   ├── 00.00-Preface.ipynb
│   │   ├── 00-Collections-Module-checkpoint.ipynb
│   │   ├── 00-Collections-Module.ipynb
│   │   ├── 00-Guide-to-Web-Scraping-checkpoint.ipynb
│   │   ├── 00-Guide-to-Web-Scraping.ipynb
│   │   ├── 00-Milestone-2-Warmup-Project-checkpoint.ipynb
│   │   ├── 00-Milestone-2-Warmup-Project.ipynb
│   │   ├── 00-Overview-of-Sending-Emails-checkpoint.ipynb
│   │   ├── 00-Overview-of-Sending-Emails.ipynb
│   │   ├── 00-Overview-of-Working-with-Images-checkpoint.ipynb
│   │   ├── 00-Overview-of-Working-with-Images.ipynb
│   │   ├── 00-Warm-Up-Project-Exercises-checkpoint.ipynb
│   │   ├── 00-Warm-Up-Project-Exercises.ipynb
│   │   ├── 00-Working-with-CSV-Files-checkpoint.ipynb
│   │   ├── 00-Working-with-CSV-Files.ipynb
│   │   ├── 01.00-Ipython-Beyond-Normal-python.ipynb
│   │   ├── 01.01-Help-And-Documentation.ipynb
│   │   ├── 01.02-Shell-Keyboard-Shortcuts.ipynb
│   │   ├── 01.03-Magic-Commands.ipynb
│   │   ├── 01.04-Input-Output-History.ipynb
│   │   ├── 01.05-Ipython-And-Shell-Commands.ipynb
│   │   ├── 01.06-Errors-and-Debugging.ipynb
│   │   ├── 01.07-Timing-and-Profiling.ipynb
│   │   ├── 01.08-More-Ipython-Resources.ipynb
│   │   ├── 01-Advanced Numbers-checkpoint.ipynb
│   │   ├── 01-Advanced Numbers.ipynb
│   │   ├── 01-Comparison Operators-checkpoint.ipynb
│   │   ├── 01-Comparison Operators.ipynb
│   │   ├── 01-Datetime-Module-checkpoint.ipynb
│   │   ├── 01-Decorators-checkpoint.ipynb
│   │   ├── 01-Decorators.ipynb
│   │   ├── 01-Errors and Exceptions Handling-checkpoint.ipynb
│   │   ├── 01-Errors and Exceptions Handling.ipynb
│   │   ├── 01-Final Capstone Project-checkpoint.ipynb
│   │   ├── 01-Final Capstone Project.ipynb
│   │   ├── 01-Image-Exercise-checkpoint.ipynb
│   │   ├── 01-Image-Exercise.ipynb
│   │   ├── 01-Interact-checkpoint.ipynb
│   │   ├── 01-Interact.ipynb
│   │   ├── 01_Introduction.ipynb
│   │   ├── 01-Introduction to python Statements-checkpoint.ipynb
│   │   ├── 01-Introduction to python Statements.ipynb
│   │   ├── 01-Iterators and Generators-checkpoint.ipynb
│   │   ├── 01-Iterators and Generators.ipynb
│   │   ├── 01-Map-checkpoint.ipynb
│   │   ├── 01-Map.ipynb
│   │   ├── 01-Methods-checkpoint.ipynb
│   │   ├── 01-Methods.ipynb
│   │   ├── 01-Milestone Project 1 - Assignment-checkpoint.ipynb
│   │   ├── 01-Milestone Project 1 - Assignment.ipynb
│   │   ├── 01-Milestone Project 2 - Assignment-checkpoint.ipynb
│   │   ├── 01-Milestone Project 2 - Assignment.ipynb
│   │   ├── 01-Modules and Packages-checkpoint.ipynb
│   │   ├── 01-Numbers-checkpoint.ipynb
│   │   ├── 01-Numbers.ipynb
│   │   ├── 01-Object Oriented Programming-checkpoint.ipynb
│   │   ├── 01-Object Oriented Programming-Copy1-checkpoint.ipynb
│   │   ├── 01-Object Oriented Programming.ipynb
│   │   ├── 01-Opening-and-Reading-Files-Folders-checkpoint.ipynb
│   │   ├── 01-Opening-and-Reading-Files-Folders.ipynb
│   │   ├── 01-Overview-of-Received-Emails-checkpoint.ipynb
│   │   ├── 01-Overview-of-Received-Emails.ipynb
│   │   ├── 01-Variable Assignment-checkpoint.ipynb
│   │   ├── 01-Variable Assignment.ipynb
│   │   ├── 01-Web-Scraping-Exercises-checkpoint.ipynb
│   │   ├── 01-Web-Scraping-Exercises.ipynb
│   │   ├── 01-Working-with-PDFs-checkpoint.ipynb
│   │   ├── 01-Working-with-PDFs.ipynb
│   │   ├── 02.00-Introduction-to-NumPy.ipynb
│   │   ├── 02.01-Understanding-Data-Types.ipynb
│   │   ├── 02.02-The-Basics-Of-NumPy-Arrays.ipynb
│   │   ├── 02.03-Computation-on-arrays-ufuncs.ipynb
│   │   ├── 02.04-Computation-on-arrays-aggregates.ipynb
│   │   ├── 02.05-Computation-on-arrays-broadcasting.ipynb
│   │   ├── 02.06-Boolean-Arrays-and-Masks.ipynb
│   │   ├── 02.07-Fancy-Indexing.ipynb
│   │   ├── 02.08-Sorting.ipynb
│   │   ├── 02.09-Structured-Data-NumPy.ipynb
│   │   ├── 02-Advanced Strings-checkpoint.ipynb
│   │   ├── 02-Advanced Strings.ipynb
│   │   ├── 02_Basicpython.ipynb
│   │   ├── 02-Chained Comparison Operators-checkpoint.ipynb
│   │   ├── 02-Chained Comparison Operators.ipynb
│   │   ├── 02-Datetime-Module-checkpoint.ipynb
│   │   ├── 02-Datetime-Module.ipynb
│   │   ├── 02-Decorators Homework-checkpoint.ipynb
│   │   ├── 02-Decorators Homework.ipynb
│   │   ├── 02-Errors and Exceptions Homework-checkpoint.ipynb
│   │   ├── 02-Errors and Exceptions Homework.ipynb
│   │   ├── 02-Exercise-Ideas-checkpoint.ipynb
│   │   ├── 02-Exercise-Ideas.ipynb
│   │   ├── 02-Final Capstone Project Ideas-checkpoint.ipynb
│   │   ├── 02-Final Capstone Project Ideas.ipynb
│   │   ├── 02-Functions-checkpoint.ipynb
│   │   ├── 02-Functions.ipynb
│   │   ├── 02-if, elif, and else Statements-checkpoint.ipynb
│   │   ├── 02-if, elif, and else Statements.ipynb
│   │   ├── 02-Image-Exercise-Solution-checkpoint.ipynb
│   │   ├── 02-Image-Exercise-Solution.ipynb
│   │   ├── 02-imdb-binary-classification.ipynb
│   │   ├── 02-Iterators and Generators Homework-checkpoint.ipynb
│   │   ├── 02-Iterators and Generators Homework.ipynb
│   │   ├── 02-Math-and-Random-Module-checkpoint.ipynb
│   │   ├── 02-Milestone Project 1 - Walkthrough Steps Workbook-checkpoint.ipynb
│   │   ├── 02-Milestone Project 1 - Walkthrough Steps Workbook.ipynb
│   │   ├── 02-Milestone Project 2 - Walkthrough Steps Workbook-checkpoint.ipynb
│   │   ├── 02-Milestone Project 2 - Walkthrough Steps Workbook.ipynb
│   │   ├── 02-Object Oriented Programming Homework-checkpoint.ipynb
│   │   ├── 02-Object Oriented Programming Homework.ipynb
│   │   ├── 02-PDFs-Spreadsheets-Puzzle-checkpoint.ipynb
│   │   ├── 02-PDFs-Spreadsheets-Puzzle.ipynb
│   │   ├── 02-Reduce-checkpoint.ipynb
│   │   ├── 02-Reduce.ipynb
│   │   ├── 02-Strings-checkpoint.ipynb
│   │   ├── 02-Strings.ipynb
│   │   ├── 02-Web-Scraping-Exercise-Solutions-checkpoint.ipynb
│   │   ├── 02-Web-Scraping-Exercise-Solutions.ipynb
│   │   ├── 02-Widget Basics-checkpoint.ipynb
│   │   ├── 02-Widget Basics.ipynb
│   │   ├── 03.00-Introduction-to-Pandas.ipynb
│   │   ├── 03.01-Introducing-Pandas-Objects.ipynb
│   │   ├── 03.02-Data-Indexing-and-Selection.ipynb
│   │   ├── 03.03-Operations-in-Pandas.ipynb
│   │   ├── 03.04-Missing-Values.ipynb
│   │   ├── 03.05-Hierarchical-Indexing.ipynb
│   │   ├── 03.06-Concat-And-Append.ipynb
│   │   ├── 03.07-Merge-and-Join.ipynb
│   │   ├── 03.08-Aggregation-and-Grouping.ipynb
│   │   ├── 03.09-Pivot-Tables.ipynb
│   │   ├── 03.10-Working-With-Strings.ipynb
│   │   ├── 03.11-Working-with-Time-Series.ipynb
│   │   ├── 03.12-Performance-Eval-and-Query.ipynb
│   │   ├── 03.13-Further-Resources.ipynb
│   │   ├── 03-Advanced Sets-checkpoint.ipynb
│   │   ├── 03-Advanced Sets.ipynb
│   │   ├── 03_Containers.ipynb
│   │   ├── 03-Errors and Exceptions Homework - Solution-checkpoint.ipynb
│   │   ├── 03-Errors and Exceptions Homework - Solution.ipynb
│   │   ├── 03-Filter-checkpoint.ipynb
│   │   ├── 03-Filter.ipynb
│   │   ├── 03-for Loops-checkpoint.ipynb
│   │   ├── 03-for Loops.ipynb
│   │   ├── 03-Function Practice Exercises-checkpoint.ipynb
│   │   ├── 03-Function Practice Exercises.ipynb
│   │   ├── 03-Iterators and Generators Homework - Solution-checkpoint.ipynb
│   │   ├── 03-Iterators and Generators Homework - Solution.ipynb
│   │   ├── 03-Lambda expressions-checkpoint.ipynb
│   │   ├── 03-Math-and-Random-Module-checkpoint.ipynb
│   │   ├── 03-Math-and-Random-Module.ipynb
│   │   ├── 03-Milestone Project 1 - Complete Walkthrough Solution-checkpoint.ipynb
│   │   ├── 03-Milestone Project 1 - Complete Walkthrough Solution.ipynb
│   │   ├── 03-Milestone Project 2 - Complete Walkthrough Solution-checkpoint.ipynb
│   │   ├── 03-Milestone Project 2 - Complete Walkthrough Solution.ipynb
│   │   ├── 03-Object Oriented Programming Homework - Solution-checkpoint.ipynb
│   │   ├── 03-Object Oriented Programming Homework - Solution.ipynb
│   │   ├── 03-PDFs-Spreadsheets-Puzzle-Solution-checkpoint.ipynb
│   │   ├── 03-PDFs-Spreadsheets-Puzzle-Solution.ipynb
│   │   ├── 03-Print Formatting with Strings-checkpoint.ipynb
│   │   ├── 03-Print Formatting with Strings.ipynb
│   │   ├── 03-python Debugger (pdb)-checkpoint.ipynb
│   │   ├── 03-Widget List-checkpoint.ipynb
│   │   ├── 03-Widget List.ipynb
│   │   ├── 04.00-Introduction-To-Matplotlib.ipynb
│   │   ├── 04.01-Simple-Line-Plots.ipynb
│   │   ├── 04.02-Simple-Scatter-Plots.ipynb
│   │   ├── 04.03-Errorbars.ipynb
│   │   ├── 04.04-Density-and-Contour-Plots.ipynb
│   │   ├── 04.05-Histograms-and-Binnings.ipynb
│   │   ├── 04.06-Customizing-Legends.ipynb
│   │   ├── 04.07-Customizing-Colorbars.ipynb
│   │   ├── 04.08-Multiple-Subplots.ipynb
│   │   ├── 04.09-Text-and-Annotation.ipynb
│   │   ├── 04.10-Customizing-Ticks.ipynb
│   │   ├── 04.11-Settings-and-Stylesheets.ipynb
│   │   ├── 04.12-Three-Dimensional-Plotting.ipynb
│   │   ├── 04.13-Geographic-Data-With-Basemap.ipynb
│   │   ├── 04.14-Visualization-With-Seaborn.ipynb
│   │   ├── 04.15-Further-Resources.ipynb
│   │   ├── 04-Advanced Dictionaries-checkpoint.ipynb
│   │   ├── 04-Advanced Dictionaries.ipynb
│   │   ├── 04_DictionariesControl.ipynb
│   │   ├── 04-Function Practice Exercises - Solutions-checkpoint.ipynb
│   │   ├── 04-Function Practice Exercises - Solutions.ipynb
│   │   ├── 04-Lists-checkpoint.ipynb
│   │   ├── 04-Lists.ipynb
│   │   ├── 04-Nested Statements and Scope-checkpoint.ipynb
│   │   ├── 04-OOP Challenge-checkpoint.ipynb
│   │   ├── 04-OOP Challenge.ipynb
│   │   ├── 04-python Debugger (pdb)-checkpoint.ipynb
│   │   ├── 04-python Debugger (pdb).ipynb
│   │   ├── 04-Timing your code - timeit-checkpoint.ipynb
│   │   ├── 04-Unit Testing-checkpoint.ipynb
│   │   ├── 04-Unit Testing.ipynb
│   │   ├── 04-while Loops-checkpoint.ipynb
│   │   ├── 04-while Loops.ipynb
│   │   ├── 04-Widget Events-checkpoint.ipynb
│   │   ├── 04-Widget Events.ipynb
│   │   ├── 04-Zip-checkpoint.ipynb
│   │   ├── 04-Zip.ipynb
│   │   ├── 05.00-Machine-Learning.ipynb
│   │   ├── 05.01-What-Is-Machine-Learning.ipynb
│   │   ├── 05.02-Introducing-Scikit-Learn.ipynb
│   │   ├── 05.03-Hyperparameters-and-Model-Validation.ipynb
│   │   ├── 05.04-Feature-Engineering.ipynb
│   │   ├── 05.05-Naive-Bayes.ipynb
│   │   ├── 05.06-Linear-Regression.ipynb
│   │   ├── 05.07-Support-Vector-Machines.ipynb
│   │   ├── 05.08-Random-Forests.ipynb
│   │   ├── 05.09-Principal-Component-Analysis.ipynb
│   │   ├── 05.10-Manifold-Learning.ipynb
│   │   ├── 05.11-K-Means.ipynb
│   │   ├── 05.12-Gaussian-Mixtures.ipynb
│   │   ├── 05.13-Kernel-Density-Estimation.ipynb
│   │   ├── 05.14-Image-Features.ipynb
│   │   ├── 05.15-Learning-More.ipynb
│   │   ├── 05-Advanced Lists-checkpoint.ipynb
│   │   ├── 05-Advanced Lists.ipynb
│   │   ├── 05-Dictionaries-checkpoint.ipynb
│   │   ├── 05-Dictionaries.ipynb
│   │   ├── 05-Enumerate-checkpoint.ipynb
│   │   ├── 05-Enumerate.ipynb
│   │   ├── 05-Functions and Methods Homework-checkpoint.ipynb
│   │   ├── 05_Functions.ipynb
│   │   ├── 05-Lambda-Expressions-Map-and-Filter-checkpoint.ipynb
│   │   ├── 05-Lambda-Expressions-Map-and-Filter.ipynb
│   │   ├── 05-OOP Challenge - Solution-checkpoint.ipynb
│   │   ├── 05-OOP Challenge - Solution.ipynb
│   │   ├── 05-Overview-of-Regular-Expressions-checkpoint.ipynb
│   │   ├── 05-Overview-of-Regular-Expressions.ipynb
│   │   ├── 05-range()-checkpoint.ipynb
│   │   ├── 05-Useful-Operators-checkpoint.ipynb
│   │   ├── 05-Useful-Operators.ipynb
│   │   ├── 05-Widget Styling-checkpoint.ipynb
│   │   ├── 05-Widget Styling.ipynb
│   │   ├── 06.00-Figure-Code.ipynb
│   │   ├── 06-Advanced python Objects Test-checkpoint.ipynb
│   │   ├── 06-Advanced python Objects Test.ipynb
│   │   ├── 06-all() and any()-checkpoint.ipynb
│   │   ├── 06-all() and any().ipynb
│   │   ├── 06-Custom Widget-checkpoint.ipynb
│   │   ├── 06-Custom Widget.ipynb
│   │   ├── 06-Functions and Methods Homework - Solutions-checkpoint.ipynb
│   │   ├── 06_FunctionsModules.ipynb
│   │   ├── 06-List Comprehensions-checkpoint.ipynb
│   │   ├── 06-List Comprehensions.ipynb
│   │   ├── 06-Nested Statements and Scope-checkpoint.ipynb
│   │   ├── 06-Nested Statements and Scope.ipynb
│   │   ├── 06-Timing your code - timeit-checkpoint.ipynb
│   │   ├── 06-Timing your code - timeit.ipynb
│   │   ├── 06-Tuples-checkpoint.ipynb
│   │   ├── 06-Tuples.ipynb
│   │   ├── 06-Unzipping-and-Zipping-Files-checkpoint.ipynb
│   │   ├── 07-Advanced-Modules-Exercise-Puzzle-checkpoint.ipynb
│   │   ├── 07-Advanced-Modules-Exercise-Puzzle.ipynb
│   │   ├── 07-Advanced python Objects Test - Solutions-checkpoint.ipynb
│   │   ├── 07-Advanced python Objects Test - Solutions.ipynb
│   │   ├── 07-Advanced Widget List-checkpoint.ipynb
│   │   ├── 07-Advanced Widget List.ipynb
│   │   ├── 07-args and kwargs-checkpoint.ipynb
│   │   ├── 07-args and kwargs.ipynb
│   │   ├── 07-Complex-checkpoint.ipynb
│   │   ├── 07-Complex.ipynb
│   │   ├── 07_InputOutput.ipynb
│   │   ├── 07-OS-Module-checkpoint.ipynb
│   │   ├── 07-Sets and Booleans-checkpoint.ipynb
│   │   ├── 07-Sets and Booleans.ipynb
│   │   ├── 07-Statements Assessment Test-checkpoint.ipynb
│   │   ├── 07-Statements Assessment Test.ipynb
│   │   ├── 07-Unzipping-and-Zipping-Files-checkpoint.ipynb
│   │   ├── 07-Unzipping-and-Zipping-Files.ipynb
│   │   ├── 08-Advanced-Modules-Exercise-Solutions-checkpoint.ipynb
│   │   ├── 08-Advanced-Modules-Exercise-Solutions.ipynb
│   │   ├── 08-Advanced Widget Styling with Layout-checkpoint.ipynb
│   │   ├── 08-Advanced Widget Styling with Layout.ipynb
│   │   ├── 08-BONUS - With Statement Context Managers.ipynb
│   │   ├── 08-Built-in Functions Assessment Test-checkpoint.ipynb
│   │   ├── 08-Built-in Functions Assessment Test.ipynb
│   │   ├── 08-Files-checkpoint.ipynb
│   │   ├── 08-Files.ipynb
│   │   ├── 08-Function Practice Exercises-checkpoint.ipynb
│   │   ├── 08-Functions and Methods Homework-checkpoint.ipynb
│   │   ├── 08-Functions and Methods Homework.ipynb
│   │   ├── 08_NumpyArrays.ipynb
│   │   ├── 08-Statements Assessment Test - Solutions-checkpoint.ipynb
│   │   ├── 08-Statements Assessment Test - Solutions.ipynb
│   │   ├── 09-Advanced-Modules-Exercise-Solutions-checkpoint.ipynb
│   │   ├── 09-Built-in Functions Assessment Test - Solution-checkpoint.ipynb
│   │   ├── 09-Built-in Functions Assessment Test - Solution.ipynb
│   │   ├── 09-Function Practice Exercises - Solutions-checkpoint.ipynb
│   │   ├── 09-Functions and Methods Homework - Solutions-checkpoint.ipynb
│   │   ├── 09-Functions and Methods Homework - Solutions.ipynb
│   │   ├── 09-Guessing Game Challenge-checkpoint.ipynb
│   │   ├── 09-Guessing Game Challenge.ipynb
│   │   ├── 09_NumpyMatplotlib.ipynb
│   │   ├── 09-Objects and Data Structures Assessment Test-checkpoint.ipynb
│   │   ├── 09-Objects and Data Structures Assessment Test.ipynb
│   │   ├── 10-Guessing Game Challenge - Solution-checkpoint.ipynb
│   │   ├── 10-Guessing Game Challenge - Solution.ipynb
│   │   ├── 10-Objects and Data Structures Assessment Test-Solution-checkpoint.ipynb
│   │   ├── 10-Objects and Data Structures Assessment Test-Solution.ipynb
│   │   ├── 10_PlotCustomization.ipynb
│   │   ├── 11_PlotTypes.ipynb
│   │   ├── 12_RadioactiveDecay.ipynb
│   │   ├── 13_AirResistance.ipynb
│   │   ├── 14_ProjectileMotion.ipynb
│   │   ├── 15_Baseball.ipynb
│   │   ├── 16_SimpleHarmonicMotion.ipynb
│   │   ├── 17_ChaosPendulum.ipynb
│   │   ├── 18_LogisticMap.ipynb
│   │   ├── 19_LinearRegression.ipynb
│   │   ├── 20_LeastSquares.ipynb
│   │   ├── 21_SpectralAnalysis.ipynb
│   │   ├── 22_FFT.ipynb
│   │   ├── 23_Laplace.ipynb
│   │   ├── 24_RelaxationMethods.ipynb
│   │   ├── 25_SOR_Poisson.ipynb
│   │   ├── 26_Quadrature.ipynb
│   │   ├── 27_SimpsonsRule.ipynb
│   │   ├── 28_MonteCarloIntegration.ipynb
│   │   ├── 29_RandomNumbers.ipynb
│   │   ├── 30_RejectionSampling.ipynb
│   │   ├── 31_IsingModelMFT.ipynb
│   │   ├── 32_IsingModelMC.ipynb
│   │   ├── acknowledgements.ipynb
│   │   ├── actor_critic.ipynb
│   │   ├── Adaptive_Forgetting.ipynb
│   │   ├── advanced_autodiff.ipynb
│   │   ├── advanced.ipynb
│   │   ├── adversarial_fgsm.ipynb
│   │   ├── Algebraic_integration_from_Ludovic_Sacchelli_s_PhD_thesis.ipynb
│   │   ├── Algorithme de Cocke-Kasami-Younger (python3).ipynb
│   │   ├── Algorithme_genetique_pour_generer_des_eclairages_modelisation_agreg.ipynb
│   │   ├── Algorithmes_pour_l_ecriture_inclusive.ipynb
│   │   ├── Algorithms_to_compute_eigen_values_and_eigen_vectors_in_Julia.ipynb
│   │   ├── An_Identiconizer_generator_implementation_in_python.ipynb
│   │   ├── Animations Using clear_output.ipynb
│   │   ├── ANSI Test.ipynb
│   │   ├── apa_basic_breakdowns.ipynb
│   │   ├── Arithmetique_sur_les_posits.ipynb
│   │   ├── A_short_study_of_Renyi_entropy.ipynb
│   │   ├── A-simple-GAN.ipynb
│   │   ├── Assignments - Dictionaries etc.ipynb
│   │   ├── Assignments - Hello World through Lists.ipynb
│   │   ├── Assignments - If Statements.ipynb
│   │   ├── Assignments - Real World Examples.ipynb
│   │   ├── Association.ipynb
│   │   ├── A tiny regex challenge solved without another regex.ipynb
│   │   ├── AUC_Derivation.ipynb
│   │   ├── autodiff.ipynb
│   │   ├── autoencoder.ipynb
│   │   ├── autograph.ipynb
│   │   ├── automatic_differentiation.ipynb
│   │   ├── Background Jobs.ipynb
│   │   ├── BanachMatchbox.ipynb
│   │   ├── baseball.ipynb
│   │   ├── basic_classification.ipynb
│   │   ├── Basic of TensorFlow.ipynb
│   │   ├── Basic plotting and matrix operations (Octave).ipynb
│   │   ├── Basic_plotting_and_matrix_operations__Octave__with_RISE_slides.ipynb
│   │   ├── basic_regression.ipynb
│   │   ├── basics.ipynb
│   │   ├── basic_text_classification.ipynb
│   │   ├── basic_training_loops.ipynb
│   │   ├── Bayesian-Belief-Network.ipynb
│   │   ├── Beat Frequencies.ipynb
│   │   ├── beginner.ipynb
│   │   ├── Benchmark_between_python_and_Julia.ipynb
│   │   ├── Benchmark_of_the_SHA256_hash_function__python_Cython_Numba.ipynb
│   │   ├── best_practices.ipynb
│   │   ├── Beyond Plain python.ipynb
│   │   ├── biking.ipynb
│   │   ├── Bitsets_en_OCaml.ipynb
│   │   ├── Blurring_a_part_of_an_image_in_python.ipynb
│   │   ├── boosted_trees.ipynb
│   │   ├── boosted_trees_model_understanding.ipynb
│   │   ├── Buffons_Needle_Sim.ipynb
│   │   ├── Calcul_d_une_paire_de_des_un_peu_particuliers.ipynb
│   │   ├── Capturing Output.ipynb
│   │   ├── Cell Magics.ipynb
│   │   ├── CGAN.ipynb
│   │   ├── ch1.ipynb
│   │   ├── ch2.ipynb
│   │   ├── ch3.ipynb
│   │   ├── ch4.ipynb
│   │   ├── ch5.ipynb
│   │   ├── ch6.ipynb
│   │   ├── ch7.ipynb
│   │   ├── ch8.ipynb
│   │   ├── ChaCha_PseudoRandom_number_generator.ipynb
│   │   ├── Chapeaux.ipynb
│   │   ├── Chapter 0 - Preface.ipynb
│   │   ├── Chapter 1 - Mining Twitter.ipynb
│   │   ├── Chapter 2 - Mining Facebook.ipynb
│   │   ├── Chapter 3 - Mining LinkedIn.ipynb
│   │   ├── Chapter 4 - Mining Google+.ipynb
│   │   ├── Chapter 5 - Mining Web Pages.ipynb
│   │   ├── Chapter 6 - Mining Mailboxes.ipynb
│   │   ├── Chapter 7 - Mining GitHub.ipynb
│   │   ├── Chapter 8 - Mining the Semantically Marked-Up Web.ipynb
│   │   ├── Chapter 9 - Twitter Cookbook.ipynb
│   │   ├── Cheat_sheet_for_Google_Colab.ipynb
│   │   ├── checkpoint.ipynb
│   │   ├── Circuits.ipynb
│   │   ├── CLAHE.ipynb
│   │   ├── classes_exercise.ipynb
│   │   ├── classes.ipynb
│   │   ├── classification.ipynb
│   │   ├── classify_text_with_bert.ipynb
│   │   ├── clothing_detection.ipynb
│   │   ├── cnn_adversarial_examples.ipynb
│   │   ├── cnn.ipynb
│   │   ├── CNN-using keras.ipynb
│   │   ├── Compressive_Sampling.ipynb
│   │   ├── Conditional_Expectation_Gaussian.ipynb
│   │   ├── Conditional_expectation_MSE_Ex.ipynb
│   │   ├── Conditional_expectation_MSE.ipynb
│   │   ├── Conditional_Expectation_Projection.ipynb
│   │   ├── conditionals_exercise.ipynb
│   │   ├── conditionals.ipynb
│   │   ├── Confidence_Intervals.ipynb
│   │   ├── Confined Output.ipynb
│   │   ├── Connected-Component-Labeling.ipynb
│   │   ├── Connecting with the Qt Console.ipynb
│   │   ├── contents.ipynb
│   │   ├── Contraintes_temporelles.ipynb
│   │   ├── Controller.ipynb
│   │   ├── Convolutional_Autoencoder.ipynb
│   │   ├── Crime_parfait.ipynb
│   │   ├── CSS Reference.ipynb
│   │   ├── csv.ipynb
│   │   ├── Custom Display Logic.ipynb
│   │   ├── Custom Keyboard Shortcuts.ipynb
│   │   ├── custom_layers.ipynb
│   │   ├── custom_training.ipynb
│   │   ├── custom_training_walkthrough.ipynb
│   │   ├── cvae.ipynb
│   │   ├── CycleGAN.ipynb
│   │   ├── data.ipynb
│   │   └── Data Structures.ipynb
│   ├── JavaScript Notebook Extensions.ipynb
│   ├── Jonglage.ipynb
│   ├── json-jupyter.ipynb
│   ├── Jupyter (ipython) Notebooks Guide.ipynb
│   ├── keras.ipynb
│   ├── keras_model_to_estimator.ipynb
│   ├── keras_tuner.ipynb
│   ├── Kullback-Leibler_divergences_in_native_python__Cython_and_Numba.ipynb
│   ├── Lambda_Calcul_en_OCaml.ipynb
│   ├── lasso_regression.ipynb
│   ├── Layout Example.ipynb
│   ├── Layout Templates.ipynb
│   ├── LectureChallenge.ipynb
│   ├── Lighthouse_problem.ipynb
│   ├── linear.ipynb
│   ├── linear_regression_scikit_learn.ipynb
│   ├── lists_exercise.ipynb
│   ├── lists.ipynb
│   ├── lists_tuples.ipynb
│   ├── Living_in_a_noisy_world_with_James_Powell_rwatch_module.ipynb
│   ├── logic_notebook.ipynb
│   ├── loop_the_loop.ipynb
│   ├── Lorenz Differential Equations.ipynb
│   ├── Making a Simple Game.ipynb
│   ├── mandelbrot.ipynb
│   ├── Manual_implementation_of_some_hash_functions.ipynb
│   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_.ipynb
│   ├── MAP_Estimation.ipynb
│   ├── Markdown Pandoc Limitations.ipynb
│   ├── markdown-regression-testing.ipynb
│   ├── Markov_chains.ipynb
│   ├── Matplotlib Switching.ipynb
│   ├── Maximum_likelihood.ipynb
│   ├── Médiane des médianes - sélection en temps linéaire.ipynb
│   ├── Media widgets.ipynb
│   ├── Mémoisation_en_python_et_OCaml.ipynb
│   ├── Merge-k-sorted-lists.ipynb
│   ├── migrate.ipynb
│   ├── Mini Projects.ipynb
│   ├── Mini_tutoriel_pour_la_resolution_de_programmes_lineaires_avec_python__ENS_Rennes_2021.ipynb
│   ├── Min-Max-Contrast-Stretching.ipynb
│   ├── mixed_precision.ipynb
│   ├── modules_and_packages.ipynb
│   ├── More_Fourier_Transform.ipynb
│   ├── more_functions.ipynb
│   ├── Mots_bien_formes.ipynb
│   ├── multi_worker_with_estimator.ipynb
│   ├── multi_worker_with_keras.ipynb
│   ├── mynotebook.ipynb
│   ├── Naive_simulations_of_the_Monty-Hall_paradox.ipynb
│   ├── NetHack's functions Rne, Rn2 and Rnz in python 3.ipynb
│   ├── neural_style_transfer.ipynb
│   ├── nmt_with_attention.ipynb
│   ├── Notebook Basics.ipynb
│   ├── notebook.ipynb
│   ├── Notebooks intro.ipynb
│   ├── numbers_exercise.ipynb
│   ├── Numbers.ipynb
│   ├── numpy.ipynb
│   ├── Obfuscated_code_or_piece_of_art.ipynb
│   ├── Oraux_CentraleSupelec_PSI__Juin_2017.ipynb
│   ├── Oraux_CentraleSupelec_PSI__Juin_2018.ipynb
│   ├── Oraux_CentraleSupelec_PSI__Juin_2019.ipynb
│   ├── Other Concepts.ipynb
│   ├── other.ipynb
│   ├── Output Widget.ipynb
│   ├── overfit_and_underfit.ipynb
│   ├── pandas_dataframe.ipynb
│   ├── Pandas.ipynb
│   ├── parameter_server_training.ipynb
│   ├── part1.ipynb
│   ├── part2.ipynb
│   ├── part3.ipynb
│   ├── part4.ipynb
│   ├── part5.ipynb
│   ├── pdes.ipynb
│   ├── Picker.ipynb
│   ├── pix2pix.ipynb
│   ├── Playing with Photos.ipynb
│   ├── Playing with Sound.ipynb
│   ├── Plotting in the Notebook.ipynb
│   ├── Plus long sous mot commun (python3).ipynb
│   ├── pragmatic-yet-unwieldy.ipynb
│   ├── preface.ipynb
│   ├── premade.ipynb
│   ├── preprocessing_layers.ipynb
│   ├── Prise de rdv automatique sur Doctolib.ipynb
│   ├── Profiling_in_a_Jupyter_notebook.ipynb
│   ├── programming_competencies.ipynb
│   ├── programming_environment_extras.ipynb
│   ├── programming_environment.ipynb
│   ├── programming_environment_linux.ipynb
│   ├── programming_environment_osx.ipynb
│   ├── programming_environment_windows.ipynb
│   ├── Programming - Teaching Notebook 1.ipynb
│   ├── Project Euler (python 3) - to problem 100.ipynb
│   ├── Project Euler (python 3) - to problem 50.ipynb
│   ├── projectiles.ipynb
│   ├── Projection_Ex.ipynb
│   ├── Projection.ipynb
│   ├── Projection_mdim.ipynb
│   ├── Project Notes.ipynb
│   ├── project_structure.ipynb
│   ├── public2012_D3.ipynb
│   ├── public2012_D3 (OCaml).ipynb
│   ├── public2012_D5_OCaml.ipynb
│   ├── public2018_D1.ipynb
│   ├── public2018_D2.ipynb
│   ├── publications.ipynb
│   ├── PubsFromBib.ipynb
│   ├── Py_Pi_Day_2017.ipynb
│   ├── pytest_fixtures.ipynb
│   ├── q_learning.ipynb
│   ├── ragged_tensor.ipynb
│   ├── ragged_tensors.ipynb
│   ├── Random_Forest_Regression_Model.ipynb
│   ├── random_numbers.ipynb
│   ├── Random_Sum.ipynb
│   ├── random_walk_random_stumble.ipynb
│   ├── Raw Input in the Notebook.ipynb
│   ├── rbm.ipynb
│   ├── recap1_exercise.ipynb
│   ├── recap2_exercise.ipynb
│   ├── Rectangle_Wedge_Tail_Decomposition.ipynb
│   ├── regression.ipynb
│   ├── resources.ipynb
│   ├── Rich Output.ipynb
│   ├── Robots.ipynb
│   ├── Robust_Statistics.ipynb
│   ├── Robust_Statistics_MMA.ipynb
│   ├── rsa_digital_signature.ipynb
│   ├── Runge-Kutta_methods_for_ODE_integration_in_Julia.ipynb
│   ├── Runge-Kutta_methods_for_ODE_integration_in_OCaml.ipynb
│   ├── Runge-Kutta_methods_for_ODE_integration_in_python.ipynb
│   ├── Running Code.ipynb
│   ├── Sampling_Monte_Carlo.ipynb
│   ├── Sampling_Theorem.ipynb
│   ├── Sampling_Theorem_Part_2.ipynb
│   ├── Sampling_Theorem_Part_2_v2.ipynb
│   ├── save_and_load.ipynb
│   ├── save_and_restore_models.ipynb
│   ├── saved_model.ipynb
│   ├── Script Magics.ipynb
│   ├── segmentation.ipynb
│   ├── Short_study_of_the_Lempel-Ziv_complexity.ipynb
│   ├── simple_audio.ipynb
│   ├── simple.ipynb
│   ├── Simulated_annealing_in_python.ipynb
│   ├── Simulations_du_jeu_de_151.ipynb
│   ├── Simulations du jeu de carte Jap-Jap.ipynb
│   ├── solve_glue_tasks_using_bert_on_tpu.ipynb
│   ├── Solving_an_equation_and_the_Lambert_W_function.ipynb
│   ├── sparse_tensor.ipynb
│   ├── std_lib1_exercise.ipynb
│   ├── std_lib2_exercise.ipynb
│   ├── std_lib2.ipynb
│   ├── std_lib.ipynb
│   ├── string_formatting.ipynb
│   ├── String_methods.ipynb
│   ├── strings_exercise.ipynb
│   ├── strings.ipynb
│   ├── style_transfer.ipynb
│   ├── Sudoku.ipynb
│   ├── Sudoku_resolus_par_algorithme_genetique.ipynb
│   ├── Sum_of__number.ipynb
│   ├── syllabus.ipynb
│   ├── SymPy.ipynb
│   ├── table-with-schema.ipynb
│   ├── talkmap.ipynb
│   ├── talks.ipynb
│   ├── Taquin.ipynb
│   ├── tensor.ipynb
│   ├── terminal_apps.ipynb
│   ├── Terminal Usage.ipynb
│   ├── test_borders.ipynb
│   ├── Test_for_Binder__access_local_packages.ipynb
│   ├── testing1_exercise.ipynb
│   ├── testing1.ipynb
│   ├── testing2_exercise.ipynb
│   ├── testing2.ipynb
│   ├── testing.ipynb
│   ├── Testing_the_lolviz_python_module.ipynb
│   ├── Test Output Callbacks.ipynb
│   ├── text_classification.ipynb
│   ├── text_classification_rnn.ipynb
│   ├── text_classification_with_hub.ipynb
│   ├── text_generation.ipynb
│   ├── text.ipynb
│   ├── tf_numpy.ipynb
│   ├── tfrecord.ipynb
│   ├── tf_records.ipynb
│   ├── Third Party Rich Output.ipynb
│   ├── time_series.ipynb
│   ├── Tissu_cellulaire.ipynb
│   ├── TP1__OCaml.ipynb
│   ├── TP1__python.ipynb
│   ├── TP2__OCaml.ipynb
│   ├── TP2__python.ipynb
│   ├── TP3__OCaml.ipynb
│   ├── TP3__python.ipynb
│   ├── TP4__OCaml.ipynb
│   ├── TP5__OCaml.ipynb
│   ├── TP6__OCaml.ipynb
│   ├── TP7__OCaml.ipynb
│   ├── TP8__Prolog.ipynb
│   ├── tpu_custom_training.ipynb
│   ├── tpu.ipynb
│   ├── training_loops.ipynb
│   ├── transfer_learning.ipynb
│   ├── transfer_learning_with_hub.ipynb
│   ├── transformer.ipynb
│   ├── Trapezoid Rule.ipynb
│   ├── Tri_a_bulle_et_tri_cocktail.ipynb
│   ├── Tri tricolore.ipynb
│   ├── Tutorial on head and tail (bash).ipynb
│   ├── Typesetting Equations.ipynb
│   ├── __Understanding the Reaction to Amazon Prime Air.ipynb
│   ├── unicode.ipynb
│   ├── Untitled-checkpoint.ipynb
│   ├── Untitled.ipynb
│   ├── Updating Displays.ipynb
│   ├── upgrade.ipynb
│   ├── Useful_Info_Notebook-checkpoint.ipynb
│   ├── Useful_Info_Notebook.ipynb
│   ├── User Input.ipynb
│   ├── Using Interact.ipynb
│   ├── Variable Inspector.ipynb
│   ├── variable.ipynb
│   ├── Variables.ipynb
│   ├── var_string_num.ipynb
│   ├── venv.ipynb
│   ├── visualization_earthquakes.ipynb
│   ├── WalkHomeAnalysis.ipynb
│   ├── web_apps.ipynb
│   ├── __What do Tim O'Reilly, Lady Gaga, and Marissa Mayer all have in common.ipynb
│   ├── What is the Jupyter Notebook.ipynb
│   ├── while_input.ipynb
│   ├── While Loops.ipynb
│   ├── Widget Alignment.ipynb
│   ├── Widget Asynchronous.ipynb
│   ├── Widget Basics.ipynb
│   ├── Widget Events.ipynb
│   ├── Widget List.ipynb
│   ├── Widget Low Level.ipynb
│   ├── Widget Styling.ipynb
│   ├── Widget TagsInput.ipynb
│   ├── Windowing.ipynb
│   ├── Windowing_Part2.ipynb
│   ├── Windowing_Part3.ipynb
│   ├── word2vec.ipynb
│   ├── word_embeddings.ipynb
│   ├── Working With External Code.ipynb
│   ├── Working With Markdown Cells.ipynb
│   └── world_series_analysis.ipynb
├── _Learning
│   ├── 00thread_lecture.py
│   ├── 01-basics.py
│   ├── 01_deques_intro.py
│   ├── 01_intro_to_queues.py
│   ├── 01-lists-intro.py
│   ├── 01_lists_intro.py
│   ├── 01locks.py
│   ├── 01_stacks_intro.py
│   ├── 01_summation.py
│   ├── 01_thread_intro.py
│   ├── 02-arithmetic.py
│   ├── 02_big_o.py
│   ├── 02deadlock.py
│   ├── 02_deque_implementation.py
│   ├── 02-lists-mutability-and-sorting.py
│   ├── 02_queue_implementation.py
│   ├── 02_stacks_implementation.py
│   ├── 02_thread_with_sleep.py
│   ├── 02_unordered_lists.py
│   ├── 03_anagram.py
│   ├── 03_balanced_parenthesis.py
│   ├── 03_daemon_threads.py
│   ├── 03_hot_potato.py
│   ├── 03-input-and-format.py
│   ├── 03-lists-math.py
│   ├── 03_palindrome.py
│   ├── 04b-complex-try-blocks.py
│   ├── 04_converting_bases.py
│   ├── 04-ducktyping.py
│   ├── 04_joining_daemon_threads.py
│   ├── 04-tuples-intro.py
│   ├── 04_type_performance.py
│   ├── 05b-extra-string-operations.py
│   ├── 05_importance_of_locks.py
│   ├── 05_prefix_postfix_infix.py
│   ├── 05-string-arithmetic.py
│   ├── 05-tuples-single-item.py
│   ├── 06-assignment.py
│   ├── 06_creating_locks.py
│   ├── 06-ranges.py
│   ├── 07b-logical-operators.py
│   ├── 07c-while-loops.py
│   ├── 07_deadlocks.py
│   ├── 07-dictionaries-intro.py
│   ├── 07-equality.py
│   ├── 08-dictionaries-more-ways.py
│   ├── 08-identity.py
│   ├── 09-functions.py
│   ├── 09-sets-intro.py
│   ├── 0Intro2python-all.py
│   ├── 10-functions-returning-functions.py
│   ├── 10-sets-usage.py
│   ├── 11b-stackqueue.py
│   ├── 11-combining-structures.py
│   ├── 12-built-ins-all-any.py
│   ├── 13-built-ins-filter-map-zip.py
│   ├── 14-built-ins-custom-sort.py
│   ├── 15a-input-validation1.py
│   ├── 15b-input-validation2.py
│   ├── 20-http-request.py
│   ├── examples
│   │   ├── args_example_1.py
│   │   ├── args_example.py
│   │   ├── class_animal_attributes_examples.py
│   │   ├── class_example_movies.py
│   │   ├── conditionals_examples.py
│   │   ├── FileRegexExample.py
│   │   ├── ListExample.py
│   │   ├── mod_example.py
│   │   ├── python-example.py
│   │   └── tuplesExample.py
│   └── problems
│       ├── Module15ProblemToCodeSolutionReadTxtMessage.py
│       ├── problem_01_while_filter.py
│       ├── problem_01_while_loop.py
│       ├── problem_01_while_map.py
│       ├── problem_02_for_filter.py
│       ├── problem_02_for_loop.py
│       ├── problem_02_for_map.py
│       ├── problem_03_list_comprehension.py
│       ├── problem_03_my_comprehension.py
│       ├── problem_04_if_statement.py
│       ├── problem_04_if_statements.py
│       ├── problem_05_class.py
│       ├── problem_06_inheritance.py
│       ├── problem_07_dictionary.py
│       ├── problem_07_dict.py
│       ├── problem_08_built_in.py
│       ├── problem_08_builtins.py
│       ├── problem_09_get_set.py
│       ├── problem_09_getters_and_setters.py
│       ├── problem_10_func.py
│       ├── problem_10_functions.py
│       └── stock_span_problem.py
├── list-org-repo.py
├── makefile
├── _Math
│   ├── bytes-n-bits
│   │   ├── add_bitwise_operator.py
│   │   ├── bitmask.py
│   │   ├── bit_operation.py
│   │   ├── bytecode.py
│   │   ├── bytes_int_conversion.py
│   │   ├── flip_bit_longest_sequence.py
│   │   ├── has_alternative_bit.py
│   │   ├── insert_bit.py
│   │   ├── pydevd_collect_bytecode_info.py
│   │   ├── pydevd_modify_bytecode.py
│   │   ├── remove_bit.py
│   │   └── reverse_bits.py
│   ├── matrix
│   │   ├── cholesky_matrix_decomposition.py
│   │   ├── count_islands_in_matrix.py
│   │   ├── crout_matrix_decomposition.py
│   │   ├── identity_matrix_recognizer.py
│   │   ├── inverse_of_matrix.py
│   │   ├── matrix_chain_order.py
│   │   ├── matrix_class.py
│   │   ├── matrix_exponentiation.py
│   │   ├── matrix_inversion.py
│   │   ├── matrix_operation.py
│   │   ├── nth_fibonacci_using_matrix_exponentiation.py
│   │   └── rotate_matrix.py
│   └── prime
│       ├── prime_factors.py
│       ├── primelib.py
│       ├── prime_numbers.py
│       ├── primeNumbers.py
│       ├── prime_sieve_eratosthenes.py
│       └── primes_sieve_of_eratosthenes.py
├── node_modules
│   ├── jquery
│   │   ├── AUTHORS.txt
│   │   ├── bower.json
│   │   ├── dist
│   │   │   ├── jquery.js
│   │   │   ├── jquery.min.js
│   │   │   ├── jquery.min.map
│   │   │   ├── jquery.slim.js
│   │   │   ├── jquery.slim.min.js
│   │   │   └── jquery.slim.min.map
│   │   ├── external
│   │   │   └── sizzle
│   │   ├── LICENSE.txt
│   │   ├── package.json
│   │   ├── README.md
│   │   └── src
│   │       ├── ajax
│   │       ├── ajax.js
│   │       ├── attributes
│   │       ├── attributes.js
│   │       ├── callbacks.js
│   │       ├── core
│   │       ├── core.js
│   │       ├── css
│   │       ├── css.js
│   │       ├── data
│   │       ├── data.js
│   │       ├── deferred
│   │       ├── deferred.js
│   │       ├── deprecated
│   │       ├── deprecated.js
│   │       ├── dimensions.js
│   │       ├── effects
│   │       ├── effects.js
│   │       ├── event
│   │       ├── event.js
│   │       ├── exports
│   │       ├── jquery.js
│   │       ├── manipulation
│   │       ├── manipulation.js
│   │       ├── offset.js
│   │       ├── queue
│   │       ├── queue.js
│   │       ├── selector.js
│   │       ├── selector-native.js
│   │       ├── selector-sizzle.js
│   │       ├── serialize.js
│   │       ├── traversing
│   │       ├── traversing.js
│   │       ├── var
│   │       └── wrap.js
│   ├── loadashes6
│   │   ├── lodash
│   │   │   ├── add.js
│   │   │   ├── after.js
│   │   │   ├── _apply.js
│   │   │   ├── _arrayAggregator.js
│   │   │   ├── _arrayEach.js
│   │   │   ├── _arrayEachRight.js
│   │   │   ├── _arrayEvery.js
│   │   │   ├── _arrayFilter.js
│   │   │   ├── _arrayIncludes.js
│   │   │   ├── _arrayIncludesWith.js
│   │   │   ├── array.js
│   │   │   ├── _arrayLikeKeys.js
│   │   │   ├── _arrayMap.js
│   │   │   ├── _arrayPush.js
│   │   │   ├── _arrayReduce.js
│   │   │   ├── _arrayReduceRight.js
│   │   │   ├── _arraySample.js
│   │   │   ├── _arraySampleSize.js
│   │   │   ├── _arrayShuffle.js
│   │   │   ├── _arraySome.js
│   │   │   ├── ary.js
│   │   │   ├── _asciiSize.js
│   │   │   ├── _asciiToArray.js
│   │   │   ├── _asciiWords.js
│   │   │   ├── assignIn.js
│   │   │   ├── assignInWith.js
│   │   │   ├── assign.js
│   │   │   ├── _assignMergeValue.js
│   │   │   ├── _assignValue.js
│   │   │   ├── assignWith.js
│   │   │   ├── _assocIndexOf.js
│   │   │   ├── at.js
│   │   │   ├── attempt.js
│   │   │   ├── _baseAggregator.js
│   │   │   ├── _baseAssignIn.js
│   │   │   ├── _baseAssign.js
│   │   │   ├── _baseAssignValue.js
│   │   │   ├── _baseAt.js
│   │   │   ├── _baseClamp.js
│   │   │   ├── _baseClone.js
│   │   │   ├── _baseConforms.js
│   │   │   ├── _baseConformsTo.js
│   │   │   ├── _baseCreate.js
│   │   │   ├── _baseDelay.js
│   │   │   ├── _baseDifference.js
│   │   │   ├── _baseEach.js
│   │   │   ├── _baseEachRight.js
│   │   │   ├── _baseEvery.js
│   │   │   ├── _baseExtremum.js
│   │   │   ├── _baseFill.js
│   │   │   ├── _baseFilter.js
│   │   │   ├── _baseFindIndex.js
│   │   │   ├── _baseFindKey.js
│   │   │   ├── _baseFlatten.js
│   │   │   ├── _baseFor.js
│   │   │   ├── _baseForOwn.js
│   │   │   ├── _baseForOwnRight.js
│   │   │   ├── _baseForRight.js
│   │   │   ├── _baseFunctions.js
│   │   │   ├── _baseGetAllKeys.js
│   │   │   ├── _baseGet.js
│   │   │   ├── _baseGetTag.js
│   │   │   ├── _baseGt.js
│   │   │   ├── _baseHasIn.js
│   │   │   ├── _baseHas.js
│   │   │   ├── _baseIndexOf.js
│   │   │   ├── _baseIndexOfWith.js
│   │   │   ├── _baseInRange.js
│   │   │   ├── _baseIntersection.js
│   │   │   ├── _baseInverter.js
│   │   │   ├── _baseInvoke.js
│   │   │   ├── _baseIsArguments.js
│   │   │   ├── _baseIsArrayBuffer.js
│   │   │   ├── _baseIsDate.js
│   │   │   ├── _baseIsEqualDeep.js
│   │   │   ├── _baseIsEqual.js
│   │   │   ├── _baseIsMap.js
│   │   │   ├── _baseIsMatch.js
│   │   │   ├── _baseIsNaN.js
│   │   │   ├── _baseIsNative.js
│   │   │   ├── _baseIsRegExp.js
│   │   │   ├── _baseIsSet.js
│   │   │   ├── _baseIsTypedArray.js
│   │   │   ├── _baseIteratee.js
│   │   │   ├── _baseKeysIn.js
│   │   │   ├── _baseKeys.js
│   │   │   ├── _baseLodash.js
│   │   │   ├── _baseLt.js
│   │   │   ├── _baseMap.js
│   │   │   ├── _baseMatches.js
│   │   │   ├── _baseMatchesProperty.js
│   │   │   ├── _baseMean.js
│   │   │   ├── _baseMergeDeep.js
│   │   │   ├── _baseMerge.js
│   │   │   ├── _baseNth.js
│   │   │   ├── _baseOrderBy.js
│   │   │   ├── _basePickBy.js
│   │   │   ├── _basePick.js
│   │   │   ├── _basePropertyDeep.js
│   │   │   ├── _baseProperty.js
│   │   │   ├── _basePropertyOf.js
│   │   │   ├── _basePullAll.js
│   │   │   ├── _basePullAt.js
│   │   │   ├── _baseRandom.js
│   │   │   ├── _baseRange.js
│   │   │   ├── _baseReduce.js
│   │   │   ├── _baseRepeat.js
│   │   │   ├── _baseRest.js
│   │   │   ├── _baseSample.js
│   │   │   ├── _baseSampleSize.js
│   │   │   ├── _baseSetData.js
│   │   │   ├── _baseSet.js
│   │   │   ├── _baseSetToString.js
│   │   │   ├── _baseShuffle.js
│   │   │   ├── _baseSlice.js
│   │   │   ├── _baseSome.js
│   │   │   ├── _baseSortBy.js
│   │   │   ├── _baseSortedIndexBy.js
│   │   │   ├── _baseSortedIndex.js
│   │   │   ├── _baseSortedUniq.js
│   │   │   ├── _baseSum.js
│   │   │   ├── _baseTimes.js
│   │   │   ├── _baseToNumber.js
│   │   │   ├── _baseToPairs.js
│   │   │   ├── _baseToString.js
│   │   │   ├── _baseTrim.js
│   │   │   ├── _baseUnary.js
│   │   │   ├── _baseUniq.js
│   │   │   ├── _baseUnset.js
│   │   │   ├── _baseUpdate.js
│   │   │   ├── _baseValues.js
│   │   │   ├── _baseWhile.js
│   │   │   ├── _baseWrapperValue.js
│   │   │   ├── _baseXor.js
│   │   │   ├── _baseZipObject.js
│   │   │   ├── before.js
│   │   │   ├── bindAll.js
│   │   │   ├── bind.js
│   │   │   ├── bindKey.js
│   │   │   ├── _cacheHas.js
│   │   │   ├── camelCase.js
│   │   │   ├── capitalize.js
│   │   │   ├── castArray.js
│   │   │   ├── _castArrayLikeObject.js
│   │   │   ├── _castFunction.js
│   │   │   ├── _castPath.js
│   │   │   ├── _castRest.js
│   │   │   ├── _castSlice.js
│   │   │   ├── ceil.js
│   │   │   ├── chain.js
│   │   │   ├── _charsEndIndex.js
│   │   │   ├── _charsStartIndex.js
│   │   │   ├── chunk.js
│   │   │   ├── clamp.js
│   │   │   ├── _cloneArrayBuffer.js
│   │   │   ├── _cloneBuffer.js
│   │   │   ├── _cloneDataView.js
│   │   │   ├── cloneDeep.js
│   │   │   ├── cloneDeepWith.js
│   │   │   ├── clone.js
│   │   │   ├── _cloneRegExp.js
│   │   │   ├── _cloneSymbol.js
│   │   │   ├── _cloneTypedArray.js
│   │   │   ├── cloneWith.js
│   │   │   ├── collection.js
│   │   │   ├── commit.js
│   │   │   ├── compact.js
│   │   │   ├── _compareAscending.js
│   │   │   ├── _compareMultiple.js
│   │   │   ├── _composeArgs.js
│   │   │   ├── _composeArgsRight.js
│   │   │   ├── concat.js
│   │   │   ├── cond.js
│   │   │   ├── conforms.js
│   │   │   ├── conformsTo.js
│   │   │   ├── constant.js
│   │   │   ├── _copyArray.js
│   │   │   ├── _copyObject.js
│   │   │   ├── _copySymbolsIn.js
│   │   │   ├── _copySymbols.js
│   │   │   ├── core.js
│   │   │   ├── _coreJsData.js
│   │   │   ├── core.min.js
│   │   │   ├── countBy.js
│   │   │   ├── _countHolders.js
│   │   │   ├── _createAggregator.js
│   │   │   ├── _createAssigner.js
│   │   │   ├── _createBaseEach.js
│   │   │   ├── _createBaseFor.js
│   │   │   ├── _createBind.js
│   │   │   ├── _createCaseFirst.js
│   │   │   ├── _createCompounder.js
│   │   │   ├── _createCtor.js
│   │   │   ├── _createCurry.js
│   │   │   ├── _createFind.js
│   │   │   ├── _createFlow.js
│   │   │   ├── _createHybrid.js
│   │   │   ├── _createInverter.js
│   │   │   ├── create.js
│   │   │   ├── _createMathOperation.js
│   │   │   ├── _createOver.js
│   │   │   ├── _createPadding.js
│   │   │   ├── _createPartial.js
│   │   │   ├── _createRange.js
│   │   │   ├── _createRecurry.js
│   │   │   ├── _createRelationalOperation.js
│   │   │   ├── _createRound.js
│   │   │   ├── _createSet.js
│   │   │   ├── _createToPairs.js
│   │   │   ├── _createWrap.js
│   │   │   ├── curry.js
│   │   │   ├── curryRight.js
│   │   │   ├── _customDefaultsAssignIn.js
│   │   │   ├── _customDefaultsMerge.js
│   │   │   ├── _customOmitClone.js
│   │   │   ├── _DataView.js
│   │   │   ├── date.js
│   │   │   ├── debounce.js
│   │   │   ├── deburr.js
│   │   │   ├── _deburrLetter.js
│   │   │   ├── defaultsDeep.js
│   │   │   ├── defaults.js
│   │   │   ├── defaultTo.js
│   │   │   ├── defer.js
│   │   │   ├── _defineProperty.js
│   │   │   ├── delay.js
│   │   │   ├── differenceBy.js
│   │   │   ├── difference.js
│   │   │   ├── differenceWith.js
│   │   │   ├── divide.js
│   │   │   ├── drop.js
│   │   │   ├── dropRight.js
│   │   │   ├── dropRightWhile.js
│   │   │   ├── dropWhile.js
│   │   │   ├── each.js
│   │   │   ├── eachRight.js
│   │   │   ├── endsWith.js
│   │   │   ├── entriesIn.js
│   │   │   ├── entries.js
│   │   │   ├── eq.js
│   │   │   ├── _equalArrays.js
│   │   │   ├── _equalByTag.js
│   │   │   ├── _equalObjects.js
│   │   │   ├── _escapeHtmlChar.js
│   │   │   ├── escape.js
│   │   │   ├── escapeRegExp.js
│   │   │   ├── _escapeStringChar.js
│   │   │   ├── every.js
│   │   │   ├── extend.js
│   │   │   ├── extendWith.js
│   │   │   ├── fill.js
│   │   │   ├── filter.js
│   │   │   ├── findIndex.js
│   │   │   ├── find.js
│   │   │   ├── findKey.js
│   │   │   ├── findLastIndex.js
│   │   │   ├── findLast.js
│   │   │   ├── findLastKey.js
│   │   │   ├── first.js
│   │   │   ├── flatMapDeep.js
│   │   │   ├── flatMapDepth.js
│   │   │   ├── flatMap.js
│   │   │   ├── _flatRest.js
│   │   │   ├── flattenDeep.js
│   │   │   ├── flattenDepth.js
│   │   │   ├── flatten.js
│   │   │   ├── flip.js
│   │   │   ├── floor.js
│   │   │   ├── flow.js
│   │   │   ├── flowRight.js
│   │   │   ├── forEach.js
│   │   │   ├── forEachRight.js
│   │   │   ├── forIn.js
│   │   │   ├── forInRight.js
│   │   │   ├── forOwn.js
│   │   │   ├── forOwnRight.js
│   │   │   ├── fp
│   │   │   ├── fp.js
│   │   │   ├── _freeGlobal.js
│   │   │   ├── fromPairs.js
│   │   │   ├── function.js
│   │   │   ├── functionsIn.js
│   │   │   ├── functions.js
│   │   │   ├── _getAllKeysIn.js
│   │   │   ├── _getAllKeys.js
│   │   │   ├── _getData.js
│   │   │   ├── _getFuncName.js
│   │   │   ├── _getHolder.js
│   │   │   ├── get.js
│   │   │   ├── _getMapData.js
│   │   │   ├── _getMatchData.js
│   │   │   ├── _getNative.js
│   │   │   ├── _getPrototype.js
│   │   │   ├── _getRawTag.js
│   │   │   ├── _getSymbolsIn.js
│   │   │   ├── _getSymbols.js
│   │   │   ├── _getTag.js
│   │   │   ├── _getValue.js
│   │   │   ├── _getView.js
│   │   │   ├── _getWrapDetails.js
│   │   │   ├── groupBy.js
│   │   │   ├── gte.js
│   │   │   ├── gt.js
│   │   │   ├── _hashClear.js
│   │   │   ├── _hashDelete.js
│   │   │   ├── _hashGet.js
│   │   │   ├── _hashHas.js
│   │   │   ├── _Hash.js
│   │   │   ├── _hashSet.js
│   │   │   ├── hasIn.js
│   │   │   ├── has.js
│   │   │   ├── _hasPath.js
│   │   │   ├── _hasUnicode.js
│   │   │   ├── _hasUnicodeWord.js
│   │   │   ├── head.js
│   │   │   ├── identity.js
│   │   │   ├── includes.js
│   │   │   ├── index.js
│   │   │   ├── indexOf.js
│   │   │   ├── _initCloneArray.js
│   │   │   ├── _initCloneByTag.js
│   │   │   ├── _initCloneObject.js
│   │   │   ├── initial.js
│   │   │   ├── inRange.js
│   │   │   ├── _insertWrapDetails.js
│   │   │   ├── intersectionBy.js
│   │   │   ├── intersection.js
│   │   │   ├── intersectionWith.js
│   │   │   ├── invertBy.js
│   │   │   ├── invert.js
│   │   │   ├── invoke.js
│   │   │   ├── invokeMap.js
│   │   │   ├── isArguments.js
│   │   │   ├── isArrayBuffer.js
│   │   │   ├── isArray.js
│   │   │   ├── isArrayLike.js
│   │   │   ├── isArrayLikeObject.js
│   │   │   ├── isBoolean.js
│   │   │   ├── isBuffer.js
│   │   │   ├── isDate.js
│   │   │   ├── isElement.js
│   │   │   ├── isEmpty.js
│   │   │   ├── isEqual.js
│   │   │   ├── isEqualWith.js
│   │   │   ├── isError.js
│   │   │   ├── isFinite.js
│   │   │   ├── _isFlattenable.js
│   │   │   ├── isFunction.js
│   │   │   ├── _isIndex.js
│   │   │   ├── isInteger.js
│   │   │   ├── _isIterateeCall.js
│   │   │   ├── _isKeyable.js
│   │   │   ├── _isKey.js
│   │   │   ├── _isLaziable.js
│   │   │   ├── isLength.js
│   │   │   ├── isMap.js
│   │   │   ├── _isMaskable.js
│   │   │   ├── _isMasked.js
│   │   │   ├── isMatch.js
│   │   │   ├── isMatchWith.js
│   │   │   ├── isNaN.js
│   │   │   ├── isNative.js
│   │   │   ├── isNil.js
│   │   │   ├── isNull.js
│   │   │   ├── isNumber.js
│   │   │   ├── isObject.js
│   │   │   ├── isObjectLike.js
│   │   │   ├── isPlainObject.js
│   │   │   ├── _isPrototype.js
│   │   │   ├── isRegExp.js
│   │   │   ├── isSafeInteger.js
│   │   │   ├── isSet.js
│   │   │   ├── _isStrictComparable.js
│   │   │   ├── isString.js
│   │   │   ├── isSymbol.js
│   │   │   ├── isTypedArray.js
│   │   │   ├── isUndefined.js
│   │   │   ├── isWeakMap.js
│   │   │   ├── isWeakSet.js
│   │   │   ├── iteratee.js
│   │   │   ├── _iteratorToArray.js
│   │   │   ├── join.js
│   │   │   ├── kebabCase.js
│   │   │   ├── keyBy.js
│   │   │   ├── keysIn.js
│   │   │   ├── keys.js
│   │   │   ├── lang.js
│   │   │   ├── lastIndexOf.js
│   │   │   ├── last.js
│   │   │   ├── _lazyClone.js
│   │   │   ├── _lazyReverse.js
│   │   │   ├── _lazyValue.js
│   │   │   ├── _LazyWrapper.js
│   │   │   ├── _listCacheClear.js
│   │   │   ├── _listCacheDelete.js
│   │   │   ├── _listCacheGet.js
│   │   │   ├── _listCacheHas.js
│   │   │   ├── _ListCache.js
│   │   │   ├── _listCacheSet.js
│   │   │   ├── lodash.js
│   │   │   ├── lodash.min.js
│   │   │   ├── _LodashWrapper.js
│   │   │   ├── lowerCase.js
│   │   │   ├── lowerFirst.js
│   │   │   ├── lte.js
│   │   │   ├── lt.js
│   │   │   ├── _mapCacheClear.js
│   │   │   ├── _mapCacheDelete.js
│   │   │   ├── _mapCacheGet.js
│   │   │   ├── _mapCacheHas.js
│   │   │   ├── _MapCache.js
│   │   │   ├── _mapCacheSet.js
│   │   │   ├── map.js
│   │   │   ├── _Map.js
│   │   │   ├── mapKeys.js
│   │   │   ├── _mapToArray.js
│   │   │   ├── mapValues.js
│   │   │   ├── matches.js
│   │   │   ├── matchesProperty.js
│   │   │   ├── _matchesStrictComparable.js
│   │   │   ├── math.js
│   │   │   ├── maxBy.js
│   │   │   ├── max.js
│   │   │   ├── meanBy.js
│   │   │   ├── mean.js
│   │   │   ├── _memoizeCapped.js
│   │   │   ├── memoize.js
│   │   │   ├── _mergeData.js
│   │   │   ├── merge.js
│   │   │   ├── mergeWith.js
│   │   │   ├── _metaMap.js
│   │   │   ├── method.js
│   │   │   ├── methodOf.js
│   │   │   ├── minBy.js
│   │   │   ├── min.js
│   │   │   ├── mixin.js
│   │   │   ├── multiply.js
│   │   │   ├── _nativeCreate.js
│   │   │   ├── _nativeKeysIn.js
│   │   │   ├── _nativeKeys.js
│   │   │   ├── negate.js
│   │   │   ├── next.js
│   │   │   ├── _nodeUtil.js
│   │   │   ├── noop.js
│   │   │   ├── now.js
│   │   │   ├── nthArg.js
│   │   │   ├── nth.js
│   │   │   ├── number.js
│   │   │   ├── object.js
│   │   │   ├── _objectToString.js
│   │   │   ├── omitBy.js
│   │   │   ├── omit.js
│   │   │   ├── once.js
│   │   │   ├── orderBy.js
│   │   │   ├── _overArg.js
│   │   │   ├── overArgs.js
│   │   │   ├── overEvery.js
│   │   │   ├── over.js
│   │   │   ├── _overRest.js
│   │   │   ├── overSome.js
│   │   │   ├── padEnd.js
│   │   │   ├── pad.js
│   │   │   ├── padStart.js
│   │   │   ├── _parent.js
│   │   │   ├── parseInt.js
│   │   │   ├── partial.js
│   │   │   ├── partialRight.js
│   │   │   ├── partition.js
│   │   │   ├── pickBy.js
│   │   │   ├── pick.js
│   │   │   ├── plant.js
│   │   │   ├── _Promise.js
│   │   │   ├── property.js
│   │   │   ├── propertyOf.js
│   │   │   ├── pullAllBy.js
│   │   │   ├── pullAll.js
│   │   │   ├── pullAllWith.js
│   │   │   ├── pullAt.js
│   │   │   ├── pull.js
│   │   │   ├── random.js
│   │   │   ├── range.js
│   │   │   ├── rangeRight.js
│   │   │   ├── README.md
│   │   │   ├── _realNames.js
│   │   │   ├── rearg.js
│   │   │   ├── reduce.js
│   │   │   ├── reduceRight.js
│   │   │   ├── _reEscape.js
│   │   │   ├── _reEvaluate.js
│   │   │   ├── _reInterpolate.js
│   │   │   ├── reject.js
│   │   │   ├── release.md
│   │   │   ├── remove.js
│   │   │   ├── _reorder.js
│   │   │   ├── repeat.js
│   │   │   ├── _replaceHolders.js
│   │   │   ├── replace.js
│   │   │   ├── rest.js
│   │   │   ├── result.js
│   │   │   ├── reverse.js
│   │   │   ├── _root.js
│   │   │   ├── round.js
│   │   │   ├── _safeGet.js
│   │   │   ├── sample.js
│   │   │   ├── sampleSize.js
│   │   │   ├── seq.js
│   │   │   ├── _setCacheAdd.js
│   │   │   ├── _setCacheHas.js
│   │   │   ├── _SetCache.js
│   │   │   ├── _setData.js
│   │   │   ├── set.js
│   │   │   ├── _Set.js
│   │   │   ├── _setToArray.js
│   │   │   ├── _setToPairs.js
│   │   │   ├── _setToString.js
│   │   │   ├── setWith.js
│   │   │   ├── _setWrapToString.js
│   │   │   ├── _shortOut.js
│   │   │   ├── shuffle.js
│   │   │   ├── _shuffleSelf.js
│   │   │   ├── size.js
│   │   │   ├── slice.js
│   │   │   ├── snakeCase.js
│   │   │   ├── some.js
│   │   │   ├── sortBy.js
│   │   │   ├── sortedIndexBy.js
│   │   │   ├── sortedIndex.js
│   │   │   ├── sortedIndexOf.js
│   │   │   ├── sortedLastIndexBy.js
│   │   │   ├── sortedLastIndex.js
│   │   │   ├── sortedLastIndexOf.js
│   │   │   ├── sortedUniqBy.js
│   │   │   ├── sortedUniq.js
│   │   │   ├── split.js
│   │   │   ├── spread.js
│   │   │   ├── _stackClear.js
│   │   │   ├── _stackDelete.js
│   │   │   ├── _stackGet.js
│   │   │   ├── _stackHas.js
│   │   │   ├── _Stack.js
│   │   │   ├── _stackSet.js
│   │   │   ├── startCase.js
│   │   │   ├── startsWith.js
│   │   │   ├── _strictIndexOf.js
│   │   │   ├── _strictLastIndexOf.js
│   │   │   ├── string.js
│   │   │   ├── _stringSize.js
│   │   │   ├── _stringToArray.js
│   │   │   ├── _stringToPath.js
│   │   │   ├── stubArray.js
│   │   │   ├── stubFalse.js
│   │   │   ├── stubObject.js
│   │   │   ├── stubString.js
│   │   │   ├── stubTrue.js
│   │   │   ├── subtract.js
│   │   │   ├── sumBy.js
│   │   │   ├── sum.js
│   │   │   ├── _Symbol.js
│   │   │   ├── tail.js
│   │   │   ├── take.js
│   │   │   ├── takeRight.js
│   │   │   ├── takeRightWhile.js
│   │   │   ├── takeWhile.js
│   │   │   ├── tap.js
│   │   │   ├── template.js
│   │   │   ├── templateSettings.js
│   │   │   ├── throttle.js
│   │   │   ├── thru.js
│   │   │   ├── times.js
│   │   │   ├── toArray.js
│   │   │   ├── toFinite.js
│   │   │   ├── toInteger.js
│   │   │   ├── toIterator.js
│   │   │   ├── toJSON.js
│   │   │   ├── _toKey.js
│   │   │   ├── toLength.js
│   │   │   ├── toLower.js
│   │   │   ├── toNumber.js
│   │   │   ├── toPairsIn.js
│   │   │   ├── toPairs.js
│   │   │   ├── toPath.js
│   │   │   ├── toPlainObject.js
│   │   │   ├── toSafeInteger.js
│   │   │   ├── _toSource.js
│   │   │   ├── toString.js
│   │   │   ├── toUpper.js
│   │   │   ├── transform.js
│   │   │   ├── trimEnd.js
│   │   │   ├── trim.js
│   │   │   ├── _trimmedEndIndex.js
│   │   │   ├── trimStart.js
│   │   │   ├── truncate.js
│   │   │   ├── _Uint8Array.js
│   │   │   ├── unary.js
│   │   │   ├── _unescapeHtmlChar.js
│   │   │   ├── unescape.js
│   │   │   ├── _unicodeSize.js
│   │   │   ├── _unicodeToArray.js
│   │   │   ├── _unicodeWords.js
│   │   │   ├── unionBy.js
│   │   │   ├── union.js
│   │   │   ├── unionWith.js
│   │   │   ├── uniqBy.js
│   │   │   ├── uniq.js
│   │   │   ├── uniqueId.js
│   │   │   ├── uniqWith.js
│   │   │   ├── unset.js
│   │   │   ├── unzip.js
│   │   │   ├── unzipWith.js
│   │   │   ├── update.js
│   │   │   ├── updateWith.js
│   │   │   ├── _updateWrapDetails.js
│   │   │   ├── upperCase.js
│   │   │   ├── upperFirst.js
│   │   │   ├── util.js
│   │   │   ├── value.js
│   │   │   ├── valueOf.js
│   │   │   ├── valuesIn.js
│   │   │   ├── values.js
│   │   │   ├── _WeakMap.js
│   │   │   ├── without.js
│   │   │   ├── words.js
│   │   │   ├── wrap.js
│   │   │   ├── wrapperAt.js
│   │   │   ├── wrapperChain.js
│   │   │   ├── _wrapperClone.js
│   │   │   ├── wrapperLodash.js
│   │   │   ├── wrapperReverse.js
│   │   │   ├── wrapperValue.js
│   │   │   ├── xorBy.js
│   │   │   ├── xor.js
│   │   │   ├── xorWith.js
│   │   │   ├── zip.js
│   │   │   ├── zipObjectDeep.js
│   │   │   ├── zipObject.js
│   │   │   └── zipWith.js
│   │   └── package.json
│   ├── mout
│   │   ├── array
│   │   │   ├── append.d.ts
│   │   │   ├── append.js
│   │   │   ├── collect.d.ts
│   │   │   ├── collect.js
│   │   │   ├── combine.d.ts
│   │   │   ├── combine.js
│   │   │   ├── compact.d.ts
│   │   │   ├── compact.js
│   │   │   ├── contains.d.ts
│   │   │   ├── contains.js
│   │   │   ├── difference.d.ts
│   │   │   ├── difference.js
│   │   │   ├── equals.d.ts
│   │   │   ├── equals.js
│   │   │   ├── every.d.ts
│   │   │   ├── every.js
│   │   │   ├── filter.d.ts
│   │   │   ├── filter.js
│   │   │   ├── find.d.ts
│   │   │   ├── findIndex.d.ts
│   │   │   ├── findIndex.js
│   │   │   ├── find.js
│   │   │   ├── findLast.d.ts
│   │   │   ├── findLastIndex.d.ts
│   │   │   ├── findLastIndex.js
│   │   │   ├── findLast.js
│   │   │   ├── flatten.d.ts
│   │   │   ├── flatten.js
│   │   │   ├── forEach.d.ts
│   │   │   ├── forEach.js
│   │   │   ├── groupBy.d.ts
│   │   │   ├── groupBy.js
│   │   │   ├── indexOf.d.ts
│   │   │   ├── indexOf.js
│   │   │   ├── indicesOf.d.ts
│   │   │   ├── indicesOf.js
│   │   │   ├── insert.d.ts
│   │   │   ├── insert.js
│   │   │   ├── intersection.d.ts
│   │   │   ├── intersection.js
│   │   │   ├── invoke.d.ts
│   │   │   ├── invoke.js
│   │   │   ├── join.d.ts
│   │   │   ├── join.js
│   │   │   ├── last.d.ts
│   │   │   ├── lastIndexOf.d.ts
│   │   │   ├── lastIndexOf.js
│   │   │   ├── last.js
│   │   │   ├── map.d.ts
│   │   │   ├── map.js
│   │   │   ├── max.d.ts
│   │   │   ├── max.js
│   │   │   ├── min.d.ts
│   │   │   ├── min.js
│   │   │   ├── pick.d.ts
│   │   │   ├── pick.js
│   │   │   ├── pluck.d.ts
│   │   │   ├── pluck.js
│   │   │   ├── range.d.ts
│   │   │   ├── range.js
│   │   │   ├── reduce.d.ts
│   │   │   ├── reduce.js
│   │   │   ├── reduceRight.d.ts
│   │   │   ├── reduceRight.js
│   │   │   ├── reject.d.ts
│   │   │   ├── reject.js
│   │   │   ├── removeAll.d.ts
│   │   │   ├── removeAll.js
│   │   │   ├── remove.d.ts
│   │   │   ├── remove.js
│   │   │   ├── repeat.d.ts
│   │   │   ├── repeat.js
│   │   │   ├── reverse.d.ts
│   │   │   ├── reverse.js
│   │   │   ├── shuffle.d.ts
│   │   │   ├── shuffle.js
│   │   │   ├── slice.d.ts
│   │   │   ├── slice.js
│   │   │   ├── some.d.ts
│   │   │   ├── some.js
│   │   │   ├── sortBy.d.ts
│   │   │   ├── sortBy.js
│   │   │   ├── sort.d.ts
│   │   │   ├── sort.js
│   │   │   ├── split.d.ts
│   │   │   ├── split.js
│   │   │   ├── take.d.ts
│   │   │   ├── take.js
│   │   │   ├── toLookup.d.ts
│   │   │   ├── toLookup.js
│   │   │   ├── union.d.ts
│   │   │   ├── union.js
│   │   │   ├── unique.d.ts
│   │   │   ├── unique.js
│   │   │   ├── xor.d.ts
│   │   │   ├── xor.js
│   │   │   ├── zip.d.ts
│   │   │   └── zip.js
│   │   ├── array.d.ts
│   │   ├── array.js
│   │   ├── CHANGELOG.md
│   │   ├── collection
│   │   │   ├── contains.d.ts
│   │   │   ├── contains.js
│   │   │   ├── every.d.ts
│   │   │   ├── every.js
│   │   │   ├── filter.d.ts
│   │   │   ├── filter.js
│   │   │   ├── find.d.ts
│   │   │   ├── find.js
│   │   │   ├── forEach.d.ts
│   │   │   ├── forEach.js
│   │   │   ├── make_.d.ts
│   │   │   ├── make_.js
│   │   │   ├── map.d.ts
│   │   │   ├── map.js
│   │   │   ├── max.d.ts
│   │   │   ├── max.js
│   │   │   ├── min.d.ts
│   │   │   ├── min.js
│   │   │   ├── pluck.d.ts
│   │   │   ├── pluck.js
│   │   │   ├── reduce.d.ts
│   │   │   ├── reduce.js
│   │   │   ├── reject.d.ts
│   │   │   ├── reject.js
│   │   │   ├── size.d.ts
│   │   │   ├── size.js
│   │   │   ├── some.d.ts
│   │   │   └── some.js
│   │   ├── collection.d.ts
│   │   ├── collection.js
│   │   ├── CONTRIBUTING.md
│   │   ├── date
│   │   │   ├── dayOfTheYear.d.ts
│   │   │   ├── dayOfTheYear.js
│   │   │   ├── diff.d.ts
│   │   │   ├── diff.js
│   │   │   ├── i18n
│   │   │   ├── i18n_.d.ts
│   │   │   ├── i18n_.js
│   │   │   ├── isLeapYear.d.ts
│   │   │   ├── isLeapYear.js
│   │   │   ├── isSame.d.ts
│   │   │   ├── isSame.js
│   │   │   ├── parseIso.d.ts
│   │   │   ├── parseIso.js
│   │   │   ├── quarter.d.ts
│   │   │   ├── quarter.js
│   │   │   ├── startOf.d.ts
│   │   │   ├── startOf.js
│   │   │   ├── strftime.d.ts
│   │   │   ├── strftime.js
│   │   │   ├── timezoneAbbr.d.ts
│   │   │   ├── timezoneAbbr.js
│   │   │   ├── timezoneOffset.d.ts
│   │   │   ├── timezoneOffset.js
│   │   │   ├── totalDaysInMonth.d.ts
│   │   │   ├── totalDaysInMonth.js
│   │   │   ├── totalDaysInYear.d.ts
│   │   │   ├── totalDaysInYear.js
│   │   │   ├── weekOfTheYear.d.ts
│   │   │   └── weekOfTheYear.js
│   │   ├── date.d.ts
│   │   ├── date.js
│   │   ├── doc
│   │   │   ├── array.md
│   │   │   ├── collection.md
│   │   │   ├── date.md
│   │   │   ├── function.md
│   │   │   ├── lang.md
│   │   │   ├── math.md
│   │   │   ├── number.md
│   │   │   ├── object.md
│   │   │   ├── queryString.md
│   │   │   ├── random.md
│   │   │   ├── string.md
│   │   │   └── time.md
│   │   ├── function
│   │   │   ├── after.d.ts
│   │   │   ├── after.js
│   │   │   ├── awaitDelay.d.ts
│   │   │   ├── awaitDelay.js
│   │   │   ├── bind.d.ts
│   │   │   ├── bind.js
│   │   │   ├── compose.d.ts
│   │   │   ├── compose.js
│   │   │   ├── constant.d.ts
│   │   │   ├── constant.js
│   │   │   ├── debounce.d.ts
│   │   │   ├── debounce.js
│   │   │   ├── func.d.ts
│   │   │   ├── func.js
│   │   │   ├── identity.d.ts
│   │   │   ├── identity.js
│   │   │   ├── makeIterator_.d.ts
│   │   │   ├── makeIterator_.js
│   │   │   ├── memoize.d.ts
│   │   │   ├── memoize.js
│   │   │   ├── partial.d.ts
│   │   │   ├── partial.js
│   │   │   ├── prop.d.ts
│   │   │   ├── prop.js
│   │   │   ├── series.d.ts
│   │   │   ├── series.js
│   │   │   ├── throttle.d.ts
│   │   │   ├── throttle.js
│   │   │   ├── timeout.d.ts
│   │   │   ├── timeout.js
│   │   │   ├── times.d.ts
│   │   │   ├── times.js
│   │   │   ├── wrap.d.ts
│   │   │   └── wrap.js
│   │   ├── function.d.ts
│   │   ├── function.js
│   │   ├── index.d.ts
│   │   ├── index.js
│   │   ├── lang
│   │   │   ├── clone.d.ts
│   │   │   ├── clone.js
│   │   │   ├── createObject.d.ts
│   │   │   ├── createObject.js
│   │   │   ├── ctorApply.d.ts
│   │   │   ├── ctorApply.js
│   │   │   ├── deepClone.d.ts
│   │   │   ├── deepClone.js
│   │   │   ├── deepEquals.d.ts
│   │   │   ├── deepEquals.js
│   │   │   ├── defaults.d.ts
│   │   │   ├── defaults.js
│   │   │   ├── GLOBAL.d.ts
│   │   │   ├── GLOBAL.js
│   │   │   ├── inheritPrototype.d.ts
│   │   │   ├── inheritPrototype.js
│   │   │   ├── isArguments.d.ts
│   │   │   ├── isArguments.js
│   │   │   ├── isArray.d.ts
│   │   │   ├── isArray.js
│   │   │   ├── isBoolean.d.ts
│   │   │   ├── isBoolean.js
│   │   │   ├── isDate.d.ts
│   │   │   ├── isDate.js
│   │   │   ├── is.d.ts
│   │   │   ├── isEmpty.d.ts
│   │   │   ├── isEmpty.js
│   │   │   ├── isFinite.d.ts
│   │   │   ├── isFinite.js
│   │   │   ├── isFunction.d.ts
│   │   │   ├── isFunction.js
│   │   │   ├── isInteger.d.ts
│   │   │   ├── isInteger.js
│   │   │   ├── is.js
│   │   │   ├── isKind.d.ts
│   │   │   ├── isKind.js
│   │   │   ├── isNaN.d.ts
│   │   │   ├── isNaN.js
│   │   │   ├── isnt.d.ts
│   │   │   ├── isnt.js
│   │   │   ├── isNull.d.ts
│   │   │   ├── isNull.js
│   │   │   ├── isNumber.d.ts
│   │   │   ├── isNumber.js
│   │   │   ├── isObject.d.ts
│   │   │   ├── isObject.js
│   │   │   ├── isPlainObject.d.ts
│   │   │   ├── isPlainObject.js
│   │   │   ├── isPrimitive.d.ts
│   │   │   ├── isPrimitive.js
│   │   │   ├── isRegExp.d.ts
│   │   │   ├── isRegExp.js
│   │   │   ├── isString.d.ts
│   │   │   ├── isString.js
│   │   │   ├── isUndefined.d.ts
│   │   │   ├── isUndefined.js
│   │   │   ├── kindOf.d.ts
│   │   │   ├── kindOf.js
│   │   │   ├── toArray.d.ts
│   │   │   ├── toArray.js
│   │   │   ├── toNumber.d.ts
│   │   │   ├── toNumber.js
│   │   │   ├── toString.d.ts
│   │   │   └── toString.js
│   │   ├── lang.d.ts
│   │   ├── lang.js
│   │   ├── LICENSE.md
│   │   ├── math
│   │   │   ├── ceil.d.ts
│   │   │   ├── ceil.js
│   │   │   ├── clamp.d.ts
│   │   │   ├── clamp.js
│   │   │   ├── countSteps.d.ts
│   │   │   ├── countSteps.js
│   │   │   ├── floor.d.ts
│   │   │   ├── floor.js
│   │   │   ├── inRange.d.ts
│   │   │   ├── inRange.js
│   │   │   ├── isNear.d.ts
│   │   │   ├── isNear.js
│   │   │   ├── lerp.d.ts
│   │   │   ├── lerp.js
│   │   │   ├── loop.d.ts
│   │   │   ├── loop.js
│   │   │   ├── map.d.ts
│   │   │   ├── map.js
│   │   │   ├── norm.d.ts
│   │   │   ├── norm.js
│   │   │   ├── overflow.d.ts
│   │   │   ├── overflow.js
│   │   │   ├── round.d.ts
│   │   │   └── round.js
│   │   ├── math.d.ts
│   │   ├── math.js
│   │   ├── number
│   │   │   ├── abbreviate.d.ts
│   │   │   ├── abbreviate.js
│   │   │   ├── currencyFormat.d.ts
│   │   │   ├── currencyFormat.js
│   │   │   ├── enforcePrecision.d.ts
│   │   │   ├── enforcePrecision.js
│   │   │   ├── isNaN.d.ts
│   │   │   ├── isNaN.js
│   │   │   ├── MAX_INT.d.ts
│   │   │   ├── MAX_INT.js
│   │   │   ├── MAX_SAFE_INTEGER.d.ts
│   │   │   ├── MAX_SAFE_INTEGER.js
│   │   │   ├── MAX_UINT.d.ts
│   │   │   ├── MAX_UINT.js
│   │   │   ├── MIN_INT.d.ts
│   │   │   ├── MIN_INT.js
│   │   │   ├── nth.d.ts
│   │   │   ├── nth.js
│   │   │   ├── ordinal.d.ts
│   │   │   ├── ordinal.js
│   │   │   ├── pad.d.ts
│   │   │   ├── pad.js
│   │   │   ├── rol.d.ts
│   │   │   ├── rol.js
│   │   │   ├── ror.d.ts
│   │   │   ├── ror.js
│   │   │   ├── sign.d.ts
│   │   │   ├── sign.js
│   │   │   ├── toInt.d.ts
│   │   │   ├── toInt.js
│   │   │   ├── toUInt31.d.ts
│   │   │   ├── toUInt31.js
│   │   │   ├── toUInt.d.ts
│   │   │   └── toUInt.js
│   │   ├── number.d.ts
│   │   ├── number.js
│   │   ├── object
│   │   │   ├── bindAll.d.ts
│   │   │   ├── bindAll.js
│   │   │   ├── contains.d.ts
│   │   │   ├── contains.js
│   │   │   ├── deepFillIn.d.ts
│   │   │   ├── deepFillIn.js
│   │   │   ├── deepMatches.d.ts
│   │   │   ├── deepMatches.js
│   │   │   ├── deepMixIn.d.ts
│   │   │   ├── deepMixIn.js
│   │   │   ├── equals.d.ts
│   │   │   ├── equals.js
│   │   │   ├── every.d.ts
│   │   │   ├── every.js
│   │   │   ├── fillIn.d.ts
│   │   │   ├── fillIn.js
│   │   │   ├── filter.d.ts
│   │   │   ├── filter.js
│   │   │   ├── find.d.ts
│   │   │   ├── find.js
│   │   │   ├── flatten.d.ts
│   │   │   ├── flatten.js
│   │   │   ├── forIn.d.ts
│   │   │   ├── forIn.js
│   │   │   ├── forOwn.d.ts
│   │   │   ├── forOwn.js
│   │   │   ├── functions.d.ts
│   │   │   ├── functions.js
│   │   │   ├── get.d.ts
│   │   │   ├── get.js
│   │   │   ├── has.d.ts
│   │   │   ├── has.js
│   │   │   ├── hasOwn.d.ts
│   │   │   ├── hasOwn.js
│   │   │   ├── keys.d.ts
│   │   │   ├── keys.js
│   │   │   ├── map.d.ts
│   │   │   ├── map.js
│   │   │   ├── matches.d.ts
│   │   │   ├── matches.js
│   │   │   ├── max.d.ts
│   │   │   ├── max.js
│   │   │   ├── merge.d.ts
│   │   │   ├── merge.js
│   │   │   ├── min.d.ts
│   │   │   ├── min.js
│   │   │   ├── mixIn.d.ts
│   │   │   ├── mixIn.js
│   │   │   ├── namespace.d.ts
│   │   │   ├── namespace.js
│   │   │   ├── omit.d.ts
│   │   │   ├── omit.js
│   │   │   ├── pick.d.ts
│   │   │   ├── pick.js
│   │   │   ├── pluck.d.ts
│   │   │   ├── pluck.js
│   │   │   ├── reduce.d.ts
│   │   │   ├── reduce.js
│   │   │   ├── reject.d.ts
│   │   │   ├── reject.js
│   │   │   ├── result.d.ts
│   │   │   ├── result.js
│   │   │   ├── set.d.ts
│   │   │   ├── set.js
│   │   │   ├── size.d.ts
│   │   │   ├── size.js
│   │   │   ├── some.d.ts
│   │   │   ├── some.js
│   │   │   ├── unset.d.ts
│   │   │   ├── unset.js
│   │   │   ├── values.d.ts
│   │   │   └── values.js
│   │   ├── object.d.ts
│   │   ├── object.js
│   │   ├── package.json
│   │   ├── queryString
│   │   │   ├── contains.d.ts
│   │   │   ├── contains.js
│   │   │   ├── decode.d.ts
│   │   │   ├── decode.js
│   │   │   ├── encode.d.ts
│   │   │   ├── encode.js
│   │   │   ├── getParam.d.ts
│   │   │   ├── getParam.js
│   │   │   ├── getQuery.d.ts
│   │   │   ├── getQuery.js
│   │   │   ├── parse.d.ts
│   │   │   ├── parse.js
│   │   │   ├── setParam.d.ts
│   │   │   └── setParam.js
│   │   ├── queryString.d.ts
│   │   ├── queryString.js
│   │   ├── random
│   │   │   ├── choice.d.ts
│   │   │   ├── choice.js
│   │   │   ├── guid.d.ts
│   │   │   ├── guid.js
│   │   │   ├── randBit.d.ts
│   │   │   ├── randBit.js
│   │   │   ├── randBool.d.ts
│   │   │   ├── randBool.js
│   │   │   ├── rand.d.ts
│   │   │   ├── randHex.d.ts
│   │   │   ├── randHex.js
│   │   │   ├── randInt.d.ts
│   │   │   ├── randInt.js
│   │   │   ├── rand.js
│   │   │   ├── random.d.ts
│   │   │   ├── random.js
│   │   │   ├── randSign.d.ts
│   │   │   ├── randSign.js
│   │   │   ├── randString.d.ts
│   │   │   └── randString.js
│   │   ├── random.d.ts
│   │   ├── random.js
│   │   ├── README.md
│   │   ├── src
│   │   │   ├── array
│   │   │   ├── array.ts
│   │   │   ├── collection
│   │   │   ├── collection.ts
│   │   │   ├── date
│   │   │   ├── date.ts
│   │   │   ├── function
│   │   │   ├── function.ts
│   │   │   ├── index.ts
│   │   │   ├── lang
│   │   │   ├── lang.ts
│   │   │   ├── math
│   │   │   ├── math.ts
│   │   │   ├── number
│   │   │   ├── number.ts
│   │   │   ├── object
│   │   │   ├── object.ts
│   │   │   ├── queryString
│   │   │   ├── queryString.ts
│   │   │   ├── random
│   │   │   ├── random.ts
│   │   │   ├── string
│   │   │   ├── string.ts
│   │   │   ├── time
│   │   │   └── time.ts
│   │   ├── string
│   │   │   ├── camelCase.d.ts
│   │   │   ├── camelCase.js
│   │   │   ├── contains.d.ts
│   │   │   ├── contains.js
│   │   │   ├── crop.d.ts
│   │   │   ├── crop.js
│   │   │   ├── endsWith.d.ts
│   │   │   ├── endsWith.js
│   │   │   ├── escapeHtml.d.ts
│   │   │   ├── escapeHtml.js
│   │   │   ├── escapeRegExp.d.ts
│   │   │   ├── escapeRegExp.js
│   │   │   ├── escapeUnicode.d.ts
│   │   │   ├── escapeUnicode.js
│   │   │   ├── hyphenate.d.ts
│   │   │   ├── hyphenate.js
│   │   │   ├── insert.d.ts
│   │   │   ├── insert.js
│   │   │   ├── interpolate.d.ts
│   │   │   ├── interpolate.js
│   │   │   ├── lowerCase.d.ts
│   │   │   ├── lowerCase.js
│   │   │   ├── lpad.d.ts
│   │   │   ├── lpad.js
│   │   │   ├── ltrim.d.ts
│   │   │   ├── ltrim.js
│   │   │   ├── makePath.d.ts
│   │   │   ├── makePath.js
│   │   │   ├── normalizeLineBreaks.d.ts
│   │   │   ├── normalizeLineBreaks.js
│   │   │   ├── pascalCase.d.ts
│   │   │   ├── pascalCase.js
│   │   │   ├── properCase.d.ts
│   │   │   ├── properCase.js
│   │   │   ├── removeNonASCII.d.ts
│   │   │   ├── removeNonASCII.js
│   │   │   ├── removeNonWord.d.ts
│   │   │   ├── removeNonWord.js
│   │   │   ├── repeat.d.ts
│   │   │   ├── repeat.js
│   │   │   ├── replaceAccents.d.ts
│   │   │   ├── replaceAccents.js
│   │   │   ├── replace.d.ts
│   │   │   ├── replace.js
│   │   │   ├── rpad.d.ts
│   │   │   ├── rpad.js
│   │   │   ├── rtrim.d.ts
│   │   │   ├── rtrim.js
│   │   │   ├── sentenceCase.d.ts
│   │   │   ├── sentenceCase.js
│   │   │   ├── slugify.d.ts
│   │   │   ├── slugify.js
│   │   │   ├── startsWith.d.ts
│   │   │   ├── startsWith.js
│   │   │   ├── stripHtmlTags.d.ts
│   │   │   ├── stripHtmlTags.js
│   │   │   ├── stripMargin.d.ts
│   │   │   ├── stripMargin.js
│   │   │   ├── trim.d.ts
│   │   │   ├── trim.js
│   │   │   ├── truncate.d.ts
│   │   │   ├── truncate.js
│   │   │   ├── typecast.d.ts
│   │   │   ├── typecast.js
│   │   │   ├── unCamelCase.d.ts
│   │   │   ├── unCamelCase.js
│   │   │   ├── underscore.d.ts
│   │   │   ├── underscore.js
│   │   │   ├── unescapeHtml.d.ts
│   │   │   ├── unescapeHtml.js
│   │   │   ├── unescapeUnicode.d.ts
│   │   │   ├── unescapeUnicode.js
│   │   │   ├── unhyphenate.d.ts
│   │   │   ├── unhyphenate.js
│   │   │   ├── upperCase.d.ts
│   │   │   ├── upperCase.js
│   │   │   ├── WHITE_SPACES.d.ts
│   │   │   └── WHITE_SPACES.js
│   │   ├── string.d.ts
│   │   ├── string.js
│   │   ├── time
│   │   │   ├── convert.d.ts
│   │   │   ├── convert.js
│   │   │   ├── now.d.ts
│   │   │   ├── now.js
│   │   │   ├── parseMs.d.ts
│   │   │   ├── parseMs.js
│   │   │   ├── toTimeString.d.ts
│   │   │   └── toTimeString.js
│   │   ├── time.d.ts
│   │   ├── time.js
│   │   └── tsconfig.json
│   └── web-dev-utils
│       ├── bootstrap-4.3.1-dist
│       │   ├── css
│       │   └── js
│       ├── GIT-HTML-PREVIEW-TOOL
│       │   └── GIT-HTML-PREVIEW-TOOL-master
│       ├── images
│       │   ├── copy-2-clip.PNG
│       │   ├── git-preview.gif
│       │   ├── markdown-template.png
│       │   ├── text-file-2-JS-Array.PNG
│       │   ├── TexTools.PNG
│       │   └── toc.PNG
│       ├── Markdown-Templates
│       │   └── Markdown-Templates-master
│       ├── package.json
│       ├── personal-utilities
│       │   ├── Auto-table-Of-Contents
│       │   ├── copy-2-clip
│       │   ├── css
│       │   ├── js
│       │   └── prism
│       ├── README.md
│       ├── renovate.json
│       └── TexTools
│           └── TexTools-master
├── package-lock.json
├── __pycache__
│   └── json.cpython-39.pyc
├── README.md
├── robots.txt
├── _Specs
│   ├── all_pairs_shortest_path.py
│   ├── backend_test_util.py
│   ├── bfs_shortest_path.py
│   ├── bfs_zero_one_shortest_path.py
│   ├── breadth_first_search_shortest_path.py
│   ├── conftest.py
│   ├── difference_testing.py
│   ├── flask_migrate_test.py
│   ├── greatest_common_divisor.py
│   ├── lucas_lehmer_primality_test.py
│   ├── next_greatest_letter.py
│   ├── pydev_runfiles_pytest2.py
│   ├── pydev_runfiles_unittest.py
│   ├── pytestplugin.py
│   ├── pytest.py
│   ├── shortest_distance_from_all_buildings.py
│   ├── test_01_simple_web_responses.py
│   ├── test_02_db.py
│   ├── testapp.py
│   ├── test_array.py
│   ├── test_automata.py
│   ├── test_backend.py
│   ├── test_backtrack.py
│   ├── test_bfs.py
│   ├── test_bit.py
│   ├── test_bytecode.py
│   ├── test_cfg.py
│   ├── test_channel.py
│   ├── test-charmap.py
│   ├── test_cli.py
│   ├── test_code.py
│   ├── test_compression.py
│   ├── test_concrete.py
│   ├── test_cost.py
│   ├── test_countPaths.py
│   ├── test_cran.py
│   ├── test_cte.py
│   ├── test_data.py
│   ├── test_ddl.py
│   ├── test_dfs.py
│   ├── test_dialect.py
│   ├── test_digital_image_processing.py
│   ├── test_doctests.py
│   ├── test_dp.py
│   ├── test_dtw.py
│   ├── test_dtw_s.py
│   ├── test_fastvsnormal.py
│   ├── test_fetch_github_info.py
│   ├── test_flags.py
│   ├── test_functional.py
│   ├── test_graph.py
│   ├── test_heap.py
│   ├── test_histogram.py
│   ├── test_import.py
│   ├── testing.py
│   ├── test_insert.py
│   ├── test_instr.py
│   ├── test_issues.py
│   ├── test_iterative_segment_tree.py
│   ├── test_js_client.py
│   ├── test_linear_algebra.py
│   ├── test_linkedlist.py
│   ├── test_map.py
│   ├── test_maths.py
│   ├── test_matrix_operation.py
│   ├── test_matrix.py
│   ├── test_min_spanning_tree_kruskal.py
│   ├── test_min_spanning_tree_prim.py
│   ├── test_misc.py
│   ├── test_ml.py
│   ├── test_monomial.py
│   ├── test_node.py
│   ├── testofdivisibility.py
│   ├── test_peephole_opt.py
│   ├── test_poker_hand.py
│   ├── test_polynomial.py
│   ├── test_prime_check.py
│   ├── test_problem_01_while_loop.py
│   ├── test_problem_01_while_map.py
│   ├── test_problem_02_for_loop.py
│   ├── test_problem_02_for_map.py
│   ├── test_problem_03_list_comprehension.py
│   ├── test_problem_04_if_statement.py
│   ├── test_problem_05_class.py
│   ├── test_problem_06_inheritance.py
│   ├── test_problem_07_dictionary.py
│   ├── test_problem_08_builtins.py
│   ├── test_problem_09_getters_and_setters.py
│   ├── test_problem_10_functions.py
│   ├── test.py
│   ├── test_pytest.py
│   ├── test_python_client.py
│   ├── test_python_types.py
│   ├── test_python_type_stubs.py
│   ├── test_queues.py
│   ├── test_reflection.py
│   ├── test_results.py
│   ├── test_roman_numerals_pytest.py
│   ├── test_roman_numerals_unittest.py
│   ├── test_search.py
│   ├── test_select.py
│   ├── test_send_file.py
│   ├── test_sequence.py
│   ├── test_set.py
│   ├── test_sort.py
│   ├── tests.py
│   ├── test_stack.py
│   ├── test_stone_internal.py
│   ├── test_stone_route_whitelist.py
│   ├── test_strings.py
│   ├── test_tree.py
│   ├── test_tsd_client.py
│   ├── test_tsd_types.py
│   ├── test_types.py
│   ├── test_unit.py
│   ├── test_unittest.py
│   ├── test_unix.py
│   ├── test_update_delete.py
│   └── test_views.py
├── tree2.md
├── tree.md
└── _Unsorted_Mega_Folder
    ├── 1
    │   ├── 0001_initial.py
    │   ├── 0002_entry.py
    │   ├── 0003_topic_owner.py
    │   ├── 01-basics.py
    │   ├── 01_deques_intro.py
    │   ├── 01_intro_to_queues.py
    │   ├── 01-lists-intro.py
    │   ├── 01_lists_intro.py
    │   ├── 01_stacks_intro.py
    │   ├── 01_summation.py
    │   ├── 02-arithmetic.py
    │   ├── 02_big_o.py
    │   ├── 02_deque_implementation.py
    │   ├── 02-lists-mutability-and-sorting.py
    │   ├── 02_queue_implementation.py
    │   ├── 02_stacks_implementation.py
    │   ├── 02_unordered_lists.py
    │   ├── 03_anagram.py
    │   ├── 03_balanced_parenthesis.py
    │   ├── 03_hot_potato.py
    │   ├── 03-input-and-format.py
    │   ├── 03-lists-math.py
    │   ├── 03_palindrome.py
    │   ├── 04b-complex-try-blocks.py
    │   ├── 04_converting_bases.py
    │   ├── 04-ducktyping.py
    │   ├── 04-tuples-intro.py
    │   ├── 04_type_performance.py
    │   ├── 05b-extra-string-operations.py
    │   ├── 05_prefix_postfix_infix.py
    │   ├── 05-string-arithmetic.py
    │   ├── 05-tuples-single-item.py
    │   ├── 06-assignment.py
    │   ├── 06-ranges.py
    │   ├── 07b-logical-operators.py
    │   ├── 07c-while-loops.py
    │   ├── 07-dictionaries-intro.py
    │   ├── 07-equality.py
    │   ├── 08-dictionaries-more-ways.py
    │   ├── 08-identity.py
    │   ├── 09-functions.py
    │   ├── 09-sets-intro.py
    │   ├── 0Intro2python-all.py
    │   ├── 10-functions-returning-functions.py
    │   ├── 10-sets-usage.py
    │   ├── 11b-stackqueue.py
    │   ├── 11-combining-structures.py
    │   ├── 12-built-ins-all-any.py
    │   ├── 13-built-ins-filter-map-zip.py
    │   ├── 14-built-ins-custom-sort.py
    │   ├── 15a-input-validation1.py
    │   ├── 15b-input-validation2.py
    │   ├── 1.py
    │   ├── 2
    │   │   ├── accelerator_partitioner.py
    │   │   ├── ad2012R2.py
    │   │   ├── _add_newdocs (2).py
    │   │   ├── add_newdocs (2).py
    │   │   ├── _add_newdocs (3).py
    │   │   ├── add_newdocs (3).py
    │   │   ├── _add_newdocs.py
    │   │   ├── add_newdocs.py
    │   │   ├── adodbapi (2).py
    │   │   ├── adodbapi (3).py
    │   │   ├── adodbapi.py
    │   │   ├── adodbapitest (2).py
    │   │   ├── adodbapitest (3).py
    │   │   ├── adodbapitest.py
    │   │   ├── advapi32 (2).py
    │   │   ├── advapi32 (3).py
    │   │   ├── advapi32 (4).py
    │   │   ├── advapi32 (5).py
    │   │   ├── advapi32.py
    │   │   ├── _agglomerative.py
    │   │   ├── algorithms.py
    │   │   ├── android (10).py
    │   │   ├── android (11).py
    │   │   ├── android (12).py
    │   │   ├── android (13).py
    │   │   ├── android (14).py
    │   │   ├── android (15).py
    │   │   ├── android (16).py
    │   │   ├── android (17).py
    │   │   ├── android (18).py
    │   │   ├── android (19).py
    │   │   ├── android (20).py
    │   │   ├── android (21).py
    │   │   ├── android (22).py
    │   │   ├── android (23).py
    │   │   ├── android (24).py
    │   │   ├── android (25).py
    │   │   ├── android (2).py
    │   │   ├── android (3).py
    │   │   ├── android (4).py
    │   │   ├── android (5).py
    │   │   ├── android (6).py
    │   │   ├── android (7).py
    │   │   ├── android (8).py
    │   │   ├── android (9).py
    │   │   ├── android.py
    │   │   ├── animation.py
    │   │   ├── apache.py
    │   │   ├── api (2).py
    │   │   ├── api (3).py
    │   │   ├── api (4).py
    │   │   ├── api (5).py
    │   │   ├── apidoc.py
    │   │   ├── api.py
    │   │   ├── app (2).py
    │   │   ├── app (3).py
    │   │   ├── app (4).py
    │   │   ├── app (5).py
    │   │   ├── application (2).py
    │   │   ├── application (3).py
    │   │   ├── application.py
    │   │   ├── app.py
    │   │   ├── arabic_stemmer.py
    │   │   ├── argparse (2).py
    │   │   ├── argparse (3).py
    │   │   ├── argparse (4).py
    │   │   ├── argparse (5).py
    │   │   ├── argparse.py
    │   │   ├── arpack.py
    │   │   ├── arraypad.py
    │   │   ├── arrayprint (2).py
    │   │   ├── arrayprint (3).py
    │   │   ├── arrayprint.py
    │   │   ├── array.py
    │   │   ├── artist.py
    │   │   ├── assembler (2).py
    │   │   ├── assembler (3).py
    │   │   ├── assembler (4).py
    │   │   ├── assembler (5).py
    │   │   ├── assembler.py
    │   │   ├── associationproxy (2).py
    │   │   ├── associationproxy (3).py
    │   │   ├── associationproxy (4).py
    │   │   ├── associationproxy (5).py
    │   │   ├── associationproxy.py
    │   │   ├── ast.py
    │   │   ├── attributes (2).py
    │   │   ├── attributes (3).py
    │   │   ├── attributes (4).py
    │   │   ├── attributes (5).py
    │   │   ├── attributes.py
    │   │   ├── auth (2).py
    │   │   ├── auth (3).py
    │   │   ├── auth.py
    │   │   ├── autocomplete.py
    │   │   ├── automap (2).py
    │   │   ├── automap (3).py
    │   │   ├── automap (4).py
    │   │   ├── automap (5).py
    │   │   ├── automap.py
    │   │   ├── autopep8 (2).py
    │   │   ├── autopep8 (3).py
    │   │   ├── autopep8.py
    │   │   ├── axes3d.py
    │   │   ├── _axes.py
    │   │   ├── axis_artist.py
    │   │   ├── axis.py
    │   │   ├── backend_bases.py
    │   │   ├── backend_ctypes (2).py
    │   │   ├── backend_ctypes (3).py
    │   │   ├── backend_ctypes (4).py
    │   │   ├── backend_ctypes (5).py
    │   │   ├── backend_ctypes.py
    │   │   ├── backend_pdf.py
    │   │   ├── backend_pgf.py
    │   │   ├── backend_ps.py
    │   │   ├── backend.py
    │   │   ├── backend_svg.py
    │   │   ├── backend_wx.py
    │   │   ├── _bagging.py
    │   │   ├── base (10).py
    │   │   ├── base (11).py
    │   │   ├── base (12).py
    │   │   ├── base (13).py
    │   │   ├── base (14).py
    │   │   ├── base (15).py
    │   │   ├── base (16).py
    │   │   ├── base (17).py
    │   │   ├── base (18).py
    │   │   ├── base (19).py
    │   │   ├── base (20).py
    │   │   ├── base (21).py
    │   │   ├── base (22).py
    │   │   ├── base (23).py
    │   │   ├── base (24).py
    │   │   ├── base (25).py
    │   │   ├── base (26).py
    │   │   ├── base (27).py
    │   │   ├── base (28).py
    │   │   ├── base (29).py
    │   │   ├── _base (2).py
    │   │   ├── base (2).py
    │   │   ├── base (30).py
    │   │   ├── base (31).py
    │   │   ├── base (32).py
    │   │   ├── base (33).py
    │   │   ├── base (34).py
    │   │   ├── base (35).py
    │   │   ├── base (36).py
    │   │   ├── base (37).py
    │   │   ├── base (38).py
    │   │   ├── base (39).py
    │   │   ├── _base (3).py
    │   │   ├── base (3).py
    │   │   ├── base (40).py
    │   │   ├── base (41).py
    │   │   ├── base (42).py
    │   │   ├── base (43).py
    │   │   ├── base (44).py
    │   │   ├── base (45).py
    │   │   ├── base (46).py
    │   │   ├── base (47).py
    │   │   ├── base (48).py
    │   │   ├── base (49).py
    │   │   ├── _base (4).py
    │   │   ├── base (4).py
    │   │   ├── base (50).py
    │   │   ├── base (51).py
    │   │   ├── _base (5).py
    │   │   ├── base (5).py
    │   │   ├── base (6).py
    │   │   ├── base (7).py
    │   │   ├── base (8).py
    │   │   ├── base (9).py
    │   │   ├── base_events (2).py
    │   │   ├── base_events (3).py
    │   │   ├── base_events (4).py
    │   │   ├── base_events (5).py
    │   │   ├── base_events.py
    │   │   ├── _base.py
    │   │   ├── base.py
    │   │   ├── _basic (2).py
    │   │   ├── _basic (3).py
    │   │   ├── _basic.py
    │   │   ├── basic.py
    │   │   ├── bcrypt.py
    │   │   ├── blocks.py
    │   │   ├── _bootstrap_external (2).py
    │   │   ├── _bootstrap_external (3).py
    │   │   ├── _bootstrap_external (4).py
    │   │   ├── _bootstrap_external (5).py
    │   │   ├── _bootstrap_external.py
    │   │   ├── _bootstrap.py
    │   │   ├── breakpoint (2).py
    │   │   ├── breakpoint (3).py
    │   │   ├── breakpoint (4).py
    │   │   ├── breakpoint (5).py
    │   │   ├── breakpoint.py
    │   │   ├── buffer (2).py
    │   │   ├── buffer (3).py
    │   │   ├── buffer.py
    │   │   ├── _bvp.py
    │   │   ├── case (2).py
    │   │   ├── case (3).py
    │   │   ├── case (4).py
    │   │   ├── case (5).py
    │   │   ├── case.py
    │   │   ├── cast.py
    │   │   ├── categorical.py
    │   │   ├── ccompiler (10).py
    │   │   ├── ccompiler (11).py
    │   │   ├── ccompiler (2).py
    │   │   ├── ccompiler (3).py
    │   │   ├── ccompiler (4).py
    │   │   ├── ccompiler (5).py
    │   │   ├── ccompiler (6).py
    │   │   ├── ccompiler (7).py
    │   │   ├── ccompiler (8).py
    │   │   ├── ccompiler (9).py
    │   │   ├── ccompiler_opt.py
    │   │   ├── ccompiler.py
    │   │   ├── census2010.py
    │   │   ├── cfuncs (2).py
    │   │   ├── cfuncs (3).py
    │   │   ├── cfuncs (4).py
    │   │   ├── cfuncs (5).py
    │   │   ├── cfuncs.py
    │   │   ├── channel.py
    │   │   ├── chebyshev (2).py
    │   │   ├── chebyshev (3).py
    │   │   ├── chebyshev (4).py
    │   │   ├── chebyshev (5).py
    │   │   ├── chebyshev.py
    │   │   ├── _classes (2).py
    │   │   ├── _classes (3).py
    │   │   ├── _classes.py
    │   │   ├── classes.py
    │   │   ├── _classification.py
    │   │   ├── client (10).py
    │   │   ├── client (11).py
    │   │   ├── client (2).py
    │   │   ├── client (3).py
    │   │   ├── client (4).py
    │   │   ├── client (5).py
    │   │   ├── client (6).py
    │   │   ├── client (7).py
    │   │   ├── client (8).py
    │   │   ├── client (9).py
    │   │   ├── client.py
    │   │   ├── cli.py
    │   │   ├── cloudpickle.py
    │   │   ├── cmake (10).py
    │   │   ├── cmake (11).py
    │   │   ├── cmake (12).py
    │   │   ├── cmake (13).py
    │   │   ├── cmake (14).py
    │   │   ├── cmake (15).py
    │   │   ├── cmake (16).py
    │   │   ├── cmake (17).py
    │   │   ├── cmake (18).py
    │   │   ├── cmake (19).py
    │   │   ├── cmake (20).py
    │   │   ├── cmake (21).py
    │   │   ├── cmake (22).py
    │   │   ├── cmake (23).py
    │   │   ├── cmake (24).py
    │   │   ├── cmake (25).py
    │   │   ├── cmake (2).py
    │   │   ├── cmake (3).py
    │   │   ├── cmake (4).py
    │   │   ├── cmake (5).py
    │   │   ├── cmake (6).py
    │   │   ├── cmake (7).py
    │   │   ├── cmake (8).py
    │   │   ├── cmake (9).py
    │   │   ├── cmake.py
    │   │   ├── _cm_listed.py
    │   │   ├── _cm.py
    │   │   ├── codata.py
    │   │   ├── codegen (2).py
    │   │   ├── codegen (3).py
    │   │   ├── codegen (4).py
    │   │   ├── codegen (5).py
    │   │   ├── codegen.py
    │   │   ├── collections (2).py
    │   │   ├── collections (3).py
    │   │   ├── collections (4).py
    │   │   ├── collections (5).py
    │   │   ├── collections (6).py
    │   │   ├── collections (7).py
    │   │   ├── collections.py
    │   │   ├── colorbar.py
    │   │   ├── colors.py
    │   │   ├── commctrl (2).py
    │   │   ├── commctrl (3).py
    │   │   ├── commctrl.py
    │   │   ├── common_methods_invocations.py
    │   │   ├── common_nn.py
    │   │   ├── common.py
    │   │   ├── common_quantization.py
    │   │   ├── common_utils.py
    │   │   ├── compat (10).py
    │   │   ├── compat (11).py
    │   │   ├── compat (2).py
    │   │   ├── compat (3).py
    │   │   ├── compat (4).py
    │   │   ├── compat (5).py
    │   │   ├── compat (6).py
    │   │   ├── compat (7).py
    │   │   ├── compat (8).py
    │   │   ├── compat (9).py
    │   │   ├── compat.py
    │   │   ├── compiler (10).py
    │   │   ├── compiler (11).py
    │   │   ├── compiler (12).py
    │   │   ├── compiler (13).py
    │   │   ├── compiler (14).py
    │   │   ├── compiler (15).py
    │   │   ├── compiler (16).py
    │   │   ├── compiler (17).py
    │   │   ├── compiler (2).py
    │   │   ├── compiler (3).py
    │   │   ├── compiler (4).py
    │   │   ├── compiler (5).py
    │   │   ├── compiler (6).py
    │   │   ├── compiler (7).py
    │   │   ├── compiler (8).py
    │   │   ├── compiler (9).py
    │   │   ├── compiler.py
    │   │   ├── completer (2).py
    │   │   ├── completer (3).py
    │   │   ├── completer.py
    │   │   ├── compressed.py
    │   │   ├── configdialog (2).py
    │   │   ├── configdialog (3).py
    │   │   ├── configdialog (4).py
    │   │   ├── configdialog (5).py
    │   │   ├── configdialog.py
    │   │   ├── configparser (2).py
    │   │   ├── configparser (3).py
    │   │   ├── configparser (4).py
    │   │   ├── configparser (5).py
    │   │   ├── configparser.py
    │   │   ├── connection (2).py
    │   │   ├── connection (3).py
    │   │   ├── connection.py
    │   │   ├── console_widget (2).py
    │   │   ├── console_widget (3).py
    │   │   ├── console_widget.py
    │   │   ├── constants (10).py
    │   │   ├── constants (11).py
    │   │   ├── constants (12).py
    │   │   ├── constants (13).py
    │   │   ├── constants (2).py
    │   │   ├── constants (3).py
    │   │   ├── constants (4).py
    │   │   ├── constants (5).py
    │   │   ├── constants (6).py
    │   │   ├── constants (7).py
    │   │   ├── constants (8).py
    │   │   ├── constants (9).py
    │   │   ├── constants.py
    │   │   ├── containers (2).py
    │   │   ├── containers (3).py
    │   │   ├── containers.py
    │   │   ├── context.py
    │   │   ├── _continuous_distns.py
    │   │   ├── contour.py
    │   │   ├── conv.py
    │   │   ├── cookiejar (2).py
    │   │   ├── cookiejar (3).py
    │   │   ├── cookiejar (4).py
    │   │   ├── cookiejar (5).py
    │   │   ├── cookiejar.py
    │   │   ├── _coordinate_descent.py
    │   │   ├── core (10).py
    │   │   ├── core (11).py
    │   │   ├── core (12).py
    │   │   ├── core (13).py
    │   │   ├── core (14).py
    │   │   ├── core (15).py
    │   │   ├── core (16).py
    │   │   ├── core (17).py
    │   │   ├── core (18).py
    │   │   ├── core (19).py
    │   │   ├── core (20).py
    │   │   ├── core (21).py
    │   │   ├── core (2).py
    │   │   ├── core (3).py
    │   │   ├── core (4).py
    │   │   ├── core (5).py
    │   │   ├── core (6).py
    │   │   ├── core (7).py
    │   │   ├── core (8).py
    │   │   ├── core (9).py
    │   │   ├── _core.py
    │   │   ├── core.py
    │   │   ├── core_test.py
    │   │   ├── c_parser (2).py
    │   │   ├── cparser (2).py
    │   │   ├── c_parser (3).py
    │   │   ├── cparser (3).py
    │   │   ├── c_parser (4).py
    │   │   ├── cparser (4).py
    │   │   ├── c_parser (5).py
    │   │   ├── cparser (5).py
    │   │   ├── c_parser.py
    │   │   ├── cparser.py
    │   │   ├── cpp_extension.py
    │   │   ├── crackfortran (2).py
    │   │   ├── crackfortran (3).py
    │   │   ├── crackfortran (4).py
    │   │   ├── crackfortran (5).py
    │   │   ├── crackfortran.py
    │   │   ├── crash (2).py
    │   │   ├── crash (3).py
    │   │   ├── crash (4).py
    │   │   ├── crash (5).py
    │   │   ├── crash.py
    │   │   ├── cuda_to_hip_mappings.py
    │   │   ├── cursor.py
    │   │   ├── cx_oracle (2).py
    │   │   ├── cx_oracle (3).py
    │   │   ├── cx_oracle (4).py
    │   │   ├── cx_oracle (5).py
    │   │   ├── cx_oracle.py
    │   │   ├── database (10).py
    │   │   ├── database (11).py
    │   │   ├── database (2).py
    │   │   ├── database (3).py
    │   │   ├── database (4).py
    │   │   ├── database (5).py
    │   │   ├── database (6).py
    │   │   ├── database (7).py
    │   │   ├── database (8).py
    │   │   ├── database (9).py
    │   │   ├── database.py
    │   │   ├── _dataclasses (2).py
    │   │   ├── dataclasses (2).py
    │   │   ├── _dataclasses (3).py
    │   │   ├── dataclasses (3).py
    │   │   ├── _dataclasses (4).py
    │   │   ├── dataclasses (4).py
    │   │   ├── _dataclasses (5).py
    │   │   ├── dataclasses (5).py
    │   │   ├── dataclasses (6).py
    │   │   ├── dataclasses (7).py
    │   │   ├── dataclasses (8).py
    │   │   ├── dataclasses (9).py
    │   │   ├── _dataclasses.py
    │   │   ├── dataclasses.py
    │   │   ├── dataloader.py
    │   │   ├── data_parallel_model.py
    │   │   ├── data_parallel_model_test.py
    │   │   ├── _data.py
    │   │   ├── datastructures (2).py
    │   │   ├── datastructures (3).py
    │   │   ├── datastructures.py
    │   │   ├── dates (2).py
    │   │   ├── dates (3).py
    │   │   ├── dates.py
    │   │   ├── datetime (10).py
    │   │   ├── datetime (11).py
    │   │   ├── datetime (12).py
    │   │   ├── datetime (13).py
    │   │   ├── datetime (14).py
    │   │   ├── datetime (15).py
    │   │   ├── datetime (16).py
    │   │   ├── datetime (2).py
    │   │   ├── datetime (3).py
    │   │   ├── datetime (4).py
    │   │   ├── datetime (5).py
    │   │   ├── datetime (6).py
    │   │   ├── datetime (7).py
    │   │   ├── datetime (8).py
    │   │   ├── datetime (9).py
    │   │   ├── datetimelike.py
    │   │   ├── datetime.py
    │   │   ├── datetimes.py
    │   │   ├── datetimetester.py
    │   │   ├── dbghelp (2).py
    │   │   ├── dbghelp (3).py
    │   │   ├── dbghelp (4).py
    │   │   ├── dbghelp (5).py
    │   │   ├── dbghelp.py
    │   │   ├── ddl (2).py
    │   │   ├── ddl (3).py
    │   │   ├── ddl (4).py
    │   │   ├── ddl (5).py
    │   │   ├── ddl.py
    │   │   ├── debug (2).py
    │   │   ├── debug (3).py
    │   │   ├── debug (4).py
    │   │   ├── debug (5).py
    │   │   ├── debug.py
    │   │   ├── decoder.py
    │   │   ├── decomp.py
    │   │   ├── default (2).py
    │   │   ├── default (3).py
    │   │   ├── default (4).py
    │   │   ├── default (5).py
    │   │   ├── default.py
    │   │   ├── defchararray (2).py
    │   │   ├── defchararray (3).py
    │   │   ├── defchararray (4).py
    │   │   ├── defchararray (5).py
    │   │   ├── defchararray.py
    │   │   ├── dependency (2).py
    │   │   ├── dependency (3).py
    │   │   ├── dependency (4).py
    │   │   ├── dependency (5).py
    │   │   ├── dependency.py
    │   │   ├── des.py
    │   │   ├── _dict_learning.py
    │   │   ├── _differentialevolution.py
    │   │   ├── difflib (2).py
    │   │   ├── difflib (3).py
    │   │   ├── difflib (4).py
    │   │   ├── difflib (5).py
    │   │   ├── difflib.py
    │   │   ├── display (2).py
    │   │   ├── display (3).py
    │   │   ├── display.py
    │   │   ├── dist (10).py
    │   │   ├── dist (11).py
    │   │   ├── dist (12).py
    │   │   ├── dist (13).py
    │   │   ├── dist (2).py
    │   │   ├── dist (3).py
    │   │   ├── dist (4).py
    │   │   ├── dist (5).py
    │   │   ├── dist (6).py
    │   │   ├── dist (7).py
    │   │   ├── dist (8).py
    │   │   ├── dist (9).py
    │   │   ├── distance.py
    │   │   ├── dist_autograd_test.py
    │   │   ├── _distn_infrastructure.py
    │   │   ├── dist.py
    │   │   ├── distributed_c10d.py
    │   │   ├── distributed.py
    │   │   ├── distributed_test.py
    │   │   ├── distro (10).py
    │   │   ├── distro (11).py
    │   │   ├── distro (12).py
    │   │   ├── distro (13).py
    │   │   ├── distro (2).py
    │   │   ├── distro (3).py
    │   │   ├── distro (4).py
    │   │   ├── distro (5).py
    │   │   ├── distro (6).py
    │   │   ├── distro (7).py
    │   │   ├── distro (8).py
    │   │   ├── distro (9).py
    │   │   ├── distro.py
    │   │   ├── docbuilder.py
    │   │   ├── docintrospecter.py
    │   │   ├── docparser.py
    │   │   ├── docstringparser.py
    │   │   ├── doctest (2).py
    │   │   ├── doctest (3).py
    │   │   ├── doctest (4).py
    │   │   ├── doctest (5).py
    │   │   ├── doctest.py
    │   │   ├── dotgraph.py
    │   │   ├── download.py
    │   │   ├── ds389.py
    │   │   ├── easy_install (2).py
    │   │   ├── easy_install (3).py
    │   │   ├── easy_install (4).py
    │   │   ├── easy_install (5).py
    │   │   ├── easy_install (6).py
    │   │   ├── easy_install (7).py
    │   │   ├── easy_install (8).py
    │   │   ├── easy_install (9).py
    │   │   ├── easy_install.py
    │   │   ├── Ecriture inclusive (2).py
    │   │   ├── Ecriture inclusive (3).py
    │   │   ├── Ecriture inclusive (4).py
    │   │   ├── Ecriture inclusive (5).py
    │   │   ├── Ecriture inclusive.py
    │   │   ├── edir888.py
    │   │   ├── edir914.py
    │   │   ├── editor (2).py
    │   │   ├── editor (3).py
    │   │   ├── editor (4).py
    │   │   ├── editor (5).py
    │   │   ├── editor.py
    │   │   ├── einsumfunc (2).py
    │   │   ├── einsumfunc (3).py
    │   │   ├── einsumfunc.py
    │   │   ├── elements (2).py
    │   │   ├── elements (3).py
    │   │   ├── elements (4).py
    │   │   ├── elements (5).py
    │   │   ├── elements.py
    │   │   ├── ElementTree (2).py
    │   │   ├── ElementTree (3).py
    │   │   ├── ElementTree (4).py
    │   │   ├── ElementTree (5).py
    │   │   ├── ElementTree.py
    │   │   ├── emitter (2).py
    │   │   ├── emitter (3).py
    │   │   ├── emitter.py
    │   │   ├── emoji1_db.py
    │   │   ├── emsabtags (2).py
    │   │   ├── emsabtags (3).py
    │   │   ├── emsabtags.py
    │   │   ├── entities (2).py
    │   │   ├── entities (3).py
    │   │   ├── entities (4).py
    │   │   ├── entities (5).py
    │   │   ├── entities.py
    │   │   ├── environment (10).py
    │   │   ├── environment (11).py
    │   │   ├── environment (2).py
    │   │   ├── environment (3).py
    │   │   ├── environment (4).py
    │   │   ├── environment (5).py
    │   │   ├── environment (6).py
    │   │   ├── environment (7).py
    │   │   ├── environment (8).py
    │   │   ├── environment (9).py
    │   │   ├── environment.py
    │   │   ├── epytext.py
    │   │   ├── errors (10).py
    │   │   ├── errors (11).py
    │   │   ├── errors (2).py
    │   │   ├── errors (3).py
    │   │   ├── errors (4).py
    │   │   ├── errors (5).py
    │   │   ├── errors (6).py
    │   │   ├── errors (7).py
    │   │   ├── errors (8).py
    │   │   ├── errors (9).py
    │   │   ├── errors.py
    │   │   ├── estimator_checks.py
    │   │   ├── etcd_rendezvous.py
    │   │   ├── event (2).py
    │   │   ├── event (3).py
    │   │   ├── event (4).py
    │   │   ├── event (5).py
    │   │   ├── event.py
    │   │   ├── events (10).py
    │   │   ├── events (11).py
    │   │   ├── events (2).py
    │   │   ├── events (3).py
    │   │   ├── events (4).py
    │   │   ├── events (5).py
    │   │   ├── events (6).py
    │   │   ├── events (7).py
    │   │   ├── events (8).py
    │   │   ├── events (9).py
    │   │   ├── events.py
    │   │   ├── execution (2).py
    │   │   ├── execution (3).py
    │   │   ├── execution.py
    │   │   ├── extensions.py
    │   │   ├── extras (10).py
    │   │   ├── extras (11).py
    │   │   ├── extras (2).py
    │   │   ├── extras (3).py
    │   │   ├── extras (4).py
    │   │   ├── extras (5).py
    │   │   ├── extras (6).py
    │   │   ├── extras (7).py
    │   │   ├── extras (8).py
    │   │   ├── extras (9).py
    │   │   ├── extras.py
    │   │   ├── fftpack.py
    │   │   ├── figure.py
    │   │   ├── filter_design.py
    │   │   ├── filters (2).py
    │   │   ├── filters (3).py
    │   │   ├── filters (4).py
    │   │   ├── filters (5).py
    │   │   ├── filters (6).py
    │   │   ├── filters (7).py
    │   │   ├── filters (8).py
    │   │   ├── filters (9).py
    │   │   ├── filters.py
    │   │   ├── fir_filter_design.py
    │   │   ├── fitpack2.py
    │   │   ├── _fitpack_impl.py
    │   │   ├── font_manager.py
    │   │   ├── _forest.py
    │   │   ├── format.py
    │   │   ├── frame.py
    │   │   ├── french_stemmer.py
    │   │   ├── fromnumeric (2).py
    │   │   ├── fromnumeric (3).py
    │   │   ├── fromnumeric (4).py
    │   │   ├── fromnumeric (5).py
    │   │   ├── fromnumeric.py
    │   │   ├── functional (2).py
    │   │   ├── functional (3).py
    │   │   ├── functional (4).py
    │   │   ├── functional (5).py
    │   │   ├── functional (6).py
    │   │   ├── functional (7).py
    │   │   ├── functional.py
    │   │   ├── function_base (2).py
    │   │   ├── function_base (3).py
    │   │   ├── function_base (4).py
    │   │   ├── function_base (5).py
    │   │   ├── function_base.py
    │   │   ├── _gb.py
    │   │   ├── gemoji_db.py
    │   │   ├── _generate_pyx.py
    │   │   ├── generic (2).py
    │   │   ├── generic (3).py
    │   │   ├── generic.py
    │   │   ├── gen.py
    │   │   ├── genpy (2).py
    │   │   ├── genpy (3).py
    │   │   ├── genpy.py
    │   │   ├── gradient_boosting.py
    │   │   ├── graph.py
    │   │   ├── greek_stemmer.py
    │   │   ├── groupby.py
    │   │   ├── gui.py
    │   │   ├── handlers (2).py
    │   │   ├── handlers (3).py
    │   │   ├── handlers (4).py
    │   │   ├── handlers (5).py
    │   │   ├── handlers (6).py
    │   │   ├── handlers (7).py
    │   │   ├── handlers (8).py
    │   │   ├── handlers (9).py
    │   │   ├── handlers.py
    │   │   ├── _header_value_parser (2).py
    │   │   ├── _header_value_parser (3).py
    │   │   ├── _header_value_parser (4).py
    │   │   ├── _header_value_parser (5).py
    │   │   ├── _header_value_parser.py
    │   │   ├── helpers.py
    │   │   ├── hermite (2).py
    │   │   ├── hermite (3).py
    │   │   ├── hermite (4).py
    │   │   ├── hermite (5).py
    │   │   ├── hermite_e (2).py
    │   │   ├── hermite_e (3).py
    │   │   ├── hermite_e (4).py
    │   │   ├── hermite_e (5).py
    │   │   ├── hermite_e.py
    │   │   ├── hermite.py
    │   │   ├── hierarchy.py
    │   │   ├── histograms (2).py
    │   │   ├── histograms (3).py
    │   │   ├── histograms.py
    │   │   ├── html5parser (10).py
    │   │   ├── html5parser (11).py
    │   │   ├── html5parser (12).py
    │   │   ├── html5parser (13).py
    │   │   ├── html5parser (2).py
    │   │   ├── html5parser (3).py
    │   │   ├── html5parser (4).py
    │   │   ├── html5parser (5).py
    │   │   ├── html5parser (6).py
    │   │   ├── html5parser (7).py
    │   │   ├── html5parser (8).py
    │   │   ├── html5parser (9).py
    │   │   ├── html5parser.py
    │   │   ├── _html_base.py
    │   │   ├── html.py
    │   │   ├── http (2).py
    │   │   ├── http (3).py
    │   │   ├── http.py
    │   │   ├── httpserver_test (2).py
    │   │   ├── httpserver_test (3).py
    │   │   ├── httpserver_test.py
    │   │   ├── hybrid.py
    │   │   ├── hypothesis_test.py
    │   │   ├── idnadata (10).py
    │   │   ├── idnadata (11).py
    │   │   ├── idnadata (2).py
    │   │   ├── idnadata (3).py
    │   │   ├── idnadata (4).py
    │   │   ├── idnadata (5).py
    │   │   ├── idnadata (6).py
    │   │   ├── idnadata (7).py
    │   │   ├── idnadata (8).py
    │   │   ├── idnadata (9).py
    │   │   ├── idnadata.py
    │   │   ├── image (2).py
    │   │   ├── image (3).py
    │   │   ├── Image.py
    │   │   ├── imaplib (2).py
    │   │   ├── imaplib (3).py
    │   │   ├── imaplib (4).py
    │   │   ├── imaplib (5).py
    │   │   ├── imaplib.py
    │   │   ├── indexing.py
    │   │   ├── index.py
    │   │   ├── infinite_recursion (2).py
    │   │   ├── infinite_recursion (3).py
    │   │   ├── infinite_recursion.py
    │   │   ├── __init__ (10).py
    │   │   ├── __init__ (11).py
    │   │   ├── __init__ (12).py
    │   │   ├── __init__ (13).py
    │   │   ├── __init__ (14).py
    │   │   ├── __init__ (15).py
    │   │   ├── __init__ (16).py
    │   │   ├── __init__ (17).py
    │   │   ├── __init__ (18).py
    │   │   ├── __init__ (19).py
    │   │   ├── __init__ (20).py
    │   │   ├── __init__ (21).py
    │   │   ├── __init__ (22).py
    │   │   ├── __init__ (23).py
    │   │   ├── __init__ (24).py
    │   │   ├── __init__ (25).py
    │   │   ├── __init__ (26).py
    │   │   ├── __init__ (27).py
    │   │   ├── __init__ (28).py
    │   │   ├── __init__ (29).py
    │   │   ├── __init__ (2).py
    │   │   ├── __init__ (30).py
    │   │   ├── __init__ (31).py
    │   │   ├── __init__ (32).py
    │   │   ├── __init__ (33).py
    │   │   ├── __init__ (34).py
    │   │   ├── __init__ (35).py
    │   │   ├── __init__ (36).py
    │   │   ├── __init__ (37).py
    │   │   ├── __init__ (38).py
    │   │   ├── __init__ (39).py
    │   │   ├── __init__ (3).py
    │   │   ├── __init__ (40).py
    │   │   ├── __init__ (41).py
    │   │   ├── __init__ (42).py
    │   │   ├── __init__ (43).py
    │   │   ├── __init__ (44).py
    │   │   ├── __init__ (45).py
    │   │   ├── __init__ (46).py
    │   │   ├── __init__ (47).py
    │   │   ├── __init__ (48).py
    │   │   ├── __init__ (49).py
    │   │   ├── __init__ (4).py
    │   │   ├── __init__ (50).py
    │   │   ├── __init__ (51).py
    │   │   ├── __init__ (52).py
    │   │   ├── __init__ (53).py
    │   │   ├── __init__ (54).py
    │   │   ├── __init__ (55).py
    │   │   ├── __init__ (56).py
    │   │   ├── __init__ (57).py
    │   │   ├── __init__ (58).py
    │   │   ├── __init__ (59).py
    │   │   ├── __init__ (5).py
    │   │   ├── __init__ (60).py
    │   │   ├── __init__ (61).py
    │   │   ├── __init__ (62).py
    │   │   ├── __init__ (63).py
    │   │   ├── __init__ (64).py
    │   │   ├── __init__ (65).py
    │   │   ├── __init__ (66).py
    │   │   ├── __init__ (67).py
    │   │   ├── __init__ (68).py
    │   │   ├── __init__ (69).py
    │   │   ├── __init__ (6).py
    │   │   ├── __init__ (70).py
    │   │   ├── __init__ (71).py
    │   │   ├── __init__ (72).py
    │   │   ├── __init__ (73).py
    │   │   ├── __init__ (74).py
    │   │   ├── __init__ (75).py
    │   │   ├── __init__ (76).py
    │   │   ├── __init__ (77).py
    │   │   ├── __init__ (78).py
    │   │   ├── __init__ (79).py
    │   │   ├── __init__ (7).py
    │   │   ├── __init__ (80).py
    │   │   ├── __init__ (81).py
    │   │   ├── __init__ (82).py
    │   │   ├── __init__ (83).py
    │   │   ├── __init__ (84).py
    │   │   ├── __init__ (85).py
    │   │   ├── __init__ (86).py
    │   │   ├── __init__ (87).py
    │   │   ├── __init__ (88).py
    │   │   ├── __init__ (89).py
    │   │   ├── __init__ (8).py
    │   │   ├── __init__ (90).py
    │   │   ├── __init__ (91).py
    │   │   ├── __init__ (92).py
    │   │   ├── __init__ (93).py
    │   │   ├── __init__ (94).py
    │   │   ├── __init__ (95).py
    │   │   ├── __init__ (96).py
    │   │   ├── __init__ (97).py
    │   │   ├── __init__ (9).py
    │   │   ├── __init__.py
    │   │   ├── input (10).py
    │   │   ├── input (11).py
    │   │   ├── input (12).py
    │   │   ├── input (13).py
    │   │   ├── input (14).py
    │   │   ├── input (15).py
    │   │   ├── input (16).py
    │   │   ├── input (17).py
    │   │   ├── input (18).py
    │   │   ├── input (19).py
    │   │   ├── input (20).py
    │   │   ├── input (21).py
    │   │   ├── input (22).py
    │   │   ├── input (23).py
    │   │   ├── input (24).py
    │   │   ├── input (25).py
    │   │   ├── input (2).py
    │   │   ├── input (3).py
    │   │   ├── input (4).py
    │   │   ├── input (5).py
    │   │   ├── input (6).py
    │   │   ├── input (7).py
    │   │   ├── input (8).py
    │   │   ├── input (9).py
    │   │   ├── input.py
    │   │   ├── inspect (2).py
    │   │   ├── inspect (3).py
    │   │   ├── inspect (4).py
    │   │   ├── inspect (5).py
    │   │   ├── inspect.py
    │   │   ├── interactive (2).py
    │   │   ├── interactive (3).py
    │   │   ├── interactive (4).py
    │   │   ├── interactive (5).py
    │   │   ├── interactive.py
    │   │   ├── interactiveshell (2).py
    │   │   ├── interactiveshell (3).py
    │   │   ├── interactiveshell.py
    │   │   ├── interfaces (2).py
    │   │   ├── interfaces (3).py
    │   │   ├── interfaces (4).py
    │   │   ├── interfaces (5).py
    │   │   ├── interfaces.py
    │   │   ├── interpolate.py
    │   │   ├── _interpolative_backend.py
    │   │   ├── interval (2).py
    │   │   ├── interval (3).py
    │   │   ├── interval.py
    │   │   ├── int_fiction (2).py
    │   │   ├── int_fiction (3).py
    │   │   ├── int_fiction (4).py
    │   │   ├── int_fiction (5).py
    │   │   ├── int_fiction (6).py
    │   │   ├── int_fiction (7).py
    │   │   ├── int_fiction.py
    │   │   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling (2).py
    │   │   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling (3).py
    │   │   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling (4).py
    │   │   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling (5).py
    │   │   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling.py
    │   │   ├── ioloop (2).py
    │   │   ├── ioloop (3).py
    │   │   ├── ioloop.py
    │   │   ├── iostream (2).py
    │   │   ├── iostream (3).py
    │   │   ├── iostream.py
    │   │   ├── iostream_test (2).py
    │   │   ├── iostream_test (3).py
    │   │   ├── iostream_test.py
    │   │   ├── ipaddress (10).py
    │   │   ├── ipaddress (11).py
    │   │   ├── ipaddress (12).py
    │   │   ├── ipaddress (13).py
    │   │   ├── ipaddress (14).py
    │   │   ├── ipaddress (15).py
    │   │   ├── ipaddress (2).py
    │   │   ├── ipaddress (3).py
    │   │   ├── ipaddress (4).py
    │   │   ├── ipaddress (5).py
    │   │   ├── ipaddress (6).py
    │   │   ├── ipaddress (7).py
    │   │   ├── ipaddress (8).py
    │   │   ├── ipaddress (9).py
    │   │   ├── ipaddress.py
    │   │   ├── ipython_directive (2).py
    │   │   ├── ipython_directive (3).py
    │   │   ├── ipython_directive.py
    │   │   ├── isort.py
    │   │   ├── javascript (2).py
    │   │   ├── javascript (3).py
    │   │   ├── javascript (4).py
    │   │   ├── javascript (5).py
    │   │   ├── javascript (6).py
    │   │   ├── javascript (7).py
    │   │   ├── javascript.py
    │   │   ├── _jit_internal.py
    │   │   ├── jvm (2).py
    │   │   ├── jvm (3).py
    │   │   ├── jvm (4).py
    │   │   ├── jvm (5).py
    │   │   ├── jvm (6).py
    │   │   ├── jvm (7).py
    │   │   ├── jvm.py
    │   │   ├── kernel32 (2).py
    │   │   ├── kernel32 (3).py
    │   │   ├── kernel32 (4).py
    │   │   ├── kernel32 (5).py
    │   │   ├── kernel32.py
    │   │   ├── kernels.py
    │   │   ├── _kmeans.py
    │   │   ├── Kullback_Leibler_divergences_in_native_python__Cython_and_Numba (2).py
    │   │   ├── Kullback_Leibler_divergences_in_native_python__Cython_and_Numba (3).py
    │   │   ├── Kullback_Leibler_divergences_in_native_python__Cython_and_Numba (4).py
    │   │   ├── Kullback_Leibler_divergences_in_native_python__Cython_and_Numba (5).py
    │   │   ├── Kullback_Leibler_divergences_in_native_python__Cython_and_Numba.py
    │   │   ├── laguerre (2).py
    │   │   ├── laguerre (3).py
    │   │   ├── laguerre (4).py
    │   │   ├── laguerre (5).py
    │   │   ├── laguerre.py
    │   │   ├── langhelpers (2).py
    │   │   ├── langhelpers (3).py
    │   │   ├── langhelpers (4).py
    │   │   ├── langhelpers (5).py
    │   │   ├── langhelpers.py
    │   │   ├── _lasso_builtins (2).py
    │   │   ├── _lasso_builtins (3).py
    │   │   ├── _lasso_builtins (4).py
    │   │   ├── _lasso_builtins (5).py
    │   │   ├── _lasso_builtins (6).py
    │   │   ├── _lasso_builtins (7).py
    │   │   ├── _lasso_builtins.py
    │   │   ├── latex.py
    │   │   ├── layers_test.py
    │   │   ├── _least_angle.py
    │   │   ├── legend.py
    │   │   ├── legendre (2).py
    │   │   ├── legendre (3).py
    │   │   ├── legendre (4).py
    │   │   ├── legendre (5).py
    │   │   ├── legendre.py
    │   │   ├── lex (2).py
    │   │   ├── lex (3).py
    │   │   ├── lex (4).py
    │   │   ├── lex (5).py
    │   │   ├── lex (6).py
    │   │   ├── lex (7).py
    │   │   ├── lex.py
    │   │   ├── linalg (2).py
    │   │   ├── linalg (3).py
    │   │   ├── linalg (4).py
    │   │   ├── linalg (5).py
    │   │   ├── linalg.py
    │   │   ├── lines.py
    │   │   ├── _linprog_ip.py
    │   │   ├── _linprog_util.py
    │   │   ├── lisp (2).py
    │   │   ├── lisp (3).py
    │   │   ├── lisp (4).py
    │   │   ├── lisp (5).py
    │   │   ├── lisp (6).py
    │   │   ├── lisp (7).py
    │   │   ├── lisp.py
    │   │   ├── _lobpcg.py
    │   │   ├── locale (2).py
    │   │   ├── locale (3).py
    │   │   ├── locale (4).py
    │   │   ├── locale (5).py
    │   │   ├── locale.py
    │   │   ├── locators (10).py
    │   │   ├── locators (11).py
    │   │   ├── locators (2).py
    │   │   ├── locators (3).py
    │   │   ├── locators (4).py
    │   │   ├── locators (5).py
    │   │   ├── locators (6).py
    │   │   ├── locators (7).py
    │   │   ├── locators (8).py
    │   │   ├── locators (9).py
    │   │   ├── locators.py
    │   │   ├── _logistic.py
    │   │   ├── loss.py
    │   │   ├── lr_scheduler.py
    │   │   ├── ltisys.py
    │   │   ├── mailbox (2).py
    │   │   ├── mailbox (3).py
    │   │   ├── mailbox (4).py
    │   │   ├── mailbox (5).py
    │   │   ├── mailbox.py
    │   │   ├── main (2).py
    │   │   ├── main (3).py
    │   │   ├── main (4).py
    │   │   ├── main (5).py
    │   │   ├── main (6).py
    │   │   ├── main (7).py
    │   │   ├── main.py
    │   │   ├── make (10).py
    │   │   ├── make (11).py
    │   │   ├── make (12).py
    │   │   ├── make (13).py
    │   │   ├── make (14).py
    │   │   ├── make (15).py
    │   │   ├── make (16).py
    │   │   ├── make (17).py
    │   │   ├── make (18).py
    │   │   ├── make (19).py
    │   │   ├── make (20).py
    │   │   ├── make (21).py
    │   │   ├── make (22).py
    │   │   ├── make (23).py
    │   │   ├── make (24).py
    │   │   ├── make (25).py
    │   │   ├── _make (2).py
    │   │   ├── make (2).py
    │   │   ├── _make (3).py
    │   │   ├── make (3).py
    │   │   ├── make (4).py
    │   │   ├── make (5).py
    │   │   ├── make (6).py
    │   │   ├── make (7).py
    │   │   ├── make (8).py
    │   │   ├── make (9).py
    │   │   ├── _make.py
    │   │   ├── make.py
    │   │   ├── managers (2).py
    │   │   ├── managers (3).py
    │   │   ├── managers (4).py
    │   │   ├── managers (5).py
    │   │   ├── managers (6).py
    │   │   ├── managers (7).py
    │   │   ├── managers.py
    │   │   ├── Manual_implementation_of_some_hash_functions (2).py
    │   │   ├── Manual_implementation_of_some_hash_functions (3).py
    │   │   ├── Manual_implementation_of_some_hash_functions (4).py
    │   │   ├── Manual_implementation_of_some_hash_functions (5).py
    │   │   ├── Manual_implementation_of_some_hash_functions.py
    │   │   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_ (2).py
    │   │   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_ (3).py
    │   │   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_ (4).py
    │   │   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_ (5).py
    │   │   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_.py
    │   │   ├── mapitags (2).py
    │   │   ├── mapitags (3).py
    │   │   ├── mapitags.py
    │   │   ├── mapper (2).py
    │   │   ├── mapper (3).py
    │   │   ├── mapper (4).py
    │   │   ├── mapper (5).py
    │   │   ├── mapper.py
    │   │   ├── _mapping (2).py
    │   │   ├── _mapping (3).py
    │   │   ├── _mapping (4).py
    │   │   ├── _mapping (5).py
    │   │   ├── _mapping (6).py
    │   │   ├── _mapping (7).py
    │   │   ├── _mapping.py
    │   │   ├── math2html (2).py
    │   │   ├── math2html (3).py
    │   │   ├── math2html.py
    │   │   ├── _mathtext_data.py
    │   │   ├── mathtext.py
    │   │   ├── measurements.py
    │   │   ├── merge.py
    │   │   ├── message (2).py
    │   │   ├── message (3).py
    │   │   ├── message (4).py
    │   │   ├── message (5).py
    │   │   ├── message (6).py
    │   │   ├── message (7).py
    │   │   ├── message.py
    │   │   ├── messaging (2).py
    │   │   ├── messaging (3).py
    │   │   ├── messaging.py
    │   │   ├── migration (2).py
    │   │   ├── migration (3).py
    │   │   ├── migration (4).py
    │   │   ├── migration (5).py
    │   │   ├── migration.py
    │   │   ├── minidom (2).py
    │   │   ├── minidom (3).py
    │   │   ├── minidom (4).py
    │   │   ├── minidom (5).py
    │   │   ├── minidom.py
    │   │   ├── misc_util (2).py
    │   │   ├── misc_util (3).py
    │   │   ├── misc_util (4).py
    │   │   ├── misc_util (5).py
    │   │   ├── misc_util.py
    │   │   ├── mock (2).py
    │   │   ├── mock (3).py
    │   │   ├── mock (4).py
    │   │   ├── mock (5).py
    │   │   ├── mockBase.py
    │   │   ├── mock.py
    │   │   ├── model.py
    │   │   ├── model_solution (2).py
    │   │   ├── model_solution (3).py
    │   │   ├── model_solution (4).py
    │   │   ├── model_solution (5).py
    │   │   ├── model_solution (6).py
    │   │   ├── model_solution (7).py
    │   │   ├── model_solution (8).py
    │   │   ├── model_solution (9).py
    │   │   ├── model_solution.py
    │   │   ├── modula2 (2).py
    │   │   ├── modula2 (3).py
    │   │   ├── modula2 (4).py
    │   │   ├── modula2 (5).py
    │   │   ├── modula2 (6).py
    │   │   ├── modula2 (7).py
    │   │   ├── modula2.py
    │   │   ├── module (2).py
    │   │   ├── module (3).py
    │   │   ├── module (4).py
    │   │   ├── module (5).py
    │   │   ├── module (6).py
    │   │   ├── module (7).py
    │   │   ├── module.py
    │   │   ├── morestats.py
    │   │   ├── morphology.py
    │   │   ├── mstats_basic.py
    │   │   ├── msvc (2).py
    │   │   ├── msvc (3).py
    │   │   ├── msvc (4).py
    │   │   ├── msvc (5).py
    │   │   ├── msvc (6).py
    │   │   ├── msvc (7).py
    │   │   ├── msvc (8).py
    │   │   ├── msvc (9).py
    │   │   ├── msvc.py
    │   │   ├── msvs (10).py
    │   │   ├── msvs (11).py
    │   │   ├── msvs (12).py
    │   │   ├── msvs (13).py
    │   │   ├── msvs (14).py
    │   │   ├── msvs (15).py
    │   │   ├── msvs (16).py
    │   │   ├── msvs (17).py
    │   │   ├── msvs (18).py
    │   │   ├── msvs (19).py
    │   │   ├── msvs (20).py
    │   │   ├── msvs (21).py
    │   │   ├── msvs (22).py
    │   │   ├── msvs (23).py
    │   │   ├── msvs (24).py
    │   │   ├── msvs (25).py
    │   │   ├── msvs (2).py
    │   │   ├── msvs (3).py
    │   │   ├── msvs (4).py
    │   │   ├── msvs (5).py
    │   │   ├── msvs (6).py
    │   │   ├── msvs (7).py
    │   │   ├── msvs (8).py
    │   │   ├── msvs (9).py
    │   │   ├── msvs_emulation (10).py
    │   │   ├── msvs_emulation (11).py
    │   │   ├── msvs_emulation (12).py
    │   │   ├── msvs_emulation (13).py
    │   │   ├── msvs_emulation (14).py
    │   │   ├── msvs_emulation (15).py
    │   │   ├── msvs_emulation (16).py
    │   │   ├── msvs_emulation (17).py
    │   │   ├── msvs_emulation (18).py
    │   │   ├── msvs_emulation (19).py
    │   │   ├── msvs_emulation (20).py
    │   │   ├── msvs_emulation (21).py
    │   │   ├── msvs_emulation (22).py
    │   │   ├── msvs_emulation (23).py
    │   │   ├── msvs_emulation (24).py
    │   │   ├── msvs_emulation (25).py
    │   │   ├── msvs_emulation (2).py
    │   │   ├── msvs_emulation (3).py
    │   │   ├── msvs_emulation (4).py
    │   │   ├── msvs_emulation (5).py
    │   │   ├── msvs_emulation (6).py
    │   │   ├── msvs_emulation (7).py
    │   │   ├── msvs_emulation (8).py
    │   │   ├── msvs_emulation (9).py
    │   │   ├── msvs_emulation.py
    │   │   ├── msvs.py
    │   │   ├── MSVSSettings (10).py
    │   │   ├── MSVSSettings (11).py
    │   │   ├── MSVSSettings (12).py
    │   │   ├── MSVSSettings (13).py
    │   │   ├── MSVSSettings (14).py
    │   │   ├── MSVSSettings (15).py
    │   │   ├── MSVSSettings (16).py
    │   │   ├── MSVSSettings (17).py
    │   │   ├── MSVSSettings (18).py
    │   │   ├── MSVSSettings (19).py
    │   │   ├── MSVSSettings (20).py
    │   │   ├── MSVSSettings (21).py
    │   │   ├── MSVSSettings (22).py
    │   │   ├── MSVSSettings (23).py
    │   │   ├── MSVSSettings (24).py
    │   │   ├── MSVSSettings (25).py
    │   │   ├── MSVSSettings (2).py
    │   │   ├── MSVSSettings (3).py
    │   │   ├── MSVSSettings (4).py
    │   │   ├── MSVSSettings (5).py
    │   │   ├── MSVSSettings (6).py
    │   │   ├── MSVSSettings (7).py
    │   │   ├── MSVSSettings (8).py
    │   │   ├── MSVSSettings (9).py
    │   │   ├── MSVSSettings.py
    │   │   ├── MSVSSettings_test (10).py
    │   │   ├── MSVSSettings_test (11).py
    │   │   ├── MSVSSettings_test (12).py
    │   │   ├── MSVSSettings_test (13).py
    │   │   ├── MSVSSettings_test (14).py
    │   │   ├── MSVSSettings_test (15).py
    │   │   ├── MSVSSettings_test (16).py
    │   │   ├── MSVSSettings_test (17).py
    │   │   ├── MSVSSettings_test (18).py
    │   │   ├── MSVSSettings_test (19).py
    │   │   ├── MSVSSettings_test (20).py
    │   │   ├── MSVSSettings_test (21).py
    │   │   ├── MSVSSettings_test (22).py
    │   │   ├── MSVSSettings_test (23).py
    │   │   ├── MSVSSettings_test (24).py
    │   │   ├── MSVSSettings_test (25).py
    │   │   ├── MSVSSettings_test (2).py
    │   │   ├── MSVSSettings_test (3).py
    │   │   ├── MSVSSettings_test (4).py
    │   │   ├── MSVSSettings_test (5).py
    │   │   ├── MSVSSettings_test (6).py
    │   │   ├── MSVSSettings_test (7).py
    │   │   ├── MSVSSettings_test (8).py
    │   │   ├── MSVSSettings_test (9).py
    │   │   ├── MSVSSettings_test.py
    │   │   ├── multiarray (2).py
    │   │   ├── multiarray (3).py
    │   │   ├── multiarray.py
    │   │   ├── _multilayer_perceptron.py
    │   │   ├── multi.py
    │   │   ├── _multivariate.py
    │   │   ├── naive_bayes.py
    │   │   ├── nanfunctions (2).py
    │   │   ├── nanfunctions (3).py
    │   │   ├── nanfunctions (4).py
    │   │   ├── nanfunctions (5).py
    │   │   ├── nanfunctions.py
    │   │   ├── nanops.py
    │   │   ├── nbextensions (2).py
    │   │   ├── nbextensions (3).py
    │   │   ├── nbextensions.py
    │   │   ├── ncl (2).py
    │   │   ├── ncl (3).py
    │   │   ├── ncl (4).py
    │   │   ├── ncl (5).py
    │   │   ├── ncl.py
    │   │   ├── nesting (2).py
    │   │   ├── nesting (3).py
    │   │   ├── nesting.py
    │   │   ├── ninja (10).py
    │   │   ├── ninja (11).py
    │   │   ├── ninja (12).py
    │   │   ├── ninja (13).py
    │   │   ├── ninja (14).py
    │   │   ├── ninja (15).py
    │   │   ├── ninja (16).py
    │   │   ├── ninja (17).py
    │   │   ├── ninja (18).py
    │   │   ├── ninja (19).py
    │   │   ├── ninja (20).py
    │   │   ├── ninja (21).py
    │   │   ├── ninja (22).py
    │   │   ├── ninja (23).py
    │   │   ├── ninja (24).py
    │   │   ├── ninja (25).py
    │   │   ├── ninja (2).py
    │   │   ├── ninja (3).py
    │   │   ├── ninja (4).py
    │   │   ├── ninja (5).py
    │   │   ├── ninja (6).py
    │   │   ├── ninja (7).py
    │   │   ├── ninja (8).py
    │   │   ├── ninja (9).py
    │   │   ├── ninja.py
    │   │   ├── _nmf.py
    │   │   ├── nntplib (2).py
    │   │   ├── nntplib (3).py
    │   │   ├── nntplib (4).py
    │   │   ├── nntplib (5).py
    │   │   ├── nntplib.py
    │   │   ├── node_classes.py
    │   │   ├── nodes (2).py
    │   │   ├── nodes (3).py
    │   │   ├── nodes.py
    │   │   ├── nonlin.py
    │   │   ├── notebookapp (2).py
    │   │   ├── notebookapp (3).py
    │   │   ├── notebookapp.py
    │   │   ├── npyio (2).py
    │   │   ├── npyio (3).py
    │   │   ├── npyio (4).py
    │   │   ├── npyio (5).py
    │   │   ├── npyio.py
    │   │   ├── numeric (2).py
    │   │   ├── numeric (3).py
    │   │   ├── numeric (4).py
    │   │   ├── numeric (5).py
    │   │   ├── numeric.py
    │   │   ├── observer.py
    │   │   ├── _ode.py
    │   │   ├── odrpack.py
    │   │   ├── offsetbox.py
    │   │   ├── oid.py
    │   │   ├── _openedge_builtins (2).py
    │   │   ├── _openedge_builtins (3).py
    │   │   ├── _openedge_builtins (4).py
    │   │   ├── _openedge_builtins (5).py
    │   │   ├── _openedge_builtins (6).py
    │   │   ├── _openedge_builtins (7).py
    │   │   ├── _openedge_builtins.py
    │   │   ├── operators (2).py
    │   │   ├── operators (3).py
    │   │   ├── operators (4).py
    │   │   ├── operators (5).py
    │   │   ├── operators.py
    │   │   ├── ops (2).py
    │   │   ├── ops (3).py
    │   │   ├── ops (4).py
    │   │   ├── ops (5).py
    │   │   ├── ops.py
    │   │   ├── optimize.py
    │   │   ├── optimizer.py
    │   │   ├── optparse (2).py
    │   │   ├── optparse (3).py
    │   │   ├── optparse (4).py
    │   │   ├── optparse (5).py
    │   │   ├── optparse.py
    │   │   ├── orthogonal.py
    │   │   ├── overrides.py
    │   │   ├── pairwise.py
    │   │   ├── parallel.py
    │   │   ├── parse (2).py
    │   │   ├── parse (3).py
    │   │   ├── parse (4).py
    │   │   ├── parse (5).py
    │   │   ├── parse.py
    │   │   ├── _parser (2).py
    │   │   ├── _parser (3).py
    │   │   ├── _parser (4).py
    │   │   ├── _parser (5).py
    │   │   ├── _parser (6).py
    │   │   ├── _parser (7).py
    │   │   ├── _parser (8).py
    │   │   ├── _parser (9).py
    │   │   ├── _parser.py
    │   │   ├── parser.py
    │   │   ├── parsers.py
    │   │   ├── patches.py
    │   │   ├── pathlib (2).py
    │   │   ├── pathlib (3).py
    │   │   ├── pathlib (4).py
    │   │   ├── pathlib (5).py
    │   │   ├── pathlib.py
    │   │   ├── pdb (2).py
    │   │   ├── pdb (3).py
    │   │   ├── pdb (4).py
    │   │   ├── pdb (5).py
    │   │   ├── pdb.py
    │   │   ├── _peak_finding.py
    │   │   ├── peb_teb (2).py
    │   │   ├── peb_teb (3).py
    │   │   ├── peb_teb (4).py
    │   │   ├── peb_teb (5).py
    │   │   ├── peb_teb.py
    │   │   ├── persistence (2).py
    │   │   ├── persistence (3).py
    │   │   ├── persistence (4).py
    │   │   ├── persistence (5).py
    │   │   ├── persistence.py
    │   │   ├── pg_logger.py
    │   │   ├── _php_builtins (2).py
    │   │   ├── _php_builtins (3).py
    │   │   ├── _php_builtins (4).py
    │   │   ├── _php_builtins (5).py
    │   │   ├── _php_builtins (6).py
    │   │   ├── _php_builtins (7).py
    │   │   ├── _php_builtins.py
    │   │   ├── pickle (2).py
    │   │   ├── pickle (3).py
    │   │   ├── pickle (4).py
    │   │   ├── pickle (5).py
    │   │   ├── pickle.py
    │   │   ├── pickletester.py
    │   │   ├── pickletools (2).py
    │   │   ├── pickletools (3).py
    │   │   ├── pickletools (4).py
    │   │   ├── pickletools (5).py
    │   │   ├── pickletools.py
    │   │   ├── platform (2).py
    │   │   ├── platform (3).py
    │   │   ├── platform (4).py
    │   │   ├── platform (5).py
    │   │   ├── platform.py
    │   │   ├── PngImagePlugin.py
    │   │   ├── png.py
    │   │   ├── _pocketfft (2).py
    │   │   ├── _pocketfft (3).py
    │   │   ├── _pocketfft.py
    │   │   ├── polar.py
    │   │   ├── polynomial (2).py
    │   │   ├── polynomial (3).py
    │   │   ├── polynomial (4).py
    │   │   ├── polynomial (5).py
    │   │   ├── polynomial (6).py
    │   │   ├── polynomial (7).py
    │   │   ├── polynomial (8).py
    │   │   ├── polynomial (9).py
    │   │   ├── polynomial.py
    │   │   ├── pooling.py
    │   │   ├── process (2).py
    │   │   ├── process (3).py
    │   │   ├── process (4).py
    │   │   ├── process (5).py
    │   │   ├── process_executor.py
    │   │   ├── process.py
    │   │   ├── profiler.py
    │   │   ├── prompt (2).py
    │   │   ├── prompt (3).py
    │   │   ├── prompt.py
    │   │   ├── prune.py
    │   │   ├── pscon (2).py
    │   │   ├── pscon (3).py
    │   │   ├── pscon.py
    │   │   ├── _pslinux.py
    │   │   ├── pycodestyle (2).py
    │   │   ├── pycodestyle (3).py
    │   │   ├── pycodestyle (4).py
    │   │   ├── pycodestyle (5).py
    │   │   ├── pycodestyle.py
    │   │   ├── _pydecimal (2).py
    │   │   ├── _pydecimal (3).py
    │   │   ├── _pydecimal (4).py
    │   │   ├── _pydecimal (5).py
    │   │   ├── _pydecimal.py
    │   │   ├── pydevd (2).py
    │   │   ├── pydevd (3).py
    │   │   ├── pydevd (4).py
    │   │   ├── pydevd (5).py
    │   │   ├── pydevd_api (2).py
    │   │   ├── pydevd_api (3).py
    │   │   ├── pydevd_api (4).py
    │   │   ├── pydevd_api (5).py
    │   │   ├── pydevd_api.py
    │   │   ├── pydevd_comm (2).py
    │   │   ├── pydevd_comm (3).py
    │   │   ├── pydevd_comm (4).py
    │   │   ├── pydevd_comm (5).py
    │   │   ├── pydevd_comm.py
    │   │   ├── pydevd_frame (2).py
    │   │   ├── pydevd_frame (3).py
    │   │   ├── pydevd_frame (4).py
    │   │   ├── pydevd_frame (5).py
    │   │   ├── pydevd_frame.py
    │   │   ├── pydevd_process_net_command_json (2).py
    │   │   ├── pydevd_process_net_command_json (3).py
    │   │   ├── pydevd_process_net_command_json (4).py
    │   │   ├── pydevd_process_net_command_json (5).py
    │   │   ├── pydevd_process_net_command_json.py
    │   │   ├── pydevd.py
    │   │   ├── pydevd_schema (2).py
    │   │   ├── pydevd_schema (3).py
    │   │   ├── pydevd_schema (4).py
    │   │   ├── pydevd_schema (5).py
    │   │   ├── pydevd_schema.py
    │   │   ├── _pydev_xmlrpclib (2).py
    │   │   ├── _pydev_xmlrpclib (3).py
    │   │   ├── _pydev_xmlrpclib (4).py
    │   │   ├── _pydev_xmlrpclib (5).py
    │   │   ├── _pydev_xmlrpclib.py
    │   │   ├── pydoc (2).py
    │   │   ├── pydoc (3).py
    │   │   ├── pydoc (4).py
    │   │   ├── pydoc (5).py
    │   │   ├── pydoc.py
    │   │   ├── _pyio (2).py
    │   │   ├── _pyio (3).py
    │   │   ├── _pyio (4).py
    │   │   ├── _pyio (5).py
    │   │   ├── _pyio.py
    │   │   ├── pylinter.py
    │   │   ├── pyparsing (10).py
    │   │   ├── pyparsing (11).py
    │   │   ├── pyparsing (12).py
    │   │   ├── pyparsing (13).py
    │   │   ├── pyparsing (14).py
    │   │   ├── pyparsing (15).py
    │   │   ├── pyparsing (16).py
    │   │   ├── pyparsing (17).py
    │   │   ├── pyparsing (18).py
    │   │   ├── pyparsing (19).py
    │   │   ├── pyparsing (20).py
    │   │   ├── pyparsing (21).py
    │   │   ├── pyparsing (22).py
    │   │   ├── pyparsing (23).py
    │   │   ├── pyparsing (24).py
    │   │   ├── pyparsing (25).py
    │   │   ├── pyparsing (26).py
    │   │   ├── pyparsing (27).py
    │   │   ├── pyparsing (28).py
    │   │   ├── pyparsing (29).py
    │   │   ├── pyparsing (2).py
    │   │   ├── pyparsing (30).py
    │   │   ├── pyparsing (31).py
    │   │   ├── pyparsing (32).py
    │   │   ├── pyparsing (33).py
    │   │   ├── pyparsing (34).py
    │   │   ├── pyparsing (35).py
    │   │   ├── pyparsing (3).py
    │   │   ├── pyparsing (4).py
    │   │   ├── pyparsing (5).py
    │   │   ├── pyparsing (6).py
    │   │   ├── pyparsing (7).py
    │   │   ├── pyparsing (8).py
    │   │   ├── pyparsing (9).py
    │   │   ├── pyparsing.py
    │   │   ├── Py_Pi_Day_2017 (2).py
    │   │   ├── Py_Pi_Day_2017 (3).py
    │   │   ├── Py_Pi_Day_2017 (4).py
    │   │   ├── Py_Pi_Day_2017 (5).py
    │   │   ├── Py_Pi_Day_2017.py
    │   │   ├── pyplot.py
    │   │   ├── pyshell (2).py
    │   │   ├── pyshell (3).py
    │   │   ├── pyshell (4).py
    │   │   ├── pyshell (5).py
    │   │   ├── pyshell.py
    │   │   ├── pytables.py
    │   │   ├── python (2).py
    │   │   ├── python3.py
    │   │   ├── python (3).py
    │   │   ├── python (4).py
    │   │   ├── python (5).py
    │   │   ├── python (6).py
    │   │   ├── python (7).py
    │   │   ├── python.py
    │   │   ├── quantization_patterns.py
    │   │   ├── quantize.py
    │   │   ├── query (2).py
    │   │   ├── query (3).py
    │   │   ├── query (4).py
    │   │   ├── query (5).py
    │   │   ├── query.py
    │   │   ├── quiver.py
    │   │   ├── _ranking.py
    │   │   ├── rcsetup.py
    │   │   ├── recfunctions (2).py
    │   │   ├── recfunctions (3).py
    │   │   ├── recfunctions.py
    │   │   ├── recompiler (2).py
    │   │   ├── recompiler (3).py
    │   │   ├── recompiler (4).py
    │   │   ├── recompiler (5).py
    │   │   ├── recompiler.py
    │   │   ├── refactoring.py
    │   │   ├── relationships (2).py
    │   │   ├── relationships (3).py
    │   │   ├── relationships (4).py
    │   │   ├── relationships (5).py
    │   │   ├── relationships.py
    │   │   ├── request (2).py
    │   │   ├── request (3).py
    │   │   ├── request (4).py
    │   │   ├── request (5).py
    │   │   ├── request.py
    │   │   ├── resample.py
    │   │   ├── resolver.py
    │   │   ├── result (2).py
    │   │   ├── result (3).py
    │   │   ├── result (4).py
    │   │   ├── result (5).py
    │   │   ├── result.py
    │   │   ├── rfc4511.py
    │   │   ├── _ridge.py
    │   │   ├── rnn (2).py
    │   │   ├── rnn (3).py
    │   │   ├── rnn (4).py
    │   │   ├── rnn_cell.py
    │   │   ├── rnn_cell_test.py
    │   │   ├── rnn.py
    │   │   ├── rolling.py
    │   │   ├── rotation.py
    │   │   ├── routing (2).py
    │   │   ├── routing (3).py
    │   │   ├── routing.py
    │   │   ├── rpc_test (2).py
    │   │   ├── rpc_test (3).py
    │   │   ├── rpc_test (4).py
    │   │   ├── rpc_test.py
    │   │   ├── rrule (2).py
    │   │   ├── rrule (3).py
    │   │   ├── rrule (4).py
    │   │   ├── rrule (5).py
    │   │   ├── rrule (6).py
    │   │   ├── rrule (7).py
    │   │   ├── rrule (8).py
    │   │   ├── rrule (9).py
    │   │   ├── rrule.py
    │   │   ├── rules (2).py
    │   │   ├── rules (3).py
    │   │   ├── rules (4).py
    │   │   ├── rules (5).py
    │   │   ├── rules.py
    │   │   ├── run_test.py
    │   │   ├── _samples_generator.py
    │   │   ├── scanner (2).py
    │   │   ├── scanner (3).py
    │   │   ├── scanner.py
    │   │   ├── schema (10).py
    │   │   ├── schema (11).py
    │   │   ├── schema (12).py
    │   │   ├── schema (13).py
    │   │   ├── schema (14).py
    │   │   ├── schema (15).py
    │   │   ├── schema (16).py
    │   │   ├── schema (17).py
    │   │   ├── schema (18).py
    │   │   ├── schema (19).py
    │   │   ├── schema (2).py
    │   │   ├── schema (3).py
    │   │   ├── schema (4).py
    │   │   ├── schema (5).py
    │   │   ├── schema (6).py
    │   │   ├── schema (7).py
    │   │   ├── schema (8).py
    │   │   ├── schema (9).py
    │   │   ├── schema.py
    │   │   ├── _scilab_builtins (2).py
    │   │   ├── _scilab_builtins (3).py
    │   │   ├── _scilab_builtins (4).py
    │   │   ├── _scilab_builtins (5).py
    │   │   ├── _scilab_builtins (6).py
    │   │   ├── _scilab_builtins (7).py
    │   │   ├── _scilab_builtins.py
    │   │   ├── scintillacon (2).py
    │   │   ├── scintillacon (3).py
    │   │   ├── scintillacon.py
    │   │   ├── scoped_nodes.py
    │   │   ├── scrape_module (2).py
    │   │   ├── scrape_module (3).py
    │   │   ├── scrape_module.py
    │   │   ├── scripting (2).py
    │   │   ├── scripting (3).py
    │   │   ├── scripting (4).py
    │   │   ├── scripting (5).py
    │   │   ├── scripting (6).py
    │   │   ├── scripting (7).py
    │   │   ├── scripting.py
    │   │   ├── _script.py
    │   │   ├── _search.py
    │   │   ├── selectable (2).py
    │   │   ├── selectable (3).py
    │   │   ├── selectable (4).py
    │   │   ├── selectable (5).py
    │   │   ├── selectable.py
    │   │   ├── serializer.py
    │   │   ├── series.py
    │   │   ├── server (2).py
    │   │   ├── server (3).py
    │   │   ├── server (4).py
    │   │   ├── server (5).py
    │   │   ├── server.py
    │   │   ├── session (2).py
    │   │   ├── session (3).py
    │   │   ├── session (4).py
    │   │   ├── session (5).py
    │   │   ├── session.py
    │   │   ├── setup (2).py
    │   │   ├── setup (3).py
    │   │   ├── setup (4).py
    │   │   ├── setup (5).py
    │   │   ├── setup.py
    │   │   ├── shellcon (2).py
    │   │   ├── shellcon (3).py
    │   │   ├── shellcon.py
    │   │   ├── _shgo.py
    │   │   ├── shutil (2).py
    │   │   ├── shutil (3).py
    │   │   ├── shutil (4).py
    │   │   ├── shutil (5).py
    │   │   ├── shutil.py
    │   │   ├── signaltools.py
    │   │   ├── slapd24.py
    │   │   ├── smtplib (2).py
    │   │   ├── smtplib (3).py
    │   │   ├── smtplib (4).py
    │   │   ├── smtplib (5).py
    │   │   ├── smtplib.py
    │   │   ├── spawner.py
    │   │   ├── spectral.py
    │   │   ├── _split.py
    │   │   ├── sql.py
    │   │   ├── sqltypes (2).py
    │   │   ├── sqltypes (3).py
    │   │   ├── sqltypes (4).py
    │   │   ├── sqltypes (5).py
    │   │   ├── sqltypes.py
    │   │   ├── sre_parse (2).py
    │   │   ├── sre_parse (3).py
    │   │   ├── sre_parse (4).py
    │   │   ├── sre_parse (5).py
    │   │   ├── sre_parse.py
    │   │   ├── sshtunnel.py
    │   │   ├── ssl (2).py
    │   │   ├── ssl (3).py
    │   │   ├── ssl (4).py
    │   │   ├── ssl (5).py
    │   │   ├── ssl.py
    │   │   ├── stata.py
    │   │   ├── statemachine (2).py
    │   │   ├── statemachine (3).py
    │   │   ├── statemachine.py
    │   │   ├── states (2).py
    │   │   ├── states (3).py
    │   │   ├── states.py
    │   │   ├── statistics.py
    │   │   ├── stats.py
    │   │   ├── _stochastic_gradient.py
    │   │   ├── strategies (2).py
    │   │   ├── strategies (3).py
    │   │   ├── strategies (4).py
    │   │   ├── strategies (5).py
    │   │   ├── strategies.py
    │   │   ├── strategy_options (2).py
    │   │   ├── strategy_options (3).py
    │   │   ├── strategy_options (4).py
    │   │   ├── strategy_options (5).py
    │   │   ├── strategy_options.py
    │   │   ├── strings.py
    │   │   ├── string_tests.py
    │   │   ├── style.py
    │   │   ├── subprocess (2).py
    │   │   ├── subprocess (3).py
    │   │   ├── subprocess (4).py
    │   │   ├── subprocess (5).py
    │   │   ├── subprocess.py
    │   │   ├── symbolic_opset9.py
    │   │   ├── system (2).py
    │   │   ├── system (3).py
    │   │   ├── system (4).py
    │   │   ├── system (5).py
    │   │   ├── system_info (2).py
    │   │   ├── system_info (3).py
    │   │   ├── system_info (4).py
    │   │   ├── system_info (5).py
    │   │   ├── system_info.py
    │   │   ├── system.py
    │   │   ├── table_wide (2).py
    │   │   ├── table_wide (3).py
    │   │   ├── table_wide.py
    │   │   ├── table_zero (2).py
    │   │   ├── table_zero (3).py
    │   │   ├── table_zero.py
    │   │   ├── tamil_stemmer.py
    │   │   ├── tarfile (10).py
    │   │   ├── tarfile (11).py
    │   │   ├── tarfile (12).py
    │   │   ├── tarfile (13).py
    │   │   ├── tarfile (14).py
    │   │   ├── tarfile (15).py
    │   │   ├── tarfile (16).py
    │   │   ├── tarfile (17).py
    │   │   ├── tarfile (2).py
    │   │   ├── tarfile (3).py
    │   │   ├── tarfile (4).py
    │   │   ├── tarfile (5).py
    │   │   ├── tarfile (6).py
    │   │   ├── tarfile (7).py
    │   │   ├── tarfile (8).py
    │   │   ├── tarfile (9).py
    │   │   ├── tarfile.py
    │   │   ├── templates (2).py
    │   │   ├── templates (3).py
    │   │   ├── templates (4).py
    │   │   ├── templates (5).py
    │   │   ├── templates (6).py
    │   │   ├── templates (7).py
    │   │   ├── templates.py
    │   │   ├── _tensor_docs.py
    │   │   ├── _tensor.py
    │   │   ├── test_algos.py
    │   │   ├── test_analytics.py
    │   │   ├── test_api.py
    │   │   ├── test_argparse.py
    │   │   ├── test_arithmetic.py
    │   │   ├── test_array (2).py
    │   │   ├── test_array (3).py
    │   │   ├── test_arraypad (2).py
    │   │   ├── test_arraypad (3).py
    │   │   ├── test_arraypad (4).py
    │   │   ├── test_arraypad (5).py
    │   │   ├── test_arraypad.py
    │   │   ├── test_array.py
    │   │   ├── test_ast.py
    │   │   ├── test_autograd.py
    │   │   ├── test_axes.py
    │   │   ├── test_base (2).py
    │   │   ├── test_base (3).py
    │   │   ├── test_base_events.py
    │   │   ├── test_base.py
    │   │   ├── test_basic (2).py
    │   │   ├── test_basic (3).py
    │   │   ├── test_basic.py
    │   │   ├── test_bdb.py
    │   │   ├── test_benchmark_utils.py
    │   │   ├── test_bigmem.py
    │   │   ├── test_binary_ufuncs.py
    │   │   ├── test_bsplines.py
    │   │   ├── test_buffer.py
    │   │   ├── test_builtin.py
    │   │   ├── test_bytes.py
    │   │   ├── test_c10d.py
    │   │   ├── test_calendar.py
    │   │   ├── test_case (2).py
    │   │   ├── test_case (3).py
    │   │   ├── test_case.py
    │   │   ├── test_categorical.py
    │   │   ├── test_classification.py
    │   │   ├── test_class_type.py
    │   │   ├── test_client.py
    │   │   ├── test_codeccallbacks.py
    │   │   ├── test_codecs.py
    │   │   ├── test_collections.py
    │   │   ├── test_column_transformer.py
    │   │   ├── test_common (2).py
    │   │   ├── test_common (3).py
    │   │   ├── test_common.py
    │   │   ├── test_compileall.py
    │   │   ├── test_concat.py
    │   │   ├── test_concrete (2).py
    │   │   ├── test_concrete (3).py
    │   │   ├── test_concrete.py
    │   │   ├── test_concurrent_futures.py
    │   │   ├── test_configdialog (2).py
    │   │   ├── test_configdialog (3).py
    │   │   ├── test_configdialog (4).py
    │   │   ├── test_configdialog (5).py
    │   │   ├── test_configdialog.py
    │   │   ├── test_configparser.py
    │   │   ├── test_constructors (2).py
    │   │   ├── test_constructors (3).py
    │   │   ├── test_constructors.py
    │   │   ├── test_core (2).py
    │   │   ├── test_core (3).py
    │   │   ├── test_core (4).py
    │   │   ├── test_core (5).py
    │   │   ├── test_core (6).py
    │   │   ├── test_core (7).py
    │   │   ├── test_core (8).py
    │   │   ├── test_core (9).py
    │   │   ├── test_core.py
    │   │   ├── test_coroutines.py
    │   │   ├── test_csv.py
    │   │   ├── test_cuda.py
    │   │   ├── test_dataclasses.py
    │   │   ├── test_dataloader.py
    │   │   ├── test_data.py
    │   │   ├── test_datetime (2).py
    │   │   ├── test_datetime (3).py
    │   │   ├── test_datetime (4).py
    │   │   ├── test_datetime (5).py
    │   │   ├── test_datetime64.py
    │   │   ├── test_datetime_index.py
    │   │   ├── test_datetimelike.py
    │   │   ├── test_datetime.py
    │   │   ├── test_decimal.py
    │   │   ├── test_decomp.py
    │   │   ├── test_decomp_update.py
    │   │   ├── test_descr.py
    │   │   ├── test_dict.py
    │   │   ├── test__differential_evolution.py
    │   │   ├── test_dis.py
    │   │   ├── test_distance.py
    │   │   ├── test_distributions (2).py
    │   │   ├── test_distributions (3).py
    │   │   ├── test_distributions.py
    │   │   ├── test_doctest.py
    │   │   ├── test_dtype (2).py
    │   │   ├── test_dtype (3).py
    │   │   ├── test_dtype.py
    │   │   ├── test_einsum (2).py
    │   │   ├── test_einsum (3).py
    │   │   ├── test_einsum.py
    │   │   ├── test_email.py
    │   │   ├── test_embed.py
    │   │   ├── test_enum.py
    │   │   ├── test_eval.py
    │   │   ├── test_events.py
    │   │   ├── test_exceptions.py
    │   │   ├── test_execute.py
    │   │   ├── test_extras (2).py
    │   │   ├── test_extras (3).py
    │   │   ├── test_extras (4).py
    │   │   ├── test_extras (5).py
    │   │   ├── test_extras.py
    │   │   ├── test_filter_design.py
    │   │   ├── test_fitpack2.py
    │   │   ├── test_fixers (2).py
    │   │   ├── test_fixers (3).py
    │   │   ├── test_fixers.py
    │   │   ├── test_float.py
    │   │   ├── test_foreach.py
    │   │   ├── test_forest.py
    │   │   ├── test_format.py
    │   │   ├── test_frame_apply.py
    │   │   ├── test_frame.py
    │   │   ├── test_freezing.py
    │   │   ├── test_fstring.py
    │   │   ├── test_ftplib.py
    │   │   ├── test_function_base (2).py
    │   │   ├── test_function_base (3).py
    │   │   ├── test_function_base (4).py
    │   │   ├── test_function_base (5).py
    │   │   ├── test_function_base.py
    │   │   ├── test_functools.py
    │   │   ├── test_fx_experimental.py
    │   │   ├── test_fx.py
    │   │   ├── test_gaussian_mixture.py
    │   │   ├── test_gc.py
    │   │   ├── test_gdb.py
    │   │   ├── test_generator_mt19937 (2).py
    │   │   ├── test_generator_mt19937 (3).py
    │   │   ├── test_generator_mt19937.py
    │   │   ├── test_generators.py
    │   │   ├── test_geometry_managers (2).py
    │   │   ├── test_geometry_managers (3).py
    │   │   ├── test_geometry_managers.py
    │   │   ├── test_getargs2.py
    │   │   ├── test_gettext.py
    │   │   ├── test_gradient_boosting.py
    │   │   ├── test_grammar.py
    │   │   ├── test_groupby.py
    │   │   ├── test_hashlib.py
    │   │   ├── test_headerregistry.py
    │   │   ├── test__header_value_parser.py
    │   │   ├── test_hierarchy.py
    │   │   ├── test_http_cookiejar.py
    │   │   ├── test_httplib.py
    │   │   ├── test_httpservers.py
    │   │   ├── test_imaplib.py
    │   │   ├── test_impute.py
    │   │   ├── test_indexing (2).py
    │   │   ├── test_indexing (3).py
    │   │   ├── test_indexing (4).py
    │   │   ├── test_indexing (5).py
    │   │   ├── test_indexing (6).py
    │   │   ├── test_indexing (7).py
    │   │   ├── test_indexing.py
    │   │   ├── test_inference.py
    │   │   ├── _testing.py
    │   │   ├── test_inspect.py
    │   │   ├── test_internals.py
    │   │   ├── test_interpolate.py
    │   │   ├── test_io (2).py
    │   │   ├── test_io (3).py
    │   │   ├── test_io (4).py
    │   │   ├── test_io (5).py
    │   │   ├── test_io (6).py
    │   │   ├── test_io (7).py
    │   │   ├── test_io.py
    │   │   ├── test_ipaddress.py
    │   │   ├── test_itertools.py
    │   │   ├── test_jit_fuser_te.py
    │   │   ├── test_jit.py
    │   │   ├── test_kdtree.py
    │   │   ├── test_k_means.py
    │   │   ├── test_lapack.py
    │   │   ├── test_linalg (2).py
    │   │   ├── test_linalg (3).py
    │   │   ├── test_linalg (4).py
    │   │   ├── test_linalg (5).py
    │   │   ├── test_linalg (6).py
    │   │   ├── test_linalg (7).py
    │   │   ├── test_linalg.py
    │   │   ├── test_linprog.py
    │   │   ├── test_list_dict.py
    │   │   ├── test_loader (2).py
    │   │   ├── test_loader (3).py
    │   │   ├── test_loader.py
    │   │   ├── test_logging.py
    │   │   ├── test_logistic.py
    │   │   ├── test_long.py
    │   │   ├── test_ltisys.py
    │   │   ├── test_lzma.py
    │   │   ├── test_mailbox.py
    │   │   ├── test_math.py
    │   │   ├── test_memmapping.py
    │   │   ├── test_merge_asof.py
    │   │   ├── test_merge.py
    │   │   ├── test_minidom.py
    │   │   ├── test_mio.py
    │   │   ├── test_mkldnn.py
    │   │   ├── test_mlab.py
    │   │   ├── testmock (2).py
    │   │   ├── testmock (3).py
    │   │   ├── testmock.py
    │   │   ├── test_morestats.py
    │   │   ├── test_mpmath.py
    │   │   ├── test_mstats_basic.py
    │   │   ├── test_multiarray (2).py
    │   │   ├── test_multiarray (3).py
    │   │   ├── test_multiarray (4).py
    │   │   ├── test_multiarray (5).py
    │   │   ├── test_multiarray.py
    │   │   ├── test_multilevel.py
    │   │   ├── _test_multiprocessing.py
    │   │   ├── test_multivariate.py
    │   │   ├── test_namedtensor.py
    │   │   ├── test_ndimage.py
    │   │   ├── test_nditer (2).py
    │   │   ├── test_nditer (3).py
    │   │   ├── test_nditer (4).py
    │   │   ├── test_nditer (5).py
    │   │   ├── test_nditer.py
    │   │   ├── test_neighbors.py
    │   │   ├── test_nn.py
    │   │   ├── test_nntplib.py
    │   │   ├── test_numeric (2).py
    │   │   ├── test_numeric (3).py
    │   │   ├── test_numeric (4).py
    │   │   ├── test_numeric (5).py
    │   │   ├── test_numeric (6).py
    │   │   ├── test_numeric (7).py
    │   │   ├── test_numeric.py
    │   │   ├── test_offsets.py
    │   │   ├── test_openml.py
    │   │   ├── test_optimize.py
    │   │   ├── test_optim.py
    │   │   ├── test_optparse.py
    │   │   ├── test_os.py
    │   │   ├── test_pairwise.py
    │   │   ├── test_pandas.py
    │   │   ├── test_parallel.py
    │   │   ├── test_parse_dates.py
    │   │   ├── testpatch (2).py
    │   │   ├── testpatch (3).py
    │   │   ├── testpatch.py
    │   │   ├── test_pathlib.py
    │   │   ├── test_pdb.py
    │   │   ├── test_period (2).py
    │   │   ├── test_period (3).py
    │   │   ├── test_period.py
    │   │   ├── test_pipeline.py
    │   │   ├── test_pivot.py
    │   │   ├── test_posix.py
    │   │   ├── test_pprint.py
    │   │   ├── test_preprocessor.py
    │   │   ├── test_pydoc.py
    │   │   ├── test_pytorch_onnx_caffe2.py
    │   │   ├── test_pytorch_onnx_onnxruntime.py
    │   │   ├── test_qhull.py
    │   │   ├── test_quantized_module.py
    │   │   ├── test_quantized_op.py
    │   │   ├── test_quantized_tensor.py
    │   │   ├── test_quantize_fx.py
    │   │   ├── test_quantize_jit.py
    │   │   ├── test_quantize.py
    │   │   ├── test_query_eval.py
    │   │   ├── test_random (2).py
    │   │   ├── test_random (3).py
    │   │   ├── test_random (4).py
    │   │   ├── test_random (5).py
    │   │   ├── test_random.py
    │   │   ├── test_randomstate (2).py
    │   │   ├── test_randomstate (3).py
    │   │   ├── test_randomstate.py
    │   │   ├── test_ranking.py
    │   │   ├── test_readers.py
    │   │   ├── test_recfunctions (2).py
    │   │   ├── test_recfunctions (3).py
    │   │   ├── test_recfunctions.py
    │   │   ├── test_reductions (2).py
    │   │   ├── test_reductions (3).py
    │   │   ├── test_reductions.py
    │   │   ├── test_reflection (2).py
    │   │   ├── test_reflection (3).py
    │   │   ├── test_reflection (4).py
    │   │   ├── test_reflection (5).py
    │   │   ├── test_reflection.py
    │   │   ├── test_regression (2).py
    │   │   ├── test_regression (3).py
    │   │   ├── test_regression (4).py
    │   │   ├── test_regression (5).py
    │   │   ├── test_regression.py
    │   │   ├── test_regrtest.py
    │   │   ├── test_replace.py
    │   │   ├── test_re.py
    │   │   ├── test_reshape.py
    │   │   ├── test_ridge.py
    │   │   ├── test_rollbar.py
    │   │   ├── test_sax.py
    │   │   ├── test_search.py
    │   │   ├── test_selector_events.py
    │   │   ├── test_set.py
    │   │   ├── test_sgd.py
    │   │   ├── test_shutil.py
    │   │   ├── test_signal.py
    │   │   ├── test_signaltools.py
    │   │   ├── test_smtpd.py
    │   │   ├── test_smtplib.py
    │   │   ├── test_socket.py
    │   │   ├── test_sparse.py
    │   │   ├── test_spectral_ops.py
    │   │   ├── test_spectral.py
    │   │   ├── test_split.py
    │   │   ├── test_sql.py
    │   │   ├── test_ssl.py
    │   │   ├── test_stata.py
    │   │   ├── test_statistics.py
    │   │   ├── test_stats.py
    │   │   ├── test_store.py
    │   │   ├── test_strings.py
    │   │   ├── test_style.py
    │   │   ├── test_subprocess.py
    │   │   ├── test_svm.py
    │   │   ├── test_sys.py
    │   │   ├── test_sys_settrace.py
    │   │   ├── test_tarfile.py
    │   │   ├── test_tasks.py
    │   │   ├── test_tempfile.py
    │   │   ├── test_tensor_creation_ops.py
    │   │   ├── test_tensorexpr.py
    │   │   ├── test_text.py
    │   │   ├── test_threading.py
    │   │   ├── test_ticker.py
    │   │   ├── test_timedelta64.py
    │   │   ├── test_time.py
    │   │   ├── test_timezones.py
    │   │   ├── test_to_csv.py
    │   │   ├── test_to_datetime.py
    │   │   ├── test_tokenize.py
    │   │   ├── test_torch.py
    │   │   ├── test_traceback.py
    │   │   ├── test_tracer.py
    │   │   ├── test_traitlets (2).py
    │   │   ├── test_traitlets (3).py
    │   │   ├── test_traitlets.py
    │   │   ├── test_transform.py
    │   │   ├── test_tree.py
    │   │   ├── test_triangulation.py
    │   │   ├── test_type_promotion.py
    │   │   ├── test_types.py
    │   │   ├── test_typing.py
    │   │   ├── test_ufunc (2).py
    │   │   ├── test_ufunc (3).py
    │   │   ├── test_ufunc (4).py
    │   │   ├── test_ufunc (5).py
    │   │   ├── test_ufunc.py
    │   │   ├── test_umath (2).py
    │   │   ├── test_umath (3).py
    │   │   ├── test_umath (4).py
    │   │   ├── test_umath (5).py
    │   │   ├── test_umath.py
    │   │   ├── test_unary_ufuncs.py
    │   │   ├── test_unicode.py
    │   │   ├── test_unix_events.py
    │   │   ├── test_urllib2.py
    │   │   ├── test_urllib.py
    │   │   ├── test_urlparse.py
    │   │   ├── test_utils (2).py
    │   │   ├── test_utils (3).py
    │   │   ├── test_utils.py
    │   │   ├── test_uuid.py
    │   │   ├── test_validation (2).py
    │   │   ├── test_validation (3).py
    │   │   ├── test_validation.py
    │   │   ├── test_validators (2).py
    │   │   ├── test_validators (3).py
    │   │   ├── test_validators.py
    │   │   ├── test_view_ops.py
    │   │   ├── test_vmap.py
    │   │   ├── test_weakref.py
    │   │   ├── test_widgets (2).py
    │   │   ├── test_widgets (3).py
    │   │   ├── test_widgets (4).py
    │   │   ├── test_widgets (5).py
    │   │   ├── test_widgets (6).py
    │   │   ├── test_widgets (7).py
    │   │   ├── test_widgets.py
    │   │   ├── test_win32file (2).py
    │   │   ├── test_win32file (3).py
    │   │   ├── test_win32file.py
    │   │   ├── test_workflow_module.py
    │   │   ├── test_writers.py
    │   │   ├── test_xml_etree.py
    │   │   ├── test_xmlrpc.py
    │   │   ├── test_xnnpack_integration.py
    │   │   ├── test__xxsubinterpreters.py
    │   │   ├── test_yqm_offsets.py
    │   │   ├── test_zipfile.py
    │   │   ├── test_zoneinfo.py
    │   │   ├── texi2html (2).py
    │   │   ├── texi2html (3).py
    │   │   ├── texi2html.py
    │   │   ├── text (2).py
    │   │   ├── text (3).py
    │   │   ├── textio (2).py
    │   │   ├── textio (3).py
    │   │   ├── textio (4).py
    │   │   ├── textio (5).py
    │   │   ├── textio.py
    │   │   ├── text.py
    │   │   ├── thread (2).py
    │   │   ├── thread (3).py
    │   │   ├── thread (4).py
    │   │   ├── thread (5).py
    │   │   ├── threading (2).py
    │   │   ├── threading (3).py
    │   │   ├── threading (4).py
    │   │   ├── threading (5).py
    │   │   ├── threading.py
    │   │   ├── thread.py
    │   │   ├── ticker.py
    │   │   ├── TiffImagePlugin.py
    │   │   ├── tix (2).py
    │   │   ├── tix (3).py
    │   │   ├── tix (4).py
    │   │   ├── tix (5).py
    │   │   ├── tix.py
    │   │   ├── _tokenizer (10).py
    │   │   ├── _tokenizer (11).py
    │   │   ├── _tokenizer (12).py
    │   │   ├── _tokenizer (13).py
    │   │   ├── _tokenizer (2).py
    │   │   ├── _tokenizer (3).py
    │   │   ├── _tokenizer (4).py
    │   │   ├── _tokenizer (5).py
    │   │   ├── _tokenizer (6).py
    │   │   ├── _tokenizer (7).py
    │   │   ├── _tokenizer (8).py
    │   │   ├── _tokenizer (9).py
    │   │   ├── _tokenizer.py
    │   │   ├── topics (2).py
    │   │   ├── topics (3).py
    │   │   ├── topics (4).py
    │   │   ├── topics (5).py
    │   │   ├── topics.py
    │   │   ├── _torch_docs.py
    │   │   ├── totp.py
    │   │   ├── TP2__python (2).py
    │   │   ├── TP2__python (3).py
    │   │   ├── TP2__python (4).py
    │   │   ├── TP2__python (5).py
    │   │   ├── TP2__python.py
    │   │   ├── TP3__python (2).py
    │   │   ├── TP3__python (3).py
    │   │   ├── TP3__python (4).py
    │   │   ├── TP3__python (5).py
    │   │   ├── TP3__python.py
    │   │   ├── _trace.py
    │   │   ├── traitlets (2).py
    │   │   ├── traitlets (3).py
    │   │   ├── traitlets.py
    │   │   ├── transforms.py
    │   │   ├── transport.py
    │   │   ├── triinterpolate.py
    │   │   ├── ttk (2).py
    │   │   ├── ttk (3).py
    │   │   ├── ttk (4).py
    │   │   ├── ttk (5).py
    │   │   ├── ttk.py
    │   │   ├── turkish_stemmer.py
    │   │   ├── turtle (2).py
    │   │   ├── turtle (3).py
    │   │   ├── turtle (4).py
    │   │   ├── turtle (5).py
    │   │   ├── turtle.py
    │   │   ├── twemoji_db.py
    │   │   ├── type_api (2).py
    │   │   ├── type_api (3).py
    │   │   ├── type_api (4).py
    │   │   ├── type_api (5).py
    │   │   ├── type_api.py
    │   │   ├── typecheck.py
    │   │   ├── typing (2).py
    │   │   ├── typing (3).py
    │   │   ├── typing (4).py
    │   │   ├── typing (5).py
    │   │   ├── typing_extensions (2).py
    │   │   ├── typing_extensions (3).py
    │   │   ├── typing_extensions.py
    │   │   ├── typing.py
    │   │   ├── tz (2).py
    │   │   ├── tz (3).py
    │   │   ├── tz (4).py
    │   │   ├── tz (5).py
    │   │   ├── tz (6).py
    │   │   ├── tz (7).py
    │   │   ├── tz (8).py
    │   │   ├── tz (9).py
    │   │   ├── tz.py
    │   │   ├── ufunc_docstrings.py
    │   │   ├── ultratb (2).py
    │   │   ├── ultratb (3).py
    │   │   ├── ultratb.py
    │   │   ├── unistring (2).py
    │   │   ├── unistring (3).py
    │   │   ├── unistring (4).py
    │   │   ├── unistring (5).py
    │   │   ├── unistring (6).py
    │   │   ├── unistring (7).py
    │   │   ├── unistring.py
    │   │   ├── univ.py
    │   │   ├── unix_events (2).py
    │   │   ├── unix_events (3).py
    │   │   ├── unix_events (4).py
    │   │   ├── unix_events (5).py
    │   │   ├── unix_events.py
    │   │   ├── user32 (2).py
    │   │   ├── user32 (3).py
    │   │   ├── user32 (4).py
    │   │   ├── user32 (5).py
    │   │   ├── user32.py
    │   │   ├── util (10).py
    │   │   ├── util (11).py
    │   │   ├── util (12).py
    │   │   ├── util (13).py
    │   │   ├── util (14).py
    │   │   ├── util (15).py
    │   │   ├── util (16).py
    │   │   ├── util (17).py
    │   │   ├── util (2).py
    │   │   ├── util (3).py
    │   │   ├── util (4).py
    │   │   ├── util (5).py
    │   │   ├── util (6).py
    │   │   ├── util (7).py
    │   │   ├── util (8).py
    │   │   ├── util (9).py
    │   │   ├── util.py
    │   │   ├── utils (10).py
    │   │   ├── utils (11).py
    │   │   ├── utils (12).py
    │   │   ├── utils (13).py
    │   │   ├── utils (14).py
    │   │   ├── utils (15).py
    │   │   ├── utils (2).py
    │   │   ├── utils (3).py
    │   │   ├── utils (4).py
    │   │   ├── utils (5).py
    │   │   ├── utils (6).py
    │   │   ├── utils (7).py
    │   │   ├── utils (8).py
    │   │   ├── utils (9).py
    │   │   ├── utils.py
    │   │   ├── uts46data (10).py
    │   │   ├── uts46data (11).py
    │   │   ├── uts46data (12).py
    │   │   ├── uts46data (13).py
    │   │   ├── uts46data (14).py
    │   │   ├── uts46data (15).py
    │   │   ├── uts46data (2).py
    │   │   ├── uts46data (3).py
    │   │   ├── uts46data (4).py
    │   │   ├── uts46data (5).py
    │   │   ├── uts46data (6).py
    │   │   ├── uts46data (7).py
    │   │   ├── uts46data (8).py
    │   │   ├── uts46data (9).py
    │   │   ├── uts46data.py
    │   │   ├── _validation.py
    │   │   ├── validation.py
    │   │   ├── variables.py
    │   │   ├── vengine_cpy (2).py
    │   │   ├── vengine_cpy (3).py
    │   │   ├── vengine_cpy (4).py
    │   │   ├── vengine_cpy (5).py
    │   │   ├── vengine_cpy.py
    │   │   ├── vi (2).py
    │   │   ├── vi (3).py
    │   │   ├── views.py
    │   │   ├── _vim_builtins (2).py
    │   │   ├── _vim_builtins (3).py
    │   │   ├── _vim_builtins (4).py
    │   │   ├── _vim_builtins (5).py
    │   │   ├── _vim_builtins (6).py
    │   │   ├── _vim_builtins (7).py
    │   │   ├── _vim_builtins.py
    │   │   ├── vi.py
    │   │   ├── VirtualBox_constants.py
    │   │   ├── web (2).py
    │   │   ├── web (3).py
    │   │   ├── web.py
    │   │   ├── websocket (2).py
    │   │   ├── websocket (3).py
    │   │   ├── websocket.py
    │   │   ├── web_test (2).py
    │   │   ├── web_test (3).py
    │   │   ├── web_test.py
    │   │   ├── _weight_boosting.py
    │   │   ├── wheel (2).py
    │   │   ├── wheel (3).py
    │   │   ├── wheel (4).py
    │   │   ├── wheel (5).py
    │   │   ├── wheel (6).py
    │   │   ├── wheel (7).py
    │   │   ├── wheel.py
    │   │   ├── widgets.py
    │   │   ├── win32con (2).py
    │   │   ├── win32con (3).py
    │   │   ├── win32con.py
    │   │   ├── win32cryptcon (2).py
    │   │   ├── win32cryptcon (3).py
    │   │   ├── win32cryptcon.py
    │   │   ├── win32inetcon (2).py
    │   │   ├── win32inetcon (3).py
    │   │   ├── win32inetcon.py
    │   │   ├── windows.py
    │   │   ├── winerror (2).py
    │   │   ├── winerror (3).py
    │   │   ├── winerror.py
    │   │   ├── writer.py
    │   │   ├── xcode (10).py
    │   │   ├── xcode (11).py
    │   │   ├── xcode (12).py
    │   │   ├── xcode (13).py
    │   │   ├── xcode (14).py
    │   │   ├── xcode (15).py
    │   │   ├── xcode (16).py
    │   │   ├── xcode (17).py
    │   │   ├── xcode (18).py
    │   │   ├── xcode (19).py
    │   │   ├── xcode (20).py
    │   │   ├── xcode (21).py
    │   │   ├── xcode (22).py
    │   │   ├── xcode (23).py
    │   │   ├── xcode (24).py
    │   │   ├── xcode (25).py
    │   │   ├── xcode (2).py
    │   │   ├── xcode (3).py
    │   │   ├── xcode (4).py
    │   │   ├── xcode (5).py
    │   │   ├── xcode (6).py
    │   │   ├── xcode (7).py
    │   │   ├── xcode (8).py
    │   │   ├── xcode (9).py
    │   │   ├── xcode_emulation (10).py
    │   │   ├── xcode_emulation (11).py
    │   │   ├── xcode_emulation (12).py
    │   │   ├── xcode_emulation (13).py
    │   │   ├── xcode_emulation (14).py
    │   │   ├── xcode_emulation (15).py
    │   │   ├── xcode_emulation (16).py
    │   │   ├── xcode_emulation (17).py
    │   │   ├── xcode_emulation (18).py
    │   │   ├── xcode_emulation (19).py
    │   │   ├── xcode_emulation (20).py
    │   │   ├── xcode_emulation (21).py
    │   │   ├── xcode_emulation (22).py
    │   │   ├── xcode_emulation (23).py
    │   │   ├── xcode_emulation (24).py
    │   │   ├── xcode_emulation (25).py
    │   │   ├── xcode_emulation (2).py
    │   │   ├── xcode_emulation (3).py
    │   │   ├── xcode_emulation (4).py
    │   │   ├── xcode_emulation (5).py
    │   │   ├── xcode_emulation (6).py
    │   │   ├── xcode_emulation (7).py
    │   │   ├── xcode_emulation (8).py
    │   │   ├── xcode_emulation (9).py
    │   │   ├── xcode_emulation.py
    │   │   ├── xcodeproj_file (10).py
    │   │   ├── xcodeproj_file (11).py
    │   │   ├── xcodeproj_file (12).py
    │   │   ├── xcodeproj_file (13).py
    │   │   ├── xcodeproj_file (14).py
    │   │   ├── xcodeproj_file (15).py
    │   │   ├── xcodeproj_file (16).py
    │   │   ├── xcodeproj_file (17).py
    │   │   ├── xcodeproj_file (18).py
    │   │   ├── xcodeproj_file (19).py
    │   │   ├── xcodeproj_file (20).py
    │   │   ├── xcodeproj_file (21).py
    │   │   ├── xcodeproj_file (22).py
    │   │   ├── xcodeproj_file (23).py
    │   │   ├── xcodeproj_file (24).py
    │   │   ├── xcodeproj_file (2).py
    │   │   ├── xcodeproj_file (3).py
    │   │   ├── xcodeproj_file (4).py
    │   │   ├── xcodeproj_file (5).py
    │   │   ├── xcodeproj_file (6).py
    │   │   ├── xcodeproj_file (7).py
    │   │   ├── xcodeproj_file (8).py
    │   │   ├── xcodeproj_file (9).py
    │   │   ├── xcodeproj_file.py
    │   │   ├── xcode.py
    │   │   ├── yacc (2).py
    │   │   ├── yacc (3).py
    │   │   ├── yacc (4).py
    │   │   ├── yacc (5).py
    │   │   ├── yacc (6).py
    │   │   ├── yacc (7).py
    │   │   ├── yacc.py
    │   │   ├── yacctab (2).py
    │   │   ├── yacctab (3).py
    │   │   ├── yacctab (4).py
    │   │   ├── yacctab (5).py
    │   │   ├── yacctab.py
    │   │   ├── zeros.py
    │   │   ├── zipfile (2).py
    │   │   ├── zipfile (3).py
    │   │   ├── zipfile (4).py
    │   │   ├── zipfile (5).py
    │   │   ├── zipfile.py
    │   │   └── zone.py
    │   ├── 215 Kth Largest Element in an Array.py
    │   ├── 2dArray.py
    │   ├── 2_hidden_layers_neural_network.py
    │   ├── 3c1e4b6eda55_.py
    │   ├── 3n_plus_1.py
    │   ├── 3.py
    │   ├── 4be95337cf96_.py
    │   ├── 5
    │   │   ├── 05_prefix_postfix_infix.py
    │   │   ├── 09d53fca90c7_.py
    │   │   ├── 7.7-question (2).py
    │   │   ├── 7.7-question (3).py
    │   │   ├── 7.7-question.py
    │   │   ├── abitype (2).py
    │   │   ├── abitype.py
    │   │   ├── absoft (2).py
    │   │   ├── absoft (3).py
    │   │   ├── absoft.py
    │   │   ├── abstract.py
    │   │   ├── acquire.py
    │   │   ├── adadelta_test.py
    │   │   ├── adagrad_test.py
    │   │   ├── adam.py
    │   │   ├── adamw.py
    │   │   ├── adaptive_weight.py
    │   │   ├── addMembersToGroups.py
    │   │   ├── adodbapitestconfig (2).py
    │   │   ├── adodbapitestconfig.py
    │   │   ├── advanced (2).py
    │   │   ├── advanced.py
    │   │   ├── aead (2).py
    │   │   ├── aead.py
    │   │   ├── agent.py
    │   │   ├── algebra (2).py
    │   │   ├── algebra (3).py
    │   │   ├── algebra (4).py
    │   │   ├── algebra.py
    │   │   ├── alien_invasion (2).py
    │   │   ├── alien_invasion (3).py
    │   │   ├── alien_invasion (4).py
    │   │   ├── alien_invasion (5).py
    │   │   ├── alien_invasion (6).py
    │   │   ├── alien_invasion.py
    │   │   ├── align.py
    │   │   ├── analysis (2).py
    │   │   ├── analysis (3).py
    │   │   ├── analysis (4).py
    │   │   ├── analysis (5).py
    │   │   ├── analysis (6).py
    │   │   ├── analysis.py
    │   │   ├── An_Identiconizer_generator_implementation_in_python (2).py
    │   │   ├── An_Identiconizer_generator_implementation_in_python (3).py
    │   │   ├── An_Identiconizer_generator_implementation_in_python.py
    │   │   ├── annotation (2).py
    │   │   ├── annotation (3).py
    │   │   ├── annotation.py
    │   │   ├── ansi (2).py
    │   │   ├── ansi (3).py
    │   │   ├── ansi (4).py
    │   │   ├── ansi.py
    │   │   ├── api (10).py
    │   │   ├── api (11).py
    │   │   ├── api (2).py
    │   │   ├── api (3).py
    │   │   ├── api (4).py
    │   │   ├── api (5).py
    │   │   ├── api (6).py
    │   │   ├── api (7).py
    │   │   ├── api (8).py
    │   │   ├── api (9).py
    │   │   ├── api.py
    │   │   ├── api_test (2).py
    │   │   ├── api_test.py
    │   │   ├── app (2).py
    │   │   ├── application.py
    │   │   ├── app.py
    │   │   ├── apps.py
    │   │   ├── arc_cosine_feature_map.py
    │   │   ├── archive_util (2).py
    │   │   ├── archive_util (3).py
    │   │   ├── archive_util (4).py
    │   │   ├── archive_util (5).py
    │   │   ├── archive_util.py
    │   │   ├── area.py
    │   │   ├── area_volume_calculator.py
    │   │   ├── array (2).py
    │   │   ├── array.py
    │   │   ├── arrayterator (2).py
    │   │   ├── arrayterator (3).py
    │   │   ├── arrayterator.py
    │   │   ├── asserts (2).py
    │   │   ├── asserts.py
    │   │   ├── ast (2).py
    │   │   ├── ast (3).py
    │   │   ├── ast.py
    │   │   ├── async_case (2).py
    │   │   ├── async_case (3).py
    │   │   ├── async_case.py
    │   │   ├── asyncio_test (2).py
    │   │   ├── asyncio_test.py
    │   │   ├── asyncsupport (2).py
    │   │   ├── asyncsupport (3).py
    │   │   ├── asyncsupport (4).py
    │   │   ├── asyncsupport (5).py
    │   │   ├── asyncsupport (6).py
    │   │   ├── asyncsupport.py
    │   │   ├── attach_script (2).py
    │   │   ├── attach_script (3).py
    │   │   ├── attach_script.py
    │   │   ├── attr_list.py
    │   │   ├── autocompletion (2).py
    │   │   ├── autocompletion (3).py
    │   │   ├── autocompletion (4).py
    │   │   ├── autocompletion (5).py
    │   │   ├── autocompletion.py
    │   │   ├── auto_suggest (2).py
    │   │   ├── auto_suggest.py
    │   │   ├── avl_tree (2).py
    │   │   ├── avl_tree (3).py
    │   │   ├── avl_tree.py
    │   │   ├── axes_size.py
    │   │   ├── B04170_05_python_Draft_01 (2).py
    │   │   ├── B04170_05_python_Draft_01.py
    │   │   ├── backend (2).py
    │   │   ├── backend (3).py
    │   │   ├── backend (4).py
    │   │   ├── backend_inline (2).py
    │   │   ├── backend_inline.py
    │   │   ├── backend_macosx.py
    │   │   ├── backend.py
    │   │   ├── Background (2).py
    │   │   ├── Background (3).py
    │   │   ├── Background (4).py
    │   │   ├── Background.py
    │   │   ├── back_propagation_neural_network (10).py
    │   │   ├── back_propagation_neural_network (11).py
    │   │   ├── back_propagation_neural_network (2).py
    │   │   ├── back_propagation_neural_network (3).py
    │   │   ├── back_propagation_neural_network (4).py
    │   │   ├── back_propagation_neural_network (5).py
    │   │   ├── back_propagation_neural_network (6).py
    │   │   ├── back_propagation_neural_network (7).py
    │   │   ├── back_propagation_neural_network (8).py
    │   │   ├── back_propagation_neural_network (9).py
    │   │   ├── back_propagation_neural_network.py
    │   │   ├── backup (2).py
    │   │   ├── backup.py
    │   │   ├── base (10).py
    │   │   ├── base (11).py
    │   │   ├── base (12).py
    │   │   ├── base (13).py
    │   │   ├── base (14).py
    │   │   ├── base (15).py
    │   │   ├── base (16).py
    │   │   ├── base (17).py
    │   │   ├── base (18).py
    │   │   ├── base (2).py
    │   │   ├── base (3).py
    │   │   ├── base (4).py
    │   │   ├── base (5).py
    │   │   ├── base (6).py
    │   │   ├── base (7).py
    │   │   ├── base (8).py
    │   │   ├── base (9).py
    │   │   ├── base_checker.py
    │   │   ├── base_command.py
    │   │   ├── base_frontend_mixin (2).py
    │   │   ├── base_frontend_mixin.py
    │   │   ├── base.py
    │   │   ├── basic (2).py
    │   │   ├── basic_graphs (10).py
    │   │   ├── basic_graphs (11).py
    │   │   ├── basic_graphs (12).py
    │   │   ├── basic_graphs (13).py
    │   │   ├── basic_graphs (2).py
    │   │   ├── basic_graphs (3).py
    │   │   ├── basic_graphs (4).py
    │   │   ├── basic_graphs (5).py
    │   │   ├── basic_graphs (6).py
    │   │   ├── basic_graphs (7).py
    │   │   ├── basic_graphs (8).py
    │   │   ├── basic_graphs (9).py
    │   │   ├── basic_graphs.py
    │   │   ├── basic.py
    │   │   ├── basics.py
    │   │   ├── basic_string (10).py
    │   │   ├── basic_string (11).py
    │   │   ├── basic_string (12).py
    │   │   ├── basic_string (2).py
    │   │   ├── basic_string (3).py
    │   │   ├── basic_string (4).py
    │   │   ├── basic_string (5).py
    │   │   ├── basic_string (6).py
    │   │   ├── basic_string (7).py
    │   │   ├── basic_string (8).py
    │   │   ├── basic_string (9).py
    │   │   ├── basic_string.py
    │   │   ├── basictimerapp (2).py
    │   │   ├── basictimerapp.py
    │   │   ├── batchnorm (2).py
    │   │   ├── batchnorm.py
    │   │   ├── battleplayer.py
    │   │   ├── battleserver.py
    │   │   ├── bdist (2).py
    │   │   ├── bdist (3).py
    │   │   ├── bdist.py
    │   │   ├── benchmark_caffe2.py
    │   │   ├── benchmark_pytorch.py
    │   │   ├── bibtex_beautifier.py
    │   │   ├── bidirectional_a_star (2).py
    │   │   ├── bidirectional_a_star (3).py
    │   │   ├── bidirectional_a_star (4).py
    │   │   ├── bidirectional_a_star (5).py
    │   │   ├── bidirectional_a_star (6).py
    │   │   ├── bidirectional_a_star.py
    │   │   ├── bidirectional_breadth_first_search (10).py
    │   │   ├── bidirectional_breadth_first_search (11).py
    │   │   ├── bidirectional_breadth_first_search (12).py
    │   │   ├── bidirectional_breadth_first_search (13).py
    │   │   ├── bidirectional_breadth_first_search (2).py
    │   │   ├── bidirectional_breadth_first_search (3).py
    │   │   ├── bidirectional_breadth_first_search (4).py
    │   │   ├── bidirectional_breadth_first_search (5).py
    │   │   ├── bidirectional_breadth_first_search (6).py
    │   │   ├── bidirectional_breadth_first_search (7).py
    │   │   ├── bidirectional_breadth_first_search (8).py
    │   │   ├── bidirectional_breadth_first_search (9).py
    │   │   ├── bidirectional_breadth_first_search.py
    │   │   ├── binary_build_data.py
    │   │   ├── binary_search_tree (10).py
    │   │   ├── binary_search_tree (11).py
    │   │   ├── binary_search_tree (12).py
    │   │   ├── binary_search_tree (13).py
    │   │   ├── binary_search_tree (14).py
    │   │   ├── binary_search_tree (15).py
    │   │   ├── binary_search_tree (16).py
    │   │   ├── binary_search_tree (17).py
    │   │   ├── binary_search_tree (18).py
    │   │   ├── binary_search_tree (19).py
    │   │   ├── binary_search_tree (2).py
    │   │   ├── binary_search_tree (3).py
    │   │   ├── binary_search_tree (4).py
    │   │   ├── binary_search_tree (5).py
    │   │   ├── binary_search_tree (6).py
    │   │   ├── binary_search_tree (7).py
    │   │   ├── binary_search_tree (8).py
    │   │   ├── binary_search_tree (9).py
    │   │   ├── binary_search_tree.py
    │   │   ├── binary_smoketest.py
    │   │   ├── binding.py
    │   │   ├── bind.py
    │   │   ├── Bird (2).py
    │   │   ├── Bird (3).py
    │   │   ├── Bird (4).py
    │   │   ├── Bird.py
    │   │   ├── bisect_percentile_op_test.py
    │   │   ├── bitwise_ops.py
    │   │   ├── blackjack (2).py
    │   │   ├── blackjack (3).py
    │   │   ├── blackjack.py
    │   │   ├── blackjack_rr (2).py
    │   │   ├── blackjack_rr (3).py
    │   │   ├── blackjack_rr.py
    │   │   ├── blas_compare.py
    │   │   ├── blas_compare_setup.py
    │   │   ├── blockchain (2).py
    │   │   ├── blockchain (3).py
    │   │   ├── blockchain (4).py
    │   │   ├── blockchain (5).py
    │   │   ├── blockchain.py
    │   │   ├── brain_gi.py
    │   │   ├── brain_numpy_core_numerictypes.py
    │   │   ├── brain_six.py
    │   │   ├── broadcasting.py
    │   │   ├── browseProjects (2).py
    │   │   ├── browseProjects.py
    │   │   ├── btm_matcher (2).py
    │   │   ├── btm_matcher (3).py
    │   │   ├── btm_matcher (4).py
    │   │   ├── btm_matcher.py
    │   │   ├── buffered_pipe.py
    │   │   ├── build (2).py
    │   │   ├── build (3).py
    │   │   ├── build (4).py
    │   │   ├── build (5).py
    │   │   ├── build (6).py
    │   │   ├── build (7).py
    │   │   ├── build_bundled.py
    │   │   ├── build_clib (2).py
    │   │   ├── build_clib (3).py
    │   │   ├── build_clib (4).py
    │   │   ├── build_clib (5).py
    │   │   ├── build_clib (6).py
    │   │   ├── build_clib.py
    │   │   ├── build_env (2).py
    │   │   ├── build_env (3).py
    │   │   ├── build_env (4).py
    │   │   ├── build_env (5).py
    │   │   ├── build_env.py
    │   │   ├── build_listing_legacy.py
    │   │   ├── build.py
    │   │   ├── build_scripts (2).py
    │   │   ├── build_scripts (3).py
    │   │   ├── build_scripts (4).py
    │   │   ├── build_scripts (5).py
    │   │   ├── build_scripts (6).py
    │   │   ├── build_scripts.py
    │   │   ├── burrows_wheeler (10).py
    │   │   ├── burrows_wheeler (11).py
    │   │   ├── burrows_wheeler (12).py
    │   │   ├── burrows_wheeler (13).py
    │   │   ├── burrows_wheeler (2).py
    │   │   ├── burrows_wheeler (3).py
    │   │   ├── burrows_wheeler (4).py
    │   │   ├── burrows_wheeler (5).py
    │   │   ├── burrows_wheeler (6).py
    │   │   ├── burrows_wheeler (7).py
    │   │   ├── burrows_wheeler (8).py
    │   │   ├── burrows_wheeler (9).py
    │   │   ├── burrows_wheeler.py
    │   │   ├── bytecode (2).py
    │   │   ├── bytecode.py
    │   │   ├── cache (2).py
    │   │   ├── cache (3).py
    │   │   ├── cache (4).py
    │   │   ├── cache (5).py
    │   │   ├── cache (6).py
    │   │   ├── cache (7).py
    │   │   ├── cache (8).py
    │   │   ├── cache (9).py
    │   │   ├── cached_py_info.py
    │   │   ├── cache.py
    │   │   ├── caesar_cipher (10).py
    │   │   ├── caesar_cipher (11).py
    │   │   ├── caesar_cipher (12).py
    │   │   ├── caesar_cipher (2).py
    │   │   ├── caesar_cipher (3).py
    │   │   ├── caesar_cipher (4).py
    │   │   ├── caesar_cipher (5).py
    │   │   ├── caesar_cipher (6).py
    │   │   ├── caesar_cipher (7).py
    │   │   ├── caesar_cipher (8).py
    │   │   ├── caesar_cipher (9).py
    │   │   ├── caesar_cipher.py
    │   │   ├── calc_interets.py
    │   │   ├── callable.py
    │   │   ├── calltip (2).py
    │   │   ├── calltip (3).py
    │   │   ├── calltip.py
    │   │   ├── CallTips (2).py
    │   │   ├── CallTips.py
    │   │   ├── calltip_w (2).py
    │   │   ├── calltip_w (3).py
    │   │   ├── calltip_w.py
    │   │   ├── caret.py
    │   │   ├── category.py
    │   │   ├── cerapi (2).py
    │   │   ├── cerapi.py
    │   │   ├── cffi_opcode (2).py
    │   │   ├── cffi_opcode (3).py
    │   │   ├── cffi_opcode.py
    │   │   ├── channels (2).py
    │   │   ├── channels.py
    │   │   ├── check (10).py
    │   │   ├── check (11).py
    │   │   ├── check (2).py
    │   │   ├── check (3).py
    │   │   ├── check (4).py
    │   │   ├── check (5).py
    │   │   ├── check (6).py
    │   │   ├── check (7).py
    │   │   ├── check (8).py
    │   │   ├── check (9).py
    │   │   ├── check_backward_compatibility.py
    │   │   ├── checkers.py
    │   │   ├── checkGroupsMemberships.py
    │   │   ├── checkpoint (4).py
    │   │   ├── checkpoint.py
    │   │   ├── check.py
    │   │   ├── check_voyages-sncf.py
    │   │   ├── ciDict.py
    │   │   ├── ciphers.py
    │   │   ├── clang_coverage.py
    │   │   ├── clang_format_utils.py
    │   │   ├── class_weight.py
    │   │   ├── clean (2).py
    │   │   ├── clean (3).py
    │   │   ├── clean.py
    │   │   ├── cli (2).py
    │   │   ├── client (2).py
    │   │   ├── client_mining_blockchain (2).py
    │   │   ├── client_mining_blockchain (3).py
    │   │   ├── client_mining_blockchain (4).py
    │   │   ├── client_mining_blockchain.py
    │   │   ├── client.py
    │   │   ├── cli.py
    │   │   ├── cnn.py
    │   │   ├── codeop (2).py
    │   │   ├── codeop (3).py
    │   │   ├── codeop.py
    │   │   ├── collection.py
    │   │   ├── coloransi (2).py
    │   │   ├── coloransi.py
    │   │   ├── colorbox.py
    │   │   ├── commandpath.py
    │   │   ├── command.py
    │   │   ├── commitlist.py
    │   │   ├── common (2).py
    │   │   ├── common_cuda.py
    │   │   ├── common.py
    │   │   ├── common_quantized.py
    │   │   ├── compat (2).py
    │   │   ├── compat (3).py
    │   │   ├── compat (4).py
    │   │   ├── compatibility_tags.py
    │   │   ├── compat.py
    │   │   ├── compilerop (2).py
    │   │   ├── compilerop.py
    │   │   ├── completion (2).py
    │   │   ├── completion.py
    │   │   ├── completion_widget.py
    │   │   ├── components (2).py
    │   │   ├── components.py
    │   │   ├── compute_norm_for_blobs_test.py
    │   │   ├── concat_split_op_test.py
    │   │   ├── concurrent.py
    │   │   ├── concurrent_test (2).py
    │   │   ├── concurrent_test.py
    │   │   ├── conf (2).py
    │   │   ├── conf (3).py
    │   │   ├── conf (4).py
    │   │   ├── conf (5).py
    │   │   ├── config (2).py
    │   │   ├── config (3).py
    │   │   ├── config_manager.py
    │   │   ├── config.py
    │   │   ├── configuration (2).py
    │   │   ├── configuration.py
    │   │   ├── conf.py
    │   │   ├── conftest (2).py
    │   │   ├── conftest.py
    │   │   ├── connect (2).py
    │   │   ├── connect.py
    │   │   ├── constants.py
    │   │   ├── conv (4).py
    │   │   ├── conversion (2).py
    │   │   ├── conversion (3).py
    │   │   ├── conversion (4).py
    │   │   ├── conversion (5).py
    │   │   ├── conversion (6).py
    │   │   ├── conversion.py
    │   │   ├── conv_op_test.py
    │   │   ├── conv.py
    │   │   ├── conv_relu.py
    │   │   ├── copy_ops_test.py
    │   │   ├── copyreg (2).py
    │   │   ├── copyreg (3).py
    │   │   ├── copyreg.py
    │   │   ├── core.py
    │   │   ├── cpp_jit.py
    │   │   ├── cProfile (2).py
    │   │   ├── cProfile (3).py
    │   │   ├── cProfile.py
    │   │   ├── cpu (2).py
    │   │   ├── cpu (3).py
    │   │   ├── cpu.py
    │   │   ├── crack_caesar (2).py
    │   │   ├── crack_caesar (3).py
    │   │   ├── crack_caesar.py
    │   │   ├── crashhandler (2).py
    │   │   ├── crashhandler.py
    │   │   ├── creator.py
    │   │   ├── crypto_kx.py
    │   │   ├── crypto_scalarmult.py
    │   │   ├── csc.py
    │   │   ├── css.py
    │   │   ├── cuda.py
    │   │   ├── cudnn_recurrent_test.py
    │   │   ├── cursor.py
    │   │   ├── customprint (2).py
    │   │   ├── customprint.py
    │   │   ├── cwrap_common.py
    │   │   ├── daemon_python (2).py
    │   │   ├── daemon_python.py
    │   │   ├── danish_stemmer.py
    │   │   ├── Data_Challenge__Mangaki__September_2017 (2).py
    │   │   ├── Data_Challenge__Mangaki__September_2017 (3).py
    │   │   ├── Data_Challenge__Mangaki__September_2017.py
    │   │   ├── datastructures-avl_tree1 (2).py
    │   │   ├── datastructures-avl_tree1 (3).py
    │   │   ├── datastructures-avl_tree1.py
    │   │   ├── datastructures-avl_tree2 (2).py
    │   │   ├── datastructures-avl_tree2 (3).py
    │   │   ├── datastructures-avl_tree2.py
    │   │   ├── datastructures-binary_search_tree (2).py
    │   │   ├── datastructures-binary_search_tree (3).py
    │   │   ├── datastructures-binary_search_tree.py
    │   │   ├── datastructures-linkedlist_doubly.py
    │   │   ├── datastructures-skip_list (2).py
    │   │   ├── datastructures-skip_list (3).py
    │   │   ├── datastructures-skip_list.py
    │   │   ├── data_workers_test.py
    │   │   ├── datetime_parse (2).py
    │   │   ├── datetime_parse.py
    │   │   ├── db_file_reader.py
    │   │   ├── dbgcommands (2).py
    │   │   ├── dbgcommands.py
    │   │   ├── dbutil.py
    │   │   ├── debug (2).py
    │   │   ├── debug (3).py
    │   │   ├── debug (4).py
    │   │   ├── debug (5).py
    │   │   ├── debug (6).py
    │   │   ├── debugger (2).py
    │   │   ├── debugger (3).py
    │   │   ├── debugger (4).py
    │   │   ├── debugger.py
    │   │   ├── debughelpers (2).py
    │   │   ├── debughelpers.py
    │   │   ├── debug.py
    │   │   ├── decision_tree (10).py
    │   │   ├── decision_tree (11).py
    │   │   ├── decision_tree (2).py
    │   │   ├── decision_tree (3).py
    │   │   ├── decision_tree (4).py
    │   │   ├── decision_tree (5).py
    │   │   ├── decision_tree (6).py
    │   │   ├── decision_tree (7).py
    │   │   ├── decision_tree (8).py
    │   │   ├── decision_tree (9).py
    │   │   ├── decision_tree.py
    │   │   ├── decoder.py
    │   │   ├── decomp_lu.py
    │   │   ├── decorator (2).py
    │   │   ├── decorator.py
    │   │   ├── decor.py
    │   │   ├── default_value_test.py
    │   │   ├── depends.py
    │   │   ├── deprecations (2).py
    │   │   ├── deprecations (3).py
    │   │   ├── deprecations.py
    │   │   ├── desktopmanager (2).py
    │   │   ├── desktopmanager.py
    │   │   ├── detectron_keypoints.py
    │   │   ├── develop (2).py
    │   │   ├── develop (3).py
    │   │   ├── develop (4).py
    │   │   ├── develop (5).py
    │   │   ├── develop.py
    │   │   ├── dialog (2).py
    │   │   ├── dialog.py
    │   │   ├── diffie_hellman_key_exchange (10).py
    │   │   ├── diffie_hellman_key_exchange (2).py
    │   │   ├── diffie_hellman_key_exchange (3).py
    │   │   ├── diffie_hellman_key_exchange (4).py
    │   │   ├── diffie_hellman_key_exchange (5).py
    │   │   ├── diffie_hellman_key_exchange (6).py
    │   │   ├── diffie_hellman_key_exchange (7).py
    │   │   ├── diffie_hellman_key_exchange (8).py
    │   │   ├── diffie_hellman_key_exchange (9).py
    │   │   ├── diffie_hellman_key_exchange.py
    │   │   ├── dijkstra_algorithm (10).py
    │   │   ├── dijkstra_algorithm (11).py
    │   │   ├── dijkstra_algorithm (12).py
    │   │   ├── dijkstra_algorithm (13).py
    │   │   ├── dijkstra_algorithm (2).py
    │   │   ├── dijkstra_algorithm (3).py
    │   │   ├── dijkstra_algorithm (4).py
    │   │   ├── dijkstra_algorithm (5).py
    │   │   ├── dijkstra_algorithm (6).py
    │   │   ├── dijkstra_algorithm (7).py
    │   │   ├── dijkstra_algorithm (8).py
    │   │   ├── dijkstra_algorithm (9).py
    │   │   ├── dijkstra_algorithm.py
    │   │   ├── dijkstras.py
    │   │   ├── dimension (2).py
    │   │   ├── dimension.py
    │   │   ├── direct_url (2).py
    │   │   ├── direct_url (3).py
    │   │   ├── direct_url (4).py
    │   │   ├── direct_url (5).py
    │   │   ├── direct_url.py
    │   │   ├── dir_util (2).py
    │   │   ├── dir_util (3).py
    │   │   ├── dir_util (4).py
    │   │   ├── dir_util (5).py
    │   │   ├── dir_util (6).py
    │   │   ├── dir_util.py
    │   │   ├── dist_optimizer_test.py
    │   │   ├── distributed (2).py
    │   │   ├── distributed.py
    │   │   ├── dist_utils.py
    │   │   ├── django (10).py
    │   │   ├── django (11).py
    │   │   ├── django (12).py
    │   │   ├── django (13).py
    │   │   ├── django (2).py
    │   │   ├── django (3).py
    │   │   ├── django (4).py
    │   │   ├── django (5).py
    │   │   ├── django (6).py
    │   │   ├── django (7).py
    │   │   ├── django (8).py
    │   │   ├── django (9).py
    │   │   ├── django.py
    │   │   ├── dll (2).py
    │   │   ├── dll (3).py
    │   │   ├── dll (4).py
    │   │   ├── dll (5).py
    │   │   ├── dll (6).py
    │   │   ├── dll.py
    │   │   ├── dml (2).py
    │   │   ├── dml (3).py
    │   │   ├── dml.py
    │   │   ├── doccer.py
    │   │   ├── doc_controls_test.py
    │   │   ├── docutils_xml (2).py
    │   │   ├── docutils_xml.py
    │   │   ├── dop853_coefficients.py
    │   │   ├── doubly_linked_list (10).py
    │   │   ├── doubly_linked_list (11).py
    │   │   ├── doubly_linked_list (12).py
    │   │   ├── doubly_linked_list (13).py
    │   │   ├── doubly_linked_list (14).py
    │   │   ├── doubly_linked_list (15).py
    │   │   ├── doubly_linked_list (16).py
    │   │   ├── doubly_linked_list (17).py
    │   │   ├── doubly_linked_list (18).py
    │   │   ├── doubly_linked_list (19).py
    │   │   ├── doubly_linked_list (20).py
    │   │   ├── doubly_linked_list (21).py
    │   │   ├── doubly_linked_list (22).py
    │   │   ├── doubly_linked_list (23).py
    │   │   ├── doubly_linked_list (24).py
    │   │   ├── doubly_linked_list (25).py
    │   │   ├── doubly_linked_list (26).py
    │   │   ├── doubly_linked_list (27).py
    │   │   ├── doubly_linked_list (28).py
    │   │   ├── doubly_linked_list (29).py
    │   │   ├── doubly_linked_list (2).py
    │   │   ├── doubly_linked_list (30).py
    │   │   ├── doubly_linked_list (31).py
    │   │   ├── doubly_linked_list (3).py
    │   │   ├── doubly_linked_list (4).py
    │   │   ├── doubly_linked_list (5).py
    │   │   ├── doubly_linked_list (6).py
    │   │   ├── doubly_linked_list (7).py
    │   │   ├── doubly_linked_list (8).py
    │   │   ├── doubly_linked_list (9).py
    │   │   ├── doubly_linked_list.py
    │   │   ├── doubly_linked_list_two (10).py
    │   │   ├── doubly_linked_list_two (11).py
    │   │   ├── doubly_linked_list_two (12).py
    │   │   ├── doubly_linked_list_two (2).py
    │   │   ├── doubly_linked_list_two (3).py
    │   │   ├── doubly_linked_list_two (4).py
    │   │   ├── doubly_linked_list_two (5).py
    │   │   ├── doubly_linked_list_two (6).py
    │   │   ├── doubly_linked_list_two (7).py
    │   │   ├── doubly_linked_list_two (8).py
    │   │   ├── doubly_linked_list_two (9).py
    │   │   ├── doubly_linked_list_two.py
    │   │   ├── download (2).py
    │   │   ├── download (3).py
    │   │   ├── download (4).py
    │   │   ├── download (5).py
    │   │   ├── download.py
    │   │   ├── driver (2).py
    │   │   ├── driver (3).py
    │   │   ├── driver.py
    │   │   ├── dsa.py
    │   │   ├── dsskey.py
    │   │   ├── dynamic_arrays (2).py
    │   │   ├── dynamic_arrays (3).py
    │   │   ├── dynamic_arrays (4).py
    │   │   ├── dynamic_arrays (5).py
    │   │   ├── dynamic_arrays (6).py
    │   │   ├── dynamic_arrays.py
    │   │   ├── dynamic_params (2).py
    │   │   ├── dynamic_params (3).py
    │   │   ├── dynamic_params (4).py
    │   │   ├── dynamic_params (5).py
    │   │   ├── dynamic_params (6).py
    │   │   ├── dynamic_params.py
    │   │   ├── ecl (2).py
    │   │   ├── ecl (3).py
    │   │   ├── ecl (4).py
    │   │   ├── ecl.py
    │   │   ├── ed25519key.py
    │   │   ├── ed25519.py
    │   │   ├── ed448.py
    │   │   ├── EditSecurity (2).py
    │   │   ├── EditSecurity.py
    │   │   ├── EditServiceSecurity (2).py
    │   │   ├── EditServiceSecurity.py
    │   │   ├── edmonds_karp_multiple_source_and_sink (10).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (11).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (12).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (13).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (2).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (3).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (4).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (5).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (6).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (7).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (8).py
    │   │   ├── edmonds_karp_multiple_source_and_sink (9).py
    │   │   ├── edmonds_karp_multiple_source_and_sink.py
    │   │   ├── ElementInclude.py
    │   │   ├── elementwise_add_dnnlowp_op_test.py
    │   │   ├── elementwise_mul_dnnlowp_op_test.py
    │   │   ├── elementwise.py
    │   │   ├── elementwise_sum_op_test.py
    │   │   ├── empty_volume_cache (2).py
    │   │   ├── empty_volume_cache.py
    │   │   ├── encoder (2).py
    │   │   ├── encoder (3).py
    │   │   ├── encoder.py
    │   │   ├── entrypoints (2).py
    │   │   ├── entrypoints.py
    │   │   ├── envbuild (2).py
    │   │   ├── envbuild (3).py
    │   │   ├── envbuild (4).py
    │   │   ├── envbuild (5).py
    │   │   ├── envbuild.py
    │   │   ├── env.py
    │   │   ├── Epoch.py
    │   │   ├── epylint.py
    │   │   ├── error (2).py
    │   │   ├── error.py
    │   │   ├── error_reporting.py
    │   │   ├── errorSemantics (2).py
    │   │   ├── errorSemantics.py
    │   │   ├── escape.py
    │   │   ├── etcd_server.py
    │   │   ├── etree_lxml (2).py
    │   │   ├── etree_lxml (3).py
    │   │   ├── etree_lxml (4).py
    │   │   ├── etree_lxml (5).py
    │   │   ├── etree_lxml (6).py
    │   │   ├── etree_lxml (7).py
    │   │   ├── etree_lxml.py
    │   │   ├── evaluator.py
    │   │   ├── exc (2).py
    │   │   ├── exc (3).py
    │   │   ├── excelAddin (2).py
    │   │   ├── excelAddin.py
    │   │   ├── exceptions (10).py
    │   │   ├── exceptions (11).py
    │   │   ├── exceptions (12).py
    │   │   ├── exceptions (13).py
    │   │   ├── exceptions (14).py
    │   │   ├── exceptions (2).py
    │   │   ├── exceptions (3).py
    │   │   ├── exceptions (4).py
    │   │   ├── exceptions (5).py
    │   │   ├── exceptions (6).py
    │   │   ├── exceptions (7).py
    │   │   ├── exceptions (8).py
    │   │   ├── exceptions (9).py
    │   │   ├── exceptions.py
    │   │   ├── excolors (2).py
    │   │   ├── excolors.py
    │   │   ├── exc.py
    │   │   ├── execute.py
    │   │   ├── executor_test_util.py
    │   │   ├── expanding.py
    │   │   ├── expressions (2).py
    │   │   ├── expressions (3).py
    │   │   ├── expressions.py
    │   │   ├── ext (2).py
    │   │   ├── ext (3).py
    │   │   ├── ext (4).py
    │   │   ├── ext (5).py
    │   │   ├── ext (6).py
    │   │   ├── ext (7).py
    │   │   ├── ext (8).py
    │   │   ├── extensions (2).py
    │   │   ├── extensions (3).py
    │   │   ├── extensions (4).py
    │   │   ├── extensions (5).py
    │   │   ├── extensions.py
    │   │   ├── ext.py
    │   │   ├── extractoutput (2).py
    │   │   ├── extractoutput.py
    │   │   ├── extract.py
    │   │   ├── factory (2).py
    │   │   ├── factory.py
    │   │   ├── fc (2).py
    │   │   ├── fc.py
    │   │   ├── fdc58d9bd449_.py
    │   │   ├── featureexplore (2).py
    │   │   └── featureexplore.py
    │   ├── 5.py
    │   ├── 7.py
    │   ├── abbreviation.py
    │   ├── abs_max.py
    │   ├── abs_min.py
    │   ├── abs.py
    │   ├── abstract_build_docker.py
    │   ├── abstract_command.py
    │   ├── abstract.py
    │   ├── abstract_strategy.py
    │   ├── access.py
    │   ├── activity_selection.py
    │   ├── adapter.py
    │   ├── add_and_search.py
    │   ├── add_binary.py
    │   ├── add_bitwise_operator.py
    │   ├── add_book_info.py
    │   ├── add_navigation.py
    │   ├── add_operators.py
    │   ├── add.py
    │   ├── add_two_numbers.py
    │   ├── adler32.py
    │   ├── admin.py
    │   ├── af4cbdb2d13c_services.py
    │   ├── aff1436e3c8c_.py
    │   ├── affine_cipher.py
    │   ├── aho_corasick.py
    │   ├── aif.py
    │   ├── ai_player.py
    │   ├── aircraft_thrust.py
    │   ├── Algebraic_integration_from_Ludovic_Sacchelli_s_PhD_thesis.py
    │   ├── algolia_helper.py
    │   ├── algolia_internal_api.py
    │   ├── algolia_settings.py
    │   ├── Algorithme de Cocke-Kasami-Younger (python3).py
    │   ├── Algorithme_genetique_pour_generer_des_eclairages_modelisation_agreg.py
    │   ├── alice.py
    │   ├── alien_invasion.py
    │   ├── alien.py
    │   ├── alien_ship.py
    │   ├── aliens.py
    │   ├── aliquot_sum.py
    │   ├── all_combinations.py
    │   ├── A_short_study_of_Renyi_entropy.py
    │   ├── a_star.py
    │   ├── _ast_gen.py
    │   ├── A tiny regex challenge solved without another regex.py
    │   ├── _base.py
    │   ├── _build_tables.py
    │   ├── _bulk.py
    │   ├── _compatibility.py
    │   ├── _compat.py
    │   ├── _data.py
    │   ├── _decorators.py
    │   ├── _fscompat.py
    │   ├── _ftp_parse.py
    │   ├── _ihatexml.py
    │   ├── _imports_.py
    │   ├── __init__.py
    │   ├── _inputstream.py
    │   ├── __main__.py
    │   ├── _numpy_testing_noseclasses.py
    │   ├── _process_cli.py
    │   ├── _pydev_calltip_util.py
    │   ├── _pydev_pkgutil_old.py
    │   ├── rabin_karp_hash.py
    │   ├── random (2).py
    │   ├── random_fourier_features.py
    │   ├── random_latin_citation.py
    │   ├── randomModule.py
    │   ├── random.py
    │   ├── random_sample_test.py
    │   ├── random_topo_test.py
    │   ├── rand_quantization_op_speed_test.py
    │   ├── rank_loss_operator_test.py
    │   ├── ransom.py
    │   ├── read_english_dictionary.py
    │   ├── readfilesfromtar.py
    │   ├── readfilesfromzip.py
    │   ├── readFiles.py
    │   ├── reading-by-moonlight.py
    │   ├── rebatching_queue_test.py
    │   ├── record_function_bench.py
    │   ├── record_queue.py
    │   ├── record_queue_test.py
    │   ├── recurrent_net_executor_test.py
    │   ├── recurrent_network_test.py
    │   ├── recurrent.py
    │   ├── redirects.py
    │   ├── redirects_test.py
    │   ├── redis_store_handler_op_test.py
    │   ├── reduce_ops_test.py
    │   ├── reduction_ops_test.py
    │   ├── reduction.py
    │   ├── reductions.py
    │   ├── register_dispatch_key.py
    │   ├── registry.py
    │   ├── regularizer_context.py
    │   ├── regularizer.py
    │   ├── regularizer_test.py
    │   ├── reinforcement_learning_rpc_test.py
    │   ├── relative_ranks.py
    │   ├── relaxed_bernoulli.py
    │   ├── relaxed_categorical.py
    │   ├── relu_dnnlowp_op_test.py
    │   ├── relu_op_test.py
    │   ├── remainder_test.py
    │   ├── remote_module.py
    │   ├── remote_module_template.py
    │   ├── remote_module_test.py
    │   ├── remove-from-list (2).py
    │   ├── remove-from-list (3).py
    │   ├── remove-from-list (4).py
    │   ├── remove-from-list (5).py
    │   ├── remove-from-list (6).py
    │   ├── remove-from-list.py
    │   ├── render.py
    │   ├── rendezvous_filestore.py
    │   ├── rendezvous.py
    │   ├── repeat_benchmark.py
    │   ├── replace_nan_test.py
    │   ├── replicate.py
    │   ├── _repr.py
    │   ├── reservoir_sampling.py
    │   ├── reshape_ops_test.py
    │   ├── reshape_op_test.py
    │   ├── resize_nearest_3d_dnnlowp_op_test.py
    │   ├── resize_nearest_dnnlowp_op_test.py
    │   ├── resize_op_test.py
    │   ├── resnet50_trainer.py
    │   ├── resnet_memory_profiler.py
    │   ├── resnet.py
    │   ├── resnet_test.py
    │   ├── retrying.py
    │   ├── reverseInteger.py
    │   ├── reverse_linked_list.py
    │   ├── reverse_sort.py
    │   ├── rev-word (2).py
    │   ├── rev-word (3).py
    │   ├── rev-word (4).py
    │   ├── rev-word.py
    │   ├── rewrite_graph.py
    │   ├── rewrite_graph_test.py
    │   ├── rewriter.py
    │   ├── rglob.py
    │   ├── rmac_regions_op_test.py
    │   ├── rms_norm_op_test.py
    │   ├── rmsprop (2).py
    │   ├── rmsprop.py
    │   ├── rnn (2).py
    │   ├── rnn (3).py
    │   ├── rnn (4).py
    │   ├── rnn (5).py
    │   ├── rnn_cell.py
    │   ├── rnn_cell_test.py
    │   ├── rnn_cell_test_util.py
    │   ├── rnn_eltwise.py
    │   ├── rnn_model_with_packed_sequence.py
    │   ├── rnn.py
    │   ├── rock,paper,scissor.py
    │   ├── roi_align_rotated_op_test.py
    │   ├── routeddecoder.py
    │   ├── routes.py
    │   ├── rowwise_counter_test.py
    │   ├── rpc_agent_test_fixture.py
    │   ├── rpc_test (2).py
    │   ├── rpc_test_faulty.py
    │   ├── rpc_test.py
    │   ├── rpc_utils.py
    │   ├── rprop (2).py
    │   ├── rprop.py
    │   ├── rref_proxy.py
    │   ├── rst2md.py
    │   ├── run_config_docker.py
    │   ├── run_config.py
    │   ├── run_cuda_memcheck.py
    │   ├── runner.py
    │   ├── run.py
    │   ├── run_python_nn_smoketests.py
    │   ├── run_test.py
    │   ├── run_tests.py
    │   ├── s3_stat_parser.py
    │   ├── sample_functional.py
    │   ├── sample_module.py
    │   ├── sample.py
    │   ├── sampler.py
    │   ├── sampling_trainable_mixin.py
    │   ├── sampling_train.py
    │   ├── scale_op_test.py
    │   ├── scan (2).py
    │   ├── scan.py
    │   ├── scatter_gather.py
    │   ├── schema.py
    │   ├── schema_test.py
    │   ├── schema_type_annotation.py
    │   ├── scope.py
    │   ├── scope_test.py
    │   ├── scratch.py
    │   ├── script (2).py
    │   ├── script.py
    │   ├── searchable_level_test.py
    │   ├── searchLogfile.py
    │   ├── SecurityBijective.py
    │   ├── SecurityEncryptionScheme.py
    │   ├── SecurityFunctions2.py
    │   ├── SecurityFunctions.py
    │   ├── SecurityInverseFunction.py
    │   ├── SecurityInvolution.py
    │   ├── SecurityKeySpaces.py
    │   ├── SecurityMessageSpaceandCiphertextSpace.py
    │   ├── SecurityPermutations.py
    │   ├── seed.py
    │   ├── segment_ops_test.py
    │   ├── selecting.py
    │   ├── selection_sort (2).py
    │   ├── selection_sort (3).py
    │   ├── selection_sort.py
    │   ├── selectionSort.py
    │   ├── selector.py
    │   ├── selectors_exclude_test.py
    │   ├── selectors_parser.py
    │   ├── select_record_by_context.py
    │   ├── self_binning_histogram_test.py
    │   ├── selfvis.py
    │   ├── selu_op_test.py
    │   ├── semi_random_features.py
    │   ├── sendingEmailsInpython.py
    │   ├── seq2seq_beam_search_test.py
    │   ├── seq2seq_model_helper.py
    │   ├── seq2seq_model_helper_test.py
    │   ├── seq2seq_util.py
    │   ├── sequence_ops_test.py
    │   ├── serde.py
    │   ├── serialization.py
    │   ├── serialized_test_util.py
    │   ├── serializer.py
    │   ├── server_process_global_profiler.py
    │   ├── server.py
    │   ├── service.py
    │   ├── services.py
    │   ├── session (2).py
    │   ├── session.py
    │   ├── session_test.py
    │   ├── setting.py
    │   ├── setup (2).py
    │   ├── setup (3).py
    │   ├── setup (4).py
    │   ├── setup (5).py
    │   ├── setup (6).py
    │   ├── setup (7).py
    │   ├── setup (8).py
    │   ├── setup (9).py
    │   ├── setup.py
    │   ├── sgd (2).py
    │   ├── sgd.py
    │   ├── shape_inference_test.py
    │   ├── shape_op_test.py
    │   ├── shape_prop.py
    │   ├── shipping_form.py
    │   ├── show_pickle.py
    │   ├── shufflenet.py
    │   ├── shufflenet_test.py
    │   ├── shuffle.py
    │   ├── sigmoid_dnnlowp_op_test.py
    │   ├── sigmoid_op_test.py
    │   ├── signal_handling.py
    │   ├── SimpleAddModule.py
    │   ├── simpleArraySum.py
    │   ├── simple_copy (2).py
    │   ├── simple_copy.py
    │   ├── simple_measurement.py
    │   ├── simple.py
    │   ├── simple-python-game (2).py
    │   ├── simple-python-game.py
    │   ├── simple_timeit.py
    │   ├── simulate_nccl_errors.py
    │   ├── sinusoid_position_encoding_op_test.py
    │   ├── sitemap_test.py
    │   ├── size.py
    │   ├── sizestr.py
    │   ├── skippable.py
    │   ├── slack_helper (2).py
    │   ├── slack_helper.py
    │   ├── sleep-poisson-time.py
    │   ├── sleepWellAlarm.py
    │   ├── smooth_filter_nojit_and_jit.py
    │   ├── snippeter.py
    │   ├── softmax_ops_test.py
    │   ├── softmax_op_test.py
    │   ├── softmax.py
    │   ├── softmax_test.py
    │   ├── softplus_op_test.py
    │   ├── sol1 (100).py
    │   ├── sol1 (101).py
    │   ├── sol1 (102).py
    │   ├── sol1 (103).py
    │   ├── sol1 (104).py
    │   ├── sol1 (105).py
    │   ├── sol1 (106).py
    │   ├── sol1 (107).py
    │   ├── sol1 (108).py
    │   ├── sol1 (109).py
    │   ├── sol1 (10).py
    │   ├── sol1 (110).py
    │   ├── sol1 (111).py
    │   ├── sol1 (112).py
    │   ├── sol1 (113).py
    │   ├── sol1 (114).py
    │   ├── sol1 (115).py
    │   ├── sol1 (116).py
    │   ├── sol1 (117).py
    │   ├── sol1 (118).py
    │   ├── sol1 (119).py
    │   ├── sol1 (11).py
    │   ├── sol1 (120).py
    │   ├── sol1 (121).py
    │   ├── sol1 (122).py
    │   ├── sol1 (123).py
    │   ├── sol1 (124).py
    │   ├── sol1 (125).py
    │   ├── sol1 (126).py
    │   ├── sol1 (127).py
    │   ├── sol1 (128).py
    │   ├── sol1 (129).py
    │   ├── sol1 (12).py
    │   ├── sol1 (130).py
    │   ├── sol1 (131).py
    │   ├── sol1 (132).py
    │   ├── sol1 (133).py
    │   ├── sol1 (134).py
    │   ├── sol1 (135).py
    │   ├── sol1 (136).py
    │   ├── sol1 (137).py
    │   ├── sol1 (138).py
    │   ├── sol1 (139).py
    │   ├── sol1 (13).py
    │   ├── sol1 (140).py
    │   ├── sol1 (141).py
    │   ├── sol1 (142).py
    │   ├── sol1 (143).py
    │   ├── sol1 (144).py
    │   ├── sol1 (145).py
    │   ├── sol1 (146).py
    │   ├── sol1 (147).py
    │   ├── sol1 (148).py
    │   ├── sol1 (149).py
    │   ├── sol1 (14).py
    │   ├── sol1 (150).py
    │   ├── sol1 (151).py
    │   ├── sol1 (152).py
    │   ├── sol1 (153).py
    │   ├── sol1 (154).py
    │   ├── sol1 (155).py
    │   ├── sol1 (156).py
    │   ├── sol1 (157).py
    │   ├── sol1 (158).py
    │   ├── sol1 (159).py
    │   ├── sol1 (15).py
    │   ├── sol1 (160).py
    │   ├── sol1 (161).py
    │   ├── sol1 (162).py
    │   ├── sol1 (163).py
    │   ├── sol1 (164).py
    │   ├── sol1 (165).py
    │   ├── sol1 (166).py
    │   ├── sol1 (167).py
    │   ├── sol1 (168).py
    │   ├── sol1 (169).py
    │   ├── sol1 (16).py
    │   ├── sol1 (170).py
    │   ├── sol1 (171).py
    │   ├── sol1 (172).py
    │   ├── sol1 (173).py
    │   ├── sol1 (174).py
    │   ├── sol1 (175).py
    │   ├── sol1 (176).py
    │   ├── sol1 (177).py
    │   ├── sol1 (178).py
    │   ├── sol1 (179).py
    │   ├── sol1 (17).py
    │   ├── sol1 (180).py
    │   ├── sol1 (181).py
    │   ├── sol1 (182).py
    │   ├── sol1 (183).py
    │   ├── sol1 (184).py
    │   ├── sol1 (185).py
    │   ├── sol1 (186).py
    │   ├── sol1 (187).py
    │   ├── sol1 (188).py
    │   ├── sol1 (189).py
    │   ├── sol1 (18).py
    │   ├── sol1 (190).py
    │   ├── sol1 (191).py
    │   ├── sol1 (19).py
    │   ├── sol1 (20).py
    │   ├── sol1 (21).py
    │   ├── sol1 (22).py
    │   ├── sol1 (23).py
    │   ├── sol1 (24).py
    │   ├── sol1 (25).py
    │   ├── sol1 (26).py
    │   ├── sol1 (27).py
    │   ├── sol1 (28).py
    │   ├── sol1 (29).py
    │   ├── sol1 (2).py
    │   ├── sol1 (30).py
    │   ├── sol1 (31).py
    │   ├── sol1 (32).py
    │   ├── sol1 (33).py
    │   ├── sol1 (34).py
    │   ├── sol1 (35).py
    │   ├── sol1 (36).py
    │   ├── sol1 (37).py
    │   ├── sol1 (38).py
    │   ├── sol1 (39).py
    │   ├── sol1 (3).py
    │   ├── sol1 (40).py
    │   ├── sol1 (41).py
    │   ├── sol1 (42).py
    │   ├── sol1 (43).py
    │   ├── sol1 (44).py
    │   ├── sol1 (45).py
    │   ├── sol1 (46).py
    │   ├── sol1 (47).py
    │   ├── sol1 (48).py
    │   ├── sol1 (49).py
    │   ├── sol1 (4).py
    │   ├── sol1 (50).py
    │   ├── sol1 (51).py
    │   ├── sol1 (52).py
    │   ├── sol1 (53).py
    │   ├── sol1 (54).py
    │   ├── sol1 (55).py
    │   ├── sol1 (56).py
    │   ├── sol1 (57).py
    │   ├── sol1 (58).py
    │   ├── sol1 (59).py
    │   ├── sol1 (5).py
    │   ├── sol1 (60).py
    │   ├── sol1 (61).py
    │   ├── sol1 (62).py
    │   ├── sol1 (63).py
    │   ├── sol1 (64).py
    │   ├── sol1 (65).py
    │   ├── sol1 (66).py
    │   ├── sol1 (67).py
    │   ├── sol1 (68).py
    │   ├── sol1 (69).py
    │   ├── sol1 (6).py
    │   ├── sol1 (70).py
    │   ├── sol1 (71).py
    │   ├── sol1 (72).py
    │   ├── sol1 (73).py
    │   ├── sol1 (74).py
    │   ├── sol1 (75).py
    │   ├── sol1 (76).py
    │   ├── sol1 (77).py
    │   ├── sol1 (78).py
    │   ├── sol1 (79).py
    │   ├── sol1 (7).py
    │   ├── sol1 (80).py
    │   ├── sol1 (81).py
    │   ├── sol1 (82).py
    │   ├── sol1 (83).py
    │   ├── sol1 (84).py
    │   ├── sol1 (85).py
    │   ├── sol1 (86).py
    │   ├── sol1 (87).py
    │   ├── sol1 (88).py
    │   ├── sol1 (89).py
    │   ├── sol1 (8).py
    │   ├── sol1 (90).py
    │   ├── sol1 (91).py
    │   ├── sol1 (92).py
    │   ├── sol1 (93).py
    │   ├── sol1 (94).py
    │   ├── sol1 (95).py
    │   ├── sol1 (96).py
    │   ├── sol1 (97).py
    │   ├── sol1 (98).py
    │   ├── sol1 (99).py
    │   ├── sol1 (9).py
    │   ├── sol1.py
    │   ├── sol2 (10).py
    │   ├── sol2 (11).py
    │   ├── sol2 (12).py
    │   ├── sol2 (13).py
    │   ├── sol2 (14).py
    │   ├── sol2 (15).py
    │   ├── sol2 (16).py
    │   ├── sol2 (17).py
    │   ├── sol2 (18).py
    │   ├── sol2 (19).py
    │   ├── sol2 (20).py
    │   ├── sol2 (21).py
    │   ├── sol2 (22).py
    │   ├── sol2 (23).py
    │   ├── sol2 (24).py
    │   ├── sol2 (25).py
    │   ├── sol2 (26).py
    │   ├── sol2 (27).py
    │   ├── sol2 (28).py
    │   ├── sol2 (29).py
    │   ├── sol2 (2).py
    │   ├── sol2 (30).py
    │   ├── sol2 (31).py
    │   ├── sol2 (32).py
    │   ├── sol2 (33).py
    │   ├── sol2 (34).py
    │   ├── sol2 (35).py
    │   ├── sol2 (36).py
    │   ├── sol2 (37).py
    │   ├── sol2 (38).py
    │   ├── sol2 (39).py
    │   ├── sol2 (3).py
    │   ├── sol2 (4).py
    │   ├── sol2 (5).py
    │   ├── sol2 (6).py
    │   ├── sol2 (7).py
    │   ├── sol2 (8).py
    │   ├── sol2 (9).py
    │   ├── sol2.py
    │   ├── sol3 (10).py
    │   ├── sol3 (11).py
    │   ├── sol3 (12).py
    │   ├── sol3 (13).py
    │   ├── sol3 (14).py
    │   ├── sol3 (15).py
    │   ├── sol3 (16).py
    │   ├── sol3 (17).py
    │   ├── sol3 (18).py
    │   ├── sol3 (19).py
    │   ├── sol32.py
    │   ├── sol3 (2).py
    │   ├── sol3 (3).py
    │   ├── sol3 (4).py
    │   ├── sol3 (5).py
    │   ├── sol3 (6).py
    │   ├── sol3 (7).py
    │   ├── sol3 (8).py
    │   ├── sol3 (9).py
    │   ├── sol3.py
    │   ├── sol4 (2).py
    │   ├── sol4 (3).py
    │   ├── sol4 (4).py
    │   ├── sol4 (5).py
    │   ├── sol4 (6).py
    │   ├── sol4 (7).py
    │   ├── sol4.py
    │   ├── sol5 (2).py
    │   ├── sol5 (3).py
    │   ├── sol5.py
    │   ├── sol6.py
    │   ├── sol7.py
    │   ├── solution42.py
    │   ├── solution.py
    │   ├── sort_0_1_2.py
    │   ├── sorted_array_rotation.py
    │   ├── sortedlist.py
    │   ├── sorting_floats.py
    │   ├── sortingFunctions.py
    │   ├── sort.py
    │   ├── sortString.py
    │   ├── sparse (2).py
    │   ├── sparse_adam.py
    │   ├── sparse_binary.py
    │   ├── sparse_dropout_with_replacement_op_test.py
    │   ├── sparse_dropout_with_replacement.py
    │   ├── sparse_feature_hash.py
    │   ├── sparse_funhash_op_test.py
    │   ├── sparse_fuzzer.py
    │   ├── sparse_gradient_checker_test.py
    │   ├── sparse_lengths_sum_benchmark.py
    │   ├── sparse_lengths_sum_nbit_benchmark.py
    │   ├── sparse_lookup.py
    │   ├── sparse_lp_regularizer_test.py
    │   ├── sparse_normalize_benchmark.py
    │   ├── sparse_normalize_test.py
    │   ├── sparse_ops_test.py
    │   ├── sparse.py
    │   ├── sparse_reshape_op_test.py
    │   ├── sparse_to_dense_mask_op_test.py
    │   ├── sparse_to_dense_mask_test.py
    │   ├── sparse_to_dense_test.py
    │   ├── SparseTransformer.py
    │   ├── sparse_unary.py
    │   ├── spatial_batch_norm_dnnlowp_op_test.py
    │   ├── spatial_bn_op_test (2).py
    │   ├── spatial_bn_op_test.py
    │   ├── spawner.py
    │   ├── spawn.py
    │   ├── specialized_segment_ops_test.py
    │   ├── spectral_norm.py
    │   ├── spectral_ops_fuzz_test.py
    │   ├── spectral.py
    │   ├── split_module.py
    │   ├── split.py
    │   ├── split-string (2).py
    │   ├── split-string (3).py
    │   ├── split-string (4).py
    │   ├── split-string.py
    │   ├── split_test.py
    │   ├── sprial_rotation.py
    │   ├── squarecube.py
    │   ├── square_root_algorithm.py
    │   ├── square_root_divide_op_test.py
    │   ├── squareTurtle.py
    │   ├── squeezenet.py
    │   ├── srresnet.py
    │   ├── ssa_test.py
    │   ├── stack_imply.py
    │   ├── stack_max_o(1).py
    │   ├── stack_test.py
    │   ├── staircase copy.py
    │   ├── staircase.py
    │   ├── start_ui.py
    │   ├── start_urls_test.py
    │   ├── star_turtle.py
    │   ├── static.py
    │   ├── stats_ops_test.py
    │   ├── stats_put_ops_test.py
    │   ├── stop_urls_test.py
    │   ├── storage (2).py
    │   ├── storage.py
    │   ├── store_ops_test_util.py
    │   ├── store.py
    │   ├── storm_test.py
    │   ├── strapdown2html.py
    │   ├── stream.py
    │   ├── streams.py
    │   ├── stringify_file.py
    │   ├── stringIndexing.py
    │   ├── stringOperations.py
    │   ├── string_ops_test.py
    │   ├── stringReverse.py
    │   ├── strip_chars_test.py
    │   ├── Strong Password Checker.py
    │   ├── structured.py
    │   ├── stubs.py
    │   ├── studentT.py
    │   ├── subgraph_rewriter.py
    │   ├── subpackage.py
    │   ├── substring_search.py
    │   ├── sudoku.py
    │   ├── sum-arr (2).py
    │   ├── sum-arr (3).py
    │   ├── sum-arr (4).py
    │   ├── sum-arr (5).py
    │   ├── sum-arr (6).py
    │   ├── sum_array.py
    │   ├── sum-arr.py
    │   ├── sumAverage.py
    │   ├── summarize_jsons.py
    │   ├── summary.py
    │   ├── sum_of_arithmetic_sequence.py
    │   ├── SumOfIntegers.py
    │   ├── sum.py
    │   ├── sum_test.py
    │   ├── sun_finder.py
    │   ├── super_resolution.py
    │   ├── supported_ops.py
    │   ├── swap_case.py
    │   ├── swa_utils.py
    │   ├── swish.py
    │   ├── symbolic_caffe2.py
    │   ├── symbolic_helper.py
    │   ├── symbolic_opset10.py
    │   ├── symbolic_opset11.py
    │   ├── symbolic_opset12.py
    │   ├── symbolic_opset13.py
    │   ├── symbolic_opset7.py
    │   ├── symbolic_opset8.py
    │   ├── symbolic_opset9.py
    │   ├── symbolic_registry.py
    │   ├── symbolic_trace.py
    │   ├── _sysinfo.py
    │   ├── syslogger.py
    │   ├── systemInfo.py
    │   ├── system_process_scanner.py
    │   ├── systemstats.py
    │   ├── table_maker.py
    │   ├── table.py
    │   ├── tags.py
    │   ├── tags_test.py
    │   ├── tail_log.py
    │   ├── tail_log_test.py
    │   ├── take-a-break.py
    │   ├── tanh_dnnlowp_op_test.py
    │   ├── task.py
    │   ├── tasks.py
    │   ├── task_test.py
    │   ├── temp.py
    │   ├── tensorboard_exporter.py
    │   ├── tensorboard_exporter_test.py
    │   ├── tensorboard.py
    │   ├── tensorboard_test.py
    │   ├── tensor_constructors.py
    │   ├── tensor_copy.py
    │   ├── tensor_engine.py
    │   ├── tensorflow_docs_l10n.py
    │   ├── tensorflow.py
    │   ├── tensorpipe_rpc_agent_test_fixture.py
    │   ├── tensor.py
    │   ├── tensor_to_test.py
    │   ├── terminal2.py
    │   ├── terminal3.py
    │   ├── terminal.py
    │   ├── test_01_simple_web_responses.py
    │   ├── test_02_db.py
    │   ├── test (2).py
    │   ├── test (3).py
    │   ├── test (4).py
    │   ├── test_api (2).py
    │   ├── test_api.py
    │   ├── test_app.py
    │   ├── test_args.py
    │   ├── test_async.py
    │   ├── test_auth_expiry.py
    │   ├── test_auth.py
    │   ├── test_autodiff_subgraph_slicing.py
    │   ├── test_autograd.py
    │   ├── test_backends.py
    │   ├── test_backward_compatibility.py
    │   ├── test_balance.py
    │   ├── test_batchmatmul_nnpi_fp16.py
    │   ├── test_batchnorm_nnpi_fp16.py
    │   ├── test_benchmark_utils.py
    │   ├── test_bench.py
    │   ├── test_bias_correction.py
    │   ├── test_binary_ufuncs.py
    │   ├── test_bugs.py
    │   ├── test_builtins.py
    │   ├── test_bundled_images.py
    │   ├── test_bundled_inputs.py
    │   ├── test_c10d.py
    │   ├── test_c10d_spawn.py
    │   ├── test_caffe2_common.py
    │   ├── test-charmap.py
    │   ├── test_checkpoint.py
    │   ├── test_chunking.py
    │   ├── test_class_type.py
    │   ├── test_cli.py
    │   ├── test_complex (2).py
    │   ├── test_complexity.py
    │   ├── test_complex.py
    │   ├── test_constraints.py
    │   ├── test_copy.py
    │   ├── test_cpp_api_parity.py
    │   ├── test_cpp_extensions_aot.py
    │   ├── test_cpp_extensions_jit.py
    │   ├── test_crypto.py
    │   ├── test_cuda (2).py
    │   ├── test_cuda (3).py
    │   ├── test_cuda_primary_ctx.py
    │   ├── test_cuda.py
    │   ├── test_custom_backend.py
    │   ├── test_custom_classes.py
    │   ├── test_custom_operators.py
    │   ├── test_custom_ops (2).py
    │   ├── test_custom_ops.py
    │   ├── test_dataloader.py
    │   ├── test_data_parallel (2).py
    │   ├── test_data_parallel.py
    │   ├── test_datapipe.py
    │   ├── test_dataset.py
    │   ├── test_db.py
    │   ├── test_dce_pass.py
    │   ├── test_ddp_hooks.py
    │   ├── test_dead_link.py
    │   ├── test_deferred_batch_norm.py
    │   ├── test_dependency_api.py
    │   ├── test_dependency.py
    │   ├── test_deq_swish_quant_nnpi.py
    │   ├── test_determination.py
    │   ├── test_dispatch.py
    │   ├── test_distributed_fork.py
    │   ├── test_distributed_spawn.py
    │   ├── test_distributions.py
    │   ├── test_dummyauth.py
    │   ├── test_enum.py
    │   ├── test_equalize.py
    │   ├── test_escape.py
    │   ├── test_eventlog.py
    │   ├── test_expecttest.py
    │   ├── test_export_modes.py
    │   ├── test_faulty_agent.py
    │   ├── test_fc_nnpi_fp16.py
    │   ├── test_foreach.py
    │   ├── test_freezing.py
    │   ├── test_functional_autograd_benchmark.py
    │   ├── test_functional_blocks.py
    │   ├── test_function_schema.py
    │   ├── test_fuser_common.py
    │   ├── test_fusion_passes.py
    │   ├── test_fusions.py
    │   ├── test_future.py
    │   ├── test_futures.py
    │   ├── test_fx_const_fold.py
    │   ├── test_fx_experimental.py
    │   ├── test_fx.py
    │   ├── test_gpipe.py
    │   ├── test_hash.py
    │   ├── test_history.py
    │   ├── test_hooks_modules.py
    │   ├── test_hooks.py
    │   ├── test_ideep_net.py
    │   ├── test_importer.py
    │   ├── test_indexing.py
    │   ├── test_inplace.py
    │   ├── test_inspect_skip_layout.py
    │   ├── test_instantiator.py
    │   ├── test_int8_ops_nnpi.py
    │   ├── test_int8_quant.py
    │   ├── test_internal_ssl_connections.py
    │   ├── test_isinstance.py
    │   ├── test_jit_c10d.py
    │   ├── test_jit_cuda_fuser.py
    │   ├── test_jit_disabled.py
    │   ├── test_jit_fuser_legacy.py
    │   ├── test_jit_fuser.py
    │   ├── test_jit_fuser_te.py
    │   ├── test_jit_legacy.py
    │   ├── test_jit_profiling.py
    │   ├── test_jit.py
    │   ├── test_jit_py3.py
    │   ├── test_jit_simple.py
    │   ├── test_jit_string.py
    │   ├── test_jit_utils.py
    │   ├── test_kernel_launch_checks.py
    │   ├── test_layernorm_nnpi_fp16.py
    │   ├── test_leak.py
    │   ├── test_license.py
    │   ├── test_linalg.py
    │   ├── test_list_dict.py
    │   ├── test_lite_script_module.py
    │   ├── test_logging (2).py
    │   ├── test_logging.py
    │   ├── test_mailgun.py
    │   ├── test_mangling.py
    │   ├── test_metal.py
    │   ├── test_metrics.py
    │   ├── test_microbatch.py
    │   ├── test_misc.py
    │   ├── test_mkldnn.py
    │   ├── test_mobile_optimizer.py
    │   ├── test_model.py
    │   ├── test_models (2).py
    │   ├── test_models_onnxruntime.py
    │   ├── test_models.py
    │   ├── test_module1.py
    │   ├── test_module2.py
    │   ├── test_module_containers.py
    │   ├── test_module_interface.py
    │   ├── test_module.py
    │   ├── test_multiprocessing.py
    │   ├── test_multiprocessing_spawn.py
    │   ├── test_mypy_wrapper.py
    │   ├── test_named_servers.py
    │   ├── test_namedtensor.py
    │   ├── test_namedtuple_return_api.py
    │   ├── test_native_functions.py
    │   ├── test_nccl.py
    │   ├── test_nn.py
    │   ├── test_numba_integration.py
    │   ├── test_numeric_suite_fx.py
    │   ├── test_numeric_suite.py
    │   ├── test_numpy_interop.py
    │   ├── test_objects.py
    │   ├── testofdivisibility.py
    │   ├── test_onnx_common.py
    │   ├── test_onnx_export.py
    │   ├── test_onnxifi.py
    │   ├── test_onnx_opset.py
    │   ├── test_op_aliases.py
    │   ├── test_openmp.py
    │   ├── test_operators.py
    │   ├── test_op_nnpi_fp16.py
    │   ├── test_ops.py
    │   ├── test_optim.py
    │   ├── test_orm.py
    │   ├── test_overrides.py
    │   ├── test_package_fx.py
    │   ├── test_package.py
    │   ├── test_pages.py
    │   ├── test_pagination.py
    │   ├── test_peephole.py
    │   ├── test-permutation-algorithms.py
    │   ├── test_phony.py
    │   ├── test_pipeline.py
    │   ├── test_pipe.py
    │   ├── test_poker_hand.py
    │   ├── test_portal.py
    │   ├── test_process_group_agent.py
    │   ├── test_profiler (2).py
    │   ├── test_profiler.py
    │   ├── test_progressbar.py
    │   ├── test_proxy.py
    │   ├── test_pruning_op.py
    │   ├── test_pt_onnx_trt.py
    │   ├── test_public_bindings.py
    │   ├── test.py
    │   ├── test_python_bindings.py
    │   ├── test_python_ir.py
    │   ├── test_pytorch_common.py
    │   ├── test_pytorch_helper.py
    │   ├── test_pytorch_onnx_caffe2.py
    │   ├── test_pytorch_onnx_caffe2_quantized.py
    │   ├── test_pytorch_onnx_onnxruntime_cuda.py
    │   ├── test_pytorch_onnx_onnxruntime.py
    │   ├── test_pytorch_onnx_shape_inference.py
    │   ├── test_pytree.py
    │   ├── test_qat_module.py
    │   ├── test_quantization.py
    │   ├── test_quantized_functional.py
    │   ├── test_quantized_module.py
    │   ├── test_quantized_op.py
    │   ├── test_quantized_tensor.py
    │   ├── test_quantize_fx.py
    │   ├── test_quantize_jit.py
    │   ├── test_quantize.py
    │   ├── test_recursive_script.py
    │   ├── test_reductions.py
    │   ├── test_release_notes.py
    │   ├── test_remove_mutation.py
    │   ├── test_resources.py
    │   ├── test_rglob.py
    │   ├── test_save_load (2).py
    │   ├── test_save_load.py
    │   ├── test_scriptmod_ann.py
    │   ├── test_script.py
    │   ├── test_serialization.py
    │   ├── test_services_auth.py
    │   ├── test_services.py
    │   ├── test_set_default_mobile_cpu_allocator.py
    │   ├── test_shape_ops.py
    │   ├── test_show_pickle.py
    │   ├── test_singleuser.py
    │   ├── test_slice.py
    │   ├── test_sls_4bit_nnpi_fp16.py
    │   ├── test_sls_8bit_nnpi_fp16.py
    │   ├── test_sls_8bit_nnpi_fp32.py
    │   ├── test_sort_and_select.py
    │   ├── test_sparse.py
    │   ├── test_spawner.py
    │   ├── test_spectral_ops.py
    │   ├── tests_setup.py
    │   ├── test_stash_pop.py
    │   ├── test_static_runtime.py
    │   ├── test_stats.py
    │   ├── test_stream.py
    │   ├── test_string_formatting.py
    │   ├── test_subgraph_rewriter.py
    │   ├── test_tensorboard.py
    │   ├── test_tensor_creation_ops.py
    │   ├── test_tensorexpr.py
    │   ├── test_tensorexpr_pybind.py
    │   ├── test_tensorpipe_agent.py
    │   ├── test_test_history.py
    │   ├── test_testing.py
    │   ├── test_throughput_benchmark.py
    │   ├── test_toposort.py
    │   ├── test_torchbind.py
    │   ├── test_torch.py
    │   ├── test_tracer.py
    │   ├── test_tracker.py
    │   ├── test_traitlets.py
    │   ├── test_transforms.py
    │   ├── test_transparency.py
    │   ├── test_trt.py
    │   ├── test_type_hints.py
    │   ├── test_type_info.py
    │   ├── test_type_promotion.py
    │   ├── test_type_sharing.py
    │   ├── test_typing.py
    │   ├── test_unary_ufuncs.py
    │   ├── test_unsupported_ops.py
    │   ├── test_user.py
    │   ├── test_utility_funs.py
    │   ├── test_util.py
    │   ├── test_utils (2).py
    │   ├── test_utils (3).py
    │   ├── test_utils (4).py
    │   ├── test_utils (5).py
    │   ├── test_utils.py
    │   ├── test_verify.py
    │   ├── test_verify_skippables.py
    │   ├── test_version.py
    │   ├── test_view_ops.py
    │   ├── test_vmap.py
    │   ├── test_vulkan.py
    │   ├── test_warn.py
    │   ├── test_with.py
    │   ├── test_worker.py
    │   ├── test_workflow_module.py
    │   ├── test_xnnpack_integration.py
    │   ├── test_zero_redundancy_optimizer.py
    │   ├── te_utils.py
    │   ├── text_file_reader.py
    │   ├── text_file_reader_test.py
    │   ├── TGV.py
    │   ├── thnn.py
    │   ├── thresholded_relu_op_test.py
    │   ├── throttle (2).py
    │   ├── throttle.py
    │   ├── throughput_benchmark.py
    │   ├── TicTacToe.py
    │   ├── tile_op_test.py
    │   ├── time_conversion.py
    │   ├── timeout_guard.py
    │   ├── timer (2).py
    │   ├── timer (3).py
    │   ├── timer_interface.py
    │   ├── timer.py
    │   ├── titlecase-cli.py
    │   ├── _tokenizer.py
    │   ├── tools.py
    │   ├── top_k_test.py
    │   ├── toposort.py
    │   ├── torchaudio_models.py
    │   ├── torch_integration_test.py
    │   ├── torch_optim.py
    │   ├── torchvision_models.py
    │   ├── Tower_of_hanoi.py
    │   ├── toy_regression_test.py
    │   ├── trace_model.py
    │   ├── tracker.py
    │   ├── train (2).py
    │   ├── train.py
    │   ├── traitlets.py
    │   ├── transformations.py
    │   ├── transformations_test.py
    │   ├── transformed_distribution.py
    │   ├── transformer.py
    │   ├── transform_ideep_net.py
    │   ├── transform.py
    │   ├── transforms.py
    │   ├── translate (2).py
    │   ├── translate.py
    │   ├── transposeMatrix.py
    │   ├── transpose_op_test (2).py
    │   ├── transpose_op_test.py
    │   ├── trapping_water.py
    │   ├── traversals.py
    │   ├── traverse.py
    │   ├── traverse_test.py
    │   ├── tree_creation.py
    │   ├── tree_equal.py
    │   ├── tree-level-order-print (2).py
    │   ├── tree-level-order-print (3).py
    │   ├── tree-level-order-print.py
    │   ├── tree_mirror.py
    │   ├── tree_node (2).py
    │   ├── tree_node.py
    │   ├── tree_order (2).py
    │   ├── tree_order.py
    │   ├── trees.py
    │   ├── tree_traversal.py
    │   ├── trie.py
    │   ├── trigonometric_op_test.py
    │   ├── TripletSearch.py
    │   ├── tt_contraction_op_test.py
    │   ├── tt_core.py
    │   ├── tt_core_test.py
    │   ├── tt_pad_op_test.py
    │   ├── tuplesExample.py
    │   ├── turtleRandomWeb.py
    │   ├── two_sum.py
    │   ├── types (2).py
    │   ├── types.py
    │   ├── _typing.py
    │   ├── unary.py
    │   ├── unary_test.py
    │   ├── unicode.py
    │   ├── uniform.py
    │   ├── uniform_sampling.py
    │   ├── union_find.py
    │   ├── Union of two arrays.py
    │   ├── unique (2).py
    │   ├── unique (3).py
    │   ├── unique (4).py
    │   ├── unique_ops_test.py
    │   ├── unique.py
    │   ├── unique_uniform_fill_op_test.py
    │   ├── unsafe_coalesce_test.py
    │   ├── unscrape (2).py
    │   ├── unscrape (3).py
    │   ├── unscrape (4).py
    │   ├── unscrape.py
    │   ├── unsupported_tensor_ops.py
    │   ├── update-caffe2-models.py
    │   ├── update_commit_hash.py
    │   ├── update_item (2).py
    │   ├── update_item.py
    │   ├── update-models-from-caffe2.py
    │   ├── update_ui.py
    │   ├── update_versions.py
    │   ├── upload_binary_size_to_scuba.py
    │   ├── upload_scribe.py
    │   ├── upsample_nearest_op_test.py
    │   ├── upsample_op_test.py
    │   ├── upsampling.py
    │   ├── urls_parser.py
    │   ├── _url_tools.py
    │   ├── useful_function_mapping.py
    │   ├── userinput.py
    │   ├── user.py
    │   ├── users.py
    │   ├── utility_ops_test.py
    │   ├── utils (10).py
    │   ├── utils (11).py
    │   ├── utils (12).py
    │   ├── utils (13).py
    │   ├── utils (14).py
    │   ├── utils (15).py
    │   ├── utils (16).py
    │   ├── utils (17).py
    │   ├── utils (18).py
    │   ├── utils (19).py
    │   ├── utils (20).py
    │   ├── utils (21).py
    │   ├── utils (22).py
    │   ├── utils (23).py
    │   ├── utils (24).py
    │   ├── utils (25).py
    │   ├── utils (26).py
    │   ├── utils (27).py
    │   ├── utils (28).py
    │   ├── _utils (2).py
    │   ├── utils (2).py
    │   ├── _utils (3).py
    │   ├── utils (3).py
    │   ├── utils (4).py
    │   ├── utils (5).py
    │   ├── utils (6).py
    │   ├── utils (7).py
    │   ├── utils (8).py
    │   ├── utils (9).py
    │   ├── utils_init.py
    │   ├── _utils.py
    │   ├── utils.py
    │   ├── utils_test.py
    │   ├── util_test.py
    │   ├── _validate_init.py
    │   ├── validate.py
    │   ├── valid_parenthesis.py
    │   ├── variable.py
    │   ├── verify.py
    │   ├── _version.py
    │   ├── version.py
    │   ├── versions.py
    │   ├── videodownloader.py
    │   ├── video_input_op_test.py
    │   ├── vision_models.py
    │   ├── visualize.py
    │   ├── von_mises.py
    │   ├── vowel_count.py
    │   ├── wave.py
    │   ├── webp_animation.py
    │   ├── webp_test.py
    │   ├── website_opener.py
    │   ├── weibull.py
    │   ├── weighted_multi_sample_test.py
    │   ├── weighted_sample_test.py
    │   ├── weightedsum_op_test.py
    │   ├── weighted_sum_test.py
    │   ├── weight_norm.py
    │   ├── weight_scale_test.py
    │   ├── weight_utils.py
    │   ├── whatsYourName.py
    │   ├── whoami-flask.py
    │   ├── whoami-oauth (2).py
    │   ├── whoami-oauth-basic.py
    │   ├── whoami-oauth.py
    │   ├── whoami.py
    │   ├── wholeMinute.py
    │   ├── wikistats.py
    │   ├── windows_build_definitions.py
    │   ├── win_tool (2).py
    │   ├── win_tool.py
    │   ├── wngrad_test.py
    │   ├── word_count.py
    │   ├── word_language_model.py
    │   ├── worker (2).py
    │   ├── worker.py
    │   ├── workSchedule.py
    │   ├── workspace (2).py
    │   ├── workspace.py
    │   ├── workspace_test.py
    │   ├── writer.py
    │   ├── writingFiles.py
    │   ├── xcode (2).py
    │   ├── xcode_emulation (2).py
    │   ├── xcode_emulation.py
    │   ├── xcode_ninja (2).py
    │   ├── xcode_ninja.py
    │   ├── xcodeproj_file (2).py
    │   ├── xcodeproj_file.py
    │   ├── xcode.py
    │   ├── xcode_test (2).py
    │   ├── xcode_test.py
    │   ├── xml_fix (2).py
    │   ├── xml_fix.py
    │   ├── xor.py
    │   ├── xpath_test.py
    │   ├── youtube_playlist_spider_scrapy.py
    │   ├── YT_DOWN.py
    │   └── zero_redundancy_optimizer.py
    ├── 2
    │   ├── all_factors.py
    │   ├── allMyCats1.py
    │   ├── allMyCats2.py
    │   ├── allocation_number.py
    │   ├── all_pairs_shortest_path.py
    │   ├── all_permutations.py
    │   ├── allSizedIntegers.py
    │   ├── all_subsequences.py
    │   ├── alphabeticalattributes.py
    │   ├── alternate_disjoint_set.py
    │   ├── americas.py
    │   ├── amusement_park.py
    │   ├── anagram-detection.py
    │   ├── anagram_mappings.py
    │   ├── anagram.py
    │   ├── anagrams.py
    │   ├── analysis.py
    │   ├── anchor.py
    │   ├── anchors_test.py
    │   ├── angry_bird.py
    │   ├── An_Identiconizer_generator_implementation_in_python.py
    │   ├── annotation.py
    │   ├── announcement.py
    │   ├── apigen.py
    │   ├── api.py
    │   ├── apostrophe.py
    │   ├── appfs.py
    │   ├── app.py
    │   ├── approximate_matching.py
    │   ├── apps.py
    │   ├── archive_util (2).py
    │   ├── archive_util (3).py
    │   ├── archive_util (4).py
    │   ├── archive_util (5).py
    │   ├── archive_util (6).py
    │   ├── archive_util (7).py
    │   ├── archive_util.py
    │   ├── area.py
    │   ├── area_under_curve.py
    │   ├── area_volume_calculator.py
    │   ├── are_you_connected_to_world.py
    │   ├── args_example_1.py
    │   ├── args_example.py
    │   ├── arguments.py
    │   ├── arithmetic_progression_builder.py
    │   ├── armstrong_number.py
    │   ├── armstrong_numbers.py
    │   ├── array-pair-sum.py
    │   ├── array_parity.py
    │   ├── Array.py
    │   ├── array_sum_combinations.py
    │   ├── array_to_bst.py
    │   ├── articulation_points.py
    │   ├── asset_model.py
    │   ├── astar.py
    │   ├── ast_transforms.py
    │   ├── async_helpers.py
    │   ├── asyncio.py
    │   ├── atbash_cipher.py
    │   ├── atbash.py
    │   ├── auth_routes.py
    │   ├── autocomplete_using_trie.py
    │   ├── autogen_api.py
    │   ├── autogen_config.py
    │   ├── autogen_magics.py
    │   ├── autogen_shortcuts.py
    │   ├── average_mean.py
    │   ├── average_median.py
    │   ├── average_mode.py
    │   ├── avg.py
    │   ├── avl.py
    │   ├── avl_tree.py
    │   ├── backport_pr.py
    │   ├── back_propagation_neural_network.py
    │   ├── backup.py
    │   ├── backupToZip.py
    │   ├── bad_all.py
    │   ├── bailey_borwein_plouffe.py
    │   ├── balanced-brackets.py
    │   ├── balanced_brackets.py
    │   ├── balanced_parentheses.py
    │   ├── banned_users.py
    │   ├── bar_descriptions.py
    │   ├── base16.py
    │   ├── base32.py
    │   ├── base64_encoding.py
    │   ├── base85.py
    │   ├── base_conversion.py
    │   ├── basedb.py
    │   ├── base_handler.py
    │   ├── base.py
    │   ├── base_value.py
    │   ├── basic_binary_tree.py
    │   ├── BasicCryptanalysis.py
    │   ├── basic_graphs.py
    │   ├── basic_maths.py
    │   ├── basic.py
    │   ├── basic_string.py
    │   ├── basic_test.py
    │   ├── battleplayer.py
    │   ├── battleserver.py
    │   ├── battleship.py
    │   ├── bead_sort.py
    │   ├── beaufort_cipher.py
    │   ├── bellman_ford.py
    │   ├── Benchmark_between_python_and_Julia.py
    │   ├── Benchmark_of_the_SHA256_hash_function__python_Cython_Numba.py
    │   ├── Benchmarkpython.py
    │   ├── bench.py
    │   ├── bench_test.py
    │   ├── bezier_curve.py
    │   ├── bfs.py
    │   ├── bfs_shortest_path.py
    │   ├── bfs_zero_one_shortest_path.py
    │   ├── bibtex_beautifier.py
    │   ├── bicycles.py
    │   ├── bidirectional_a_star.py
    │   ├── bidirectional_breadth_first_search.py
    │   ├── bilateral_filter.py
    │   ├── binary_and_operator.py
    │   ├── binary_count_setbits.py
    │   ├── binary_count_trailing_zeros.py
    │   ├── binary_exp_mod.py
    │   ├── binary_exponentiation_2.py
    │   ├── binary_exponentiation.py
    │   ├── binary_gap.py
    │   ├── binary_heap.py
    │   ├── binary_or_operator.py
    │   ├── binary_search.py
    │   ├── binarySearch.py
    │   ├── binary_search_tree.py
    │   ├── binary_search_tree_recursive.py
    │   ├── binary_shifts.py
    │   ├── binary_to_decimal_conversion.py
    │   ├── binary_to_decimal_converter.py
    │   ├── binary_to_decimal.py
    │   ├── binary_to_octal.py
    │   ├── binary_tree_mirror.py
    │   ├── binary_tree_paths.py
    │   ├── binary_tree.py
    │   ├── binary_tree_traversals.py
    │   ├── binary_twos_complement.py
    │   ├── binary_xor_operator.py
    │   ├── binomial_coefficient.py
    │   ├── binomial_distribution.py
    │   ├── binomial_heap.py
    │   ├── bin_tree_to_list.py
    │   ├── birthday.py
    │   ├── birthdays.py
    │   ├── bisection.py
    │   ├── bitbucket_repolist.py
    │   ├── bit_manipulation.py
    │   ├── bitmask.py
    │   ├── bitonic_sort.py
    │   ├── bit_operation.py
    │   ├── BlackandWhite.py
    │   ├── blackjack.py
    │   ├── blueprints.py
    │   ├── Blurring_a_part_of_an_image_in_python.py
    │   ├── bmi_body_mass_index_calculator.py
    │   ├── boat.py
    │   ├── boat_routes.py
    │   ├── boats.py
    │   ├── bogo_sort.py
    │   ├── bomb_enemy.py
    │   ├── boohoo.py
    │   ├── bootstrap_config.py
    │   ├── boxPrint.py
    │   ├── boyer_moore_search.py
    │   ├── breadth_first_search_2.py
    │   ├── breadth_first_search.py
    │   ├── breadth_first_search_shortest_path.py
    │   ├── breaking_bad.py
    │   ├── browser_handler.py
    │   ├── brute_force_caesar_cipher.py
    │   ├── bst_closest_value.py
    │   ├── BSTIterator.py
    │   ├── bst.py
    │   ├── b_tree.py
    │   ├── bubble_sort.py
    │   ├── bubbleSort.py
    │   ├── bucket.py
    │   ├── bucket_sort.py
    │   ├── buggyAddingProgram.py
    │   ├── build_all_exercises_page.py
    │   ├── build_directory_md.py
    │   ├── build_docker_scraper.py
    │   ├── build_index.py
    │   ├── bulletPointAdder.py
    │   ├── bullet.py
    │   ├── burkes.py
    │   ├── burrows_wheeler.py
    │   ├── button.py
    │   ├── buy_sell_stock.py
    │   ├── bw_filter.py
    │   ├── bytes_int_conversion.py
    │   ├── cache.py
    │   ├── caesar_cipher.py
    │   ├── caesars_cipher_encryption.py
    │   ├── calc_interets.py
    │   ├── calcProd.py
    │   ├── calculate-day-in-your-head.py
    │   ├── calculator.py
    │   ├── Calcul_d_une_paire_de_des_un_peu_particuliers.py
    │   ├── calendar.py
    │   ├── calendar_this.py
    │   ├── callbacks.py
    │   ├── canny.py
    │   ├── can_string_be_rearranged_as_palindrome.py
    │   ├── capitalize.py
    │   ├── cap.py
    │   ├── capsLock.py
    │   ├── car.py
    │   ├── car_routes.py
    │   ├── cars.py
    │   ├── cartesian_plane_quadrant.py
    │   ├── c_ast.py
    │   ├── categories.py
    │   ├── category.py
    │   ├── category_routes.py
    │   ├── catnapping.py
    │   ├── ceil.py
    │   ├── census2010.py
    │   ├── c_generator.py
    │   ├── change_brightness.py
    │   ├── change_contrast.py
    │   ├── changelog.py
    │   ├── channelsabc.py
    │   ├── channels.py
    │   ├── chaos_machine.py
    │   ├── characterCount.py
    │   ├── char_prime_map.py
    │   ├── check_anagrams.py
    │   ├── check_bipartite_graph_bfs.py
    │   ├── check_bipartite_graph_dfs.py
    │   ├── check_bipartite.py
    │   ├── check_config_win.py
    │   ├── check_digraph_strongly_connected.py
    │   ├── check_IBAN.py
    │   ├── check_NIRPP.py
    │   ├── check_pangram.py
    │   ├── check-property-on-prime-numbers.py
    │   ├── check_sources.py
    │   ├── checktcpconn.py
    │   ├── check_voyages-sncf.py
    │   ├── cherrypy_print_file_interface.py
    │   ├── chess_game.py
    │   ├── chess_set.py
    │   ├── chinese_remainder_theorem.py
    │   ├── cholesky_matrix_decomposition.py
    │   ├── chudnovsky_algorithm.py
    │   ├── circular_linked_list.py
    │   ├── circular_queue.py
    │   ├── cities.py
    │   ├── class_animal_attributes_examples.py
    │   ├── class_example_movies.py
    │   ├── c_lexer.py
    │   └── c_parser.py
    ├── 3
    │   ├── classes.py
    │   ├── class_movies.py
    │   ├── clientabc.py
    │   ├── client.py
    │   ├── climbing_stairs.py
    │   ├── climbingStairs.py
    │   ├── clipboard.py
    │   ├── cli.py
    │   ├── clone_graph.py
    │   ├── closest_pair_of_points.py
    │   ├── cloud_cover_statistics_with_darksky_data.py
    │   ├── CloudWatchMetricHandlerpython3.py
    │   ├── co2_emission.py
    │   ├── cocktail_shaker_sort.py
    │   ├── cocktail_sort.py
    │   ├── code.py
    │   ├── coding_dos.py
    │   ├── coin_change.py
    │   ├── coinFlip.py
    │   ├── collatz_sequence.py
    │   ├── coloring.py
    │   ├── color-picker.py
    │   ├── combination.py
    │   ├── combinations.py
    │   ├── combination_sum.py
    │   ├── combinePdfs.py
    │   ├── comb_sort.py
    │   ├── commandpath.py
    │   ├── comment.py
    │   ├── commit_size_vs_commit_message_size.py
    │   ├── common.py
    │   ├── comparator.py
    │   ├── compare_array_elements.py
    │   ├── completion_cache.py
    │   ├── completion.py
    │   ├── compress.py
    │   ├── concurrency-primes.py
    │   ├── conditional_put.py
    │   ├── conditionals_examples.py
    │   ├── config_creator.py
    │   ├── config_distro.py
    │   ├── config_loader.py
    │   ├── config_manager.py
    │   ├── config.py
    │   ├── configtraits.py
    │   ├── config_validator.py
    │   ├── confirmed_users.py
    │   ├── conf.py
    │   ├── conftest.py
    │   ├── conjugate_gradient.py
    │   ├── connected_components.py
    │   ├── connect.py
    │   ├── constants.py
    │   ├── construct_tree_postorder_preorder.py
    │   ├── contactbook.py
    │   ├── contacts_bisect.py
    │   ├── contain_string.py
    │   ├── context.py
    │   ├── controlLoops.py
    │   ├── conversion.py
    │   ├── convert-array.py
    │   ├── convert_lib_params_linux.py
    │   ├── convert_lib_params_windows.py
    │   ├── convertPolyLine.py
    │   ├── convert_to_negative.py
    │   ├── convex_hull.py
    │   ├── convolution_neural_network.py
    │   ├── convolve.py
    │   ├── conways_game_of_life.py
    │   ├── cookie_utils.py
    │   ├── copy_notebooks.py
    │   ├── copy.py
    │   ├── copy_random_pointer.py
    │   ├── copy_transform.py
    │   ├── core.py
    │   ├── cosine_similarity.py
    │   ├── couchdbbase.py
    │   ├── count_algorithm_execution_time.py
    │   ├── count_binary_substring.py
    │   ├── count_connected_number_of_component.py
    │   ├── countdown.py
    │   ├── counter.py
    │   ├── count_flips_to_convert.py
    │   ├── counting.py
    │   ├── counting_sort.py
    │   ├── count_islands_in_matrix.py
    │   ├── count_islands.py
    │   ├── count_left_node.py
    │   ├── count_number_of_one_bits.py
    │   ├── count_ones.py
    │   ├── count_paths.py
    │   ├── count.py
    │   ├── count_py.py
    │   ├── countries.py
    │   ├── country_codes.py
    │   ├── covid_stats_via_xpath.py
    │   ├── crawl_google_results.py
    │   ├── crawl_google_scholar_citation.py
    │   ├── create_boat.py
    │   ├── createdb.py
    │   ├── create.py
    │   ├── create_table.py
    │   ├── crout_matrix_decomposition.py
    │   ├── cryptomath_module.py
    │   ├── csv2sqlite3.py
    │   ├── csv-parsing.py
    │   ├── ctes.py
    │   ├── ctokens.py
    │   ├── ctx.py
    │   ├── cuisine_routes.py
    │   ├── cuisines.py
    │   ├── currency_converter.py
    │   ├── current_stock_price.py
    │   ├── current_weather.py
    │   ├── custom_attributes_test.py
    │   ├── custom_doctests.py
    │   ├── custom_downloader_middleware.py
    │   ├── custom_dupefilter.py
    │   ├── cwd_prompt.py
    │   ├── cycle_detection.py
    │   ├── cycle.py
    │   ├── cycle_sort.py
    │   ├── d68c98b66cd4_client_description.py
    │   ├── d85a62333272_.py
    │   ├── daemonize.py
    │   ├── daft_extension.py
    │   ├── daily_horoscope.py
    │   ├── database.py
    │   ├── Data_Challenge__Mangaki__September_2017.py
    │   ├── data_fetcher_processor_handler.py
    │   ├── data_handler.py
    │   ├── data_sample_handler.py
    │   ├── data_test_webpage.py
    │   ├── data_transformations.py
    │   ├── dataurl.py
    │   ├── davis–putnam–logemann–loveland.py
    │   ├── davisputnamlogemannloveland.py
    │   ├── days_you_lived.py
    │   ├── db.py
    │   ├── dc5e6834bdb4_.py
    │   ├── death_valley_highs_lows.py
    │   ├── debugger.py
    │   ├── debughelpers.py
    │   ├── debug.py
    │   ├── decimalencoder.py
    │   ├── decimal_isolate.py
    │   ├── decimal_to_any.py
    │   ├── decimal_to_binary_converter.py
    │   ├── decimal_to_binary_ip.py
    │   ├── decimal_to_binary.py
    │   ├── decimal_to_binary_recursion.py
    │   ├── decimal_to_hexadecimal.py
    │   ├── decimal_to_octal.py
    │   ├── decision_tree.py
    │   ├── decode_string.py
    │   ├── decorator.py
    │   ├── decrypt_caesar_with_chi_squared.py
    │   ├── decrypting_caesars_cipher.py
    │   ├── deepest_left.py
    │   ├── deepl.py
    │   ├── default_settings.py
    │   ├── default_strategy.py
    │   ├── default_tag.py
    │   ├── default_tags.py
    │   ├── default_value_test.py
    │   ├── delete_item.py
    │   ├── delete_node.py
    │   ├── delete_nth.py
    │   ├── delete.py
    │   ├── delete_reoccurring.py
    │   ├── delqueen.py
    │   ├── demographic-pandas-solution.py
    │   ├── Demonstration_of_running_a_Jupyter_notebook_with_sudo_rights.py
    │   ├── deMorgans_law.py
    │   ├── deploy_config.py
    │   ├── deploy_docker_scraper_images.py
    │   ├── depth_first_search_2.py
    │   ├── depth_first_search.py
    │   ├── depth_sum.py
    │   ├── deque_doubly.py
    │   ├── Des_dates_qui_font_des_nombres_premiers.py
    │   ├── Des_dates_qui_font_des_nombres_premiers__suite.py
    │   ├── design_yelp.py
    │   ├── detecting_english_programmatically.py
    │   ├── deterministic_miller_rabin.py
    │   └── deutsch_jozsa.py
    ├── 4
    │   ├── dfa.py
    │   ├── dfs.py
    │   ├── dice_rolling_simulator.py
    │   ├── dice_visual_different_sizes.py
    │   ├── dice_visual.py
    │   ├── dictionary.py
    │   ├── die.py
    │   ├── die_visual.py
    │   ├── difference_testing.py
    │   ├── different_dice.py
    │   ├── diffie_hellman_key_exchange.py
    │   ├── diffie_hellman.py
    │   ├── diffie.py
    │   ├── diff.py
    │   ├── dijkstra_2.py
    │   ├── dijkstra_algorithm.py
    │   ├── dijkstra_bankers_algorithm.py
    │   ├── dijkstra.py
    │   ├── dijkstras_two_stack_algorithm.py
    │   ├── dimensions.py
    │   ├── dinic.py
    │   ├── diophantine_equation.py
    │   ├── directed_and_undirected_(weighted)_graph.py
    │   ├── discountPercent.py
    │   ├── discount.py
    │   ├── disjoint_set.py
    │   ├── distance_on_number_line.py
    │   ├── division_calculator.py
    │   ├── division.py
    │   ├── divMod.py
    │   ├── django.py
    │   ├── djb2.py
    │   ├── DM_Images_debut.py
    │   ├── docstrings.py
    │   ├── documentation_spider.py
    │   ├── dog.py
    │   ├── domain_extractor.py
    │   ├── domains_test.py
    │   ├── dom.py
    │   ├── doomsday.py
    │   ├── double_ended_queue.py
    │   ├── double_hash.py
    │   ├── double_linear_search.py
    │   ├── double_linear_search_recursion.py
    │   ├── double_sort.py
    │   ├── doubly_linked_list.py
    │   ├── doubly_linked_list_two.py
    │   ├── downloadXkcd.py
    │   ├── dp.py
    │   ├── Droite Discrète - public 2012 D5.py
    │   ├── dtexample.py
    │   ├── dump_dD.py
    │   ├── dynamic_arrays.py
    │   ├── dynamic_params.py
    │   ├── ec1cac3399c9_.py
    │   ├── ec2_list_instances.py
    │   ├── echo.py
    │   ├── Ecriture inclusive.py
    │   ├── edit_distance.py
    │   ├── edit_file_form.py
    │   ├── edit_folder_form.py
    │   ├── editorhooks.py
    │   ├── edmonds_karp_multiple_source_and_sink.py
    │   ├── eeb276e51423_auth_state.py
    │   ├── ef590e979b0d_.py
    │   ├── Efficient_sampling_from_a_Binomial_distribution.py
    │   ├── egg_drop.py
    │   ├── electric_car.py
    │   ├── electric_car_with_battery.py
    │   ├── electric_power.py
    │   ├── elgamal_key_generator.py
    │   ├── elias.py
    │   ├── emails_from_url.py
    │   ├── emails.py
    │   ├── embed1.py
    │   ├── embed2.py
    │   ├── embed3.py
    │   ├── embed_class_long.py
    │   ├── embed_class_short.py
    │   ├── embed_flufl.py
    │   ├── embed_function.py
    │   ├── embed_no_flufl.py
    │   ├── emojize_pngorsvg.py
    │   ├── emojize.py
    │   ├── employee.py
    │   ├── emptyclass.py
    │   ├── encode_decode.py
    │   ├── enigma_machine2.py
    │   ├── enigma_machine.py
    │   ├── entropy.py
    │   ├── enum-2-string.py
    │   ├── enum.py
    │   ├── enums.py
    │   ├── environment.py
    │   ├── env.py
    │   ├── eq_explore_data.py
    │   ├── eq_world_map.py
    │   ├── errorExample.py
    │   ├── errors.py
    │   ├── error_tools.py
    │   ├── escape.py
    │   ├── etree_lxml.py
    │   ├── etree.py
    │   ├── euclidean_distance.py
    │   ├── euclidean_gcd.py
    │   ├── eulerian_path_and_circuit_for_undirected_graph.py
    │   ├── eulers_python.py
    │   ├── eulers_totient.py
    │   ├── euler_totient.py
    │   ├── evaluate_postfix_notations.py
    │   ├── even_numbers.py
    │   ├── even-occuring-element.py
    │   ├── EvenOddArray.py
    │   ├── even_or_odd.py
    │   ├── eventful.py
    │   ├── even_tree.py
    │   ├── events.py
    │   ├── example-demo.py
    │   ├── excel.py
    │   ├── exceptions.py
    │   ├── exitExample.py
    │   ├── explicit_euler.py
    │   ├── explosion.py
    │   ├── Exponential_Integral_python.py
    │   ├── extended_euclidean_algorithm.py
    │   ├── extended_gcd.py
    │   ├── external_sort.py
    │   ├── extract.py
    │   ├── f195f9a4923d_.py
    │   ├── f1b9ee79cb54_create_packages_table.py
    │   ├── fabfile.py
    │   ├── factor_combinations.py
    │   ├── factorial_iterative.py
    │   ├── factorialLog.py
    │   ├── factorial.py
    │   ├── factorial_python.py
    │   ├── factorial_recursive.py
    │   ├── factors.py
    │   ├── fairCoin.py
    │   ├── fast_fibonacci.py
    │   ├── favorite_languages.py
    │   ├── fenwick_tree.py
    │   ├── fermat_little_theorem.py
    │   ├── fetch_bbc_news.py
    │   ├── fetchers.py
    │   ├── fetch_github_info.py
    │   ├── fetch_jobs.py
    │   ├── Février 2021 un mini challenge arithmético-algorithmique.py
    │   ├── fibonacci_module.py
    │   ├── fibonacci.py
    │   ├── fibonacci_search.py
    │   ├── fibonacci_sequence_recursion.py
    │   ├── fib.py
    │   ├── field.py
    │   ├── fields.py
    │   ├── figure determiner.py
    │   ├── file_io.py
    │   ├── file_name.py
    │   ├── file.py
    │   ├── file_reader.py
    │   ├── file_routes.py
    │   ├── file_script.py
    │   ├── filesize.py
    │   ├── file_tag.py
    │   ├── file_to_string.py
    │   ├── file_type.py
    │   ├── file_types.py
    │   ├── file_util (2).py
    │   ├── file_util (3).py
    │   ├── file_util.py
    │   ├── filters.py
    │   ├── find_all_cliques.py
    │   ├── find_cube_root.py
    │   ├── find_difference.py
    │   ├── find_dups.py
    │   ├── finder.py
    │   ├── findinfiles.py
    │   ├── finding_bridges.py
    │   ├── find_keyboard_row.py
    │   ├── findLcm.py
    │   ├── find_max.py
    │   ├── find_max_recursion.py
    │   ├── find_min.py
    │   ├── find_min_recursion.py
    │   ├── find_min_rotate.py
    │   ├── find-missing-element.py
    │   ├── find_missing_number.py
    │   ├── findNthElement.py
    │   ├── findNumber.py
    │   ├── findNumInList.py
    │   ├── find_order_simple.py
    │   ├── find_path.py
    │   ├── findPercentage.py
    │   ├── find_primitive_root_simple.py
    │   ├── find_roots.py
    │   ├── find_square_root_of_imperfect_square.py
    │   ├── find_square_root.py
    │   ├── findTheNumber.py
    │   ├── find_words.py
    │   ├── first_come_first_served.py
    │   ├── first_cyclic_node.py
    │   ├── first-non-repeated-character.py
    │   ├── first_numbers.py
    │   ├── first_occurrence.py
    │   ├── first_unique_char.py
    │   ├── fischer_yates_shuffle.py
    │   ├── fiveTimes.py
    │   ├── fix-iocaml-notebook-exports-to-pdf.py
    │   ├── fix_kernelspec.py
    │   ├── fixup_whats_new_pr.py
    │   ├── fizzbuzz.py
    │   ├── flask.py
    │   ├── flatten-array.py
    │   ├── flatten.py
    │   ├── flip_bit_longest_sequence.py
    │   ├── flipBits.py
    │   ├── flip_invert_image.py
    │   ├── Floating_point_error_propagation_in_polynomial_multiplication_with_Fast-Fourier_Transform.py
    │   ├── flock.py
    │   ├── floor.py
    │   ├── flow_analysis.py
    │   ├── floyd_warshall.py
    │   ├── folder.py
    │   ├── folder_routes.py
    │   ├── folders.py
    │   ├── foods.py
    │   ├── ford_fulkerson.py
    │   ├── for_loop_mountain.py
    │   ├── formatted_name.py
    │   ├── formFiller.py
    │   ├── forms.py
    │   ├── fractional_knapsack_2.py
    │   ├── fractional_knapsack.py
    │   ├── fraisMission.py
    │   ├── frequency_finder.py
    │   ├── frequent_pattern_graph_miner.py
    │   ├── from_sequence.py
    │   ├── ftpfs.py
    │   ├── full_name.py
    │   ├── full_path.py
    │   ├── function.py
    │   ├── functions.py
    │   ├── fuzzy_operations.py
    │   ├── gale_shapley_bigraph.py
    │   ├── game_functions.py
    │   ├── game_of_life.py
    │   ├── game.py
    │   ├── game_routes.py
    │   ├── game_stats.py
    │   ├── gamma.py
    │   ├── garage.py
    │   ├── gas_station_routes.py
    │   ├── gas_stations.py
    │   ├── gauss_easter.py
    │   ├── gaussian_elimination.py
    │   ├── gaussian_filter.py
    │   ├── gaussian_naive_bayes.py
    │   ├── gaussian.py
    │   ├── gcd.py
    │   ├── generate_abbreviations.py
    │   ├── generateBaseline.py
    │   ├── generate_contents.py
    │   ├── generate-darwin-source-and-headers.py
    │   ├── generate_email.py
    │   ├── generate-metrics.py
    │   ├── generate_parenthesis.py
    │   ├── generatePortVersionsDb.py
    │   ├── generate_static_link_cmd_linux.py
    │   ├── generate_static_link_cmd_macos.py
    │   ├── generate_static_link_cmd_windows.py
    │   ├── generate_strobogrammtic.py
    │   ├── Generating_permutations_with_python.py
    │   ├── Generer_des_fausses_citations_latines_du_Roi_Loth.py
    │   ├── generics.py
    │   ├── gen_latex_symbols.py
    │   ├── genshi.py
    │   ├── geoMean.py
    │   ├── geometric_mean.py
    │   ├── geometric_progression_builder.py
    │   ├── geometric_series.py
    │   ├── get_anchor_test.py
    │   ├── getattr_static.py
    │   ├── get_current_temperature.py
    │   ├── getDocxText.py
    │   ├── get_extra_facets_test.py
    │   ├── get-gists.py
    │   ├── get_hierarchy_radio_test.py
    │   ├── get_imdb_top_250_movies_csv.py
    │   ├── get_imdbtop.py
    │   ├── get_item.py
    │   ├── get-json-data-from-google-scholar.py
    │   ├── get_level_weight_test.py
    │   ├── get-pip.py
    │   ├── get.py
    │   ├── get_records_from_dom_test.py
    │   ├── get_settings_test.py
    │   ├── gh_api.py
    │   ├── github_gpg.py
    │   ├── github_stats.py
    │   ├── git-mpr.py
    │   ├── globals.py
    │   ├── globals_test.py
    │   ├── glob.py
    │   ├── glut.py
    │   ├── gnome_sort.py
    │   ├── grades.py
    │   ├── gradient_boosting_regressor.py
    │   ├── gradient_descent.py
    │   ├── graham_scan.py
    │   ├── grammar.py
    │   ├── graph_dfs.py
    │   ├── graph_list.py
    │   ├── graph_matrix.py
    │   ├── graph.py
    │   ├── graphs_floyd_warshall.py
    │   ├── graph_topo_sort.py
    │   ├── gravatar.py
    │   ├── greatest_common_divisor.py
    │   ├── greedy_best_first.py
    │   ├── greedy_knapsack.py
    │   ├── greedy.py
    │   ├── greet_user.py
    │   ├── greet_users.py
    │   └── g_topological_sort.py
    ├── 5
    │   ├── greeter.py
    │   ├── gronsfeld_cipher.py
    │   ├── group_anagrams.py
    │   ├── gtk3.py
    │   ├── guessing-game.py
    │   ├── guessinggame.py
    │   ├── guessTheNumber.py
    │   ├── gui-glut.py
    │   ├── gui-gtk3.py
    │   ├── gui-gtk.py
    │   ├── gui-pyglet.py
    │   ├── gui-qt.py
    │   ├── gui-tk.py
    │   ├── gui-wx.py
    │   ├── hailstone.py
    │   ├── half_adder.py
    │   ├── hamiltonian_cycle.py
    │   ├── hamming_code.py
    │   ├── handler.py
    │   ├── handwritten_hashtable.py
    │   ├── hanoi.py
    │   ├── hardy_ramanujanalgo.py
    │   ├── harmonic_series.py
    │   ├── harriscorner.py
    │   ├── has_alternative_bit.py
    │   ├── hash_table.py
    │   ├── hashtable.py
    │   ├── hash_table_with_linked_list.py
    │   ├── has_loop.py
    │   ├── haversine_distance.py
    │   ├── hb_gil.py
    │   ├── healthScore.py
    │   ├── heap_generic.py
    │   ├── heap.py
    │   ├── heaps_algorithm_iterative.py
    │   ├── heaps_algorithm.py
    │   ├── heap_sort.py
    │   ├── height.py
    │   ├── hello-flask.py
    │   ├── helloFunc2.py
    │   ├── helloFunc.py
    │   ├── hello.py
    │   ├── hello_world.py
    │   ├── helpdesk_helper.py
    │   ├── helpers_05_08.py
    │   ├── helpers.py
    │   ├── hexadecimal_to_decimal.py
    │   ├── hierarchy.py
    │   ├── highlight_code.py
    │   ├── highs_lows.py
    │   ├── hill_cipher.py
    │   ├── hill_climbing.py
    │   ├── histogram.py
    │   ├── histogram_stretch.py
    │   ├── historyapp.py
    │   ├── hn_article.py
    │   ├── hn_submissions.py
    │   ├── homeworkProblems.py
    │   ├── hooks.py
    │   ├── hosoya_triangle.py
    │   ├── hotel_routes.py
    │   ├── hotels.py
    │   ├── house_robber.py
    │   ├── html5lib_shim.py
    │   ├── html5parser.py
    │   ├── html_source.py
    │   ├── htpasswd.py
    │   ├── HTTPpython.py
    │   ├── httpserver.py
    │   ├── huffman_coding.py
    │   ├── huffman.py
    │   ├── ice_cream_parlor_two_sum.py
    │   ├── identity_matrix_recognizer.py
    │   ├── image_downloader.py
    │   ├── imports.py
    │   ├── index_calculation.py
    │   ├── index.py
    │   ├── infix_to_postfix_conversion.py
    │   ├── infix_to_prefix_conversion.py
    │   ├── info.py
    │   ├── inject_meta_charset.py
    │   ├── inline_figshow.py
    │   ├── inorder.py
    │   ├── inputtransformer.py
    │   ├── insert_bit.py
    │   ├── insertion_sort.py
    │   ├── instagram_crawler.py
    │   ├── instagram_pic.py
    │   ├── instagram_video.py
    │   ├── install_data_ext.py
    │   ├── instance.py
    │   ├── in_static_equilibrium.py
    │   ├── int_divide.py
    │   ├── integeration_by_simpson_approx.py
    │   ├── integer_partition.py
    │   ├── interactiveshell.py
    │   ├── interpolation_search.py
    │   ├── interpreter.py
    │   ├── intersection.py
    │   ├── in_the_something.py
    │   ├── Introduction_aux_algorithmes_de_bandit__comme_UCB1_et_Thompson_Sampling.py
    │   ├── intro_sort.py
    │   ├── int_to_roman.py
    │   ├── inventory.py
    │   ├── inversedNumber.py
    │   ├── inverse_of_matrix.py
    │   ├── inversions.py
    │   ├── invert_tree.py
    │   ├── invite_removeuser.py
    │   ├── invite_user.py
    │   ├── iotools.py
    │   ├── ipdoctest.py
    │   ├── iptest.py
    │   ├── IPvalidation.py
    │   ├── ipykernel_launcher.py
    │   ├── ipython_console_highlighting.py
    │   ├── ipython_directive.py
    │   ├── ipython-get-history.py
    │   ├── is_anagram.py
    │   ├── is_balanced.py
    │   ├── is_bst.py
    │   ├── is_consecutive.py
    │   ├── is_cyclic.py
    │   ├── is_isomorphic.py
    │   ├── is_palindrome.py
    │   ├── is_palindrome_two_liner.py
    │   ├── isPhoneNumber.py
    │   ├── isPower.py
    │   ├── is_rotated.py
    │   ├── is_sorted.py
    │   ├── is_square_free.py
    │   ├── is_strobogrammatic.py
    │   ├── is_subsequence.py
    │   ├── is_subtree.py
    │   ├── is_symmetric.py
    │   ├── item_index.py
    │   ├── iterable.py
    │   ├── iterating_through_submasks.py
    │   ├── iterative_merge_sort.py
    │   ├── iterative_segment_tree.py
    │   ├── jaccard_similarity.py
    │   ├── jaro_winkler.py
    │   ├── JDelta.py
    │   ├── jedi_utils.py
    │   ├── jewelsAndStones.py
    │   ├── jinja_env.py
    │   ├── job_scheduling.py
    │   ├── join_with_slash.py
    │   ├── josephus.py
    │   ├── jqvmap.py
    │   ├── js_executor.py
    │   ├── jsonutil.py
    │   ├── judge_circle.py
    │   ├── jump_search.py
    │   ├── jupyterhub_config.py
    │   ├── jupyter.py
    │   ├── kadanes.py
    │   ├── kahns_algorithm_long.py
    │   ├── kahns_algorithm_topo.py
    │   ├── karatsuba.py
    │   ├── karger.py
    │   ├── kay_sort.py
    │   ├── k_closest_points.py
    │   ├── kernel.py
    │   ├── kernelspecapp.py
    │   ├── kernelspec.py
    │   ├── keywords.py
    │   ├── KeywordTranspositionCipher.py
    │   ├── k_factor.py
    │   ├── klass.py
    │   ├── k_means_clust.py
    │   ├── knapsack.py
    │   ├── k_nearest_neighbours.py
    │   ├── knight_tour.py
    │   ├── knn_sklearn.py
    │   ├── knuth_morris_pratt.py
    │   ├── koch_snowflake.py
    │   ├── kombu_queue.py
    │   ├── krishnamurthy_number.py
    │   ├── kSubs.py
    │   ├── kth_lexicographic_permutation.py
    │   ├── kth_order_statistic.py
    │   ├── kth_smallest.py
    │   ├── kth_to_last.py
    │   ├── Kullback_Leibler_divergences_in_native_python__Cython_and_Numba.py
    │   ├── kullback.py
    │   ├── lambda_handlers.py
    │   ├── lamberts_ellipsoidal_distance.py
    │   ├── language_survey.py
    │   ├── largest-continuous-sum.py
    │   ├── largest_of_very_large_numbers.py
    │   ├── largest_subarray_sum.py
    │   ├── last_occurrence.py
    │   ├── LatencyTesterpython3.py
    │   ├── latex_symbols.py
    │   ├── launcher.py
    │   ├── lazy_segment_tree.py
    │   ├── lazy_value.py
    │   ├── least_common_multiple.py
    │   ├── least_recently_used.py
    │   ├── lecture.py
    │   ├── leet_code_105.py
    │   ├── leet_code_108.py
    │   ├── leet_code_110.py
    │   ├── left_rotation.py
    │   ├── lempel_ziv_decompress.py
    │   ├── lempel_ziv.py
    │   ├── length.py
    │   ├── lesson_settings.py
    │   ├── lesson_tests.py
    │   ├── lessThanMoreThan.py
    │   ├── letter_combination.py
    │   ├── level_order.py
    │   └── levenshtein_distance.py
    ├── 6
    │   ├── lextab.py
    │   ├── lfu_cache.py
    │   ├── lib.py
    │   ├── license_number.py
    │   ├── LiFXpython.py
    │   ├── like.py
    │   ├── limit.py
    │   ├── linear_congruential_generator.py
    │   ├── linear_discriminant_analysis.py
    │   ├── linear_regression.py
    │   ├── linear_search.py
    │   ├── line_length.py
    │   ├── linkedin__kml_utility.py
    │   ├── linked_list.py
    │   ├── linkedlist.py
    │   ├── linked_queue.py
    │   ├── linked_stack.py
    │   ├── linkifier.py
    │   ├── linksame.py
    │   ├── lint.py
    │   ├── list_comprehensions.py
    │   ├── ListExample.py
    │   ├── ListIO.py
    │   ├── listOperationsMethods.py
    │   ├── listOperations.py
    │   ├── list.py
    │   ├── listReverse.py
    │   ├── littleKid.py
    │   ├── Living_in_a_noisy_world_with_James_Powell_rwatch_module.py
    │   ├── localinterfaces.py
    │   ├── local.py
    │   ├── logarithm_integer.py
    │   ├── log_cfg.py
    │   ├── loggers_test.py
    │   ├── logging.py
    │   ├── logic.py
    │   ├── login_form.py
    │   ├── login.py
    │   ├── logistic_regression.py
    │   ├── log.py
    │   ├── longest_abs_path.py
    │   ├── longest_common_prefix.py
    │   ├── longest_common_subsequence.py
    │   ├── longest_consecutive.py
    │   ├── longest_increasing.py
    │   ├── longest_increasing_subsequence_o(nlogn).py
    │   ├── longest_increasing_subsequence.py
    │   ├── longest_non_repeat.py
    │   ├── longest_palindromic_substring.py
    │   ├── longest_sub_array.py
    │   ├── lower.py
    │   ├── lowest_common_ancestor.py
    │   ├── lru_cache.py
    │   ├── lrucache.py
    │   ├── ls_slider.py
    │   ├── lucas_lehmer_primality_test.py
    │   ├── lucas_series.py
    │   ├── lucky.py
    │   ├── lu_decomposition.py
    │   ├── madLibs.py
    │   ├── magic8Ball2.py
    │   ├── magic8Ball.py
    │   ├── magicball_8.py
    │   ├── magicdiamondpattern.py
    │   ├── magicians.py
    │   ├── magic_number.py
    │   ├── magics.py
    │   ├── mail_ghost.py
    │   ├── mail_html.py
    │   ├── mail.py
    │   ├── mail_tel.py
    │   ├── main.py
    │   ├── makeresult.py
    │   ├── make_sentence.py
    │   ├── make_tarball.py
    │   ├── making_pizzas.py
    │   ├── MalwareDefender.py
    │   ├── manacher.py
    │   ├── manage.py
    │   ├── managerabc.py
    │   ├── manager.py
    │   ├── mandelbrot.py
    │   ├── Manual_implementation_of_some_hash_functions.py
    │   ├── Manual_implementation_of_the_Mersenne_twister_PseudoRandom_Number_Generator__PRNG_.py
    │   ├── many_users.py
    │   ├── mapIt.py
    │   ├── mapper_tutorial.py
    │   ├── map.py
    │   ├── markov_chain.py
    │   ├── mark.py
    │   ├── master_password.py
    │   ├── mathjax.py
    │   ├── mathoperators.py
    │   ├── matrix_chain_order.py
    │   ├── matrix_class.py
    │   ├── matrix_exponentiation.py
    │   ├── matrix_inversion.py
    │   ├── matrix_operation.py
    │   ├── max_by_alphabetical_order.py
    │   ├── max_difference_pair.py
    │   ├── maxDifference.py
    │   ├── max_heap.py
    │   ├── max_height.py
    │   ├── maximum_flow_bfs.py
    │   ├── maximum_flow_dfs.py
    │   ├── maximum_flow.py
    │   ├── maximumStreak.py
    │   ├── max_int_in_list.py
    │   ├── max_non_adjacent_sum.py
    │   ├── max_ones_index.py
    │   ├── max_path_sum.py
    │   ├── max_product_subarray.py
    │   ├── max.py
    │   ├── max_sliding_window.py
    │   ├── max_sub_array.py
    │   ├── max_subarray.py
    │   ├── max_subarray_sum.py
    │   ├── max_sum_contiguous_subsequence.py
    │   ├── max_sum_sliding_window.py
    │   ├── maze_search.py
    │   ├── mccabe.py
    │   ├── md5.py
    │   ├── mean.py
    │   ├── meanthreshold.py
    │   ├── Médiane des médianes - sélection en temps linéaire.py
    │   ├── median_filter.py
    │   ├── median_of_two_arrays.py
    │   ├── median.py
    │   ├── media.py
    │   ├── meeting_rooms.py
    │   ├── memoisation.py
    │   ├── memoize.py
    │   ├── memoryfs.py
    │   ├── menu_item.py
    │   ├── menu_item_type.py
    │   ├── menu.py
    │   ├── merge_insertion_sort.py
    │   ├── merge_intervals.py
    │   ├── Merge-k-sorted-lists.py
    │   ├── merge_sorted_k_lists.py
    │   ├── merge_sort.py
    │   ├── mergesort.py
    │   ├── merge_string_checker.py
    │   ├── merge_two_binary_trees.py
    │   ├── merge_two_list.py
    │   ├── merge_two_lists.py
    │   ├── meta.py
    │   ├── meta_test.py
    │   ├── metrics.py
    │   ├── microKanren.py
    │   ├── middle_element_of_linked_list.py
    │   ├── migrate.py
    │   ├── miller_rabin.py
    │   ├── min_by_alphabetical_order.py
    │   ├── min_cost_path.py
    │   ├── min_cost_string_conversion.py
    │   ├── min_distance.py
    │   ├── minesweeper.py
    │   ├── min_heap.py
    │   ├── min_height.py
    │   ├── minimax.py
    │   ├── minimum_coin_change.py
    │   ├── minimum_cost_path.py
    │   ├── minimum_cut.py
    │   ├── minimumMoves.py
    │   ├── minimum_partition.py
    │   ├── minimum_spanning_tree_boruvka.py
    │   ├── minimum_spanning_tree_kruskal2.py
    │   ├── minimum_spanning_tree_kruskal.py
    │   ├── minimum_spanning_tree_prims2.py
    │   ├── minimum_spanning_tree_prims.py
    │   ├── minimum_spanning_tree.py
    │   ├── minimum_steps_to_one.py
    │   ├── min_indexed_level_test.py
    │   ├── min_int_in_list.py
    │   ├── Mini_tutoriel_pour_la_resolution_de_programmes_lineaires_avec_python__ENS_Rennes_2021.py
    │   ├── min.py
    │   ├── mirror.py
    │   ├── missing_ranges.py
    │   ├── mistune.py
    │   ├── mixed_keyword_cypher.py
    │   ├── mixed.py
    │   ├── mknbindex.py
    │   ├── mobius_function.py
    │   ├── mocked_init.py
    │   ├── mocking.py
    │   ├── mockserverapp.py
    │   ├── mockservice.py
    │   ├── mocksu.py
    │   ├── models.py
    │   ├── mode.py
    │   ├── mod_example.py
    │   ├── modified_selection_sort.py
    │   ├── modify_emails.py
    │   ├── modular_division.py
    │   ├── modular_exponential.py
    │   ├── modular_inverse.py
    │   ├── Module10Lists.py
    │   ├── Module11WriteFileExtraCreditChallenge.py
    │   ├── Module11WriteFileGuestListChallenge.py
    │   ├── Module11WritingFiles.py
    │   ├── Module12ReadCSVfileChallenge.py
    │   ├── Module12ReadingFromFiles.py
    │   ├── Module13Function.py
    │   ├── Module13Functions.py
    │   ├── Module14FileMissingErrorChallengeSolution.py
    │   ├── Module15ProblemToCodeSolutionReadTxtMessage.py
    │   ├── module1.py
    │   ├── Module2DisplayingText.py
    │   ├── Module2DisplayPoemChallengeSolution.py
    │   ├── module2.py
    │   ├── Module3PersonalizedStorySolution.py
    │   ├── Module3StringsAndVariables.py
    │   ├── Module4MortgageCalculatorChallengeSolution.py
    │   ├── module4WorkingWithNumbers.py
    │   ├── Module5DeadlineChallengeSolution.py
    │   ├── Module6MakingDecisionsWithCode.py
    │   ├── Module6ShippingTotalChallengeSolution.py
    │   ├── Module7TaxesChallengeSolution.py
    │   ├── Module8DrawingChallenge.py
    │   ├── Module9EtchASketchChallengeSolution.py
    │   ├── Module9WhileLoops.py
    │   └── module.py
    ├── 7
    │   ├── molecular_chemistry.py
    │   ├── mongodbbase.py
    │   ├── mono_alphabetic_ciphers.py
    │   ├── monte_carlo_dice.py
    │   ├── monte_carlo.py
    │   ├── morse_code_decoder.py
    │   ├── morse_code_implementation.py
    │   ├── morse_code.py
    │   ├── mosaic.py
    │   ├── motorcycles.py
    │   ├── mountain_poll.py
    │   ├── mountfs.py
    │   ├── mouseNow2.py
    │   ├── mouseNow.py
    │   ├── move.py
    │   ├── move_zeros.py
    │   ├── moving_average.py
    │   ├── mpl_squares.py
    │   ├── MqttClientpython3.py
    │   ├── msvs_emulation.py
    │   ├── multidownloadXkcd.py
    │   ├── multifs.py
    │   ├── multi_heuristic_astar.py
    │   ├── multikernelmanager.py
    │   ├── multilayer_perceptron_classifier.py
    │   ├── multiples-of-3-and-5.py
    │   ├── multiplication-table.py
    │   ├── multiplicationTables.py
    │   ├── multiplying.py
    │   ├── multiply.py
    │   ├── multiply_strings.py
    │   ├── multiprocessing_queue.py
    │   ├── my_car.py
    │   ├── my_cars.py
    │   ├── my_electric_car.py
    │   ├── mymodule.py
    │   ├── my_name.py
    │   ├── myPets.py
    │   ├── myprogram.py
    │   ├── my-small-slack-bot.py
    │   ├── mysqlbase.py
    │   ├── mysubscript.py
    │   ├── Naive_simulations_of_the_Monty-Hall_paradox.py
    │   ├── naive_string_search.py
    │   ├── name_function.py
    │   ├── name.py
    │   ├── namespace.py
    │   ├── names.py
    │   ├── na_populations.py
    │   ├── natural_sort.py
    │   ├── nb_hits_updater.py
    │   ├── nearest_neighbor.py
    │   ├── nearest_square_and_its_root.py
    │   ├── nest_asyncio.py
    │   ├── nested_brackets.py
    │   ├── NetHack's functions Rne, Rn2 and Rnz in python 3.py
    │   ├── new_file_form.py
    │   ├── new_folder_form.py
    │   ├── newOnContacts.py
    │   ├── newton_forward_interpolation.py
    │   ├── newton_method.py
    │   ├── newton_raphson.py
    │   ├── next_bigger.py
    │   ├── next_greater_element.py
    │   ├── next_greatest_letter.py
    │   ├── next_perfect_square.py
    │   ├── ninja_syntax.py
    │   ├── node.py
    │   ├── nonascii2.py
    │   ├── nonascii.py
    │   ├── non_multiples.py
    │   ├── non_recursive_segment_tree.py
    │   ├── normalizer.py
    │   ├── notebook.py
    │   ├── notebook_routes.py
    │   ├── note.py
    │   ├── note_routes.py
    │   ├── notes.py
    │   ├── note_tag.py
    │   ├── note_tag_routes.py
    │   ├── note_tags.py
    │   ├── not_gate.py
    │   ├── n_queens_math.py
    │   ├── n_queens.py
    │   ├── n_sum.py
    │   ├── nth_digit.py
    │   ├── nth_fibonacci_using_matrix_exponentiation.py
    │   ├── number_guessing_game.py
    │   ├── number_lesser_greater.py
    │   ├── number_of_digits.py
    │   ├── number_of_possible_binary_trees.py
    │   ├── numberOfRoutes.py
    │   ├── number_reader.py
    │   ├── numbers.py
    │   ├── number_writer.py
    │   ├── num_decodings.py
    │   ├── num_empty.py
    │   ├── numerical_integration.py
    │   ├── numUniqueEmails.py
    │   ├── oauth.py
    │   ├── Obfuscated_code_or_piece_of_art.py
    │   ├── octal_to_decimal.py
    │   ├── odd_even_sort.py
    │   ├── odd_even_transposition_parallel.py
    │   ├── odd_even_transposition_single_threaded.py
    │   ├── oddNumbers.py
    │   ├── ohms_law.py
    │   ├── once.py
    │   ├── one_dimensional.py
    │   ├── one_edit_distance.py
    │   ├── onepad_cipher.py
    │   ├── one.py
    │   ├── openpy.py
    │   ├── open_selenium_browser_test.py
    │   ├── optimal_binary_search_tree.py
    │   ├── optionaltags.py
    │   ├── Oraux_CentraleSupelec_PSI__Juin_2017.py
    │   ├── Oraux_CentraleSupelec_PSI__Juin_2018.py
    │   ├── Oraux_CentraleSupelec_PSI__Juin_2019.py
    │   ├── order_detail.py
    │   ├── ordered_binary_search.py
    │   ├── ordered_stack.py
    │   ├── order.py
    │   ├── orders.py
    │   ├── order_up.py
    │   ├── osfs.py
    │   ├── os.py
    │   ├── osx.py
    │   ├── otherAngle.py
    │   ├── pacific_atlantic.py
    │   ├── package_tracker.py
    │   ├── page_rank.py
    │   ├── page_rank_test.py
    │   ├── palindrome_partitioning.py
    │   ├── Palindrome.py
    │   ├── pancake_sort.py
    │   ├── param.py
    │   ├── parrot.py
    │   ├── parse.py
    │   ├── parser_cache.py
    │   ├── parser_utils.py
    │   ├── partition.py
    │   └── p_series.py
    ├── 8
    │   ├── passingReference.py
    │   ├── password_creator.py
    │   ├── password_generator.py
    │   ├── path_between_two_vertices_in_digraph.py
    │   ├── path_finder.py
    │   ├── path_finders.py
    │   ├── path_sum2.py
    │   ├── path_sum.py
    │   ├── patience_sort.py
    │   ├── pattern_match.py
    │   ├── peak.py
    │   ├── peak_signal_to_noise_ratio.py
    │   ├── pelicanconf.py
    │   ├── pep8.py
    │   ├── percentageCalc.py
    │   ├── percentage_increase_decrease.py
    │   ├── perceptron.py
    │   ├── perfect_cube.py
    │   ├── perfect_number.py
    │   ├── perfect_square.py
    │   ├── permute.py
    │   ├── permute_unique.py
    │   ├── personality_teller.py
    │   ├── person.py
    │   ├── pets.py
    │   ├── phoneAndEmail.py
    │   ├── physics.py
    │   ├── pi_birthday.py
    │   ├── pickleshare.py
    │   ├── pickleutil.py
    │   ├── picnicTable.py
    │   ├── pidutil.py
    │   ├── pigeonhole_sort.py
    │   ├── pigeon_sort.py
    │   ├── piggyBank.py
    │   ├── pigLatin.py
    │   ├── pi_monte_carlo_estimation.py
    │   ├── ping_host.py
    │   ├── ping_pong.py
    │   ├── pi_string.py
    │   ├── pizza.py
    │   ├── players.py
    │   ├── playfair_cipher.py
    │   ├── playground_config.py
    │   ├── plotnotes.py
    │   ├── Plus long sous mot commun (python3).py
    │   ├── plus_one.py
    │   ├── plyparser.py
    │   ├── policyserver.py
    │   ├── polybius_square_cipher.py
    │   ├── polymonial_regression.py
    │   ├── polynom_for_points.py
    │   ├── polynomial_evaluation.py
    │   ├── polynomial.py
    │   ├── populate_db.py
    │   ├── porta_cipher.py
    │   ├── postfix_evaluation.py
    │   ├── post_gen_project.py
    │   ├── postorder.py
    │   ├── post_save_hook.py
    │   ├── power_iteration.py
    │   ├── powerMod.py
    │   ├── power_of_two.py
    │   ├── power.py
    │   ├── power_using_recursion.py
    │   ├── pprint.py
    │   ├── predecessor.py
    │   ├── prefix_conversions.py
    │   ├── prefix_evaluation.py
    │   ├── prefix_function.py
    │   ├── prefix.py
    │   ├── pre_gen_project.py
    │   ├── preorder.py
    │   ├── prettyCharacterCount.py
    │   ├── pretty_print.py
    │   ├── pretty.py
    │   ├── prime_between.py
    │   ├── prime_check.py
    │   ├── prime_factors.py
    │   ├── primelib.py
    │   ├── prime-number.py
    │   ├── prime_numbers.py
    │   ├── primeNumbers.py
    │   ├── prime.py
    │   ├── prime_sieve_eratosthenes.py
    │   ├── primes.py
    │   ├── primes_sieve_of_eratosthenes.py
    │   ├── prim.py
    │   ├── prims_minimum_spanning.py
    │   ├── print_argv.py
    │   ├── printFunction.py
    │   ├── printing_models.py
    │   ├── printRandom.py
    │   ├── print_reverse.py
    │   ├── prioritization.py
    │   ├── priority_queue.py
    │   ├── priority_queue_using_list.py
    │   ├── Prise de rdv automatique sur Doctolib.py
    │   ├── problem_01_while_filter.py
    │   ├── problem_01_while_loop.py
    │   ├── problem_02_for_filter.py
    │   ├── problem_02_for_loop.py
    │   ├── problem_03_list_comprehension.py
    │   ├── problem_03_my_comprehension.py
    │   ├── problem_04_if_statement.py
    │   ├── problem_04_if_statements.py
    │   ├── problem_05_class.py
    │   ├── problem_06_inheritance.py
    │   ├── problem_07_dictionary.py
    │   ├── problem_07_dict.py
    │   ├── problem_08_built_in.py
    │   ├── problem_08_builtins.py
    │   ├── problem_09_get_set.py
    │   ├── problem_09_getters_and_setters.py
    │   ├── problem_10_func.py
    │   ├── problem_10_functions.py
    │   ├── processor.py
    │   ├── profitLoss.py
    │   ├── progressbar.py
    │   ├── projectdb.py
    │   ├── project_module.py
    │   ├── prompts.py
    │   ├── protego.py
    │   ├── provider.py
    │   ├── ptshell.py
    │   ├── public2012_D3.py
    │   ├── publications.py
    │   ├── publishconf.py
    │   ├── publish.py
    │   ├── pubsFromBib.py
    │   ├── put_item.py
    │   ├── pw.py
    │   ├── py3compat.py
    │   ├── PyColorize.py
    │   ├── pydev_console_utils.py
    │   ├── pydevd_extension_utils.py
    │   ├── pydevd_file_utils.py
    │   ├── pydevd_utils.py
    │   ├── pyglet.py
    │   ├── pyKeywords.py
    │   ├── Py_Pi_Day_2017.py
    │   ├── py.py
    │   ├── pytest.py
    │   ├── pythagoras.py
    │   ├── python_files_compiler.py
    │   ├── python_repos.py
    │   ├── python_repos_updated.py
    │   ├── python_repos_visual.py
    │   ├── py-tutorial.py
    │   ├── qr_decomposition.py
    │   ├── qt_for_kernel.py
    │   ├── qt_loaders.py
    │   ├── quadratic_equations_complex_numbers.py
    │   ├── quadratic_probing.py
    │   ├── quantum_entanglement.py
    │   ├── queen.py
    │   ├── query_history.py
    │   ├── queue_on_list.py
    │   ├── queue_on_pseudo_stack.py
    │   ├── queue.py
    │   ├── quick_select.py
    │   ├── quick_sort_3_partition.py
    │   ├── quick-sort.py
    │   ├── quick_sort.py
    │   ├── quickWeather.py
    │   ├── quine_mc_cluskey.py
    │   ├── rabbitmq.py
    │   ├── rabin_karp_hash.py
    │   ├── rabin_karp.py
    │   ├── rabin_miller.py
    │   ├── radians.py
    │   ├── radix2_fft.py
    │   ├── radix_sort.py
    │   ├── rail_fence_cipher.py
    │   ├── random_forest_classifier.py
    │   ├── random_forest_regressor.py
    │   ├── randomized_heap.py
    │   ├── randomized_set.py
    │   ├── random_latin_citation.py
    │   ├── randomModule.py
    │   ├── random_normal_distribution_quicksort.py
    │   ├── random_pivot_quick_sort.py
    │   ├── randomQuizGenerator.py
    │   └── random_walk.py
    ├── 9
    │   ├── ransom.py
    │   ├── RaspberryPiGpiopython3.py
    │   ├── rat_in_maze.py
    │   ├── rayleigh_quotient.py
    │   ├── readCensusExcel.py
    │   ├── readDocx.py
    │   ├── read_english_dictionary.py
    │   ├── readFiles.py
    │   ├── recaptcha_verification.py
    │   ├── receive_file.py
    │   ├── reconstruct_queue.py
    │   ├── recursion.py
    │   ├── recursive_binomial_coefficient.py
    │   ├── recursive_bubble_sort.py
    │   ├── recursive_insertion_sort.py
    │   ├── recursive_quick_sort.py
    │   ├── red_black_tree.py
    │   ├── redis_queue.py
    │   ├── refbug.py
    │   ├── references.py
    │   ├── regex_matching.py
    │   ├── registry.py
    │   ├── relative_ranks.py
    │   ├── relu.py
    │   ├── remember_me.py
    │   ├── remove_bit.py
    │   ├── removeCsvHeader.py
    │   ├── remove_duplicate.py
    │   ├── remove_duplicates.py
    │   ├── remove-from-list.py
    │   ├── remove_input_references.py
    │   ├── remove_min.py
    │   ├── remove_range.py
    │   ├── renameDates.py
    │   ├── rendering.py
    │   ├── render.py
    │   ├── repeat_string.py
    │   ├── repeat_substring.py
    │   ├── replstartup.py
    │   ├── request.py
    │   ├── reservation_form.py
    │   ├── reservation.py
    │   ├── reservation_routes.py
    │   ├── resizeAndAddLogo.py
    │   ├── resize.py
    │   ├── response.py
    │   ├── restarter.py
    │   ├── restaurant_cuisines.py
    │   ├── restaurant_routes.py
    │   ├── restaurants.py
    │   ├── resultdb.py
    │   ├── result_dump.py
    │   ├── result.py
    │   ├── result_worker.py
    │   ├── resume_field.py
    │   ├── resume_fields.py
    │   ├── resume.py
    │   ├── resume_routes.py
    │   ├── resumes.py
    │   ├── retar.py
    │   ├── reverse_bits.py
    │   ├── reverseInteger.py
    │   ├── reverse_letters.py
    │   ├── reverse_linked_list.py
    │   ├── reverse.py
    │   ├── reverse_sort.py
    │   ├── reverse_string.py
    │   ├── reverse_vowel.py
    │   ├── reverse_words.py
    │   ├── rev-word.py
    │   ├── rglob.py
    │   ├── ripple_adder_classic.py
    │   ├── rle_compression.py
    │   ├── rm_pyc.py
    │   ├── rocket.py
    │   ├── rock,paper,scissor.py
    │   ├── rod_cut.py
    │   ├── rod_cutting.py
    │   ├── rollercoaster.py
    │   ├── roman_numerals.py
    │   ├── roman_to_int.py
    │   ├── rot13.py
    │   ├── rotate_image.py
    │   ├── rotate_list.py
    │   ├── rotate_matrix.py
    │   ├── rotate.py
    │   ├── rotation.py
    │   ├── round_robin.py
    │   ├── routes.py
    │   ├── routing.py
    │   ├── rsa_cipher.py
    │   ├── rsa_factorization.py
    │   ├── rsa_key_generator.py
    │   ├── rsa.py
    │   ├── rst2md.py
    │   ├── run_config_docker.py
    │   ├── run_config.py
    │   ├── Runge-Kutta_methods_for_ODE_integration_in_python.py
    │   ├── runge_kutta.py
    │   ├── run.py
    │   ├── run_tests.py
    │   ├── rw_visual.py
    │   ├── sameName2.py
    │   ├── sameName3.py
    │   ├── sameName4.py
    │   ├── sameName.py
    │   ├── same_tree.py
    │   ├── sample_handler.py
    │   ├── sanitizer.py
    │   ├── satisfiability.py
    │   ├── sax.py
    │   ├── say_something.py
    │   ├── scan.py
    │   ├── scatter_squares.py
    │   ├── scc_kosaraju.py
    │   ├── scheduler.py
    │   ├── schema.py
    │   ├── scoreboard.py
    │   ├── scoring_algorithm.py
    │   ├── scoring_functions.py
    │   ├── script.py
    │   ├── sdbm.py
    │   ├── sdes.py
    │   ├── searchable_level_test.py
    │   ├── searching_in_sorted_matrix.py
    │   ├── search_insert.py
    │   ├── search_in_sorted_matrix.py
    │   ├── searchLogfile.py
    │   ├── search_range.py
    │   ├── search_rotate.py
    │   ├── secant_method.py
    │   ├── SecretsManagerpython3.py
    │   ├── SecurityBijective.py
    │   ├── SecurityEncryptionScheme.py
    │   ├── SecurityFunctions2.py
    │   ├── SecurityFunctions.py
    │   ├── SecurityInverseFunction.py
    │   ├── SecurityInvolution.py
    │   ├── SecurityKeySpaces.py
    │   ├── SecurityMessageSpaceandCiphertextSpace.py
    │   ├── SecurityPermutations.py
    │   ├── security.py
    │   ├── seed.py
    │   ├── segmented_sieve.py
    │   ├── segment_tree_other.py
    │   ├── segment_tree.py
    │   ├── selection_sort.py
    │   ├── selectionSort.py
    │   ├── selectors_exclude_test.py
    │   ├── selectors_parser.py
    │   ├── selfvis.py
    │   ├── sendDuesReminders.py
    │   ├── send_file.py
    │   ├── sendingEmailsInpython.py
    │   ├── sentinel_linear_search.py
    │   ├── separate_chaining_hashtable.py
    │   ├── sepia.py
    │   ├── sequential_minimum_optimization.py
    │   ├── serialize_deserialize.py
    │   ├── serializer.py
    │   ├── server.py
    │   ├── service.py
    │   ├── session.py
    │   ├── sessions.py
    │   ├── set_covering.py
    │   ├── settings_old.py
    │   ├── settings.py
    │   ├── setupegg.py
    │   ├── setup.py
    │   ├── sha1.py
    │   ├── SharedFile1python3.py
    │   ├── SharedFile2python3.py
    │   ├── shell_sort.py
    │   ├── sherman_morrison.py
    │   ├── sh_game.py
    │   ├── shipping_form.py
    │   ├── ship.py
    │   ├── shortest_distance_from_all_buildings.py
    │   ├── shortest_job_first.py
    │   ├── Short_study_of_the_Lempel-Ziv_complexity.py
    │   ├── show_hide_output.py
    │   ├── show_refs.py
    │   ├── shuffled_shift_cipher.py
    │   ├── shuffle.py
    │   ├── sierpinski_triangle.py
    │   ├── sieve_of_eratosthenes.py
    │   ├── sigmoid.py
    │   ├── signals.py
    │   ├── signature.py
    │   ├── signatures.py
    │   ├── signup_form.py
    │   ├── similarity_search.py
    │   ├── simple1.py
    │   ├── simple2.py
    │   ├── simpleArraySum.py
    │   ├── simple_binary_search.py
    │   ├── simpleerr.py
    │   ├── simple_keyword_cypher.py
    │   ├── simple.py
    │   ├── simple-python-game.py
    │   ├── simple_substitution_cipher.py
    │   ├── simplevars.py
    │   ├── simplify_path.py
    │   ├── simpson_rule.py
    │   ├── Simulated_annealing_in_python.py
    │   ├── simulated_annealing.py
    │   ├── Simulations_du_jeu_de_151.py
    │   ├── Simulations du jeu de carte Jap-Jap.py
    │   ├── single_bit_manipulation_operations.py
    │   ├── single_number2.py
    │   ├── single_number3.py
    │   ├── single_number.py
    │   ├── single_qubit_measure.py
    │   ├── singly_linked_list.py
    │   ├── sitemap_test.py
    │   ├── sitka_highs_lows.py
    │   ├── sitka_highs.py
    │   ├── sizestr.py
    │   ├── skew_heap.py
    │   ├── skip_list.py
    │   ├── skyline.py
    │   ├── slack_helper.py
    │   ├── slack_message.py
    │   ├── sleep-poisson-time.py
    │   ├── sleepWellAlarm.py
    │   ├── sliding_window_max.py
    │   ├── slowsort.py
    │   ├── smooth_filter_nojit_and_jit.py
    │   ├── snippeter.py
    │   ├── sobel_filter.py
    │   ├── SocketServerpython.py
    │   ├── softmax.py
    │   ├── sol1.py
    │   ├── sol2.py
    │   ├── sol32.py
    │   ├── sol3.py
    │   ├── sol4.py
    │   ├── sol5.py
    │   ├── sol6.py
    │   ├── sol7.py
    │   ├── solution42.py
    │   ├── solution.py
    │   ├── Solving_an_equation_and_the_Lambert_W_function.py
    │   ├── some_main_script.py
    │   ├── sort_colors.py
    │   ├── sortedlist.py
    │   ├── sorting_floats.py
    │   ├── sortingFunctions.py
    │   ├── sort_matrix_diagonally.py
    │   ├── sort.py
    │   ├── sortString.py
    │   ├── sound_effects.py
    │   ├── sparse_dot_vector.py
    │   ├── sparse_mul.py
    │   ├── spelling_practice.py
    │   ├── spiral_print.py
    │   ├── spiral_traversal.py
    │   ├── split.py
    │   ├── split-string.py
    │   ├── spritesheet.py
    │   ├── sqlalchemybase.py
    │   ├── sqlcompletion.py
    │   ├── sqlitebase.py
    │   ├── sqrt_precision_factor.py
    │   ├── squarecube.py
    │   ├── square_root_algorithm.py
    │   ├── square_root.py
    │   ├── squares.py
    │   ├── squareTurtle.py
    │   ├── ssl.py
    │   ├── stack.py
    │   ├── stack_using_dll.py
    │   ├── staircase copy.py
    │   ├── staircase.py
    │   ├── star_args.py
    │   ├── start_ipython_config.py
    │   ├── start_ui.py
    │   ├── start_urls_test.py
    │   ├── star_turtle.py
    │   ├── stdlib.py
    │   ├── stitch.py
    │   ├── stock_span_problem.py
    │   ├── stooge_sort.py
    │   ├── stop_cuisines.py
    │   ├── Stop.py
    │   ├── stop_routes.py
    │   ├── stops.py
    │   ├── stop_urls_test.py
    │   ├── stopwatch.py
    │   ├── strand_sort.py
    │   ├── strassen_matrix_multiplication.py
    │   ├── strdispatch.py
    │   ├── stringIndexing.py
    │   ├── stringOperations.py
    │   ├── stringReverse.py
    │   ├── strings.py
    │   ├── strip_chars_test.py
    │   ├── strip_url_params.py
    │   ├── strongly_connected_components.py
    │   ├── strong_password.py
    │   ├── stub_value.py
    │   ├── stutter.py
    │   ├── style.py
    │   ├── styles.py
    │   ├── subfs.py
    │   ├── subset_generation.py
    │   ├── subsets.py
    │   ├── subsets_unique.py
    │   ├── successor.py
    │   ├── sudoku.py
    │   ├── sudoku_solver.py
    │   ├── sudoku_validator.py
    │   ├── sum_array.py
    │   ├── sum-arr.py
    │   ├── sumAverage.py
    │   ├── summarize_ranges.py
    │   ├── summing_digits.py
    │   ├── sum_of_arithmetic_sequence.py
    │   ├── sum_of_arithmetic_series.py
    │   ├── sum_of_digits.py
    │   ├── sum_of_geometric_progression.py
    │   ├── SumOfIntegers.py
    │   ├── sum_of_subset.py
    │   ├── sum_of_subsets.py
    │   ├── sum.py
    │   ├── sum_sub_squares.py
    │   ├── support_vector_machines.py
    │   ├── survey.py
    │   ├── swap_case.py
    │   ├── swap_in_pairs.py
    │   ├── swap_nodes.py
    │   ├── swap_pair.py
    │   ├── switch_pairs.py
    │   ├── swordfish.py
    │   ├── symmetry_group_cycle_index.py
    │   ├── syntax_tree.py
    │   ├── syslogger.py
    │   ├── sys_path.py
    │   ├── systemInfo.py
    │   ├── systemstats.py
    │   ├── table_maker.py
    │   ├── table.py
    │   ├── tables.py
    │   ├── tabu_search.py
    │   ├── tag.py
    │   ├── tag_routes.py
    │   ├── tags.py
    │   ├── tags_test.py
    │   ├── take-a-break.py
    │   ├── talkmap.py
    │   ├── talks.py
    │   ├── tarfs.py
    │   ├── tarjan.py
    │   ├── tarjans_scc.py
    │   ├── taskdb.py
    │   ├── task.py
    │   ├── task_queue.py
    │   ├── tclass.py
    │   ├── tempdir.py
    │   ├── temperature_conversions.py
    │   ├── tempfs.py
    │   ├── template_default_tag.py
    │   ├── template_default_tags.py
    │   ├── template_field.py
    │   ├── template_fields.py
    │   ├── template.py
    │   ├── template_routes.py
    │   ├── templates.py
    │   ├── templating.py
    │   ├── ternary_search.py
    │   ├── test_01_simple_web_responses.py
    │   ├── test_02_db.py
    │   ├── test_alias.py
    │   ├── test_api.py
    │   ├── test_appfs.py
    │   ├── test_application.py
    │   ├── test_app.py
    │   ├── test_archives.py
    │   ├── test_arithmetic.py
    │   ├── test_array.py
    │   ├── test_assigment.py
    │   ├── test_async_helpers.py
    │   ├── test_auth_expiry.py
    │   ├── test_autocall.py
    │   ├── test_automata.py
    │   ├── test_backtrack.py
    │   ├── test_base_handler.py
    │   ├── test_base.py
    │   ├── test_bench.py
    │   ├── test_bfs.py
    │   ├── test_bit.py
    │   ├── test_bitwise.py
    │   ├── test_break.py
    │   ├── test_bulk.py
    │   ├── test_cap.py
    │   ├── test_class_and_instance_variables.py
    │   ├── test_class_definition.py
    │   ├── test_class_objects.py
    │   ├── test_clipboard.py
    │   ├── test_cli.py
    │   ├── test_comparison.py
    │   ├── test_completerlib.py
    │   ├── test_compression.py
    │   ├── test_continue.py
    │   ├── test_copy.py
    │   ├── test_counter.py
    │   ├── test_crypto.py
    │   ├── test_database.py
    │   ├── test_datetime.py
    │   ├── test_db.py
    │   ├── test_dead_link.py
    │   ├── test_decorators.py
    │   ├── test_dfs.py
    │   ├── test_dictionaries.py
    │   ├── test_digital_image_processing.py
    │   ├── test_dir2.py
    │   ├── test_displayhook.py
    │   ├── test_dp.py
    │   ├── test_editorhooks.py
    │   ├── test_encoding.py
    │   ├── test_enums.py
    │   ├── test_errors.py
    │   ├── test_error_tools.py
    │   ├── test_escape.py
    │   ├── test_eventlog.py
    │   ├── test_events.py
    │   ├── test_extension.py
    │   ├── test_fetcher_processor.py
    │   ├── test_fetcher.py
    │   ├── test_fetch_github_info.py
    │   ├── test_file_methods.py
    │   ├── test_file_reading.py
    │   ├── test_filesize.py
    │   ├── test_formatters.py
    │   ├── test_for.py
    │   ├── test_fscompat.py
    │   ├── test_ftpfs.py
    │   ├── test_ftp_parse.py
    │   ├── test_function_annotations.py
    │   ├── test_function_arbitrary_arguments.py
    │   ├── test_function_decorators.py
    │   ├── test_function_default_arguments.py
    │   ├── test_function_definition.py
    │   ├── test_function_documentation_string.py
    │   ├── test_function_keyword_arguments.py
    │   ├── test_function_scopes.py
    │   ├── test_function_unpacking_arguments.py
    │   ├── test_generate.py
    │   ├── test_generators.py
    │   ├── test_glob.py
    │   ├── test_graph.py
    │   ├── test_greedy_knapsack.py
    │   ├── test_handle_exceptions.py
    │   ├── test_handler.py
    │   ├── test_handlers.py
    │   ├── test_heap.py
    │   ├── test_histogram.py
    │   ├── test_history.py
    │   ├── test_hooks.py
    │   ├── test_identity.py
    │   ├── test_if.py
    │   ├── test_imports.py
    │   ├── test_info.py
    │   ├── testing.py
    │   ├── Testing_the_lolviz_python_module.py
    │   ├── test_inheritance.py
    │   ├── test_inputtransformer2_line.py
    │   ├── test_inputtransformer2.py
    │   ├── test_inputtransformer.py
    │   ├── test_install.py
    │   ├── test_instance_objects.py
    │   ├── test_internal_ssl_connections.py
    │   ├── test_iotools.py
    │   ├── test_ipdoctest.py
    │   ├── test_iplib.py
    │   ├── test_iterative_segment_tree.py
    │   ├── test_json.py
    │   ├── test_knapsack.py
    │   ├── test_lambda_expressions.py
    │   ├── test_linear_algebra.py
    │   ├── test_linkedlist.py
    │   ├── test_links.py
    │   ├── test_lists.py
    │   ├── test_logger.py
    │   ├── test_logical.py
    │   ├── test_lrucache.py
    │   ├── test_magic.py
    │   ├── test_magic_terminal.py
    │   ├── test_map.py
    │   ├── test_math.py
    │   ├── test_maths.py
    │   ├── test_matrix_operation.py
    │   ├── test_matrix.py
    │   ├── test_membership.py
    │   ├── test_memoryfs.py
    │   ├── test_message_queue.py
    │   ├── test_method_objects.py
    │   ├── test_metrics.py
    │   ├── test_min_spanning_tree_kruskal.py
    │   ├── test_min_spanning_tree_prim.py
    │   ├── test_mirror.py
    │   ├── test_ml.py
    │   ├── test_mode.py
    │   ├── test_modules.py
    │   ├── test_monomial.py
    │   ├── test_mountfs.py
    │   ├── test_move.py
    │   ├── test_multifs.py
    │   ├── test_multiple_inheritance.py
    │   ├── test_named_servers.py
    │   ├── test_name_function.py
    │   ├── test_new_name.py
    │   ├── test_node.py
    │   ├── test_numbers.py
    │   ├── test_objects.py
    │   ├── testofdivisibility.py
    │   ├── test_opener.py
    │   ├── test_openpy.py
    │   ├── test_osfs.py
    │   ├── test_packages.py
    │   ├── test_pages.py
    │   ├── test_pagination.py
    │   ├── test_pass.py
    │   ├── test_path.py
    │   ├── test_paths.py
    │   ├── test_permissions.py
    │   ├── test-permutation-algorithms.py
    │   ├── test_poker_hand.py
    │   ├── test_polynomial.py
    │   ├── test_prefilter.py
    │   ├── test_prime_check.py
    │   ├── test_processor.py
    │   ├── test_profile.py
    │   ├── test_progressbar.py
    │   ├── test_prompts.py
    │   ├── test_proxy.py
    │   ├── test.py
    │   ├── test_queues.py
    │   ├── test_raise_exceptions.py
    │   ├── test_refs.py
    │   ├── test_re.py
    │   ├── test_response.py
    │   ├── test_result_dump.py
    │   ├── test_result_worker.py
    │   ├── test_rglob.py
    │   ├── test_roman_numerals_pytest.py
    │   ├── test_roman_numerals_unittest.py
    │   ├── test_run.py
    │   ├── test_scheduler.py
    │   ├── test_search.py
    │   ├── test_security.py
    │   ├── test_send_file.py
    │   ├── test_services_auth.py
    │   ├── test_services.py
    │   ├── test_set.py
    │   ├── test_sets.py
    │   ├── test_shimmodule.py
    │   ├── test_singleuser.py
    │   ├── test_sort.py
    │   ├── test_splitinput.py
    │   ├── tests.py
    │   ├── test_stack.py
    │   ├── test_storemagic.py
    │   ├── test_strings.py
    │   ├── test_subfs.py
    │   ├── test_survey.py
    │   ├── test_tarfs.py
    │   ├── test_task_queue.py
    │   ├── test_tempfs.py
    │   ├── test_time.py
    │   ├── test_tools.py
    │   ├── test_toposort.py
    │   ├── test_traitlets.py
    │   ├── test_tree.py
    │   ├── test_try.py
    │   ├── test_tuples.py
    │   ├── test_type_casting.py
    │   ├── test_ultratb.py
    │   ├── test_unix.py
    │   ├── test_url_tools.py
    │   ├── test_usage.py
    │   ├── test_user.py
    │   ├── test_utils.py
    │   ├── test_variables.py
    │   ├── test_version.py
    │   ├── test_walk.py
    │   ├── test_webdav.py
    │   ├── test_webui.py
    │   ├── test_while.py
    │   ├── test_wildcard.py
    │   ├── test_wrapfs.py
    │   ├── test_wrap.py
    │   ├── test_zipfs.py
    │   ├── test_zlib.py
    │   ├── text_edit_utils.py
    │   ├── text_justification.py
    │   ├── textMyself.py
    │   ├── text.py
    │   ├── TGV.py
    │   ├── threadDemo.py
    │   ├── threaded.py
    │   ├── three_sum.py
    │   ├── ticTacToe.py
    │   ├── time_conversion.py
    │   ├── time.py
    │   ├── timer.py
    │   ├── tim_sort.py
    │   ├── titlecase-cli.py
    │   ├── title_generator.py
    │   ├── Tkinter-animation.py
    │   ├── Tkinter-grocery-cost-calculator.py
    │   ├── Tkinter-Tron-GUI.py
    │   ├── todo_model.py
    │   ├── token_bucket.py
    │   ├── tokenize.py
    │   ├── token.py
    │   ├── toollib.py
    │   ├── tools.py
    │   ├── top_1.py
    │   ├── topological_sort.py
    │   ├── toposort.py
    │   ├── toppings.py
    │   ├── top_sort.py
    │   ├── tornado_fetcher.py
    │   ├── torrentStarter.py
    │   ├── tower_of_hanoi.py
    │   ├── TP1__python.py
    │   ├── TP2__python.py
    │   ├── TP3__python.py
    │   ├── trafid_cipher.py
    │   ├── traitlets.py
    │   ├── transformations_2d.py
    │   ├── transitive_closure_dfs.py
    │   ├── transposeMatrix.py
    │   ├── transposition_cipher_encrypt_decrypt_file.py
    │   ├── transposition_cipher.py
    │   ├── trapezoidal_rule.py
    │   ├── traversal.py
    │   ├── travis_after_all.py
    │   ├── treap.py
    │   ├── tree_equal.py
    │   ├── tree-level-order-print.py
    │   ├── tree_mirror.py
    │   ├── tree_node.py
    │   ├── tree_order.py
    │   ├── tree.py
    │   ├── tree_sort.py
    │   ├── tree_traversal.py
    │   ├── trie.py
    │   ├── trimmean.py
    │   ├── triplet_sum.py
    │   ├── trip_routes.py
    │   ├── trips.py
    │   ├── Tri tricolore.py
    │   ├── tuplesExample.py
    │   ├── turtleRandomWeb.py
    │   ├── two_pointer.py
    │   ├── two.py
    │   ├── two_sum.py
    │   ├── typeshed.py
    │   ├── type_var.py
    │   ├── typing.py
    │   ├── ugly_numbers.py
    │   ├── ulinecache.py
    │   ├── Un exercice d'algorithmique - mise en page de paragraphe, résolutions gourmande et dynamique.py
    │   ├── unicode.py
    │   ├── union_find.py
    │   ├── unique_bst.py
    │   ├── unique_morse.py
    │   ├── unique.py
    │   ├── unknown_sort.py
    │   ├── update_item.py
    │   ├── updateProduce.py
    │   ├── update.py
    │   ├── update_ui.py
    │   ├── update_whatsnew.py
    │   ├── upload.py
    │   ├── upper.py
    │   ├── url.py
    │   ├── urls_parser.py
    │   ├── urls.py
    │   ├── url_validate.py
    │   ├── usage.py
    │   ├── useful_function_mapping.py
    │   ├── user_profile.py
    │   ├── user.py
    │   ├── user_resume_tag.py
    │   ├── user_resume_tags.py
    │   ├── user_routes.py
    │   ├── users.py
    │   ├── user_tag.py
    │   ├── user_tags.py
    │   ├── util (22).py
    │   ├── util (23).py
    │   ├── util (24).py
    │   ├── util (4).py
    │   ├── utils (10).py
    │   ├── utils (11).py
    │   ├── utils (12).py
    │   ├── utils (13).py
    │   ├── utils (14).py
    │   ├── utils (15).py
    │   ├── utils (16).py
    │   ├── utils (17).py
    │   ├── utils (18).py
    │   ├── utils (19).py
    │   ├── utils (20).py
    │   ├── utils (3).py
    │   ├── utils (4).py
    │   ├── utils (5).py
    │   ├── utils (6).py
    │   ├── utils (7).py
    │   ├── utils (8).py
    │   ├── utils (9).py
    │   ├── validate_coordinates.py
    │   ├── validate_filenames.py
    │   ├── validateInput.py
    │   ├── validate.py
    │   ├── validate_solutions.py
    │   ├── validators.py
    │   ├── valid_parenthesis.py
    │   ├── valid_sudoku.py
    │   ├── value.py
    │   ├── vampire2.py
    │   ├── vampire.py
    │   ├── vector3_for_2d_rendering.py
    │   ├── Verification_de_numeros_CB_RIB_NIRPP_IBAN.py
    │   ├── version_01.py
    │   ├── version_02.py
    │   ├── version_03.py
    │   ├── version_04.py
    │   ├── videodownloader.py
    │   ├── views.py
    │   ├── vigenere_cipher.py
    │   ├── visualize_data.py
    │   ├── volume.py
    │   ├── voting.py
    │   ├── vowel_count.py
    │   ├── walk.py
    │   ├── walls_and_gates.py
    │   ├── wav.py
    │   ├── webdav.py
    │   ├── website_opener.py
    │   ├── weight_conversion.py
    │   ├── whatsYourName.py
    │   ├── whitespace.py
    │   ├── whoami-flask.py
    │   ├── whoami-oauth-basic.py
    │   ├── whoami-oauth.py
    │   ├── whoami.py
    │   ├── wholeMinute.py
    │   ├── wiggle_sort.py
    │   ├── wikistats.py
    │   ├── word_break.py
    │   ├── word_count.py
    │   ├── word_frequency_functions.py
    │   ├── word_ladder.py
    │   ├── word_occurrence.py
    │   ├── word_pattern.py
    │   ├── word_patterns.py
    │   ├── word_squares.py
    │   ├── workSchedule.py
    │   ├── world_covid19_stats.py
    │   ├── world_population.py
    │   ├── wrapfs.py
    │   ├── wrappers.py
    │   ├── wrap.py
    │   ├── write_message.py
    │   ├── writingFiles.py
    │   ├── wsgi.py
    │   ├── wx.py
    │   ├── xor_cipher.py
    │   ├── xpath_test.py
    │   ├── yacctab.py
    │   ├── ygen.py
    │   ├── youtube_playlist_spider_scrapy.py
    │   ├── zellers_congruence.py
    │   ├── zeroDivide.py
    │   ├── z_function.py
    │   ├── zigzagiterator.py
    │   ├── zigzag.py
    │   └── zipfs.py
    ├── _Unsorted
    │   ├── abbreviation.py
    │   ├── abs_max.py
    │   ├── abs_min.py
    │   ├── abs.py
    │   ├── add_binary.py
    │   ├── add_operators.py
    │   ├── add.py
    │   ├── add_two_numbers.py
    │   ├── adler32.py
    │   ├── aircraft_thrust.py
    │   ├── alias.py
    │   ├── aliquot_sum.py
    │   ├── all_factors.py
    │   ├── allocation_number.py
    │   ├── all.py
    │   ├── alternate_disjoint_set.py
    │   ├── anagram.py
    │   ├── anagrams.py
    │   ├── analysis.py
    │   ├── angry_bird.py
    │   ├── annotation.py
    │   ├── api.py
    │   ├── app-container.py
    │   ├── appengine.py
    │   ├── app.py
    │   ├── archive_util.py
    │   ├── area.py
    │   ├── area_under_curve.py
    │   ├── area_volume_calculator.py
    │   ├── are_you_connected_to_world.py
    │   ├── arguments.py
    │   ├── armstrong_number.py
    │   ├── armstrong_numbers.py
    │   ├── articulation_points.py
    │   ├── assertions.py
    │   ├── a_star.py
    │   ├── astar.py
    │   ├── ast.py
    │   ├── asyncfilters.py
    │   ├── asyncsupport.py
    │   ├── atbash_cipher.py
    │   ├── attach_pydevd.py
    │   ├── attach_script.py
    │   ├── auth.py
    │   ├── average_mean.py
    │   ├── average_median.py
    │   ├── average_mode.py
    │   ├── avg.py
    │   ├── avl.py
    │   ├── avl_tree.py
    │   ├── backend.py
    │   ├── back_propagation_neural_network.py
    │   ├── backtrack.py
    │   ├── bailey_borwein_plouffe.py
    │   ├── balanced_parentheses.py
    │   ├── base_command.py
    │   ├── base_conversion.py
    │   ├── base.py
    │   ├── base_request.py
    │   ├── base_response.py
    │   ├── base_value.py
    │   ├── basic_maths.py
    │   ├── batch.py
    │   ├── battleship.py
    │   ├── bazaar.py
    │   ├── bccache.py
    │   ├── bcppcompiler.py
    │   ├── bellman_ford.py
    │   ├── bidirectional_a_star.py
    │   ├── bilateral_filter.py
    │   ├── binary_exp_mod.py
    │   ├── binary_exponentiation_2.py
    │   ├── binary_exponentiation.py
    │   ├── binary_gap.py
    │   ├── binary_heap.py
    │   ├── binary_to_decimal_conversion.py
    │   ├── binary_to_decimal_converter.py
    │   ├── binary_to_decimal.py
    │   ├── binary_to_octal.py
    │   ├── binomial_coefficient.py
    │   ├── binomial_distribution.py
    │   ├── binomial_heap.py
    │   ├── bin_tree_to_list.py
    │   ├── bisection.py
    │   ├── blueprints.py
    │   ├── bmi_body_mass_index_calculator.py
    │   ├── bomb_enemy.py
    │   ├── bootstrap.py
    │   ├── breaking_bad.py
    │   ├── b_tree.py
    │   ├── burkes.py
    │   ├── burrows_wheeler.py
    │   ├── buy_sell_stock.py
    │   ├── cache.py
    │   ├── caesar_cipher.py
    │   ├── caesars_cipher_encryption.py
    │   ├── calculator.py
    │   ├── calendar.py
    │   ├── calendar_this.py
    │   ├── candidate.py
    │   ├── candidates.py
    │   ├── canny.py
    │   ├── can_string_be_rearranged_as_palindrome.py
    │   ├── capitalize.py
    │   ├── cars.py
    │   ├── cartesian_plane_quadrant.py
    │   ├── ccompiler.py
    │   ├── ceil.py
    │   ├── certs.py
    │   ├── cfg.py
    │   ├── chaos_machine.py
    │   ├── chardetect.py
    │   ├── chudnovsky_algorithm.py
    │   ├── circular_linked_list.py
    │   ├── classes.py
    │   ├── class_movies.py
    │   ├── clean.py
    │   ├── cli_helpers.py
    │   ├── climbing_stairs.py
    │   ├── cli.py
    │   ├── cmdoptions.py
    │   ├── cmd.py
    │   ├── co2_emission.py
    │   ├── codec.py
    │   ├── code.py
    │   ├── coin_change.py
    │   ├── collatz_sequence.py
    │   ├── collections_abc.py
    │   ├── collector.py
    │   ├── combination.py
    │   ├── combinations.py
    │   ├── combination_sum.py
    │   ├── command_context.py
    │   ├── command.py
    │   ├── comments.py
    │   ├── common_descriptors.py
    │   ├── compare.py
    │   ├── compatibility_tags.py
    │   ├── compiler.py
    │   ├── completion_cache.py
    │   ├── completion.py
    │   ├── complex-number-multiplication.py
    │   ├── concrete.py
    │   ├── conditional_put.py
    │   ├── conf.py
    │   ├── conjugate_gradient.py
    │   ├── connected_components.py
    │   ├── connectionpool.py
    │   ├── connection.py
    │   ├── console.py
    │   ├── constants_postaction.py
    │   ├── constants.py
    │   ├── constructors.py
    │   ├── construct_tree_postorder_preorder.py
    │   ├── contain_string.py
    │   ├── contextlib2.py
    │   ├── context.py
    │   ├── controller.py
    │   ├── conversion.py
    │   ├── convertPolyLine.py
    │   ├── convert.py
    │   ├── convert_to_negative.py
    │   ├── convolution_neural_network.py
    │   ├── convolve.py
    │   ├── cookies.py
    │   ├── copy_random_pointer.py
    │   ├── copy_transform.py
    │   ├── core.py
    │   ├── cors.py
    │   ├── cosine_similarity.py
    │   ├── cost.py
    │   ├── count_algorithm_execution_time.py
    │   ├── count_connected_number_of_component.py
    │   ├── count_flips_to_convert.py
    │   ├── count_islands.py
    │   ├── count_left_node.py
    │   ├── count_ones.py
    │   ├── count_paths.py
    │   ├── count.py
    │   ├── covid_stats_via_xpath.py
    │   ├── crawl_google_results.py
    │   ├── crawl_google_scholar_citation.py
    │   ├── create_table.py
    │   ├── ctx.py
    │   ├── cuisines.py
    │   ├── currency_converter.py
    │   ├── current_stock_price.py
    │   ├── current_weather.py
    │   ├── cycle_detection.py
    │   ├── cygwinccompiler.py
    │   ├── daily_horoscope.py
    │   ├── daily-temperatures.py
    │   ├── datastructures.py
    │   ├── data_transformations.py
    │   ├── data_types.py
    │   ├── datetime.py
    │   ├── davis–putnam–logemann–loveland.py
    │   ├── days_you_lived.py
    │   ├── debughelpers.py
    │   ├── debug.py
    │   ├── decimal_isolate.py
    │   ├── decimal_to_any.py
    │   ├── decimal_to_binary_converter.py
    │   ├── decimal_to_binary_ip.py
    │   ├── decimal_to_binary.py
    │   ├── decimal_to_binary_recursion.py
    │   ├── decimal_to_hexadecimal.py
    │   ├── decimal_to_octal.py
    │   ├── decision_tree.py
    │   ├── decoder.py
    │   ├── decode_string.py
    │   ├── decorator.py
    │   ├── decorators.py
    │   ├── decrypting_caesars_cipher.py
    │   ├── deepest_left.py
    │   ├── defaults.py
    │   ├── delete_item.py
    │   ├── delete_node.py
    │   ├── delete_nth.py
    │   ├── delete_reoccurring.py
    │   ├── deMorgans_law.py
    │   ├── depends.py
    │   ├── deprecation.py
    │   ├── depth_sum.py
    │   ├── dep_util.py
    │   ├── deque_doubly.py
    │   ├── design_yelp.py
    │   ├── detecting_english_programmatically.py
    │   ├── develop.py
    │   ├── dfa.py
    │   ├── diameter-of-binary-tree.py
    │   ├── dice_rolling_simulator.py
    │   ├── dictionary.py
    │   ├── diff.py
    │   ├── dijkstra_2.py
    │   ├── dijkstra_algorithm.py
    │   ├── dijkstra_bankers_algorithm.py
    │   ├── dijkstra.py
    │   ├── dinic.py
    │   ├── direct_url_helpers.py
    │   ├── direct_url.py
    │   ├── dirtools.py
    │   ├── dir_util.py
    │   ├── discountPercent.py
    │   ├── discount.py
    │   ├── disjoint_set.py
    │   ├── dispatcher.py
    │   ├── distance_on_number_line.py
    │   ├── distance.py
    │   ├── distribute-candies.py
    │   ├── django_debug.py
    │   ├── django.py
    │   ├── djb2.py
    │   ├── docstrings.py
    │   ├── domain_extractor.py
    │   ├── dom.py
    │   ├── doomsday.py
    │   ├── double_hash.py
    │   ├── doubly_linked_list.py
    │   ├── doubly_linked_list_two.py
    │   ├── download.py
    │   ├── dynamic_params.py
    │   ├── easter.py
    │   ├── ec2_list_instances.py
    │   ├── editable_legacy.py
    │   ├── editor.py
    │   ├── edmonds_karp_multiple_source_and_sink.py
    │   ├── egg_drop.py
    │   ├── egg_info.py
    │   ├── electric_power.py
    │   ├── elias.py
    │   ├── emails_from_url.py
    │   ├── encode_decode.py
    │   ├── encoder.py
    │   ├── encoding.py
    │   ├── enigma_machine.py
    │   ├── entropy.py
    │   ├── entrypoints.py
    │   ├── entry.py
    │   ├── enum.py
    │   ├── enums.py
    │   ├── environment.py
    │   ├── env.py
    │   ├── etag.py
    │   ├── etree_lxml.py
    │   ├── etree.py
    │   ├── euclidean_distance.py
    │   ├── euclidean_gcd.py
    │   ├── eulers_python.py
    │   ├── eulers_totient.py
    │   ├── euler_totient.py
    │   ├── evaluate_postfix_notations.py
    │   ├── even_tree.py
    │   ├── ex1.stoneg.py
    │   ├── ex2.stoneg.py
    │   ├── ex3.stoneg.py
    │   ├── exception.py
    │   ├── exc.py
    │   ├── explicit_euler.py
    │   ├── extended_euclidean_algorithm.py
    │   ├── extended_gcd.py
    │   ├── extension.py
    │   ├── ext.py
    │   ├── extract.py
    │   ├── ez_setup.py
    │   ├── f1b9ee79cb54_create_packages_table.py
    │   ├── factor_combinations.py
    │   ├── factorial_iterative.py
    │   ├── factorial.py
    │   ├── factorial_python.py
    │   ├── factorial_recursive.py
    │   ├── factors.py
    │   ├── factory.py
    │   ├── fallback.py
    │   ├── fancy_getopt.py
    │   ├── fast_fibonacci.py
    │   ├── fenwick_tree.py
    │   ├── fermat_little_theorem.py
    │   ├── fetch_bbc_news.py
    │   ├── fetch_github_info.py
    │   ├── fetch_jobs.py
    │   ├── fibonacci.py
    │   ├── fibonacci_sequence_recursion.py
    │   ├── fib.py
    │   ├── fields.py
    │   ├── figure determiner.py
    │   ├── file_io.py
    │   ├── filelist.py
    │   ├── file_name.py
    │   ├── filepost.py
    │   ├── filesystem.py
    │   ├── filetypes.py
    │   ├── file_util.py
    │   ├── filewrapper.py
    │   ├── filters.py
    │   ├── find_all_cliques.py
    │   ├── find_cube_root.py
    │   ├── find_difference.py
    │   ├── finder.py
    │   ├── finders.py
    │   ├── findinfiles.py
    │   ├── finding_bridges.py
    │   ├── find_keyboard_row.py
    │   ├── findLcm.py
    │   ├── find_max.py
    │   ├── find_max_recursion.py
    │   ├── find_min.py
    │   ├── find_min_recursion.py
    │   ├── find_min_rotate.py
    │   ├── find_missing_number.py
    │   ├── find_order_simple.py
    │   ├── find_path.py
    │   ├── find_primitive_root_simple.py
    │   ├── find_roots.py
    │   ├── find_square_root_of_imperfect_square.py
    │   ├── find_square_root.py
    │   ├── find_words.py
    │   ├── first_cyclic_node.py
    │   ├── first_occurrence.py
    │   ├── first_unique_char.py
    │   ├── fischer_yates_shuffle.py
    │   ├── fixture_functions.py
    │   ├── fixtures.py
    │   ├── fizzbuzz.py
    │   ├── flags.py
    │   ├── flatten.py
    │   ├── floor.py
    │   ├── flow_analysis.py
    │   ├── floyd_warshall.py
    │   ├── for_loop_mountain.py
    │   ├── format_control.py
    │   ├── format.py
    │   ├── formatting.py
    │   ├── formparser.py
    │   ├── forms.py
    │   ├── found_candidates.py
    │   ├── fractional_knapsack_2.py
    │   ├── fractional_knapsack.py
    │   ├── freeze.py
    │   ├── frequency_estimator.py
    │   ├── frequency_finder.py
    │   ├── friend-circles.py
    │   ├── from_sequence.py
    │   ├── frontend.py
    │   ├── full_path.py
    │   ├── function.py
    │   ├── functions.py
    │   ├── fuzzy_operations.py
    │   ├── game_of_life.py
    │   ├── game.py
    │   ├── gamma.py
    │   ├── garage.py
    │   ├── gas_stations.py
    │   ├── gauss_easter.py
    │   ├── gaussian_filter.py
    │   ├── gaussian_naive_bayes.py
    │   ├── gaussian.py
    │   ├── gcd.py
    │   ├── generate_abbreviations.py
    │   ├── generate_parenthesis.py
    │   ├── generate_strobogrammtic.py
    │   ├── generics.py
    │   ├── genshi.py
    │   ├── geoMean.py
    │   ├── geometric_series.py
    │   ├── getattr_static.py
    │   ├── get_item.py
    │   ├── git.py
    │   ├── glibc.py
    │   ├── globals.py
    │   ├── glob.py
    │   ├── gradient_boosting_regressor.py
    │   ├── gradient_descent.py
    │   ├── grammar.py
    │   ├── greedy_best_first.py
    │   ├── greedy.py
    │   ├── group_anagrams.py
    │   ├── guessing-game.py
    │   ├── guessinggame.py
    │   ├── hailstone.py
    │   ├── hamming_code.py
    │   ├── hangman.py
    │   ├── hardy_ramanujanalgo.py
    │   ├── harmonic_series.py
    │   ├── hashes.py
    │   ├── hash.py
    │   ├── hashset.py
    │   ├── hash_table.py
    │   ├── hashtable.py
    │   ├── hash_table_with_linked_list.py
    │   ├── has_loop.py
    │   ├── healthScore.py
    │   ├── heap_generic.py
    │   ├── Heap.py
    │   ├── height.py
    │   ├── hello.py
    │   ├── hello_world.py
    │   ├── helpers.py
    │   ├── help.py
    │   ├── heuristics.py
    │   ├── hexadecimal_to_decimal.py
    │   ├── hill_climbing.py
    │   ├── histogram.py
    │   ├── histogram_stretch.py
    │   ├── hooks.py
    │   ├── hosoya_triangle.py
    │   ├── hotels.py
    │   ├── house_robber.py
    │   ├── h.py
    │   ├── html_source.py
    │   ├── http_proxy.py
    │   ├── huffman_coding.py
    │   ├── huffman.py
    │   ├── hz.py
    │   ├── idnadata.py
    │   ├── idtracking.py
    │   ├── image_downloader.py
    │   ├── impl.py
    │   ├── imports.py
    │   ├── index_calculation.py
    │   ├── infix_to_postfix_conversion.py
    │   ├── infix_to_prefix_conversion.py
    │   ├── initialize_params_parser.py
    │   ├── inject_meta_charset.py
    │   ├── inject_securetransport.py
    │   ├── inorder.py
    │   ├── inputhooktk.py
    │   ├── insert-delete-getrandom-o1-duplicates-allowed.py
    │   ├── insert-delete-getrandom-o1.py
    │   ├── instagram_crawler.py
    │   ├── instagram_pic.py
    │   ├── instagram_video.py
    │   ├── instance.py
    │   ├── instr.py
    │   ├── int_divide.py
    │   ├── integeration_by_simpson_approx.py
    │   ├── integer_partition.py
    │   ├── interpreter.py
    │   ├── intersection.py
    │   ├── in_the_something.py
    │   ├── intl-date-to-us.py
    │   ├── intranges.py
    │   ├── intro.py
    │   ├── int_to_roman.py
    │   ├── inventory.py
    │   ├── invert_tree.py
    │   ├── io.py
    │   ├── ipython.py
    │   ├── ir_generator.py
    │   ├── is_anagram.py
    │   ├── is_balanced.py
    │   ├── is_consecutive.py
    │   ├── is_cyclic.py
    │   ├── is_isomorphic.py
    │   ├── isoparser.py
    │   ├── is_palindrome.py
    │   ├── is_palindrome_two_liner.py
    │   ├── is_rotated.py
    │   ├── is_square_free.py
    │   ├── is_strobogrammatic.py
    │   ├── is_subtree.py
    │   ├── is_symmetric.py
    │   ├── item_index.py
    │   ├── iterable.py
    │   ├── iterating_through_submasks.py
    │   ├── iterative_segment_tree.py
    │   ├── jaccard_similarity.py
    │   ├── jaro_winkler.py
    │   ├── jedi_utils.py
    │   ├── jinja2_debug.py
    │   ├── job_scheduling.py
    │   ├── join_with_slash.py
    │   ├── josephus.py
    │   ├── js_client.py
    │   ├── js_helpers.py
    │   ├── js_types.py
    │   ├── judge_circle.py
    │   ├── jws.py
    │   ├── kadanes.py
    │   ├── kahns_algorithm_long.py
    │   ├── kahns_algorithm_topo.py
    │   ├── karatsuba.py
    │   ├── karger.py
    │   ├── k_closest_points.py
    │   ├── keywords.py
    │   ├── k_factor.py
    │   ├── klass.py
    │   ├── k_means_clust.py
    │   ├── knapsack.py
    │   ├── k_nearest_neighbours.py
    │   ├── knn_sklearn.py
    │   ├── knuth_morris_pratt.py
    │   ├── krishnamurthy_number.py
    │   ├── kth_smallest.py
    │   ├── kth_to_last.py
    │   ├── langhelpers.py
    │   ├── largest_of_very_large_numbers.py
    │   ├── last_occurrence.py
    │   ├── launch.py
    │   ├── lazy_segment_tree.py
    │   ├── lazy_value.py
    │   ├── lazy_wheel.py
    │   ├── least_common_multiple.py
    │   ├── least_recently_used.py
    │   ├── lecture.py
    │   ├── leet_code_105.py
    │   ├── leet_code_108.py
    │   ├── leet_code_110.py
    │   ├── legacy.py
    │   ├── lempel_ziv_decompress.py
    │   ├── lempel_ziv.py
    │   ├── length.py
    │   ├── lessThanMoreThan.py
    │   ├── letter_combination.py
    │   ├── level_order.py
    │   ├── levenshtein_distance.py
    │   ├── lexer.py
    │   ├── lfu_cache.py
    │   ├── lib2to3_ex.py
    │   ├── lib.py
    │   ├── license_number.py
    │   ├── limit.py
    │   ├── linear_congruential_generator.py
    │   ├── linear_discriminant_analysis.py
    │   ├── linear_regression.py
    │   ├── line_length.py
    │   ├── linked_list.py
    │   ├── linkedlist.py
    │   ├── link.py
    │   ├── list_comp.py
    │   ├── list_comprehensions.py
    │   ├── listOperationsMethods.py
    │   ├── listOperations.py
    │   ├── list.py
    │   ├── listReverse.py
    │   ├── literal.py
    │   ├── loaders.py
    │   ├── local.py
    │   ├── locations.py
    │   ├── logarithm_integer.py
    │   ├── logging.py
    │   ├── login_form.py
    │   ├── login.py
    │   ├── logistic_regression.py
    │   ├── logo.py
    │   ├── logParser.py
    │   ├── lonely-pixel-ii.py
    │   ├── lonely-pixel-i.py
    │   ├── longest_abs_path.py
    │   ├── longest_common_prefix.py
    │   ├── longest_common_subsequence.py
    │   ├── longest_consecutive.py
    │   ├── longest_increasing.py
    │   ├── longest_increasing_subsequence_o(nlogn).py
    │   ├── longest_increasing_subsequence.py
    │   ├── longest_non_repeat.py
    │   ├── longest-uncommon-subsequence-i.py
    │   ├── lower.py
    │   ├── lowest_common_ancestor.py
    │   ├── low_level.py
    │   ├── lru_cache.py
    │   ├── lucas_series.py
    │   ├── macosx_libfile.py
    │   ├── madLibs.py
    │   ├── magicball_8.py
    │   ├── magicdiamondpattern.py
    │   ├── magic_number.py
    │   ├── main_parser.py
    │   ├── main.py
    │   ├── makefile.py
    │   ├── make_sentence.py
    │   ├── manacher.py
    │   ├── manage.py
    │   ├── map.py
    │   ├── markers.py
    │   ├── markov_chain.py
    │   ├── mathoperators.py
    │   ├── matplotlibtools.py
    │   ├── max_heap.py
    │   ├── max_height.py
    │   ├── maximum_flow_bfs.py
    │   ├── maximum_flow_dfs.py
    │   ├── maximum_flow.py
    │   ├── max_int_in_list.py
    │   ├── max_non_adjacent_sum.py
    │   ├── max_ones_index.py
    │   ├── max_path_sum.py
    │   ├── max.py
    │   ├── max_sliding_window.py
    │   ├── max_sum_contiguous_subsequence.py
    │   ├── max_sum_sliding_window.py
    │   ├── md5.py
    │   ├── mean.py
    │   ├── median_filter.py
    │   ├── median.py
    │   ├── meeting_rooms.py
    │   ├── mercurial.py
    │   ├── merge_intervals.py
    │   ├── merge_two_list.py
    │   ├── merge_two_lists.py
    │   ├── metadata_legacy.py
    │   ├── meta.py
    │   ├── middle_element_of_linked_list.py
    │   ├── migration.py
    │   ├── miller_rabin.py
    │   ├── min_cost_path.py
    │   ├── min_cost_string_conversion.py
    │   ├── min_distance.py
    │   ├── minesweeper.py
    │   ├── min_heap.py
    │   ├── min_height.py
    │   ├── minimum_cost_path.py
    │   ├── minimum_partition.py
    │   ├── minimum_spanning_tree_boruvka.py
    │   ├── minimum_spanning_tree_kruskal2.py
    │   ├── minimum_spanning_tree_kruskal.py
    │   ├── minimum_spanning_tree_prims2.py
    │   ├── minimum_spanning_tree_prims.py
    │   ├── minimum_spanning_tree.py
    │   ├── minimum_steps_to_one.py
    │   ├── min_int_in_list.py
    │   ├── min.py
    │   ├── missing_ranges.py
    │   ├── mixed.py
    │   ├── mobius_function.py
    │   ├── mode.py
    │   ├── modular_exponential.py
    │   ├── modular_inverse.py
    │   ├── Module10Lists.py
    │   ├── Module11WriteFileExtraCreditChallenge.py
    │   ├── Module11WriteFileGuestListChallenge.py
    │   ├── Module11WritingFiles.py
    │   ├── Module12ReadCSVfileChallenge.py
    │   ├── Module12ReadingFromFiles.py
    │   ├── Module13Function.py
    │   ├── Module13Functions.py
    │   ├── Module14FileMissingErrorChallengeSolution.py
    │   ├── module1.py
    │   ├── Module2DisplayingText.py
    │   ├── Module2DisplayPoemChallengeSolution.py
    │   ├── module2.py
    │   ├── Module3PersonalizedStorySolution.py
    │   ├── Module3StringsAndVariables.py
    │   ├── Module4MortgageCalculatorChallengeSolution.py
    │   ├── module4WorkingWithNumbers.py
    │   ├── Module5DeadlineChallengeSolution.py
    │   ├── Module6MakingDecisionsWithCode.py
    │   ├── Module6ShippingTotalChallengeSolution.py
    │   ├── Module7TaxesChallengeSolution.py
    │   ├── Module8DrawingChallenge.py
    │   ├── Module9EtchASketchChallengeSolution.py
    │   ├── Module9WhileLoops.py
    │   ├── module.py
    │   ├── molecular_chemistry.py
    │   ├── mongo_client.py
    │   ├── mongo_service.py
    │   ├── monkey.py
    │   ├── monte_carlo_dice.py
    │   ├── monte_carlo.py
    │   ├── more-classes.py
    │   ├── more-on-classes.py
    │   ├── morse_code_decoder.py
    │   ├── move.py
    │   ├── move_zeros.py
    │   ├── moving_average.py
    │   ├── mssql.py
    │   ├── msvc9compiler.py
    │   ├── msvccompiler.py
    │   ├── msvc.py
    │   ├── multi_heuristic_astar.py
    │   ├── multilayer_perceptron_classifier.py
    │   ├── multiplicationTables.py
    │   ├── multiply.py
    │   ├── multiply_strings.py
    │   ├── my_name.py
    │   ├── mysql.py
    │   ├── namespace.py
    │   ├── namespaces_and_docstrings.py
    │   ├── namespaces.py
    │   ├── names.py
    │   ├── nativetypes.py
    │   ├── nearest_neighbor.py
    │   ├── nearest_square_and_its_root.py
    │   ├── nested_brackets.py
    │   ├── newOnContacts.py
    │   ├── newton_raphson.py
    │   ├── next_bigger.py
    │   ├── next_greater_element.py
    │   ├── next_perfect_square.py
    │   ├── ninja_syntax.py
    │   ├── node_python_bridge.py
    │   ├── nodes.py
    │   ├── non_multiples.py
    │   ├── non_recursive_segment_tree.py
    │   ├── normalizer.py
    │   ├── notebook.py
    │   ├── note.py
    │   ├── notes.py
    │   ├── note_tag.py
    │   ├── note_tags.py
    │   ├── n_sum.py
    │   ├── nth_digit.py
    │   ├── ntlmpool.py
    │   ├── number_guessing_game.py
    │   ├── number_lesser_greater.py
    │   ├── number_of_digits.py
    │   ├── number_of_possible_binary_trees.py
    │   ├── num_decodings.py
    │   ├── num_empty.py
    │   ├── numerical_integration.py
    │   ├── obj_c_client.py
    │   ├── obj_c_helpers.py
    │   ├── obj_c_types.py
    │   ├── octal_to_decimal.py
    │   ├── odd_even_transposition_parallel.py
    │   ├── odd_even_transposition_single_threaded.py
    │   ├── ohms_law.py
    │   ├── op.py
    │   ├── ops.py
    │   ├── optimizer.py
    │   ├── optionaltags.py
    │   ├── oracle.py
    │   ├── ordered.py
    │   ├── ordered_set.py
    │   ├── orders.py
    │   ├── order_up.py
    │   ├── otherAngle.py
    │   ├── output.py
    │   ├── override.py
    │   ├── pacific_atlantic.py
    │   ├── package_data.py
    │   ├── package_finder.py
    │   ├── package_index.py
    │   ├── packages.py
    │   ├── package_tracker.py
    │   ├── packaging.py
    │   ├── pack.py
    │   ├── page_rank.py
    │   ├── palindrome_partitioning.py
    │   ├── palindrome.py
    │   ├── parabolic.py
    │   ├── parallel.py
    │   ├── param.py
    │   ├── parse.py
    │   ├── parser_cache.py
    │   ├── parser.py
    │   ├── parser_utils.py
    │   ├── partition.py
    │   ├── password_creator.py
    │   ├── password_generator.py
    │   ├── path_finder.py
    │   ├── path_finders.py
    │   ├── path_sum2.py
    │   ├── path_sum.py
    │   ├── pattern_match.py
    │   ├── peak_signal_to_noise_ratio.py
    │   ├── peephole_opt.py
    │   ├── pep8.py
    │   ├── percentageCalc.py
    │   ├── percentage_increase_decrease.py
    │   ├── perceptron.py
    │   ├── perfect_cube.py
    │   ├── perfect-number.py
    │   ├── perfect_number.py
    │   ├── perfect_square.py
    │   ├── permute.py
    │   ├── permute_unique.py
    │   ├── personality_teller.py
    │   ├── pets.py
    │   ├── physics.py
    │   ├── piggyBank.py
    │   ├── pigLatin.py
    │   ├── pi_monte_carlo_estimation.py
    │   ├── ping_host.py
    │   ├── ping_pong.py
    │   ├── pizza_time.py
    │   ├── pkginfo.py
    │   ├── pkg_resources.py
    │   ├── place.py
    │   ├── playground.py
    │   ├── plugin_base.py
    │   ├── plus_one.py
    │   ├── polymonial_regression.py
    │   ├── polynom_for_points.py
    │   ├── polynomial_evaluation.py
    │   ├── polynomial.py
    │   ├── poolmanager.py
    │   ├── posixemulation.py
    │   ├── postfix_evaluation.py
    │   ├── postgresql.py
    │   ├── postorder.py
    │   ├── post_save_hook.py
    │   ├── power_iteration.py
    │   ├── power_of_two.py
    │   ├── power.py
    │   ├── power_using_recursion.py
    │   ├── predecessor.py
    │   ├── prefix_conversions.py
    │   ├── prefix_evaluation.py
    │   ├── prefix_function.py
    │   ├── prefix.py
    │   ├── preorder.py
    │   ├── prepare.py
    │   ├── pretty_print.py
    │   ├── prim.py
    │   ├── prims_minimum_spanning.py
    │   ├── print_reverse.py
    │   ├── profiler.py
    │   ├── profiles.py
    │   ├── profitLoss.py
    │   ├── progress_bars.py
    │   ├── project.py
    │   ├── provider.py
    │   ├── providers.py
    │   ├── provision.py
    │   ├── proxy_fix.py
    │   ├── p_series.py
    │   ├── publish.py
    │   ├── put_item.py
    │   ├── py27.py
    │   ├── py2.py
    │   ├── py31compat.py
    │   ├── py34compat.py
    │   ├── py35compat.py
    │   ├── py35.py
    │   ├── py36compat.py
    │   ├── py36.py
    │   ├── py37compat.py
    │   ├── py37.py
    │   ├── py38compat.py
    │   ├── py38.py
    │   ├── py39.py
    │   ├── py3.py
    │   ├── pydev_app_engine_debug_startup.py
    │   ├── pydevconsole_code_for_ironpython.py
    │   ├── pydevconsole.py
    │   ├── pydev_console_utils.py
    │   ├── pydev_coverage.py
    │   ├── pydevd_additional_thread_info.py
    │   ├── pydevd_additional_thread_info_regular.py
    │   ├── pydevd_api.py
    │   ├── pydevd_base_schema.py
    │   ├── pydevd_breakpoints.py
    │   ├── pydevd_code_to_source.py
    │   ├── pydevd_command_line_handling.py
    │   ├── pydevd_comm_constants.py
    │   ├── pydevd_comm.py
    │   ├── pydevd_concurrency_logger.py
    │   ├── pydevd_console.py
    │   ├── pydevd_constants.py
    │   ├── pydevd_custom_frames.py
    │   ├── pydevd_cython_wrapper.py
    │   ├── pydevd_daemon_thread.py
    │   ├── pydevd_defaults.py
    │   ├── pydevd_dont_trace_files.py
    │   ├── pydevd_dont_trace.py
    │   ├── pydevd_exec2.py
    │   ├── pydevd_exec.py
    │   ├── pydevd_extension_api.py
    │   ├── pydevd_extension_utils.py
    │   ├── pydevd_file_utils.py
    │   ├── pydevd_filtering.py
    │   ├── pydevd_fix_code.py
    │   ├── pydevd_frame_eval_cython_wrapper.py
    │   ├── pydevd_frame_eval_main.py
    │   ├── pydevd_frame.py
    │   ├── pydevd_frame_tracing.py
    │   ├── pydevd_frame_utils.py
    │   ├── pydevd_helpers.py
    │   ├── pydevd_import_class.py
    │   ├── pydevd_io.py
    │   ├── pydevd_json_debug_options.py
    │   ├── pydevd_net_command_factory_json.py
    │   ├── pydevd_net_command_factory_xml.py
    │   ├── pydevd_net_command.py
    │   ├── pydevd_plugin_numpy_types.py
    │   ├── pydevd_plugins_django_form_str.py
    │   ├── pydevd_plugin_utils.py
    │   ├── pydevd_process_net_command_json.py
    │   ├── pydevd_process_net_command.py
    │   ├── pydevd_referrers.py
    │   ├── pydevd_reload.py
    │   ├── pydevd_resolver.py
    │   ├── pydevd_save_locals.py
    │   ├── pydevd_schema_log.py
    │   ├── pydevd_signature.py
    │   ├── pydevd_source_mapping.py
    │   ├── pydevd_suspended_frames.py
    │   ├── pydevd_thread_lifecycle.py
    │   ├── pydevd_thread_wrappers.py
    │   ├── pydevd_timeout.py
    │   ├── pydevd_trace_api.py
    │   ├── pydevd_trace_dispatch.py
    │   ├── pydevd_trace_dispatch_regular.py
    │   ├── pydevd_traceproperty.py
    │   ├── pydevd_tracing.py
    │   ├── pydevd_utils.py
    │   ├── pydevd_vars.py
    │   ├── pydevd_vm_type.py
    │   ├── pydevd_xml.py
    │   ├── pydev_import_hook.py
    │   ├── pydev_imports.py
    │   ├── pydev_ipython_console_011.py
    │   ├── pydev_ipython_console.py
    │   ├── pydev_is_thread_alive.py
    │   ├── pydev_localhost.py
    │   ├── pydev_log.py
    │   ├── pydev_monkey.py
    │   ├── pydev_monkey_qt.py
    │   ├── pydev_override.py
    │   ├── pydev_pysrc.py
    │   ├── pydev_runfiles_coverage.py
    │   ├── pydev_runfiles_nose.py
    │   ├── pydev_runfiles_parallel_client.py
    │   ├── pydev_runfiles_parallel.py
    │   ├── pydev_runfiles.py
    │   ├── pydev_runfiles_xml_rpc.py
    │   ├── pydev_run_in_console.py
    │   ├── pyfiles.py
    │   ├── pygls_type_overrides.py
    │   ├── pygls_utils.py
    │   ├── pyKeywords.py
    │   ├── pyopenssl.py
    │   ├── pyproject.py
    │   ├── py.py
    │   ├── pythagoras.py
    │   ├── python_client.py
    │   ├── python_files_compiler.py
    │   ├── python_helpers.py
    │   ├── python-notes.py
    │   ├── python_type_mapping.py
    │   ├── python_types.py
    │   ├── python_type_stubs.py
    │   ├── py-tutorial.py
    │   ├── qr_decomposition.py
    │   ├── qt_for_kernel.py
    │   ├── qt_loaders.py
    │   ├── qt.py
    │   ├── quadratic_equations_complex_numbers.py
    │   ├── quadratic_probing.py
    │   ├── quick_select.py
    │   ├── rabin_karp.py
    │   ├── rabin_miller.py
    │   ├── radians.py
    │   ├── radix2_fft.py
    │   ├── railroad_diagrams.py
    │   ├── random_forest_classifier.py
    │   ├── random_forest_regressor.py
    │   ├── randomized_heap.py
    │   ├── randomized_set.py
    │   ├── randomModule.py
    │   ├── random-pick-index.py
    │   ├── rayleigh_quotient.py
    │   ├── rdtw.py
    │   ├── readFiles.py
    │   ├── rebuild.py
    │   ├── recaptcha_verification.py
    │   ├── receive_file.py
    │   ├── recursion.py
    │   ├── recursive_binomial_coefficient.py
    │   ├── red_black_tree.py
    │   ├── redis_cache.py
    │   ├── refactor.py
    │   ├── references.py
    │   ├── regex_matching.py
    │   ├── register.py
    │   ├── registry.py
    │   ├── relativedelta.py
    │   ├── relu.py
    │   ├── remove_duplicate.py
    │   ├── remove_duplicates.py
    │   ├── remove_min.py
    │   ├── remove_range.py
    │   ├── render.py
    │   ├── repeat_string.py
    │   ├── replay_fixture.py
    │   ├── replstartup.py
    │   ├── reporters.py
    │   ├── repr.py
    │   ├── req_command.py
    │   ├── req_file.py
    │   ├── req_set.py
    │   ├── req_tracker.py
    │   ├── request.py
    │   ├── requirements.py
    │   ├── resize.py
    │   ├── resolver.py
    │   ├── resolvers.py
    │   ├── response.py
    │   ├── restaurant_cuisines.py
    │   ├── restaurants.py
    │   ├── result.py
    │   ├── retry.py
    │   ├── reverse_letters.py
    │   ├── reverse.py
    │   ├── reverse-string-ii.py
    │   ├── reverse_string.py
    │   ├── reverse_vowel.py
    │   ├── reverse-words-in-a-string-iii.py
    │   ├── reverse_words.py
    │   ├── revision.py
    │   ├── rewriter.py
    │   ├── rle_compression.py
    │   ├── rock,paper,scissor.py
    │   ├── rod_cut.py
    │   ├── rod_cutting.py
    │   ├── roman_numerals.py
    │   ├── roman_to_integer.py
    │   ├── roman_to_int.py
    │   ├── rotate_image.py
    │   ├── rotate_list.py
    │   ├── rotate.py
    │   ├── rotation.py
    │   ├── routing.py
    │   ├── roxypick.py
    │   ├── rrule.py
    │   ├── rsa.py
    │   ├── runge_kutta.py
    │   ├── runJediLanguageServer.py
    │   ├── run.py
    │   ├── runserver.py
    │   ├── runtime.py
    │   ├── same_tree.py
    │   ├── sample_data$wts.Page_gpostaction.py
    │   ├── sample_data$wts.Page.React.List_gpostaction.py
    │   ├── sample_data.py
    │   ├── sanitizer.py
    │   ├── satisfiability.py
    │   ├── saveopts.py
    │   ├── sax.py
    │   ├── scan.py
    │   ├── scc_kosaraju.py
    │   ├── schemaobj.py
    │   ├── scheme.py
    │   ├── score.py
    │   ├── scoring_algorithm.py
    │   ├── scoring_functions.py
    │   ├── scrape-domain-names.py
    │   ├── script.py
    │   ├── sdes.py
    │   ├── sections.py
    │   ├── security.py
    │   ├── segmented_sieve.py
    │   ├── segment_tree_other.py
    │   ├── segment_tree.py
    │   ├── selection_prefs.py
    │   ├── send_file.py
    │   ├── sendingEmailsInpython.py
    │   ├── separate_chaining_hashtable.py
    │   ├── sepia.py
    │   ├── sequential_minimum_optimization.py
    │   ├── serialize_deserialize.py
    │   ├── serialize.py
    │   ├── serializer.py
    │   ├── server_postaction.py
    │   ├── server.py
    │   ├── serving.py
    │   ├── session.py
    │   ├── set_covering.py
    │   ├── setopt.py
    │   ├── settings.py
    │   ├── setup_cython.py
    │   ├── setuptools_commands.py
    │   ├── sha1.py
    │   ├── shared_data.py
    │   ├── sherman_morrison.py
    │   ├── shipping_form.py
    │   ├── show.py
    │   ├── sierpinski_triangle.py
    │   ├── sieve_of_eratosthenes.py
    │   ├── sigmoid.py
    │   ├── signals.py
    │   ├── signature.py
    │   ├── signer.py
    │   ├── signup_form.py
    │   ├── simplify_path.py
    │   ├── simpson_rule.py
    │   ├── simulated_annealing.py
    │   ├── single_number2.py
    │   ├── single_number3.py
    │   ├── single_number.py
    │   ├── singly_linked_list.py
    │   ├── sitecustomize.py
    │   ├── skew_heap.py
    │   ├── skip_list.py
    │   ├── skyline.py
    │   ├── slack_helper.py
    │   ├── slack_message.py
    │   ├── sleepWellAlarm.py
    │   ├── sliding_window_max.py
    │   ├── sobel_filter.py
    │   ├── socks.py
    │   ├── softmax.py
    │   ├── sol1.py
    │   ├── sol2.py
    │   ├── sol32.py
    │   ├── sol3.py
    │   ├── sol4.py
    │   ├── sol5.py
    │   ├── sol6.py
    │   ├── sol7.py
    │   ├── solution42.py
    │   ├── solution (5).py
    │   ├── solution.py
    │   ├── solution_py.py
    │   ├── sparse_dot_vector.py
    │   ├── sparse_mul.py
    │   ├── spawn.py
    │   ├── specifiers.py
    │   ├── spinners.py
    │   ├── spiral_print.py
    │   ├── spiral_traversal.py
    │   ├── split.py
    │   ├── sqla_compat.py
    │   ├── sql_client.py
    │   ├── sqlite.py
    │   ├── sql_service.py
    │   ├── sqrt_precision_factor.py
    │   ├── squarecube.py
    │   ├── square_root_algorithm.py
    │   ├── square_root.py
    │   ├── squareTurtle.py
    │   ├── ssl_.py
    │   ├── ssl_support.py
    │   ├── star_args.py
    │   ├── startup.py
    │   ├── star_turtle.py
    │   ├── status_codes.py
    │   ├── stdlib.py
    │   ├── step_pattern.py
    │   ├── stone_base.py
    │   ├── stone_serializers.py
    │   ├── stone_validators.py
    │   ├── stop_cuisines.py
    │   ├── Stop.py
    │   ├── stops.py
    │   ├── stringIndexing.py
    │   ├── stringOperations.py
    │   ├── stringReverse.py
    │   ├── strings.py
    │   ├── strip_url_params.py
    │   ├── strongly_connected_components.py
    │   ├── strong_password.py
    │   ├── structs.py
    │   ├── structures.py
    │   ├── stub_value.py
    │   ├── student_class.py
    │   ├── stutter.py
    │   ├── subpackage.py
    │   ├── subprocess.py
    │   ├── subset_generation.py
    │   ├── subsets.py
    │   ├── subsets_unique.py
    │   ├── subversion.py
    │   ├── successor.py
    │   ├── sudoku_solver.py
    │   ├── sudoku_validator.py
    │   ├── sum-arr.py
    │   ├── sumAverage.py
    │   ├── summarize_ranges.py
    │   ├── summing_digits.py
    │   ├── sum_of_arithmetic_sequence.py
    │   ├── sum_of_arithmetic_series.py
    │   ├── sum_of_digits.py
    │   ├── sum_of_geometric_progression.py
    │   ├── sum-of-square-numbers.py
    │   ├── sum_of_subset.py
    │   ├── sum.py
    │   ├── sum_sub_squares.py
    │   ├── support_vector_machines.py
    │   ├── swap_case.py
    │   ├── swap_in_pairs.py
    │   ├── swap_nodes.py
    │   ├── swap_pair.py
    │   ├── swift_client.py
    │   ├── swift_helpers.py
    │   ├── swift_types.py
    │   ├── switch_pairs.py
    │   ├── symmetry_group_cycle_index.py
    │   ├── syntax_tree.py
    │   ├── sys_path.py
    │   ├── systemInfo.py
    │   ├── table_maker.py
    │   ├── tag.py
    │   ├── tags.py
    │   ├── take-a-break.py
    │   ├── target_python.py
    │   ├── tarjan.py
    │   ├── tarjans_scc.py
    │   ├── tbtools.py
    │   ├── temp_dir.py
    │   ├── temperature_conversions.py
    │   ├── templating.py
    │   ├── tensorboard_launcher.py
    │   ├── termui.py
    │   ├── text_edit_utils.py
    │   ├── text_file.py
    │   ├── text_justification.py
    │   ├── three_sum.py
    │   ├── tic_tac_toe.py
    │   ├── time_conversion.py
    │   ├── timed.py
    │   ├── timeout.py
    │   ├── timer.py
    │   ├── toimpl.py
    │   ├── tokenize.py
    │   ├── token.py
    │   ├── top_1.py
    │   ├── tower_of_hanoi.py
    │   ├── train.py
    │   ├── transformations_2d.py
    │   ├── transitive_closure_dfs.py
    │   ├── trapezoidal_rule.py
    │   ├── traversal.py
    │   ├── treap.py
    │   ├── tree_node.py
    │   ├── tree_order.py
    │   ├── tree.py
    │   ├── trie.py
    │   ├── trimmean.py
    │   ├── triplet_sum.py
    │   ├── trips.py
    │   ├── tsd_client.py
    │   ├── tsd_helpers.py
    │   ├── tsd_types.py
    │   ├── turtleRandomWeb.py
    │   ├── two_pointer.py
    │   ├── two_sum.py
    │   ├── type_map.py
    │   ├── typeshed.py
    │   ├── type_var.py
    │   ├── typing_hacks.py
    │   ├── typing.py
    │   ├── tz.py
    │   ├── tzwin.py
    │   ├── ugly_numbers.py
    │   ├── unicode.py
    │   ├── unicode_utils.py
    │   ├── unique_morse.py
    │   ├── unixccompiler.py
    │   ├── unpacking.py
    │   ├── unpack.py
    │   ├── unstone.stoneg.py
    │   ├── update-gyp.py
    │   ├── update_item.py
    │   ├── upload_docs.py
    │   ├── upload.py
    │   ├── upper.py
    │   ├── url.py
    │   ├── url_safe.py
    │   ├── urls.py
    │   ├── useful_function_mapping.py
    │   ├── user_agent.py
    │   ├── useragents.py
    │   ├── user.py
    │   ├── users.py
    │   ├── utils.py
    │   ├── validate_coordinates.py
    │   ├── validate_filenames.py
    │   ├── validate_solutions.py
    │   ├── valid_parenthesis.py
    │   ├── valid_sudoku.py
    │   ├── value.py
    │   ├── version_01.py
    │   ├── version_02.py
    │   ├── version_03.py
    │   ├── version_04.py
    │   ├── versioncontrol.py
    │   ├── versionpredicate.py
    │   ├── videodownloader.py
    │   ├── views.py
    │   ├── virtualenv.py
    │   ├── visitor.py
    │   ├── volume.py
    │   ├── vowel_count.py
    │   ├── vscodeDataFrame.py
    │   ├── vscodeGetVariableInfo.py
    │   ├── wait.py
    │   ├── walls_and_gates.py
    │   ├── website_opener.py
    │   ├── weight_conversion.py
    │   ├── wheelfile.py
    │   ├── wheel_legacy.py
    │   ├── wheel.py
    │   ├── whitespace.py
    │   ├── why_does_pythons_for_have_an_else.py
    │   ├── wiki-years.py
    │   ├── word_break.py
    │   ├── word_frequency_functions.py
    │   ├── word_ladder.py
    │   ├── word_occurrence.py
    │   ├── word_pattern.py
    │   ├── word_patterns.py
    │   ├── word_squares.py
    │   ├── world_covid19_stats.py
    │   ├── wrap_modes.py
    │   ├── wrapper.py
    │   ├── wrappers.py
    │   ├── wrap.py
    │   ├── write_hooks.py
    │   ├── writingFiles.py
    │   ├── wsgi.py
    │   ├── xmlrpc.py
    │   ├── zellers_congruence.py
    │   ├── z_function.py
    │   ├── zigzagiterator.py
    │   └── zigzag.py
    └── whole-bunch-of-py
        ├── 0001_initial.py
        ├── 00_hello.py
        ├── 01_bignum.py
        ├── 01_deques_intro.py
        ├── 01-FindUniqueIntegers.py
        ├── 01_intro_to_queues.py
        ├── 01_lists_intro.py
        ├── 01_stacks_intro.py
        ├── 02_datatypes.py
        ├── 02_find_all_links (2).py
        ├── 02_find_all_links (3).py
        ├── 02_find_all_links (4).py
        ├── 02_find_all_links (5).py
        ├── 02_find_all_links.py
        ├── 02_geocode_wikipedia_edits.py
        ├── 02-MinMovesString.py
        ├── 02_stacks_implementation.py
        ├── 02_unordered_lists.py
        ├── 03_anagram.py
        ├── 03_balanced_parenthesis.py
        ├── 03-FindPossibleValidIPs.py
        ├── 03_hot_potato.py
        ├── 03_modules.py
        ├── 03_scrapy_pagination.py
        ├── 03_simple_twitter_manager (2).py
        ├── 03_simple_twitter_manager (3).py
        ├── 03_simple_twitter_manager (4).py
        ├── 03_simple_twitter_manager (5).py
        ├── 03_simple_twitter_manager.py
        ├── 04-MinSwapsPalindrome.py
        ├── 04_printing.py
        ├── 04_rename_with_slice (2).py
        ├── 04_rename_with_slice (3).py
        ├── 04_rename_with_slice (4).py
        ├── 04_rename_with_slice (5).py
        ├── 04_rename_with_slice.py
        ├── 04_so_word_cloud.py
        ├── 04-tuples-intro.py
        ├── 05b-extra-string-operations.py
        ├── 05_lists.py
        ├── 05_load_json_without_dupes (2).py
        ├── 05_load_json_without_dupes (3).py
        ├── 05_load_json_without_dupes (4).py
        ├── 05_load_json_without_dupes (5).py
        ├── 05_load_json_without_dupes.py
        ├── 05-LongestSubstringWithout3.py
        ├── 05_mixed_sorting.py
        ├── 05_prefix_postfix_infix.py
        ├── 06_execution_time (2).py
        ├── 06_execution_time (3).py
        ├── 06_execution_time (4).py
        ├── 06_execution_time (5).py
        ├── 06_execution_time.py
        ├── 06_gets_with_twisted.py
        ├── 06-LexicographicallySmallestString.py
        ├── 06_tuples.py
        ├── 06_visualize_wikipedia_links.py
        ├── 07_benchmark_permissions_loading_django (2).py
        ├── 07_benchmark_permissions_loading_django (3).py
        ├── 07_benchmark_permissions_loading_django (4).py
        ├── 07_benchmark_permissions_loading_django (5).py
        ├── 07_benchmark_permissions_loading_django.py
        ├── 07_degrees_of_separation.py
        ├── 07_slices.py
        ├── 07-StringWithout3IdenticalConLetters.py
        ├── 08_basic_email_web_crawler (2).py
        ├── 08_basic_email_web_crawler (3).py
        ├── 08_basic_email_web_crawler (4).py
        ├── 08_basic_email_web_crawler (5).py
        ├── 08_basic_email_web_crawler.py
        ├── 08_comprehensions.py
        ├── 08_limit_depth.py
        ├── 08-LongestSemiAlternatingSubString.py
        ├── 09_basic_link_web_crawler (2).py
        ├── 09_basic_link_web_crawler (3).py
        ├── 09_basic_link_web_crawler (4).py
        ├── 09_basic_link_web_crawler (5).py
        ├── 09_basic_link_web_crawler.py
        ├── 09d53fca90c7_.py
        ├── 09_dictionaries.py
        ├── 09-MinStepsToMakePilesEqualHeight.py
        ├── 0Intro2python-all.py
        ├── 10.10-question.py
        ├── 10.11-question.py
        ├── 10.1-question.py
        ├── 10.2-question.py
        ├── 10.3-question.py
        ├── 10.4-question.py
        ├── 10.5-question.py
        ├── 10.6-question.py
        ├── 10.7-question.py
        ├── 10.8-question.py
        ├── 10.9-question.py
        ├── 10-DayOfWeekThatIsKDaysLater.py
        ├── 10_find_files_recursively (2).py
        ├── 10_find_files_recursively (3).py
        ├── 10_find_files_recursively (4).py
        ├── 10_find_files_recursively (5).py
        ├── 10_find_files_recursively.py
        ├── 10_forms_auth.py
        ├── 10_functions.py
        ├── 1.1-answer.py
        ├── 11_args.py
        ├── 11_hash_algo_d.py
        ├── 11-MaxInserts.py
        ├── 11_optimize_images_with_wand (2).py
        ├── 11_optimize_images_with_wand (3).py
        ├── 11_optimize_images_with_wand (4).py
        ├── 11_optimize_images_with_wand (5).py
        ├── 11_optimize_images_with_wand.py
        ├── 1.1-question.py
        ├── 1.2-answer.py
        ├── 12_csv_split (2).py
        ├── 12_csv_split (3).py
        ├── 12_csv_split (4).py
        ├── 12_csv_split (5).py
        ├── 12_csv_split.py
        ├── 12_parse_differently_based_on_rules.py
        ├── 1.2-question.py
        ├── 12-RottingOranges.py
        ├── 12_scopes.py
        ├── 13.1-question.py
        ├── 13.2-question.py
        ├── 13.3-question.py
        ├── 13.4-question.py
        ├── 13.5-question.py
        ├── 13.6-question.py
        ├── 13.7-question.py
        ├── 13.8-question.py
        ├── 1.3-answer.py
        ├── 13_clean_jd.py
        ├── 13_file_io.py
        ├── 13_hash_algo.py
        ├── 13-MilkBottles.py
        ├── 13_project_script copy.py
        ├── 1.3-question.py
        ├── 13_random_name_generator (2).py
        ├── 13_random_name_generator (3).py
        ├── 13_random_name_generator (4).py
        ├── 13_random_name_generator (5).py
        ├── 13_random_name_generator.py
        ├── 14.1-question.py
        ├── 14.2-question.py
        ├── 14.3-question.py
        ├── 14.4-question.py
        ├── 14.5-question.py
        ├── 14.6-question.py
        ├── 14.7-question.py
        ├── 1.4-answer.py
        ├── 14_cal.py
        ├── 14-FindLargestBSTSubtreeInGivenBT.py
        ├── 1.4-question.py
        ├── 15.1-question.py
        ├── 15.2-question.py
        ├── 15.3-question.py
        ├── 15.4-question.py
        ├── 15.5-question.py
        ├── 15.6-question.py
        ├── 15.7-question.py
        ├── 1.5-answer.py
        ├── 15_check_my_environment (2).py
        ├── 15_check_my_environment (3).py
        ├── 15_check_my_environment (4).py
        ├── 15_check_my_environment (5).py
        ├── 15_check_my_environment.py
        ├── 15_classes.py
        ├── 15_linked_list_a.py
        ├── 1.5-question.py
        ├── 15-ReturnStringsNoIdenticalNeighbors.py
        ├── 1.6-answer.py
        ├── 16_jinja_quick_load (2).py
        ├── 16_jinja_quick_load (3).py
        ├── 16_jinja_quick_load (4).py
        ├── 16_jinja_quick_load (5).py
        ├── 16_jinja_quick_load.py
        ├── 16-LargestMAlignedSubset.py
        ├── 16_linked_list_demo.py
        ├── 1.6-question.py
        ├── 16_sieve.py
        ├── 1.7-answer.py
        ├── 1.7-question.py
        ├── 1.8-answer.py
        ├── 1.8-question.py
        ├── 18_zipper (2).py
        ├── 18_zipper (3).py
        ├── 18_zipper (4).py
        ├── 18_zipper (5).py
        ├── 18_zipper.py
        ├── 1.9-answer.py
        ├── 19c0846f6344_base_revision_for_0_5.py
        ├── 1.9-question.py
        ├── 19_tsv-to-csv (2).py
        ├── 19_tsv-to-csv (3).py
        ├── 19_tsv-to-csv (4).py
        ├── 19_tsv-to-csv (5).py
        ├── 19_tsv-to-csv.py
        ├── 1cebaf56856c_session_id.py
        ├── 1_counting_digital_sums.py
        ├── 1_lowest_common_ancestor.py
        ├── 1.py
        ├── 1_rmq_segment_trees.py
        ├── 1_rolling_hashes.py
        ├── 20200625_165528_add_express_boolean_to_package.py
        ├── 20201124_124301_syyahsodja.py
        ├── 20201124_125414_updated_models.py
        ├── 20201124_125647_created_notes_table.py
        ├── 20201129_021616_updated_rest_gas_hotel_and_stop_models.py
        ├── 20201130_195313_made_some_columns_nullable_on_the_trip_.py
        ├── 20_restore_file_from_git (2).py
        ├── 20_restore_file_from_git (3).py
        ├── 20_restore_file_from_git (4).py
        ├── 20_restore_file_from_git (5).py
        ├── 20_restore_file_from_git.py
        ├── 215 Kth Largest Element in an Array.py
        ├── 2.1-answer.py
        ├── 2.1-question.py
        ├── 21_twitter_bot (2).py
        ├── 21_twitter_bot (3).py
        ├── 21_twitter_bot (4).py
        ├── 21_twitter_bot (5).py
        ├── 21_twitter_bot.py
        ├── 2.2-answer.py
        ├── 22_git_tag (2).py
        ├── 22_git_tag (3).py
        ├── 22_git_tag (4).py
        ├── 22_git_tag (5).py
        ├── 22_git_tag.py
        ├── 2.2-question.py
        ├── 22_student_1_notes.py
        ├── 2.3-answer.py
        ├── 23_flask_session_test (2).py
        ├── 23_flask_session_test (3).py
        ├── 23_flask_session_test (4).py
        ├── 23_flask_session_test (5).py
        ├── 23_flask_session_test.py
        ├── 2.3-question.py
        ├── 23_student_2_notes.py
        ├── 2.4-answer.py
        ├── 2.4-question.py
        ├── 24_sql2csv (2).py
        ├── 24_sql2csv (3).py
        ├── 24_sql2csv (4).py
        ├── 24_sql2csv (5).py
        ├── 24_sql2csv.py
        ├── 24_student_3_notes.py
        ├── 2.5-answer.py
        ├── 25_ip2geolocation (2).py
        ├── 25_ip2geolocation (3).py
        ├── 25_ip2geolocation (4).py
        ├── 25_ip2geolocation (5).py
        ├── 25_ip2geolocation.py
        ├── 2.5-question.py
        ├── 2.6-answer.py
        ├── 26_bst_demo.py
        ├── 2.6-question.py
        ├── 26_stock_scraper (2).py
        ├── 26_stock_scraper (3).py
        ├── 26_stock_scraper (4).py
        ├── 26_stock_scraper (5).py
        ├── 26_stock_scraper.py
        ├── 2.7-answer.py
        ├── 2.7-question.py
        ├── 27_send_sms (2).py
        ├── 27_send_sms (3).py
        ├── 27_send_sms (4).py
        ├── 27_send_sms (5).py
        ├── 27_send_sms.py
        ├── 27_student_6_notes.py
        ├── 2.8-answer.py
        ├── 28_income_tax_calculator (2).py
        ├── 28_income_tax_calculator (3).py
        ├── 28_income_tax_calculator (4).py
        ├── 28_income_tax_calculator (5).py
        ├── 28_income_tax_calculator.py
        ├── 2.8-question.py
        ├── 29_job_scheduler.py
        ├── 29_json_to_yaml (2).py
        ├── 29_json_to_yaml (3).py
        ├── 29_json_to_yaml (4).py
        ├── 29_json_to_yaml (5).py
        ├── 29_json_to_yaml.py
        ├── 2dArray.py
        ├── 2_hidden_layers_neural_network.py
        ├── 2_matrix_min_sum.py
        ├── 2_rmq.py
        ├── 2_segment_trees_lazy_updates.py
        ├── 2_shortest_path_and_back.py
        ├── 2_subarrays_count.py
        ├── 2 - TreeTraversal-BFT.py
        ├── 30_fullcontact (2).py
        ├── 30_fullcontact (3).py
        ├── 30_fullcontact (4).py
        ├── 30_fullcontact (5).py
        ├── 30_fullcontact.py
        ├── 3.1-answer.py
        ├── 3.1-question.py
        ├── 31_youtube_sentiment (2).py
        ├── 31_youtube_sentiment (3).py
        ├── 31_youtube_sentiment (4).py
        ├── 31_youtube_sentiment (5).py
        ├── 31_youtube_sentiment.py
        ├── 3.2-answer.py
        ├── 3.2-question.py
        ├── 32_stock_scraper (2).py
        ├── 32_stock_scraper (3).py
        ├── 32_stock_scraper (4).py
        ├── 32_stock_scraper (5).py
        ├── 32_stock_scraper.py
        ├── 3.3-answer.py
        ├── 33_country_code (2).py
        ├── 33_country_code (3).py
        ├── 33_country_code (4).py
        ├── 33_country_code (5).py
        ├── 33_country_code.py
        ├── 3.3-question.py
        ├── 3.4-answer.py
        ├── 34_git_all_repos (2).py
        ├── 34_git_all_repos (3).py
        ├── 34_git_all_repos (4).py
        ├── 34_git_all_repos (5).py
        ├── 34_git_all_repos.py
        ├── 3.4-question.py
        ├── 3.5-answer.py
        ├── 35f29b1701bd_.py
        ├── 3.5-question.py
        ├── 3.6-answer.py
        ├── 3.6-question.py
        ├── 398697dc9550_.py
        ├── 3_another_matrix_sum.py
        ├── 3_binary_indexed_trees.py
        ├── 3_binary_search_length.py
        ├── 3ec6993fe20c_encrypted_auth_state.py
        ├── 3_matrix_expo.py
        ├── 3_more_complex_matrix_sum.py
        ├── 3n_plus_1.py
        ├── 3.py
        ├── 3-tier.py
        ├── 3 - TreeTraversal-DFS.py
        ├── 4.10-answer.py
        ├── 4.10-questions.py
        ├── 4.11-answer.py
        ├── 4.11-questions.py
        ├── 4.12-answer.py
        ├── 4.12-questions.py
        ├── 4.1-answer.py
        ├── 4.1-questions.py
        ├── 4.2-answer.py
        ├── 4.2-questions.py
        ├── 4.3-answer.py
        ├── 4.3-questions.py
        ├── 4.4-answer.py
        ├── 4.4-questions.py
        ├── 4.5-answer.py
        ├── 4.5-questions.py
        ├── 4.6-answer.py
        ├── 4.6-questions.py
        ├── 4.7-answer.py
        ├── 4.7-questions.py
        ├── 4.8-answer.py
        ├── 4.8-questions.py
        ├── 4.9-answer.py
        ├── 4.9-questions.py
        ├── 4be95337cf96_.py
        ├── 4_bits_for_rmq.py
        ├── 4dc2d5a8c53c_user_options.py
        ├── 4 - DesignLinedList.py
        ├── 4_manacher.py
        ├── 4_strings_2_notes.py
        ├── 50aad68f99c2_.py
        ├── 5.1-question (2).py
        ├── 5.1-question.py
        ├── 5.2-question (2).py
        ├── 5.2-question.py
        ├── 5.3-question (2).py
        ├── 5.3-question.py
        ├── 5.4-question (2).py
        ├── 5.4-question.py
        ├── 5.5-question (2).py
        ├── 5.5-question.py
        ├── 56cc5a70207e_token_tracking.py
        ├── 5.6-question (2).py
        ├── 5.6-question.py
        ├── 5.7-question (2).py
        ├── 5.7-question.py
        ├── 5.8-question (2).py
        ├── 5.8-question.py
        ├── 5_counting_increasing_subsequences.py
        ├── 5_eratosthenes.py
        ├── 5.py
        ├── 5_treaps.py
        ├── 5_tsp_held_karp.py
        ├── 6.10-question (2).py
        ├── 6.10-question.py
        ├── 6.1-question (2).py
        ├── 6.1-question.py
        ├── 6.2-question (2).py
        ├── 6.2-question.py
        ├── 6.3-question (2).py
        ├── 6.3-question.py
        ├── 6.4-question (2).py
        ├── 6.4-question.py
        ├── 6.5-question (2).py
        ├── 6.5-question.py
        ├── 6.6-question (2).py
        ├── 6.6-question.py
        ├── 6.7-question (2).py
        ├── 6.7-question.py
        ├── 6.8-question (2).py
        ├── 6.8-question.py
        ├── 6.9-question (2).py
        ├── 6.9-question.py
        ├── 6_demos.py
        ├── 6_nums_notes.py
        ├── 7.10-question (2).py
        ├── 7.10-question.py
        ├── 7.11-question (2).py
        ├── 7.11-question.py
        ├── 7.12-question (2).py
        ├── 7.12-question.py
        ├── 7.13-question (2).py
        ├── 7.13-question.py
        ├── 7.14-question (2).py
        ├── 7.14-question.py
        ├── 7.15-question (2).py
        ├── 7.15-question.py
        ├── 7.16-question (2).py
        ├── 7.16-question.py
        ├── 7.17-question (2).py
        ├── 7.17-question.py
        ├── 7.1-question (2).py
        ├── 7.1-question.py
        ├── 7.2-question (2).py
        ├── 7.2-question.py
        ├── 7.3-question (2).py
        ├── 7.3-question.py
        ├── 7.4-question (2).py
        ├── 7.4-question.py
        ├── 7.5-question (2).py
        ├── 7.5-question.py
        ├── 7.6-question (2).py
        ├── 7.6-question.py
        ├── 7.7-question (2).py
        ├── 7.7-question.py
        ├── 7.8-question (2).py
        ├── 7.8-question.py
        ├── 7.9-question (2).py
        ├── 7.9-question.py
        ├── 7c56ea250085_.py
        ├── 7fedf8531802_.py
        ├── 7.py
        ├── 8.10-question.py
        ├── 8.11-question.py
        ├── 8.12-question.py
        ├── 8.13-question.py
        ├── 8.14-question.py
        ├── 81c7ffeffeee_.py
        ├── 8.1-question.py
        ├── 8.2-question.py
        ├── 8.3-question.py
        ├── 8.4-question.py
        ├── 8.5-question.py
        ├── 8.6-question.py
        ├── 8.7-question.py
        ├── 8.8-question.py
        ├── 896818069c98_token_expires.py
        ├── 8.9-question.py
        ├── 9.1-question.py
        ├── 9.2-question.py
        ├── 9.3-question.py
        ├── 9.4-question.py
        ├── 9.5-question.py
        ├── 9.6-question.py
        ├── 9.7-question.py
        ├── 9.8-question.py
        ├── 99a28a4418e1_user_created.py
        ├── a091c9611d20_.py
        ├── a1z26.py
        ├── a5129010c510_create_tables.py
        ├── a77a0932a568_.py
        ├── AAAA.py
        ├── abandon.py
        ├── abbreviation.py
        ├── abbr.py
        ├── abc.py
        ├── abitype.py
        ├── abnf_regexp.py
        ├── abs_max.py
        ├── abs_min.py
        ├── absoft.py
        ├── abs.py
        ├── abstract (2).py
        ├── abstract_build_docker.py
        ├── abstract_command.py
        ├── abstract_factory.py
        ├── abstract.py
        ├── abstract_strategy.py
        ├── accelerator_partitioner.py
        ├── accept.py
        ├── accessor.py
        ├── accessors.py
        ├── access.py
        ├── account_rights.py
        ├── acquire.py
        ├── actionscript.py
        ├── activate_this.py
        ├── activation (2).py
        ├── activation (3).py
        ├── activation_ops_test.py
        ├── activation.py
        ├── activator.py
        ├── activators.py
        ├── activex.py
        ├── activity_selection.py
        ├── ad2012R2.py
        ├── adadelta (2).py
        ├── adadelta.py
        ├── adadelta_test.py
        ├── adagrad.py
        ├── adagrad_test_helper.py
        ├── adagrad_test.py
        ├── adam (2).py
        ├── adamax (2).py
        ├── adamax.py
        ├── adam_op_test.py
        ├── adam.py
        ├── adam_test.py
        ├── adamw (2).py
        ├── adamw.py
        ├── adapter.py
        ├── adapters.py
        ├── adaptive.py
        ├── adaptive_weight.py
        ├── adb.py
        ├── add_and_search.py
        ├── add_bias.py
        ├── add_binary.py
        ├── add_bitwise_operator.py
        ├── add_code_to_python_process.py
        ├── addition.py
        ├── addMembersToGroups.py
        ├── add_newdocs (2).py
        ├── add_newdocs.py
        ├── add_operators.py
        ├── add_ops_list_test.py
        ├── add.py
        ├── add_test (2).py
        ├── add_test.py
        ├── add_two_numbers.py
        ├── adler32.py
        ├── admonition.py
        ├── admonitions.py
        ├── ado_consts.py
        ├── adodbapi (2).py
        ├── adodbapi.py
        ├── adodbapitest (2).py
        ├── adodbapitestconfig.py
        ├── adodbapitest.py
        ├── adsicon.py
        ├── adv (10).py
        ├── adv (11).py
        ├── adv (2).py
        ├── adv (3).py
        ├── adv (4).py
        ├── adv (5).py
        ├── adv (6).py
        ├── adv (7).py
        ├── adv (8).py
        ├── adv (9).py
        ├── advanced.py
        ├── advapi32 (2).py
        ├── advapi32 (3).py
        ├── advapi32.py
        ├── adv.py
        ├── aead.py
        ├── af4cbdb2d13c_services.py
        ├── affine_channel_op_test.py
        ├── affine_cipher.py
        ├── afm.py
        ├── af.py
        ├── AFSDB.py
        ├── afxres.py
        ├── agent.py
        ├── aggregation.py
        ├── aho_corasick.py
        ├── aifc.py
        ├── ai_player.py
        ├── aircraft_thrust.py
        ├── ajax.py
        ├── aj.py
        ├── alexa_news_headlines.py
        ├── Algebraic_integration_from_Ludovic_Sacchelli_s_PhD_thesis.py
        ├── algebra.py
        ├── algolia_helper (2).py
        ├── algolia_helper.py
        ├── algolia_internal_api.py
        ├── algolia_settings.py
        ├── algol_nu.py
        ├── algol.py
        ├── Algorithme de Cocke-Kasami-Younger (python3).py
        ├── Algorithme_genetique_pour_generer_des_eclairages_modelisation_agreg.py
        ├── algorithms.py
        ├── aliases.py
        ├── alias.py
        ├── alias_with_name_test.py
        ├── alien_invasion.py
        ├── alien.py
        ├── aligned_indent.py
        ├── align.py
        ├── aliquot_sum.py
        ├── all_combinations.py
        ├── allcompare_test.py
        ├── all_factors.py
        ├── allocation_number.py
        ├── all_pairs_shortest_path.py
        ├── all_paths_between_two_vertices.py
        ├── all_permutations.py
        ├── allSizedIntegers.py
        ├── all_subsequences.py
        ├── all_zeros (2).py
        ├── all_zeros (3).py
        ├── all_zeros (4).py
        ├── all_zeros.py
        ├── alternate_disjoint_set.py
        ├── ambient.py
        ├── ampl.py
        ├── AMTRELAY.py
        ├── anaconda_prune_defintions.py
        ├── Anagram (2).py
        ├── Anagram (3).py
        ├── Anagram (4).py
        ├── anagram (5).py
        ├── anagram-detection (2).py
        ├── anagram-detection (3).py
        ├── anagram-detection (4).py
        ├── anagram-detection.py
        ├── anagram_mappings.py
        ├── Anagram.py
        ├── anagrams (2).py
        ├── anagrams (3).py
        ├── anagrams.py
        ├── analysis.py
        ├── analyze_dxp.py
        ├── analyzer (2).py
        ├── analyzer (3).py
        ├── analyzer (4).py
        ├── analyzer (5).py
        ├── analyzer.py
        ├── ancestor2 (2).py
        ├── ancestor2.py
        ├── ancestor (2).py
        ├── ancestor (3).py
        ├── ancestor (4).py
        ├── ancestor.py
        ├── anchored_artists.py
        ├── anchor.py
        ├── anchors_test.py
        ├── android (10).py
        ├── android (11).py
        ├── android (12).py
        ├── android (13).py
        ├── android (14).py
        ├── android (2).py
        ├── android (3).py
        ├── android (4).py
        ├── android (5).py
        ├── android (6).py
        ├── android (7).py
        ├── android (8).py
        ├── android (9).py
        ├── android_definitions.py
        ├── android.py
        ├── angle_helper.py
        ├── angry_bird (2).py
        ├── angry_bird.py
        ├── An_Identiconizer_generator_implementation_in_python.py
        ├── animation.py
        ├── ann_module.py
        ├── annotated_fn_args.py
        ├── annotated_types.py
        ├── annotation.py
        ├── annotations.py
        ├── announcement.py
        ├── anomaly_mode.py
        ├── ansi_code_processor.py
        ├── ansi_escape_sequences.py
        ├── ansi.py
        ├── ansitowin32.py
        ├── AnyExpOnTerm.py
        ├── AnyExp.py
        ├── apache.py
        ├── api (10).py
        ├── api (2).py
        ├── api (3).py
        ├── api (4).py
        ├── api (5).py
        ├── api (6).py
        ├── api (7).py
        ├── api (8).py
        ├── api (9).py
        ├── apibase.py
        ├── apidoc.py
        ├── api.py
        ├── api_report_generated_pb2.py
        ├── api_test (2).py
        ├── api_test (3).py
        ├── api_test (4).py
        ├── api_test (5).py
        ├── api_test.py
        ├── APL.py
        ├── apmeter_test.py
        ├── app (10).py
        ├── app (11).py
        ├── app (12).py
        ├── app (13).py
        ├── app (2).py
        ├── app (3).py
        ├── app (4).py
        ├── app (5).py
        ├── app (6).py
        ├── app (7).py
        ├── app (8).py
        ├── app (9).py
        ├── app_controller.py
        ├── app_data.py
        ├── appdirs.py
        ├── appengine.py
        ├── application (2).py
        ├── application.py
        ├── apply.py
        ├── app.py
        ├── approximate_matching.py
        ├── apps.py
        ├── A.py
        ├── arabic_stemmer.py
        ├── arc_cosine_feature_map.py
        ├── archetype.py
        ├── archive_util.py
        ├── arduino.py
        ├── are_anagrams.py
        ├── area.py
        ├── area_under_curve.py
        ├── area_volume_calculator.py
        ├── arepl_custom_handlers.py
        ├── arepl_dump.py
        ├── arepl_examples.py
        ├── arepl_overloads.py
        ├── arepl_pickler.py
        ├── arepl_python_evaluator.py
        ├── arepl_saved.py
        ├── arepl_user_error.py
        ├── are_you_connected_to_world.py
        ├── arffread.py
        ├── argon2id.py
        ├── argon2i.py
        ├── argon2.py
        ├── arg_ops_test.py
        ├── argparse (2).py
        ├── argparse (3).py
        ├── argparse.py
        ├── args (2).py
        ├── arg_scope.py
        ├── args_example_1.py
        ├── args_example.py
        ├── args.py
        ├── arguments.py
        ├── arithmatex.py
        ├── arithmetic_mean.py
        ├── arithmetic_progression_builder.py
        ├── arithmetic.py
        ├── armstrong_number.py
        ├── armstrongnumber.py
        ├── armstrong_numbers.py
        ├── arpack.py
        ├── array (2).py
        ├── array_constructors.py
        ├── array_helpers.py
        ├── array_ops.py
        ├── arraypad.py
        ├── array-pair-sum (2).py
        ├── array-pair-sum (3).py
        ├── array-pair-sum (4).py
        ├── array-pair-sum.py
        ├── array_parity.py
        ├── arrayprint (2).py
        ├── arrayprint.py
        ├── Array.py
        ├── arrays (2).py
        ├── arraysetops.py
        ├── arrays.py
        ├── array_sum_combinations.py
        ├── arrayterator.py
        ├── arrow.py
        ├── art3d.py
        ├── articulation_points.py
        ├── artist.py
        ├── asciidoc.py
        ├── ascii.py
        ├── asgd (2).py
        ├── asgd.py
        ├── asgi.py
        ├── A_short_study_of_Renyi_entropy.py
        ├── asm.py
        ├── asn1.py
        ├── A solution to project euler problem 3.py
        ├── assembler (2).py
        ├── assembler.py
        ├── assertions.py
        ├── asserts.py
        ├── assertsql.py
        ├── assert_test.py
        ├── associationproxy (2).py
        ├── associationproxy (3).py
        ├── associationproxy.py
        ├── as_strided_test.py
        ├── as_string.py
        ├── a_star.py
        ├── astar.py
        ├── ast.py
        ├── ast_transforms.py
        ├── ast_walker.py
        ├── asyncbackend.py
        ├── async_case.py
        ├── async_context_manager.py
        ├── async_downloader.py
        ├── asyncfilters.py
        ├── async_generator.py
        ├── asynchat.py
        ├── async_helpers.py
        ├── asynchronous.py
        ├── asyncio.py
        ├── asyncio_test.py
        ├── async_net_barrier_test.py
        ├── asyncore.py
        ├── async.py
        ├── asyncquery.py
        ├── asyncresolver.py
        ├── asyncStream.py
        ├── asyncsupport.py
        ├── atbash_cipher.py
        ├── atbash.py
        ├── aten_test.py
        ├── A tiny regex challenge solved without another regex.py
        ├── atomic_ops_test.py
        ├── attach_pid_injected.py
        ├── attach_pydevd.py
        ├── attach_script.py
        ├── attention (2).py
        ├── attention.py
        ├── attrDef.py
        ├── attribute.py
        ├── attributes (2).py
        ├── attributes (3).py
        ├── attributes.py
        ├── attr_list.py
        ├── attr.py
        ├── attrsettr.py
        ├── audiobook_gen.py
        ├── audio.py
        ├── audiotests.py
        ├── audio_text_models.py
        ├── audit-tests.py
        ├── auth (2).py
        ├── auth (3).py
        ├── auth (4).py
        ├── auth (5).py
        ├── auth (6).py
        ├── auth_handler.py
        ├── auth.py
        ├── auth_routes (2).py
        ├── auth_routes.py
        ├── auth_test.py
        ├── autocall.py
        ├── autocast_mode.py
        ├── autocast_test_lists.py
        ├── autocomplete.py
        ├── autocomplete_using_trie.py
        ├── autocomplete_w.py
        ├── autocompletion.py
        ├── autodist.py
        ├── autoexpand.py
        ├── autograd.py
        ├── autohandler.py
        ├── AutoIndent.py
        ├── automap (2).py
        ├── automap (3).py
        ├── automap.py
        ├── automation.py
        ├── autopep8 (2).py
        ├── autopep8.py
        ├── auto.py
        ├── autoreload.py
        ├── autoreload_test.py
        ├── auto_suggest.py
        ├── autumn.py
        ├── auxfuncs.py
        ├── average_mean.py
        ├── average_median.py
        ├── average_mode.py
        ├── average-of-levels-in-binary-tree.py
        ├── avg (2).py
        ├── avg (3).py
        ├── avg (4).py
        ├── avg (5).py
        ├── avg (6).py
        ├── avg (7).py
        ├── avg.py
        ├── avg_xdspam_confidence.py
        ├── avl.py
        ├── avl_tree1.py
        ├── avl_tree (2).py
        ├── avl_tree (3).py
        ├── avl_tree (4).py
        ├── avl_tree.py
        ├── axes3d.py
        ├── axes_divider.py
        ├── axes_grid.py
        ├── axes_rgb.py
        ├── axes_size.py
        ├── axis3d.py
        ├── axis_artist.py
        ├── axislines.py
        ├── axisline_style.py
        ├── axis.py
        ├── axsite.py
        ├── B04170_02_python_Draft_01.py
        ├── B04170_03_python_Draft_01.py
        ├── B04170_04_python_Draft_01.py
        ├── B04170_05_python_Draft_01.py
        ├── B04170_06_python_Draft_01.py
        ├── B04170_08_python_Draft_01.py
        ├── b64.py
        ├── babelplugin.py
        ├── backcall.py
        ├── backend (2).py
        ├── backend (3).py
        ├── backend (4).py
        ├── backend_agg.py
        ├── backend_bases.py
        ├── backend_cairo.py
        ├── backend_cpp_rep.py
        ├── backend_ctypes (2).py
        ├── backend_ctypes (3).py
        ├── backend_ctypes.py
        ├── backend_gtk3agg.py
        ├── backend_gtk3cairo.py
        ├── backend_gtk3.py
        ├── backend_inline.py
        ├── backend_macosx.py
        ├── backend_managers.py
        ├── backend_mixed.py
        ├── backend_nbagg.py
        ├── backend_pdf.py
        ├── backend_pgf.py
        ├── backend_ps.py
        ├── backend.py
        ├── backend_qt5agg.py
        ├── backend_qt5cairo.py
        ├── backend_qt5.py
        ├── backend_registry.py
        ├── backend_rep.py
        ├── backend_svg.py
        ├── backend_template.py
        ├── backend_tools.py
        ├── backend_webagg_core.py
        ├── backend_webagg.py
        ├── backend_wxagg.py
        ├── backend_wxcairo.py
        ├── backend_wx.py
        ├── backgroundjobs.py
        ├── Background.py
        ├── backports.py
        ├── back_propagation_neural_network.py
        ├── backup_automater_services.py
        ├── backup.py
        ├── BackupRead_BackupWrite.py
        ├── BackupSeek_streamheaders.py
        ├── backupToZip.py
        ├── bad_builtin.py
        ├── bailey_borwein_plouffe.py
        ├── baked.py
        ├── balanced-brackets (2).py
        ├── balanced-brackets (3).py
        ├── balanced-brackets (4).py
        ├── balanced-brackets.py
        ├── balanced_brackets.py
        ├── balanced_parentheses.py
        ├── balance_parenthesis.py
        ├── balance_parenthisis (2).py
        ├── balance_parenthisis (3).py
        ├── balance_parenthisis (4).py
        ├── balance_parenthisis.py
        ├── ball.py
        ├── bare.py
        ├── bar.py
        ├── base (10).py
        ├── base (11).py
        ├── base (12).py
        ├── base (13).py
        ├── base (14).py
        ├── base (15).py
        ├── base16.py
        ├── base (16).py
        ├── base (17).py
        ├── base (18).py
        ├── base (19).py
        ├── base (20).py
        ├── base (21).py
        ├── base (22).py
        ├── base (23).py
        ├── base (24).py
        ├── base (25).py
        ├── base (26).py
        ├── base (2).py
        ├── base32.py
        ├── base (3).py
        ├── base (4).py
        ├── base (5).py
        ├── base64_codec.py
        ├── base64_encoding.py
        ├── base64mime.py
        ├── base64.py
        ├── base (6).py
        ├── base (7).py
        ├── base85.py
        ├── base (8).py
        ├── base (9).py
        ├── base_checker.py
        ├── base_command.py
        ├── base_conversion.py
        ├── basedevice.py
        ├── base_embed.py
        ├── base_events (2).py
        ├── base_events (3).py
        ├── base_events.py
        ├── base_frontend_mixin.py
        ├── base_futures.py
        ├── base_graph.py
        ├── base_job.py
        ├── base.py
        ├── base_reporter.py
        ├── base_request.py
        ├── base_response.py
        ├── bases.py
        ├── basestemmer.py
        ├── base_subprocess.py
        ├── base_tasks.py
        ├── base_value.py
        ├── basic (2).py
        ├── basic_binary_tree.py
        ├── BasicCryptanalysis.py
        ├── basic_graphs.py
        ├── basic_hashtables (2).py
        ├── basic_hashtables.py
        ├── BasicIngestpythonWithCloudFormation.py
        ├── basic_maths.py
        ├── basic.py
        ├── basic_rnn_test.py
        ├── basics.py
        ├── basic_string.py
        ├── basic_structures.py
        ├── basic_test.py
        ├── basictimerapp.py
        ├── basque_stemmer.py
        ├── batch_box_cox_test (2).py
        ├── batch_box_cox_test.py
        ├── batch_bucketize_op_test.py
        ├── batch-executemany-writes.py
        ├── batch_file_rename.py
        ├── batch_gather_test.py
        ├── batch_get_item_with_limit.py
        ├── batch_huber_loss.py
        ├── batch_lr_loss.py
        ├── batch_matmul_dnnlowp_op_test.py
        ├── batch_moments_op_test.py
        ├── batch_mse_loss.py
        ├── batchnorm (2).py
        ├── batchnorm (3).py
        ├── batch_normalization.py
        ├── batchnorm.py
        ├── batchnorm_test.py
        ├── batch_permutation_dnnlowp_op_test.py
        ├── batch.py
        ├── batch_sigmoid_cross_entropy_loss.py
        ├── batch_softmax_loss.py
        ├── batch_sparse_to_dense_op_test.py
        ├── battleplayer.py
        ├── battleserver.py
        ├── battleship.py
        ├── bazaar.py
        ├── bazel_definitions.py
        ├── bbcode.py
        ├── bbox_transform_test.py
        ├── bccache.py
        ├── bcppcompiler.py
        ├── bcrypt.py
        ├── bdb.py
        ├── BdfFontFile.py
        ├── bdf.py
        ├── bdist_dumb.py
        ├── bdist_egg.py
        ├── bdist_msi.py
        ├── bdist.py
        ├── bdist_rpm.py
        ├── bdist_wheel.py
        ├── bdist_wininst.py
        ├── bead_sort.py
        ├── beaker_cache.py
        ├── beam_search.py
        ├── beaufort_cipher.py
        ├── beejMachine.py
        ├── bellman_ford (2).py
        ├── bellman_ford.py
        ├── bench (2).py
        ├── bench_gen.py
        ├── benchmark (2).py
        ├── benchmark_all_other_test.py
        ├── benchmark_all_quantized_test.py
        ├── benchmark_all_test.py
        ├── Benchmark_between_python_and_Julia.py
        ├── benchmark_caffe2.py
        ├── benchmark_core.py
        ├── benchmark_dataset.py
        ├── benchmark_ddp_rpc.py
        ├── benchmark_generator.py
        ├── Benchmark_of_the_SHA256_hash_function__python_Cython_Numba.py
        ├── benchmark.py
        ├── Benchmarkpython.py
        ├── benchmark_pytorch.py
        ├── benchmark_runner.py
        ├── benchmark_test_generator.py
        ├── benchmark_utils.py
        ├── bench_ops.py
        ├── bench.py
        ├── bernoulli.py
        ├── ber.py
        ├── beta.py
        ├── betterem.py
        ├── bezier_curve.py
        ├── bezier.py
        ├── bfs.py
        ├── bfs_shortest_path.py
        ├── bfs_zero_one_shortest_path.py
        ├── bibtex_beautifier.py
        ├── bibtex.py
        ├── bidirectional_a_star.py
        ├── bidirectional_breadth_first_search.py
        ├── big5freq.py
        ├── big5prober.py
        ├── bignum (2).py
        ├── bignum.py
        ├── bilateral_filter.py
        ├── binary_and_operator.py
        ├── binary_build_data.py
        ├── binary_build_definitions.py
        ├── Binary Coefficients.py
        ├── binary_count_setbits.py
        ├── binary_count_trailing_zeros.py
        ├── binary_exp_mod.py
        ├── binary_exponentiation_2.py
        ├── binary_exponentiation (2).py
        ├── binary_exponentiation_3.py
        ├── binary_exponentiation (3).py
        ├── binary_exponentiation.py
        ├── binary_gap.py
        ├── binary_heap.py
        ├── binary_or_operator.py
        ├── binary.py
        ├── binary_search (10).py
        ├── binary_search (11).py
        ├── binary_search (2).py
        ├── binary_search (3).py
        ├── binary_search (4).py
        ├── binary_search (5).py
        ├── binary_search (6).py
        ├── Binary_search (7).py
        ├── binary_search (8).py
        ├── binary_search (9).py
        ├── binary_search.py
        ├── binarySearch.py
        ├── binary_search_tree (10).py
        ├── binary_search_tree (11).py
        ├── binary_search_tree (2).py
        ├── binary_search_tree (3).py
        ├── binary_search_tree (4).py
        ├── binary_search_tree (5).py
        ├── binary_search_tree (6).py
        ├── binary_search_tree (7).py
        ├── binary_search_tree (8).py
        ├── binary_search_tree (9).py
        ├── binary_search_tree.py
        ├── binary_search_tree_recursive (2).py
        ├── binary_search_tree_recursive (3).py
        ├── binary_search_tree_recursive.py
        ├── binary_shifts.py
        ├── binarysize.py
        ├── binary_smoketest.py
        ├── binary_test.py
        ├── Binary_to_Decimal (2).py
        ├── binary_to_decimal_conversion.py
        ├── binary_to_decimal_converter.py
        ├── binary_to_decimal.py
        ├── binary_to_octal.py
        ├── binary_tree_mirror.py
        ├── binary_tree_ops.py
        ├── binary_tree.py
        ├── binarytree.py
        ├── binary_tree_traversal.py
        ├── binary_tree_traversals (2).py
        ├── binary_tree_traversals (3).py
        ├── binary_tree_traversals.py
        ├── binary_twos_complement.py
        ├── binary_xor_operator.py
        ├── binding.py
        ├── bindings.py
        ├── bind.py
        ├── binhex.py
        ├── binning.py
        ├── binod.py
        ├── binomial_coefficient.py
        ├── binomial_distribution.py
        ├── binomial_heap.py
        ├── binomial.py
        ├── bintree_tree.py
        ├── bipartite_graph.py
        ├── Bird.py
        ├── bisect_cmd.py
        ├── bisection (2).py
        ├── bisection (3).py
        ├── bisection.py
        ├── bisect_percentile_op_test.py
        ├── bisect.py
        ├── bitbucket_repolist.py
        ├── bit_manipulation.py
        ├── bitmap.py
        ├── bitmask.py
        ├── bitonic_sort (2).py
        ├── bitonic_sort.py
        ├── bit_operation.py
        ├── bitwise_ops.py
        ├── BlackandWhite.py
        ├── blackboard.py
        ├── blackjack (2).py
        ├── blackjack (3).py
        ├── blackJackGUI.py
        ├── blackjack.py
        ├── blackjack_rr.py
        ├── blas_compare.py
        ├── blas_compare_setup.py
        ├── blas.py
        ├── blob_deallocation_test.py
        ├── blobs_queue_db_test (2).py
        ├── blobs_queue_db_test.py
        ├── blob_weighted_sum.py
        ├── blockchain (2).py
        ├── blockchain (3).py
        ├── blockchain.py
        ├── blocking_input.py
        ├── blocking.py
        ├── blockparser.py
        ├── blockpartition.py
        ├── blockprocessors.py
        ├── blocks.py
        ├── blog.py
        ├── blogscraping.py
        ├── blog_tensorflow_sequence_labelling.py
        ├── bloom_filter.py
        ├── bloom_test.py
        ├── BlpImagePlugin.py
        ├── blueprints.py
        ├── Blurring_a_part_of_an_image_in_python.py
        ├── bmi_body_mass_index_calculator.py
        ├── BmpImagePlugin.py
        ├── bm_queue.py
        ├── bn_relu.py
        ├── boa.py
        ├── body.py
        ├── bogo_sort (2).py
        ├── bogo_sort.py
        ├── bomb_enemy.py
        ├── boohoo.py
        ├── bookstore_manangement_system.py
        ├── boolean_mask_test.py
        ├── boolean.py
        ├── boolean_unmask_test.py
        ├── bootstrap_config.py
        ├── bootstrap.py
        ├── borg.py
        ├── borland.py
        ├── boundary_traversal.py
        ├── boxplot.py
        ├── box_with_nms_limit_op_test.py
        ├── Boyer_Moore.py
        ├── boyer_moore_search.py
        ├── bpr_loss.py
        ├── bracket_matcher.py
        ├── brain_attrs.py
        ├── brain_builtin_inference.py
        ├── brain_collections.py
        ├── brain_curses.py
        ├── brain_dataclasses.py
        ├── brain_fstrings.py
        ├── brain_functools.py
        ├── brain_gi.py
        ├── brain_hashlib.py
        ├── brain_http.py
        ├── brain_io.py
        ├── brain_multiprocessing.py
        ├── brain_namedtuple_enum.py
        ├── brain_nose.py
        ├── brain_numpy_core_function_base.py
        ├── brain_numpy_core_multiarray.py
        ├── brain_numpy_core_numeric.py
        ├── brain_numpy_core_numerictypes.py
        ├── brain_numpy_core_umath.py
        ├── brain_numpy_ndarray.py
        ├── brain_numpy_random_mtrand.py
        ├── brain_numpy_utils.py
        ├── brain_pkg_resources.py
        ├── brain_pytest.py
        ├── brain_qt.py
        ├── brain_random.py
        ├── brain_re.py
        ├── brain_responses.py
        ├── brain_scipy_signal.py
        ├── brain_six.py
        ├── brain_ssl.py
        ├── brain_subprocess.py
        ├── brain_typing.py
        ├── branch_filters.py
        ├── branch_table.py
        ├── BrandProject.py
        ├── breadth_first_search_2.py
        ├── breadth_first_search.py
        ├── breadth_first_search_shortest_path.py
        ├── breaking_bad.py
        ├── breakpoint (2).py
        ├── breakpoint (3).py
        ├── breakpoint.py
        ├── brew.py
        ├── brew_test.py
        ├── brickout-game.py
        ├── bridge.py
        ├── broadcasting.py
        ├── broadcast.py
        ├── broad_try_clause.py
        ├── brotli.py
        ├── browse_for_folder.py
        ├── browseProjects.py
        ├── browser_handler.py
        ├── browser.py
        ├── brute_force_caesar_cipher.py
        ├── Brute_force.py
        ├── BruteForce.py
        ├── bsplines.py
        ├── bsr.py
        ├── bst (2).py
        ├── bst (3).py
        ├── bst_heaps (2).py
        ├── bst_heaps.py
        ├── bst.py
        ├── btm_matcher.py
        ├── btm_utils.py
        ├── b_tree.py
        ├── bubble_sort (2).py
        ├── bubble_sort (3).py
        ├── bubble_sort (4).py
        ├── bubble_sort (5).py
        ├── Bubble_sort (6).py
        ├── bubblesortpgm.py
        ├── bubble_sort.py
        ├── bubbleSort.py
        ├── buble_sort (2).py
        ├── buble_sort (3).py
        ├── buble_sort (4).py
        ├── buble_sort.py
        ├── bucketize_op_test.py
        ├── bucket_sort (2).py
        ├── bucket_sort.py
        ├── bucket_weighted.py
        ├── buddymove_holidayiq.py
        ├── buffer (2).py
        ├── buffered_pipe.py
        ├── buffer.py
        ├── BufrStubImagePlugin.py
        ├── build_activation_images.py
        ├── build_all_exercises_page.py
        ├── build_amd.py
        ├── build_binaries_osx.py
        ├── build_binaries_windows.py
        ├── build_bundled.py
        ├── build_clib.py
        ├── build_directory_md.py
        ├── build_docker_scraper.py
        ├── build_docs.py
        ├── build_env.py
        ├── builder.py
        ├── build_ext.py
        ├── build_index.py
        ├── build_libtorch.py
        ├── build_listing_legacy.py
        ├── build_listing.py
        ├── build_meta.py
        ├── build.py
        ├── build_py.py
        ├── build_pytorch_libs.py
        ├── build_scripts.py
        ├── build_src.py
        ├── builtin.py
        ├── builtin_trap.py
        ├── bulkstamp.py
        ├── bullet.py
        ├── bundled_inputs.py
        ├── bundlerextensions.py
        ├── burkes.py
        ├── burrows_wheeler.py
        ├── business.py
        ├── button.py
        ├── buy_sell_stock.py
        ├── bvls.py
        ├── bw_filter.py
        ├── bw.py
        ├── byext.py
        ├── bytecode_helper.py
        ├── bytecode.py
        ├── bytedesign.py
        ├── byteordercodes.py
        ├── bytes_int_conversion.py
        ├── byteswapping.py
        ├── byteyears.py
        ├── bz2_codec.py
        ├── bz2.py
        ├── c2_cpu_gpu_forward_backward_test.py
        ├── C2Module.py
        ├── c2_ref_test.py
        ├── CAA.py
        ├── cached_py_info.py
        ├── cached_reader.py
        ├── cache.py
        ├── caesar_cipher (2).py
        ├── caesar_cipher.py
        ├── caesars_cipher_encryption (2).py
        ├── caesars_cipher_encryption (3).py
        ├── caesars_cipher_encryption.py
        ├── caffe2_resnet50_default_forward.py
        ├── caffe2_resnet50_default_param_update.py
        ├── caffe_translator.py
        ├── caffe_translator_test.py
        ├── cal (2).py
        ├── calc_area.py
        ├── calc_interets.py
        ├── calculate-day-in-your-head.py
        ├── calculator (10).py
        ├── calculator (2).py
        ├── calculator (3).py
        ├── calculator (4).py
        ├── calculator (5).py
        ├── calculator (6).py
        ├── calculator (7).py
        ├── calculator (8).py
        ├── calculator (9).py
        ├── calculator.py
        ├── Calcul_d_une_paire_de_des_un_peu_particuliers.py
        ├── calendar.py
        ├── calendar_this.py
        ├── calibration.py
        ├── callable.py
        ├── caller.py
        ├── calltip.py
        ├── CallTips.py
        ├── call_tip_widget.py
        ├── calltip_w.py
        ├── cal.py
        ├── candidate.py
        ├── candidates.py
        ├── canny.py
        ├── canonical_constraint.py
        ├── can_string_be_rearranged_as_palindrome.py
        ├── capabilities.py
        ├── capi_maps.py
        ├── capitalize.py
        ├── capnproto.py
        ├── capsLock.py
        ├── capture.py
        ├── ca.py
        ├── caresresolver.py
        ├── caret.py
        ├── car.py
        ├── car_routes.py
        ├── cars.py
        ├── cartesian_plane_quadrant.py
        ├── case (2).py
        ├── case (3).py
        ├── case.py
        ├── casting.py
        ├── cast_op_test.py
        ├── c_ast.py
        ├── cast.py
        ├── catalan_stemmer.py
        ├── catalog.py
        ├── categorical (2).py
        ├── categorical.py
        ├── categorize.py
        ├── category.py
        ├── cat_files.py
        ├── cat_test.py
        ├── cauchy.py
        ├── cb_rules.py
        ├── ccompiler (2).py
        ├── ccompiler (3).py
        ├── ccompiler (4).py
        ├── ccompiler (5).py
        ├── ccompiler (6).py
        ├── ccompiler_opt.py
        ├── ccompiler.py
        ├── c_cpp.py
        ├── ceil_op_test.py
        ├── ceil.py
        ├── celeb (2).py
        ├── celeb (3).py
        ├── celeb (4).py
        ├── celeb.py
        ├── cells.py
        ├── census2010.py
        ├── cerapi.py
        ├── CERT.py
        ├── certs.py
        ├── cffi_opcode.py
        ├── cfg.py
        ├── cfuncs (2).py
        ├── cfuncs (3).py
        ├── cfuncs.py
        ├── c_generator.py
        ├── cgi.py
        ├── cgitb.py
        ├── chainmap_example.py
        ├── chain_of_responsibility.py
        ├── chain.py
        ├── changeable.py
        ├── change_brightness.py
        ├── change_contrast.py
        ├── changemac.py
        ├── channel_backprop_stats_op_test.py
        ├── channel.py
        ├── channel_shuffle_dnnlowp_op_test.py
        ├── channel_shuffle_op_test.py
        ├── channelshuffle.py
        ├── channel_shuffle_test (2).py
        ├── channel_shuffle_test.py
        ├── channels.py
        ├── channel_stats_op_test.py
        ├── chaos_machine.py
        ├── chaos.py
        ├── chapel.py
        ├── chardetect.py
        ├── chardistribution.py
        ├── charmap.py
        ├── char_prime_map.py
        ├── char.py
        ├── char_rnn.py
        ├── charsetgroupprober.py
        ├── charsetprober.py
        ├── charset.py
        ├── chatactoe.py
        ├── chebyshev (2).py
        ├── chebyshev (3).py
        ├── chebyshev.py
        ├── check_all_test_suite.py
        ├── check_anagrams.py
        ├── check_backward_compatibility.py
        ├── check_bipartite_graph_bfs.py
        ├── check_bipartite_graph_dfs.py
        ├── check_bipartite.py
        ├── check_config_win.py
        ├── check_digraph_strongly_connected.py
        ├── check_elif.py
        ├── checker_board.py
        ├── checker.py
        ├── checkers.py
        ├── check_file.py
        ├── check_for_sqlite_files.py
        ├── check-github-commit-dates.py
        ├── checkGroupsMemberships.py
        ├── check_IBAN.py
        ├── check_if_graph_is_tree.py
        ├── check_input.py
        ├── check_interleaving_of_strings.py
        ├── check_internet_con.py
        ├── check_kernel_launches.py
        ├── check_mypy_version.py
        ├── check_NIRPP.py
        ├── check_pangram (2).py
        ├── check_pangram.py
        ├── check_parallel.py
        ├── check_permutations.py
        ├── checkpoint (2).py
        ├── checkpoint (3).py
        ├── checkpoint (4).py
        ├── checkpoint.py
        ├── checkpoints.py
        ├── checkpoint_test (2).py
        ├── checkpoint_test.py
        ├── check-property-on-prime-numbers.py
        ├── check.py
        ├── checkpyc.py
        ├── check_site_selenium.py
        ├── checktcpconn.py
        ├── check_voyages-sncf.py
        ├── cheese.py
        ├── cherrypy_print_file_interface.py
        ├── chess_game.py
        ├── chess_set.py
        ├── chi2.py
        ├── chicks_n_rabs.py
        ├── chinese_remainder_theorem.py
        ├── ChipViewer.py
        ├── cholesky_matrix_decomposition.py
        ├── Chrome Dino Automater.py
        ├── chudnovsky_algorithm.py
        ├── chunk.py
        ├── chunk_test.py
        ├── ciDict.py
        ├── ciphers.py
        ├── circular_linked_list.py
        ├── circularly_linked_list.py
        ├── circular_queue.py
        ├── cisco.py
        ├── citation.py
        ├── cityreader (2).py
        ├── cityreader (3).py
        ├── cityreader (4).py
        ├── cityreader (5).py
        ├── cityreader.py
        ├── clang_coverage.py
        ├── clang_format_all.py
        ├── clang_format_utils.py
        ├── clang_tidy.py
        ├── class_animal_attributes_examples.py
        ├── classes.py
        ├── class_example_movies.py
        ├── classicIndianCardMatch.py
        ├── Classification_human_or_horse.py
        ├── class_movies.py
        ├── class_validators.py
        ├── class_weight.py
        ├── cleanfuture.py
        ├── clean.py
        ├── clearmetadata.py
        ├── c_lexer.py
        ├── client (2).py
        ├── client (3).py
        ├── client (4).py
        ├── client (5).py
        ├── client (6).py
        ├── client (7).py
        ├── client (8).py
        ├── clientabc.py
        ├── client_mining_blockchain.py
        ├── client_mining_miner.py
        ├── client.py
        ├── clients.py
        ├── c_like.py
        ├── climbing_stairs (2).py
        ├── climbing_stairs (3).py
        ├── climbing_stairs (4).py
        ├── climbing_stairs.py
        ├── climbingStairs.py
        ├── clipboard.py
        ├── clipboards.py
        ├── clip_grad.py
        ├── clip_op_test.py
        ├── clip_path.py
        ├── clip_ranges_test (2).py
        ├── clip_ranges_test.py
        ├── clip_tensor_op_test.py
        ├── cli.py
        ├── CliYoutubeDownloader (2).py
        ├── CliYoutubeDownloader.py
        ├── clock.py
        ├── clone_graph.py
        ├── closest_pair_of_points.py
        ├── clothing.py
        ├── cloud_cover_statistics_with_darksky_data.py
        ├── cloudpickle_fast.py
        ├── cloudpickle.py
        ├── cloudpickle_wrapper.py
        ├── CloudWatchMetricHandlerpython3.py
        ├── CLSIDToClass.py
        ├── clsregistry.py
        ├── cmac.py
        ├── cmake (10).py
        ├── cmake (11).py
        ├── cmake (12).py
        ├── cmake (13).py
        ├── cmake (14).py
        ├── cmake (15).py
        ├── cmake (2).py
        ├── cmake (3).py
        ├── cmake (4).py
        ├── cmake (5).py
        ├── cmake (6).py
        ├── cmake (7).py
        ├── cmake (8).py
        ├── cmake (9).py
        ├── cmake.py
        ├── cmdline.py
        ├── cmdoptions.py
        ├── cmd.py
        ├── cmdserver.py
        ├── cm.py
        ├── CNAME.py
        ├── cnn.py
        ├── co2_emission.py
        ├── coalescestreams.py
        ├── cobyla.py
        ├── cocktail_shaker_sort (2).py
        ├── cocktail_shaker_sort.py
        ├── codata.py
        ├── code_action.py
        ├── code_analyzer.py
        ├── codecontainer.py
        ├── codecontext.py
        ├── codec.py
        ├── codecs.py
        ├── codegen (2).py
        ├── codegen (3).py
        ├── codegen (4).py
        ├── codegen.py
        ├── codehilite.py
        ├── code_lens.py
        ├── codenav.py
        ├── codeop.py
        ├── code.py
        ├── code_template.py
        ├── codeutil.py
        ├── coding_dos.py
        ├── codingstatemachine.py
        ├── coin_change.py
        ├── collate.py
        ├── collatz_sequence (2).py
        ├── collatz_sequence (3).py
        ├── collatz_sequence.py
        ├── collect_and_distribute_fpn_rpn_proposals_op_test.py
        ├── collect_env.py
        ├── collectinput.py
        ├── collection.py
        ├── collections (2).py
        ├── collections (3).py
        ├── collections (4).py
        ├── collections.py
        ├── collector.py
        ├── coloransi.py
        ├── colorbar.py
        ├── colorbox.py
        ├── colorchooser.py
        ├── ColorDB.py
        ├── color_depth.py
        ├── coloreditor.py
        ├── colorful.py
        ├── coloring.py
        ├── colorizer.py
        ├── colorlog.py
        ├── colormixer.py
        ├── color-picker.py
        ├── color_presentation.py
        ├── color.py
        ├── colors.py
        ├── colorsys.py
        ├── column_provider.py
        ├── combination.py
        ├── combinations.py
        ├── combination_sum (2).py
        ├── combination_sum.py
        ├── combinatorics.py
        ├── combinerefs.py
        ├── combining.py
        ├── combrowse.py
        ├── comb_sort (2).py
        ├── comb_sort.py
        ├── comm (2).py
        ├── commandpath.py
        ├── command.py
        ├── commands.py
        ├── commctrl (2).py
        ├── commctrl.py
        ├── comment_eater.py
        ├── comment.py
        ├── comments.py
        ├── commitlist.py
        ├── commit_size_vs_commit_message_size.py
        ├── common (10).py
        ├── common (11).py
        ├── common (12).py
        ├── common (13).py
        ├── common (14).py
        ├── common (2).py
        ├── common (3).py
        ├── common (4).py
        ├── common (5).py
        ├── common (6).py
        ├── common (7).py
        ├── common (8).py
        ├── common (9).py
        ├── common_cuda.py
        ├── common_descriptors.py
        ├── common_device_type.py
        ├── commondialog.py
        ├── common_distributed.py
        ├── common_jit.py
        ├── common_methods_invocations.py
        ├── common_nn.py
        ├── common.py
        ├── common_quantization.py
        ├── common_quantized.py
        ├── common_rules.py
        ├── common_test (2).py
        ├── common_test (3).py
        ├── common_test (4).py
        ├── common_test.py
        ├── common_tests.py
        ├── common_types.py
        ├── commontypes.py
        ├── common_utils.py
        ├── comm.py
        ├── comms.py
        ├── comp (2).py
        ├── comp (3).py
        ├── comp (4).py
        ├── comp (5).py
        ├── comp (6).py
        ├── comp (7).py
        ├── compact.py
        ├── compaq.py
        ├── comparator.py
        ├── compare (2).py
        ├── compare (3).py
        ├── compare (4).py
        ├── compare (5).py
        ├── compare (6).py
        ├── compare_array_elements.py
        ├── compare-fastrnn-results.py
        ├── compare.py
        ├── comparetozero.py
        ├── compare_with_baseline.py
        ├── compat (2).py
        ├── compat (3).py
        ├── compat (4).py
        ├── compat (5).py
        ├── compat (6).py
        ├── compatibility_tags.py
        ├── compat.py
        ├── compat_win32.py
        ├── compileall.py
        ├── compile_commands_json (2).py
        ├── compile_commands_json (3).py
        ├── compile_commands_json (4).py
        ├── compile_commands_json.py
        ├── compiled.py
        ├── compiler (2).py
        ├── compiler (3).py
        ├── compiler (4).py
        ├── compiler (5).py
        ├── compiler (6).py
        ├── compiler (7).py
        ├── compiler (8).py
        ├── compiler (9).py
        ├── compiler_helper.py
        ├── compilerop.py
        ├── compiler.py
        ├── compiler_unparse.py
        ├── completer (2).py
        ├── completerlib.py
        ├── completer.py
        ├── completion_html.py
        ├── completion_plain.py
        ├── completion.py
        ├── completion_widget.py
        ├── components.py
        ├── composer.py
        ├── composite.py
        ├── comp.py
        ├── compressed.py
        ├── compressor.py
        ├── compress.py
        ├── compute_equalization_scale_test.py
        ├── compute_histogram_for_blobs.py
        ├── compute_histogram_for_blobs_test.py
        ├── compute_loss.py
        ├── compute_norm_for_blobs.py
        ├── compute_norm_for_blobs_test.py
        ├── compute_statistics_for_blobs.py
        ├── compute_statistics_for_blobs_test.py
        ├── compute_topk_accuracy.py
        ├── concat (2).py
        ├── concat_benchmark.py
        ├── concat_dnnlowp_op_test.py
        ├── concatkdf.py
        ├── concat.py
        ├── concat_split_op_test (2).py
        ├── concat_split_op_test.py
        ├── concat_test.py
        ├── concrete.py
        ├── concurrency-primes.py
        ├── concurrent.py
        ├── concurrent_test.py
        ├── conda_packaging_tool.py
        ├── conditional_put (2).py
        ├── conditional_put (3).py
        ├── conditional_put.py
        ├── conditionals_examples.py
        ├── conditional_test.py
        ├── conemu.py
        ├── conf (2).py
        ├── conf (3).py
        ├── config (2).py
        ├── config (3).py
        ├── config_compiler.py
        ├── config_creator.py
        ├── configdialog (2).py
        ├── configdialog (3).py
        ├── configdialog.py
        ├── config_init.py
        ├── config_key.py
        ├── config_loader.py
        ├── config_manager.py
        ├── configparser (2).py
        ├── configparser (3).py
        ├── configparser.py
        ├── config.py
        ├── configs.py
        ├── configui.py
        ├── configurable.py
        ├── configuration.py
        ├── configure (2).py
        ├── configure.py
        ├── config_validator.py
        ├── con_file.py
        ├── confirmable.py
        ├── conf.py
        ├── conftest (2).py
        ├── conftest (3).py
        ├── conftest (4).py
        ├── conftest.py
        ├── conf_tree.py
        ├── confusion_matrix.py
        ├── conjugate_gradient.py
        ├── connected_components.py
        ├── connection (2).py
        ├── connectionpool.py
        ├── connection.py
        ├── connect.py
        ├── consoleapp.py
        ├── console.py
        ├── console_widget (2).py
        ├── console_widget.py
        ├── constant_names.py
        ├── constants (10).py
        ├── constants (11).py
        ├── constants (2).py
        ├── constants (3).py
        ├── constants (4).py
        ├── constants (5).py
        ├── constants (6).py
        ├── constants (7).py
        ├── constants (8).py
        ├── constants (9).py
        ├── constants.py
        ├── constant_weight.py
        ├── const_fold.py
        ├── constraint.py
        ├── constraint_registry.py
        ├── constraints.py
        ├── construction.py
        ├── constructor.py
        ├── constructors.py
        ├── construct.py
        ├── construct_tree_postorder_preorder.py
        ├── contactbook.py
        ├── contacts_bisect.py
        ├── ContainerIO.py
        ├── container.py
        ├── containers (2).py
        ├── containers.py
        ├── contain_string.py
        ├── contentmanager.py
        ├── context (2).py
        ├── context (3).py
        ├── context_amd64.py
        ├── context_i386.py
        ├── contextlib2.py
        ├── contextlib.py
        ├── context_managers.py
        ├── context_menu.py
        ├── context.py
        ├── contexts.py
        ├── context_test.py
        ├── contingency.py
        ├── continuous_bernoulli.py
        ├── contour.py
        ├── controller.py
        ├── controlLoops.py
        ├── control_ops_grad.py
        ├── control_ops_grad_test.py
        ├── control_ops.py
        ├── control_ops_util.py
        ├── control.py
        ├── ControlService.py
        ├── controls.py
        ├── control_test.py
        ├── conv (2).py
        ├── conv (3).py
        ├── conv (4).py
        ├── conv (5).py
        ├── conv (6).py
        ├── conv (7).py
        ├── conv_depthwise_dnnlowp_op_test.py
        ├── conv_dnnlowp_acc16_op_test.py
        ├── conv_dnnlowp_op_test.py
        ├── Conversation.py
        ├── conversion.py
        ├── conversion_test.py
        ├── convert-array (2).py
        ├── convert-array (3).py
        ├── convert-array (4).py
        ├── convert-array.py
        ├── converter.py
        ├── converters.py
        ├── convertfigures.py
        ├── convert_parameters.py
        ├── convertPolyLine.py
        ├── convert.py
        ├── convert_test.py
        ├── convert_time.py
        ├── convert_to_negative.py
        ├── convex_hull.py
        ├── conv_fused.py
        ├── convfusion_op_test.py
        ├── conv_groupwise_dnnlowp_acc16_op_test.py
        ├── conv_groupwise_dnnlowp_op_test.py
        ├── convnet_benchmarks (2).py
        ├── convnet_benchmarks.py
        ├── convnet_benchmarks_test.py
        ├── convolution_neural_network.py
        ├── convolve.py
        ├── conv_op_test.py
        ├── conv.py
        ├── conv_relu (2).py
        ├── conv_relu.py
        ├── conv_template.py
        ├── conv_test (2).py
        ├── conv_test.py
        ├── conv_transpose_test (2).py
        ├── conv_transpose_test.py
        ├── conways_game_of_life.py
        ├── cookiejar (2).py
        ├── cookiejar (3).py
        ├── cookiejar.py
        ├── cookies.py
        ├── coo.py
        ├── coordinator.py
        ├── copy (2).py
        ├── copy (3).py
        ├── copy_hook.py
        ├── copy_ops_test.py
        ├── copy_op_test.py
        ├── copy.py
        ├── copy_random_pointer.py
        ├── copyreg.py
        ├── copy_rows_to_tensor_op_test.py
        ├── copy_transform.py
        ├── core (10).py
        ├── core (2).py
        ├── core (3).py
        ├── core (4).py
        ├── core (5).py
        ├── core (6).py
        ├── core (7).py
        ├── core (8).py
        ├── core (9).py
        ├── core_gradients_test.py
        ├── core.py
        ├── core_test.py
        ├── coronacases.py
        ├── coroutines.py
        ├── cors.py
        ├── cosine_embedding_criterion_op_test.py
        ├── cosine_similarity.py
        ├── count_algorithm_execution_time.py
        ├── count_binary_substring.py
        ├── count_connected_number_of_component.py
        ├── counter_ops_test.py
        ├── counter.py
        ├── count_flips_to_convert.py
        ├── counting_sort (2).py
        ├── counting_sort.py
        ├── Counting-sort.py
        ├── Counting_Valleys (2).py
        ├── Counting_Valleys (3).py
        ├── Counting_Valleys (4).py
        ├── Counting_Valleys.py
        ├── count_islands (2).py
        ├── count_islands_in_matrix.py
        ├── count_islands.py
        ├── count_left_node.py
        ├── CountMillionCharacter.py
        ├── CountMillionCharacters-2.0.py
        ├── count_number_of_one_bits.py
        ├── count_ones.py
        ├── count_paths.py
        ├── count.py
        ├── Count sort.py
        ├── count_th (2).py
        ├── count_th (3).py
        ├── count_th (4).py
        ├── count_th (5).py
        ├── Count the characters  (2).py
        ├── Count the characters  (3).py
        ├── Count the characters  (4).py
        ├── Count the characters .py
        ├── count_th.py
        ├── count_trees.py
        ├── count_vowels.py
        ├── coverage.py
        ├── coverage_record.py
        ├── covid_stats_via_xpath.py
        ├── cov_json.py
        ├── cp037.py
        ├── cp1006.py
        ├── cp1026.py
        ├── cp1125.py
        ├── cp1140.py
        ├── cp1250.py
        ├── cp1251.py
        ├── cp1252.py
        ├── cp1253.py
        ├── cp1254.py
        ├── cp1255.py
        ├── cp1256.py
        ├── cp1257.py
        ├── cp1258.py
        ├── cp273.py
        ├── cp424.py
        ├── cp437.py
        ├── cp500.py
        ├── cp720.py
        ├── cp737.py
        ├── cp775.py
        ├── cp850.py
        ├── cp852.py
        ├── cp855.py
        ├── cp856.py
        ├── cp857.py
        ├── cp858.py
        ├── cp860.py
        ├── cp861.py
        ├── cp862.py
        ├── cp863.py
        ├── cp864.py
        ├── cp865.py
        ├── cp866.py
        ├── cp869.py
        ├── cp874.py
        ├── cp875.py
        ├── cp949prober.py
        ├── c_parser (2).py
        ├── cparser (2).py
        ├── c_parser (3).py
        ├── cparser (3).py
        ├── c_parser.py
        ├── cparser.py
        ├── cpp (2).py
        ├── cpp_extension.py
        ├── cpp_extension_test.py
        ├── cpp_jit.py
        ├── cpp.py
        ├── cProfile.py
        ├── cpu (2).py
        ├── cpu (3).py
        ├── cpuinfo.py
        ├── cpu.py
        ├── cpython2.py
        ├── cpython3.py
        ├── crack_caesar.py
        ├── crackfortran (2).py
        ├── crackfortran (3).py
        ├── crackfortran.py
        ├── crash (2).py
        ├── crash (3).py
        ├── crashhandler.py
        ├── crash.py
        ├── crawl_google_results.py
        ├── crawl_google_scholar_citation.py
        ├── crc.py
        ├── create_and_tag_table.py
        ├── create_db.py
        ├── create_dir_if_not_there.py
        ├── create_dir.py
        ├── CreateFileTransacted_MiniVersion.py
        ├── create_json.py
        ├── create_link.py
        ├── create_numpy_pickle.py
        ├── create_table (2).py
        ├── create_table (3).py
        ├── create_table.py
        ├── createwin.py
        ├── creation.py
        ├── creator.py
        ├── creators.py
        ├── Credit_Card_Validator.py
        ├── crf_predict.py
        ├── crf.py
        ├── crf_test.py
        ├── crf_viterbi_test.py
        ├── cricket_live_score.py
        ├── cricket_news.py
        ├── Cricket_score.py
        ├── critic.py
        ├── cross_entropy_ops_test.py
        ├── crout_matrix_decomposition.py
        ├── crud.py
        ├── crypto_aead.py
        ├── crypto_box.py
        ├── crypto_core.py
        ├── crypto_generichash.py
        ├── crypto_hash.py
        ├── crypto_kx.py
        ├── cryptomath_module.py
        ├── crypto_pwhash.py
        ├── crypto.py
        ├── crypto_scalarmult.py
        ├── crypto_secretbox.py
        ├── crypto_secretstream.py
        ├── crypto_shorthash.py
        ├── crypto_sign.py
        ├── crypt.py
        ├── crystal.py
        ├── csc.py
        ├── csound.py
        ├── cs.py
        ├── csrf.py
        ├── csr.py
        ├── csshtmlheader.py
        ├── css.py
        ├── csv2sqlite3.py
        ├── csv-parsing.py
        ├── csv.py
        ├── csvs.py
        ├── CSYNC.py
        ├── ctc_beam_search_decoder_op_test.py
        ├── ctc_greedy_decoder_op_test.py
        ├── ctc_ops_test.py
        ├── ctes.py
        ├── ctokens.py
        ├── ctx.py
        ├── ctypeslib.py
        ├── cuda_memcheck_common.py
        ├── cuda_profile_ops_test.py
        ├── cuda.py
        ├── cuda_to_hip_mappings.py
        ├── cudnn_recurrent_test.py
        ├── cuisine_routes.py
        ├── CurImagePlugin.py
        ├── curl_httpclient.py
        ├── curl_httpclient_test.py
        ├── currency_converter.py
        ├── current.py
        ├── current_stock_price.py
        ├── current_weather.py
        ├── curses_tests.py
        ├── cursor.py
        ├── custom_attributes_test.py
        ├── custom_build.py
        ├── custom_doctests.py
        ├── custom_downloader_middleware.py
        ├── custom_dupefilter.py
        ├── custom_lstms.py
        ├── customprint.py
        ├── cwrap_common.py
        ├── cx_oracle (2).py
        ├── cx_oracle (3).py
        ├── cx_oracle.py
        ├── cyaml.py
        ├── cycle_detection.py
        ├── cycle_in_directed_graph_iterative.py
        ├── cycle_in_directed_graph_using_colors.py
        ├── cycle_in_undirected_graph_iterative.py
        ├── cycle_in_undirected_graph_union_find.py
        ├── cycle.py
        ├── cycler.py
        ├── cycle_sort (2).py
        ├── cycle_sort (3).py
        ├── cycle_sort.py
        ├── cyclic_rotation (2).py
        ├── cyclic_rotation (3).py
        ├── cyclic_rotation (4).py
        ├── cyclic_rotation.py
        ├── cycling_iterator.py
        ├── cycling_iterator_test.py
        ├── cygwinccompiler.py
        ├── cymysql.py
        ├── cython_json.py
        ├── d39482714a2e_.py
        ├── d68c98b66cd4_client_description.py
        ├── daemon_output.py
        ├── daemon_python.py
        ├── dag_longest_path.py
        ├── dag_shortest_path.py
        ├── daily_checks.py
        ├── daily_horoscope (2).py
        ├── daily_horoscope (3).py
        ├── daily_horoscope.py
        ├── dalvik.py
        ├── danish_stemmer.py
        ├── daodump.py
        ├── da.py
        ├── database (2).py
        ├── database (3).py
        ├── database (4).py
        ├── database (5).py
        ├── database (6).py
        ├── database.py
        ├── Data_Challenge__Mangaki__September_2017.py
        ├── dataclasses (2).py
        ├── dataclasses (3).py
        ├── dataclasses (4).py
        ├── dataclasses (5).py
        ├── dataclasses.py
        ├── data_couple_op_test.py
        ├── data_directory.py
        ├── dataio.py
        ├── dataio_test.py
        ├── dataloader.py
        ├── data_parallel_model.py
        ├── data_parallel_model_test.py
        ├── data_parallel.py
        ├── datapub.py
        ├── data.py
        ├── dataset (2).py
        ├── dataset_ops_test.py
        ├── dataset.py
        ├── datastore.py
        ├── datastructures (2).py
        ├── datastructures-avl_tree1.py
        ├── datastructures-avl_tree2.py
        ├── datastructures-binary_search_tree.py
        ├── datastructures-hashtable.py
        ├── datastructures-heap_generic.py
        ├── datastructures-heap_max.py
        ├── datastructures-linkedlist_doubly.py
        ├── datastructures-linkedlist_reverse.py
        ├── datastructures-linkedlist_singly.py
        ├── datastructures-lru_cache.py
        ├── datastructures.py
        ├── datastructures-queue.py
        ├── datastructures-skip_list.py
        ├── datastructures-stack2.py
        ├── datastructures-stack.py
        ├── datastructures-tries.py
        ├── data_transformations.py
        ├── datatypefilter.py
        ├── datatypes (2).py
        ├── datatypes.py
        ├── data_workers.py
        ├── data_workers_test.py
        ├── date_converters.py
        ├── dates (2).py
        ├── dates.py
        ├── datetime (2).py
        ├── datetime (3).py
        ├── datetime (4).py
        ├── datetime (5).py
        ├── datetime (6).py
        ├── datetime (7).py
        ├── date-timeclient.py
        ├── datetimelike.py
        ├── datetime_parse.py
        ├── datetime.py
        ├── date-timeserver.py
        ├── datetimes.py
        ├── datetimetester.py
        ├── datrie.py
        ├── davis–putnam–logemann–loveland.py
        ├── davisputnamlogemannloveland.py
        ├── day10_binary (2).py
        ├── day10_binary.py
        ├── day11_2darrays (2).py
        ├── day11_2darrays.py
        ├── day12_inheritance (2).py
        ├── day12_inheritance.py
        ├── day13_abstractclasses (2).py
        ├── day13_abstractclasses.py
        ├── day1_datatypes (2).py
        ├── day1_datatypes.py
        ├── day1.py
        ├── day2_operators (2).py
        ├── day2_operators.py
        ├── day2.py
        ├── day2_work.py
        ├── day3_conditionalstatements (2).py
        ├── day3_conditionalstatements.py
        ├── day3.py
        ├── day3_work.py
        ├── day4 (2).py
        ├── day4 (3).py
        ├── day4_classvsinstance (2).py
        ├── day4_classvsinstance.py
        ├── day4.py
        ├── day5_loops (2).py
        ├── day5_loops.py
        ├── day6_review (2).py
        ├── day6_review.py
        ├── day7_arrays (2).py
        ├── day7_arrays.py
        ├── day8_dictionariesmaps (2).py
        ├── day8_dictionariesmaps.py
        ├── day9_recursion (2).py
        ├── day9_recursion.py
        ├── Day_of_week.py
        ├── days_from_date.py
        ├── days_you_lived.py
        ├── db2pickle.py
        ├── db (2).py
        ├── db (3).py
        ├── dbapi20.py
        ├── dbapi2.py
        ├── dbapi_proxy.py
        ├── dbapi.py
        ├── db_file_reader.py
        ├── dbfunctions.py
        ├── dbgcommands.py
        ├── dbghelp (2).py
        ├── dbghelp (3).py
        ├── dbghelp.py
        ├── dbgpyapp.py
        ├── db_input.py
        ├── db_print.py
        ├── db.py
        ├── dbscan.py
        ├── db_test.py
        ├── dbutil.py
        ├── dcgan.py
        ├── DcxImagePlugin.py
        ├── ddl (2).py
        ├── ddl (3).py
        ├── ddl.py
        ├── ddp_under_dist_autograd_test.py
        ├── DdsImagePlugin.py
        ├── death_valley_highs_lows.py
        ├── debug (2).py
        ├── debug (3).py
        ├── debug_embed_params.py
        ├── debuggee.py
        ├── debugger_instance.py
        ├── debugger.py
        ├── debugger_r.py
        ├── debughelpers.py
        ├── debugobj.py
        ├── debugobj_r.py
        ├── debug.py
        ├── decimal_isolate.py
        ├── decimal_to_any.py
        ├── Decimal_To_Binary (2).py
        ├── decimal_to_binary_converter.py
        ├── decimal_to_binary_ip.py
        ├── decimal_to_binary.py
        ├── decimal_to_binary_recursion.py
        ├── decimal_to_hexadecimal.py
        ├── decimal_to_octal.py
        ├── decision_tree.py
        ├── declaration.py
        ├── decode_asn1.py
        ├── decoder (2).py
        ├── decoder.py
        ├── decode_string.py
        ├── decomp_cholesky.py
        ├── decomp_lu.py
        ├── decomp.py
        ├── decomp_qr.py
        ├── decomp_schur.py
        ├── decomp_svd.py
        ├── decorator (2).py
        ├── decorator (3).py
        ├── decorator.py
        ├── decorators.py
        ├── decor.py
        ├── decrypt_caesar_with_chi_squared.py
        ├── decrypting_caesars_cipher.py
        ├── dec_to_hex.py
        ├── deepl.py
        ├── deepreload.py
        ├── default (2).py
        ├── default (3).py
        ├── default_comparator.py
        ├── default_hooks.py
        ├── default.py
        ├── defaults.py
        ├── default_strategy.py
        ├── default_value_test.py
        ├── defchararray (2).py
        ├── defchararray (3).py
        ├── defchararray.py
        ├── deferred.py
        ├── defines.py
        ├── definition.py
        ├── def_list.py
        ├── defmatrix.py
        ├── deform_conv_test.py
        ├── delegation_pattern.py
        ├── delete_item (2).py
        ├── delete_item (3).py
        ├── delete_item (4).py
        ├── delete_item.py
        ├── delete_node.py
        ├── delete_nth.py
        ├── delete.py
        ├── delete_reoccurring.py
        ├── deletion.py
        ├── delqueen.py
        ├── demographic-pandas-solution.py
        ├── demo_lgmres.py
        ├── Demonstration_of_running_a_Jupyter_notebook_with_sudo_rights.py
        ├── demo.py
        ├── deMorgans_law.py
        ├── demoserver.py
        ├── demoutils.py
        ├── dense_vector_to_id_list_op_test.py
        ├── dependency (2).py
        ├── dependency (3).py
        ├── dependency (4).py
        ├── dependency_injection.py
        ├── dependency.py
        ├── depends.py
        ├── deploy_config.py
        ├── deploy_docker_scraper_images.py
        ├── deprecated_interfaces.py
        ├── deprecated_modules.py
        ├── deprecation.py
        ├── deprecations.py
        ├── depth_first_search_2.py
        ├── depth_first_search.py
        ├── depth_sum.py
        ├── depthwise_3x3_conv_test.py
        ├── dep_util.py
        ├── de.py
        ├── dequantize_dnnlowp_op_test.py
        ├── deque_adt.py
        ├── deque_doubly.py
        ├── deque.py
        ├── describe.py
        ├── describe_rds_instance.py
        ├── descriptions.py
        ├── descriptor_props.py
        ├── des_crypt.py
        ├── Des_dates_qui_font_des_nombres_premiers.py
        ├── Des_dates_qui_font_des_nombres_premiers__suite.py
        ├── design_analysis.py
        ├── design_yelp.py
        ├── desktopmanager.py
        ├── des.py
        ├── details.py
        ├── DetailsViewer.py
        ├── detecting_english_programmatically.py
        ├── detectron_keypoints.py
        ├── deterministic_miller_rabin.py
        ├── dettach_kernel_driver.py
        ├── deutsch_jozsa.py
        ├── develop.py
        ├── device_checker.py
        ├── device_reduce_sum_bench.py
        ├── devices.py
        ├── devicetree.py
        ├── dfa.py
        ├── dfs (2).py
        ├── dfs (3).py
        ├── dfs_iterative.py
        ├── dfs.py
        ├── DHCID.py
        ├── dh.py
        ├── diadefslib.py
        ├── diagnose_protobuf.py
        ├── diagnose.py
        ├── diagnostics.py
        ├── diagrams_graphviz.py
        ├── diagrams.py
        ├── diagrams_pygraphviz.py
        ├── diag_test.py
        ├── dialog.py
        ├── dialogs.py
        ├── diamond.py
        ├── dia.py
        ├── dibdemo.py
        ├── dice.py
        ├── dice_rolling_simulator (2).py
        ├── dice_rolling_simulator.py
        ├── diceV2_dynamic.py
        ├── dict1.py
        ├── dict_factory.py
        ├── dictionary_example.py
        ├── dictionary.py
        ├── diction.py
        ├── dicts (2).py
        ├── dicts.py
        ├── difference_testing.py
        ├── Differentiate_List.py
        ├── diffie_hellman_key_exchange.py
        ├── diffie_hellman.py
        ├── diffie.py
        ├── difflib (2).py
        ├── difflib (3).py
        ├── difflib.py
        ├── diff.py
        ├── digestMd5.py
        ├── digest.py
        ├── digests.py
        ├── digital_clock.py
        ├── digraphs.py
        ├── dijkstra_2.py
        ├── dijkstra (2).py
        ├── dijkstra_algorithm.py
        ├── dijkstra_bankers_algorithm.py
        ├── dijkstra_greedy.py
        ├── dijkstra.py
        ├── dijkstras.py
        ├── dijkstras_two_stack_algorithm.py
        ├── DijsktraGreedy.py
        ├── dimension.py
        ├── dimensions.py
        ├── dinic.py
        ├── diophantine_equation.py
        ├── dir2.py
        ├── directed_and_undirected_(weighted)_graph.py
        ├── directory_compare.py
        ├── direct_url_helpers.py
        ├── direct_url.py
        ├── dirichlet.py
        ├── dirSync.py
        ├── dir_test.py
        ├── dirtools.py
        ├── dir_util.py
        ├── disable_internet.py
        ├── disasm.py
        ├── discountPercent.py
        ├── discount.py
        ├── discover.py
        ├── discovery_output_plugin.py
        ├── discovery.py
        ├── discriminant_analysis.py
        ├── disjoint_set.py
        ├── disk.py
        ├── dispatcher.py
        ├── dispatch.py
        ├── display (2).py
        ├── displayhook.py
        ├── displaypub.py
        ├── display.py
        ├── display_.py
        ├── display_trap.py
        ├── dis.py
        ├── dist (2).py
        ├── dist (3).py
        ├── dist (4).py
        ├── dist (5).py
        ├── dist (6).py
        ├── dist (7).py
        ├── distance (2).py
        ├── distance_on_number_line.py
        ├── distance_op_test.py
        ├── distance.py
        ├── dist_autograd_test (2).py
        ├── dist_autograd_test.py
        ├── Distinct Digit Array (2).py
        ├── Distinct Digit Array (3).py
        ├── Distinct Digit Array (4).py
        ├── Distinct Digit Array.py
        ├── dist_optimizer_test.py
        ├── dist.py
        ├── distributed (2).py
        ├── distributed (3).py
        ├── distributed_c10d.py
        ├── distributed.py
        ├── distributed_test (2).py
        ├── distributed_test.py
        ├── distribution.py
        ├── distro (2).py
        ├── distro (3).py
        ├── distro (4).py
        ├── distro (5).py
        ├── distro (6).py
        ├── distro.py
        ├── distutils_args.py
        ├── dist_utils.py
        ├── divisors.py
        ├── divMod.py
        ├── django (2).py
        ├── django (3).py
        ├── django (4).py
        ├── django (5).py
        ├── django_debug.py
        ├── django.py
        ├── djb2.py
        ├── dlgappcore.py
        ├── dlgappdemo.py
        ├── dlgtest.py
        ├── dll (2).py
        ├── dll.py
        ├── dll_queue.py
        ├── dll_stack.py
        ├── dlpack.py
        ├── DM_Images_debut.py
        ├── dml.py
        ├── dnd.py
        ├── dnnlowp_test_utils.py
        ├── dn.py
        ├── dnskeybase.py
        ├── dnssec.py
        ├── docbuilder.py
        ├── doccer.py
        ├── doc_controls.py
        ├── doc_controls_test.py
        ├── doc_generator_visitor.py
        ├── doc_generator_visitor_test.py
        ├── docintrospecter.py
        ├── docker_constants.py
        ├── docker_definitions.py
        ├── docker_hub.py
        ├── DockingBar.py
        ├── docparams.py
        ├── docparser.py
        ├── docscrape.py
        ├── docscrape_sphinx.py
        ├── docstring_formatter.py
        ├── docstringparser.py
        ├── docstring.py
        ├── docstrings.py
        ├── docstyle.py
        ├── doctest (2).py
        ├── doctest (3).py
        ├── doctest.py
        ├── doctree.py
        ├── documentation_spider.py
        ├── document_color.py
        ├── document_highlight.py
        ├── document_link.py
        ├── document.py
        ├── documents.py
        ├── document_symbol.py
        ├── docutils_xml.py
        ├── docview.py
        ├── do_example.py
        ├── do_exchain.py
        ├── do_exploitable.py
        ├── dogbox.py
        ├── dojobapp.py
        ├── dok.py
        ├── domain_extractor.py
        ├── domains_test.py
        ├── dom.py
        ├── domreg.py
        ├── domwidget.py
        ├── doomsday.py
        ├── do_op_test.py
        ├── dop853_coefficients.py
        ├── do_symfix.py
        ├── dot_atlas.py
        ├── dotgraph.py
        ├── dotnet.py
        ├── double_const.py
        ├── double_ended_queue.py
        ├── double_hash.py
        ├── double_linear_search.py
        ├── double_linear_search_recursion.py
        ├── double_sort.py
        ├── Doubling_stack (2).py
        ├── Doubling_stack (3).py
        ├── Doubling_stack (4).py
        ├── Doubling_stack.py
        ├── doubly_linked_list (10).py
        ├── doubly_linked_list (11).py
        ├── doubly_linked_list (12).py
        ├── doubly_linked_list (13).py
        ├── doubly_linked_list (14).py
        ├── doubly_linked_list (2).py
        ├── doubly_linked_list (3).py
        ├── doubly_linked_list (4).py
        ├── doubly_linked_list (5).py
        ├── doubly_linked_list (6).py
        ├── doubly_linked_list (7).py
        ├── doubly_linked_list (8).py
        ├── doubly_linked_list (9).py
        ├── doubly_linked_list.py
        ├── doubly_linked_list_two.py
        ├── download (2).py
        ├── download (3).py
        ├── download (4).py
        ├── download_mnist.py
        ├── download.py
        ├── downloadXkcd.py
        ├── dp.py
        ├── d.py
        ├── draw (2).py
        ├── draw.py
        ├── driver.py
        ├── Droite Discrète - public 2012 D5.py
        ├── dropout (2).py
        ├── dropout (3).py
        ├── dropout_op_test (2).py
        ├── dropout_op_test.py
        ├── dropout.py
        ├── ds389.py
        ├── dsa.py
        ├── dsbase.py
        ├── dsl.py
        ├── dsl.sh.py
        ├── dsls.py
        ├── ds_record.py
        ├── dsskey.py
        ├── ds_test.py
        ├── dtexample.py
        ├── dtype.py
        ├── dtypes.py
        ├── dual.py
        ├── dumb.py
        ├── dummy.py
        ├── dummy_threading.py
        ├── dump_all_function_schemas.py
        ├── dump_clipboard.py
        ├── dump_dD.py
        ├── dump_dependency_json (2).py
        ├── dump_dependency_json (3).py
        ├── dump_dependency_json (4).py
        ├── dump_dependency_json (5).py
        ├── dump_dependency_json.py
        ├── dumper.py
        ├── dump_link.py
        ├── dump.py
        ├── dunder_lookup.py
        ├── duplicate_operands_test.py
        ├── duplicate_removal (2).py
        ├── duplicate_removal (3).py
        ├── duplicate_removal (4).py
        ├── duplicate_removal.py
        ├── Duration.py
        ├── dutch_stemmer.py
        ├── dutree.py
        ├── dviread.py
        ├── dylan.py
        ├── dyld.py
        ├── dylib.py
        ├── dynamic_arrays.py
        ├── dynamic_params.py
        ├── dynamic.py
        ├── dyndep.py
        ├── dyndlg.py
        ├── dynoption.py
        ├── e164.py
        ├── e56913baab20_ (2).py
        ├── e56913baab20_.py
        ├── easter.py
        ├── easy_install (2).py
        ├── easy_install (3).py
        ├── easy_install (4).py
        ├── easy_install (5).py
        ├── easy_install.py
        ├── easy_xml (2).py
        ├── easy_xml (3).py
        ├── easy_xml (4).py
        ├── easy_xml (5).py
        ├── easy_xml.py
        ├── easy_xml_test (2).py
        ├── easy_xml_test (3).py
        ├── easy_xml_test (4).py
        ├── easy_xml_test (5).py
        ├── easy_xml_test.py
        ├── eating_cookies.py
        ├── ec1cac3399c9_.py
        ├── ec2_list_instances (2).py
        ├── ec2_list_instances (3).py
        ├── ec2_list_instances (4).py
        ├── ec2_list_instances.py
        ├── ecdsakey.py
        ├── echo1.py
        ├── echo2.py
        ├── echo3.py
        ├── eclipse (2).py
        ├── eclipse (3).py
        ├── eclipse (4).py
        ├── eclipse (5).py
        ├── eclipse.py
        ├── ecl.py
        ├── ec.py
        ├── Ecriture inclusive (2).py
        ├── Ecriture inclusive.py
        ├── ed25519key.py
        ├── ed25519.py
        ├── ed448.py
        ├── edir888.py
        ├── edir914.py
        ├── editable_legacy.py
        ├── edit_distance (2).py
        ├── edit_distance.py
        ├── editor (2).py
        ├── editor (3).py
        ├── editorhooks.py
        ├── editor.py
        ├── EditSecurity.py
        ├── EditServiceSecurity.py
        ├── edmonds_karp_multiple_source_and_sink.py
        ├── edns.py
        ├── eeb276e51423_auth_state.py
        ├── Efficient_sampling_from_a_Binomial_distribution.py
        ├── egg_drop.py
        ├── egg_info.py
        ├── eiffel.py
        ├── eight_puzzle.py
        ├── einsumfunc (2).py
        ├── einsumfunc.py
        ├── eintr_tester.py
        ├── elastic_distributed_sampler.py
        ├── electric_car.py
        ├── electric_car_with_battery.py
        ├── electric_power.py
        ├── ElementInclude.py
        ├── ElementPath.py
        ├── elements (2).py
        ├── elements (3).py
        ├── elements.py
        ├── ElementTree (2).py
        ├── ElementTree (3).py
        ├── ElementTree.py
        ├── elementwise_add_dnnlowp_op_test.py
        ├── elementwise_linear_dnnlowp_op_test.py
        ├── elementwise_linear_op_test.py
        ├── elementwise_linear.py
        ├── elementwise_logical_ops_test.py
        ├── elementwise_mul_dnnlowp_op_test.py
        ├── elementwise_op_broadcast_test.py
        ├── elementwise_ops_test.py
        ├── elementwise.py
        ├── elementwise_sum_dnnlowp_op_test.py
        ├── elementwise_sum_op_test.py
        ├── elgamal_key_generator.py
        ├── elias.py
        ├── elm.py
        ├── emacs.py
        ├── Email-Automation.py
        ├── email.py
        ├── emails_from_url.py
        ├── emails.py
        ├── embed (2).py
        ├── embed (3).py
        ├── embeddingbag_test.py
        ├── embedding_generation_benchmark.py
        ├── embedding_ops.py
        ├── embed.py
        ├── emb_seq.py
        ├── emitter (2).py
        ├── emitter.py
        ├── emoji1_db.py
        ├── emoji.py
        ├── emojize_pngorsvg.py
        ├── emojize.py
        ├── employee.py
        ├── emptyclass.py
        ├── emptysample_ops_test.py
        ├── emptystring.py
        ├── empty_volume_cache.py
        ├── emsabtags (2).py
        ├── emsabtags.py
        ├── enable_hist_gradient_boosting.py
        ├── encode_asn1.py
        ├── encode_decode.py
        ├── encoder.py
        ├── encoders.py
        ├── encoding.py
        ├── EncryptionTool.py
        ├── encryptsys.py
        ├── endpoint.py
        ├── end_to_end.py
        ├── endTransaction.py
        ├── enforce_finite_op_test.py
        ├── engines.py
        ├── english_stemmer.py
        ├── enigma_machine2.py
        ├── enigma_machine.py
        ├── en.py
        ├── ensure_clipped_test.py
        ├── ensure-consistency.py
        ├── ensure_cpu_output_op_test.py
        ├── entities (2).py
        ├── entities (3).py
        ├── entities.py
        ├── entropy.py
        ├── entrypoints.py
        ├── entry.py
        ├── enum-2-string (2).py
        ├── enum-2-string (3).py
        ├── enum-2-string (4).py
        ├── enum-2-string (5).py
        ├── enum-2-string.py
        ├── enumerated.py
        ├── enum.py
        ├── enums.py
        ├── env (2).py
        ├── env (3).py
        ├── env (4).py
        ├── env (5).py
        ├── envbuild.py
        ├── env_check.py
        ├── environment (2).py
        ├── environment (3).py
        ├── environment (4).py
        ├── environment (5).py
        ├── environment (6).py
        ├── environment.py
        ├── env.py
        ├── env_settings.py
        ├── eo.py
        ├── EpochConverter.py
        ├── Epoch.py
        ├── EpsImagePlugin.py
        ├── eptags.py
        ├── epylint.py
        ├── epytext.py
        ├── equality_constrained_sqp.py
        ├── equipment.py
        ├── erf_op_test.py
        ├── erlang.py
        ├── error (2).py
        ├── errorcodes.py
        ├── error_handler.py
        ├── error_handler_test.py
        ├── error.py
        ├── error_reporting.py
        ├── errors (2).py
        ├── errors (3).py
        ├── errors (4).py
        ├── errors (5).py
        ├── errors (6).py
        ├── errorSemantics.py
        ├── errors.py
        ├── error_wrappers.py
        ├── es_bulk_ingest.py
        ├── escapeall.py
        ├── escape.py
        ├── escape_test.py
        ├── escprober.py
        ├── escsm.py
        ├── es_index_document.py
        ├── esoteric.py
        ├── es.py
        ├── es_show_info_no_auth.py
        ├── estimator_checks.py
        ├── etag.py
        ├── etcd_rendezvous.py
        ├── etcd_rendezvous_test.py
        ├── etcd_server.py
        ├── etcd_server_test.py
        ├── etree_lxml.py
        ├── etree.py
        ├── eucjpprober.py
        ├── euckrfreq.py
        ├── euckrprober.py
        ├── euclidean_distance.py
        ├── euclidean_gcd.py
        ├── euctwfreq.py
        ├── euctwprober.py
        ├── euibase.py
        ├── eulerian_path_and_circuit_for_undirected_graph.py
        ├── eulers_python.py
        ├── eulers_totient.py
        ├── euler_totient.py
        ├── eval.py
        ├── evaluate_postfix_notations.py
        ├── evaluator.py
        ├── even_more_titanic.py
        ├── even-occuring-element (2).py
        ├── even-occuring-element (3).py
        ├── even-occuring-element (4).py
        ├── even-occuring-element.py
        ├── EvenOddArray.py
        ├── even.py
        ├── event (2).py
        ├── event (3).py
        ├── eventLogDemo.py
        ├── eventloops.py
        ├── event.py
        ├── even_tree.py
        ├── events (2).py
        ├── events (3).py
        ├── events (4).py
        ├── events (5).py
        ├── events (6).py
        ├── eventsApartmentThreaded.py
        ├── eventsFreeThreaded.py
        ├── events.py
        ├── EvtFormatMessage.py
        ├── ewm.py
        ├── ex1 (2).py
        ├── ex1 (3).py
        ├── ex1.py
        ├── ex1_tests (2).py
        ├── ex1_tests (3).py
        ├── ex1_tests.py
        ├── ex20.py
        ├── ex2 (2).py
        ├── ex2 (3).py
        ├── ex2.py
        ├── ex2_tests (2).py
        ├── ex2_tests (3).py
        ├── ex2_tests.py
        ├── ex3 (2).py
        ├── ex3 (3).py
        ├── ex3.py
        ├── ex3_tests (2).py
        ├── ex3_tests (3).py
        ├── ex3_tests.py
        ├── ex4 (2).py
        ├── ex4 (3).py
        ├── ex4.py
        ├── ex4_tests (2).py
        ├── ex4_tests (3).py
        ├── ex4_tests.py
        ├── ex5 (2).py
        ├── ex5 (3).py
        ├── ex5.py
        ├── ex5_tests (2).py
        ├── ex5_tests (3).py
        ├── ex5_tests.py
        ├── examples.py
        ├── excelAddin.py
        ├── excel.py
        ├── excelRTDServer.py
        ├── exception.py
        ├── exceptions.py
        ├── exclusions.py
        ├── excolors.py
        ├── exc.py
        ├── exec_command.py
        ├── execute.py
        ├── execution (2).py
        ├── execution.py
        ├── executor.py
        ├── executor_test.py
        ├── executor_test_util.py
        ├── ExifTags.py
        ├── exists_in (2).py
        ├── exists_in (3).py
        ├── exists_in (4).py
        ├── exists_in.py
        ├── expanddims_squeeze_op_test.py
        ├── expanding.py
        ├── expand_op_test.py
        ├── expatbuilder.py
        ├── expatreader.py
        ├── expecttest.py
        ├── expensive_seq.py
        ├── experiment_util.py
        ├── exp_family.py
        ├── explicit_entries.py
        ├── explicit_euler.py
        ├── explicit_resnet_forward.py
        ├── explicit_resnet_param_update.py
        ├── explorer_browser.py
        ├── expn_asy.py
        ├── Exponential_Integral_python.py
        ├── exponential.py
        ├── exporter_locator.py
        ├── exporter.py
        ├── export_onnx_tests_filter.py
        ├── export_onnx_tests_generator.py
        ├── exposition.py
        ├── expression.py
        ├── expressions.py
        ├── expr.py
        ├── extended_euclidean_algorithm.py
        ├── extended_gcd.py
        ├── extended.py
        ├── extending_distributions.py
        ├── extending.py
        ├── extension_loader.py
        ├── extension.py
        ├── extension_simple.py
        ├── extensions.py
        ├── external_sort.py
        ├── extmath.py
        ├── ext.py
        ├── extract (2).py
        ├── extract (3).py
        ├── extract (4).py
        ├── extract (5).py
        ├── extract (6).py
        ├── extractoutput.py
        ├── extract.py
        ├── Extract_Text_from_image.py
        ├── extra.py
        ├── extrarawhtml.py
        ├── extras (2).py
        ├── extras (3).py
        ├── extras (4).py
        ├── extras (5).py
        ├── extras (6).py
        ├── extras.py
        ├── ezhil.py
        ├── f195f9a4923d_.py
        ├── f1b9ee79cb54_create_packages_table.py
        ├── f2py2e.py
        ├── f2py_testing.py
        ├── f90mod_rules.py
        ├── facade.py
        ├── facebook-autologin-bot.py
        ├── facebook id hack.py
        ├── face_recofnation_first.py
        ├── factor_combinations.py
        ├── factorial (2).py
        ├── factorial (3).py
        ├── factorial (4).py
        ├── factorial (5).py
        ├── factorial (6).py
        ├── factorial (7).py
        ├── factorial (8).py
        ├── factorial (9).py
        ├── factorial_iterative.py
        ├── factorial_perm_comp.py
        ├── factorial.py
        ├── factorial_python.py
        ├── factorial_recursive.py
        ├── factor.py
        ├── factors (2).py
        ├── factors.py
        ├── factory.py
        ├── fact.py
        ├── fairCoin.py
        ├── fakefp16_transform_lib.py
        ├── fakefp16_transform_test.py
        ├── fake_quantize.py
        ├── fallback.py
        ├── fancy_getopt.py
        ├── fantom.py
        ├── fa.py
        ├── fastapi.py
        ├── fast_fibonacci.py
        ├── fast_nvcc.py
        ├── faulty_agent_backend_registry.py
        ├── faulty_rpc_agent_test_fixture.py
        ├── fc (2).py
        ├── fc_operator_test.py
        ├── fc_op_test.py
        ├── fc.py
        ├── fc_with_bootstrap.py
        ├── fc_without_bias.py
        ├── fdb.py
        ├── fdc58d9bd449_.py
        ├── feather_format.py
        ├── featureexplore.py
        ├── feature_manager.py
        ├── feature_maps_ops_test.py
        ├── feature_sparse_to_dense.py
        ├── features.py
        ├── feedparser.py
        ├── felix.py
        ├── fenced_code.py
        ├── fenwick_tree.py
        ├── fermat_little_theorem.py
        ├── fernet.py
        ├── fetch_bbc_news.py
        ├── fetchers.py
        ├── fetch_github_info.py
        ├── fetch_jobs.py
        ├── fetch_news.py
        ├── fetch.py
        ├── fetch_url.py
        ├── Février 2021 un mini challenge arithmético-algorithmique.py
        ├── ffiplatform.py
        ├── fftpack.py
        ├── fib (2).py
        ├── fib (3).py
        ├── fib (4).py
        ├── fib (5).py
        ├── fibonacci (10).py
        ├── fibonacci (2).py
        ├── fibonacci (3).py
        ├── fibonacci (4).py
        ├── fibonacci (5).py
        ├── fibonacci (6).py
        ├── fibonacci (7).py
        ├── fibonacci (8).py
        ├── fibonacci (9).py
        ├── fibonacci.py
        ├── fibonacci_search.py
        ├── fibonacci_sequence_recursion.py
        ├── fib.py
        ├── fib_series (2).py
        ├── fib_series (3).py
        ├── fib_series (4).py
        ├── fib_series.py
        ├── fields.py
        ├── figure determiner.py
        ├── figureoptions.py
        ├── figure.py
        ├── file_baton.py
        ├── file_cache.py
        ├── filecheckpoints.py
        ├── filecmp.py
        ├── filedialog.py
        ├── fileinfo.py
        ├── fileinput.py
        ├── fileio (2).py
        ├── file_io.py
        ├── fileio.py
        ├── filelist.py
        ├── filelock.py
        ├── filemanager.py
        ├── filemap (2).py
        ├── filemap.py
        ├── file_name.py
        ├── filepost.py
        ├── file.py
        ├── FileSecurityTest.py
        ├── files.py
        ├── file_state.py
        ├── file_store_handler_op_test.py
        ├── filesystem.py
        ├── file_to_string (2).py
        ├── file_to_string.py
        ├── file_util.py
        ├── filewrapper.py
        ├── filler_ops_test.py
        ├── filler_test.py
        ├── fill_test.py
        ├── filterDemo.py
        ├── filter_design.py
        ├── filter_dialog.py
        ├── filter_links.py
        ├── filter.py
        ├── filters (2).py
        ├── filters (3).py
        ├── filters (4).py
        ├── filters (5).py
        ├── filters.py
        ├── filter_stack.py
        ├── financial.py
        ├── find_all_cliques.py
        ├── find_cube_root (2).py
        ├── find_cube_root.py
        ├── find_default_config_files.py
        ├── find_difference.py
        ├── finddiv.py
        ├── finder.py
        ├── finders.py
        ├── find_file_dependencies.py
        ├── find-in-files.py
        ├── findinfiles.py
        ├── finding_bridges.py
        ├── find_keyboard_row.py
        ├── findLcm.py
        ├── find_max.py
        ├── find_max_recursion.py
        ├── find_min.py
        ├── find_min_recursion.py
        ├── find_min_rotate.py
        ├── find-missing-element (2).py
        ├── find-missing-element (3).py
        ├── find-missing-element (4).py
        ├── find-missing-element.py
        ├── find_missing_number.py
        ├── findnocoding.py
        ├── findNthElement.py
        ├── findNumber.py
        ├── findNumInList.py
        ├── find_op_test.py
        ├── find_order_simple.py
        ├── find_path.py
        ├── findPercentage.py
        ├── find_prime.py
        ├── find_primitive_root_simple.py
        ├── find.py
        ├── find_recursionlimit.py
        ├── find_roots.py
        ├── find_rotation_point.py
        ├── find_square_root_of_imperfect_square.py
        ├── find_square_root.py
        ├── Find the Duplicate Number (2).py
        ├── Find the Duplicate Number (3).py
        ├── Find the Duplicate Number (4).py
        ├── Find the Duplicate Number.py
        ├── findTheNumber.py
        ├── find_Triplets_with_zero_sum.py
        ├── find-uname.py
        ├── find_words.py
        ├── finnish_stemmer.py
        ├── fi.py
        ├── fir_filter_design.py
        ├── first_come_first_served.py
        ├── first_cyclic_node.py
        ├── first-non-repeated-character (2).py
        ├── first-non-repeated-character (3).py
        ├── first-non-repeated-character (4).py
        ├── first-non-repeated-character.py
        ├── first_occurance (2).py
        ├── first_occurance (3).py
        ├── first_occurance (4).py
        ├── first_occurance.py
        ├── first_occurrence.py
        ├── first.py
        ├── first_unique_char.py
        ├── fischer_yates_shuffle.py
        ├── fishersnedecor.py
        ├── fitpack2.py
        ├── fitpack.py
        ├── FitsStubImagePlugin.py
        ├── fix_apply.py
        ├── fix_callable.py
        ├── fixcid.py
        ├── fix_dict.py
        ├── fixdiv.py
        ├── fixer_base.py
        ├── fixer_util.py
        ├── fixes.py
        ├── fix_except.py
        ├── fix_execfile.py
        ├── fix_exitfunc.py
        ├── fix_filter.py
        ├── fix_has_key.py
        ├── fixheader.py
        ├── fix_idioms.py
        ├── fix_import.py
        ├── fix_imports.py
        ├── fix_intern.py
        ├── fix-iocaml-notebook-exports-to-pdf.py
        ├── fix_isinstance.py
        ├── fix_itertools_imports.py
        ├── fix_itertools.py
        ├── fix_map.py
        ├── fix_metaclass.py
        ├── fix_next.py
        ├── fixnotice.py
        ├── fix_operator.py
        ├── fix_paren.py
        ├── fix_print.py
        ├── fix.py
        ├── fix_raise.py
        ├── fix_renames.py
        ├── fix_set_literal.py
        ├── fix_throw.py
        ├── fix_tuple_params.py
        ├── fixture_functions.py
        ├── fixtures.py
        ├── fix_types.py
        ├── fix_unicode.py
        ├── fix_urllib.py
        ├── fix_ws_comma.py
        ├── fix_xrange.py
        ├── fix_zip.py
        ├── fizzbuzz (2).py
        ├── FizzBuzz (3).py
        ├── fizzbuzz.py
        ├── flags.py
        ├── flake8_hook.py
        ├── flappy_bird.py
        ├── Flappy Bird.py
        ├── flash.py
        ├── flask-api.py
        ├── flask_compress.py
        ├── flask_mail.py
        ├── flask_principal.py
        ├── flatten (2).py
        ├── flatten-array (2).py
        ├── flatten-array (3).py
        ├── flatten-array (4).py
        ├── flatten-array.py
        ├── flatten_op_test.py
        ├── flatten.py
        ├── flexible_top_k_test.py
        ├── FliImagePlugin.py
        ├── flinalg.py
        ├── flip_bit_longest_sequence.py
        ├── flipBits.py
        ├── flip_invert_image.py
        ├── floating_axes.py
        ├── Floating_point_error_propagation_in_polynomial_multiplication_with_Fast-Fourier_Transform.py
        ├── flock.py
        ├── flock_tool (2).py
        ├── flock_tool (3).py
        ├── flock_tool (4).py
        ├── flock_tool (5).py
        ├── flock_tool.py
        ├── floodfill.py
        ├── floor_op_test.py
        ├── floor.py
        ├── floscript.py
        ├── flow_analysis.py
        ├── floyd_warshall.py
        ├── flyweight.py
        ├── flyweight_with_metaclass.py
        ├── fmt.py
        ├── fnmatch.py
        ├── folder_size.py
        ├── folder_view.py
        ├── folding_range.py
        ├── fold.py
        ├── fontconfig_pattern.py
        ├── fontdemo.py
        ├── FontFile.py
        ├── font_manager.py
        ├── font.py
        ├── foo.py
        ├── footnotes.py
        ├── force.py
        ├── force_pydevd.py
        ├── ford_fulkerson.py
        ├── forest.py
        ├── fork_exec.py
        ├── forkserver.py
        ├── fork_wait.py
        ├── for_loop_mountain.py
        ├── format_control.py
        ├── format_helpers.py
        ├── FormatParagraph.py
        ├── format.py
        ├── formatter.py
        ├── formatters.py
        ├── formattest.py
        ├── formatting.py
        ├── formFiller.py
        ├── formparser.py
        ├── form.py
        ├── forms.py
        ├── forth.py
        ├── fortran.py
        ├── forward_ad.py
        ├── forward.py
        ├── found_candidates.py
        ├── four_digit_num_combination.py
        ├── fourier.py
        ├── foxpro.py
        ├── FpxImagePlugin.py
        ├── fractalcurves.py
        ├── fractional_knapsack_2.py
        ├── fractional_knapsack.py
        ├── fractions.py
        ├── fraisMission.py
        ├── frame.py
        ├── framework_overhead_benchmark.py
        ├── framework.py
        ├── framework_utils.py
        ├── freefem.py
        ├── FreeSMS.py
        ├── freeze.py
        ├── french_stemmer.py
        ├── frequencies.py
        ├── frequency_finder.py
        ├── frequent_pattern_graph_miner.py
        ├── friday.py
        ├── friendly.py
        ├── fromnumeric (2).py
        ├── fromnumeric (3).py
        ├── fromnumeric.py
        ├── from_sequence.py
        ├── from_template.py
        ├── front_controller.py
        ├── frontend (2).py
        ├── frontend (3).py
        ├── frontend.py
        ├── frontend_widget.py
        ├── frontmatter.py
        ├── frozen.py
        ├── fr.py
        ├── fruity.py
        ├── fshp.py
        ├── fsqla.py
        ├── fsqla_v2.py
        ├── FtexImagePlugin.py
        ├── ftplib.py
        ├── ftp_send_receive.py
        ├── fujitsu.py
        ├── fully_connected_dnnlowp_acc16_op_test.py
        ├── fully_connected_dnnlowp_op_test.py
        ├── fully_connected_fp16_test.py
        ├── fully_connected_rowwise_dnnlowp_op_test.py
        ├── func (2).py
        ├── func2subr.py
        ├── func_inspect.py
        ├── func.py
        ├── functional (2).py
        ├── functional (3).py
        ├── functional (4).py
        ├── functional (5).py
        ├── functional (6).py
        ├── functional (7).py
        ├── functional (8).py
        ├── functional (9).py
        ├── functional_adadelta.py
        ├── functional_adagrad.py
        ├── functional_adam.py
        ├── functional_adamw.py
        ├── functional_autograd_benchmark.py
        ├── functional_impl_check.py
        ├── functional_modules.py
        ├── functional.py
        ├── functional-reduce.py
        ├── functional_rmsprop.py
        ├── functional_sgd.py
        ├── functional_test.py
        ├── function_base (2).py
        ├── function_base (3).py
        ├── function_base.py
        ├── function.py
        ├── functions.py
        ├── function_wrapper.py
        ├── functools_lru_cache.py
        ├── functools.py
        ├── funhash_op_test.py
        ├── fused_8bit_rowwise_conversion_ops_test.py
        ├── fused_nbit_rowwise_conversion_ops_test.py
        ├── fused_nbit_rowwise_test_helper.py
        ├── fused.py
        ├── fused_rowwise_nbit_conversion_bench.py
        ├── fuse_modules.py
        ├── fuse.py
        ├── fuser (2).py
        ├── fuser_method_mappings.py
        ├── fuser.py
        ├── fusion_patterns.py
        ├── fusion.py
        ├── future_div.py
        ├── future.py
        ├── futures.py
        ├── fuzzer (2).py
        ├── fuzzer (3).py
        ├── fuzzer.py
        ├── fuzzy_completer.py
        ├── fuzzy_operations.py
        ├── g95.py
        ├── gaerdbms.py
        ├── gale_shapley_bigraph.py
        ├── gambler.py
        ├── game_functions.py
        ├── game_of_life.py
        ├── game_o_life.py
        ├── game_routes.py
        ├── gamma (2).py
        ├── gamma (3).py
        ├── gammainc_asy.py
        ├── gammainc_data.py
        ├── gamma.py
        ├── garage.py
        ├── garbage.py
        ├── gas_station_routes.py
        ├── gas_stations.py
        ├── gateways.py
        ├── gather_dnnlowp_op_test.py
        ├── gather_ops_test.py
        ├── gather_ranges_op_test.py
        ├── gather_record.py
        ├── gather_test.py
        ├── gauss_easter.py
        ├── gaussian_elimination.py
        ├── gaussian_filter.py
        ├── gaussian_naive_bayes.py
        ├── gaussian.py
        ├── gb2312freq.py
        ├── gb2312prober.py
        ├── gbq.py
        ├── GbrImagePlugin.py
        ├── gcc_coverage.py
        ├── gc_collector.py
        ├── gcd (2).py
        ├── gcd.py
        ├── gc_inspection.py
        ├── gcloud-tail.py
        ├── gcov_coverage_parser.py
        ├── gc.py
        ├── gdb_pretty_printer_test.py
        ├── gdi32.py
        ├── GdImageFile.py
        ├── gdscript.py
        ├── gemoji_db.py
        ├── gen (2).py
        ├── gen_all_tests_js.py
        ├── gen_annotated_fn_args.py
        ├── genapi.py
        ├── gen_autograd_functions.py
        ├── gen_autograd.py
        ├── gencache.py
        ├── gen_convert.py
        ├── general_messages.py
        ├── general_name.py
        ├── generate_abbreviations.py
        ├── generate_binary_build_matrix.py
        ├── generate_code.py
        ├── generate_config_yml.py
        ├── generate-csv-dataset-transactions.py
        ├── generate-customer-transaction-fake-data.py
        ├── GenerateDescriptions.py
        ├── generate_email.py
        ├── generate_examples.py
        ├── generate_feature_test_macro_components.py
        ├── generate_keywords.py
        ├── generate_legacy_storage_files.py
        ├── generate_lib.py
        ├── generate_lib_test.py
        ├── generate-metrics.py
        ├── generate_mtrand_c.py
        ├── generate_numpy_api.py
        ├── generate_opcode_h.py
        ├── generate_parenthesis.py
        ├── generate_pytorch_version.py
        ├── generate-random-data-in-gz-format.py
        ├── generate-random-data-into-dynamodb.py
        ├── generate-random-data-into-elasticsearch.py
        ├── generate-random-data-into-mongodb.py
        ├── generate-random-people-data-json.py
        ├── generate-random-people-data-with-faker.py
        ├── generate_sparsetools.py
        ├── generate_strobogrammtic.py
        ├── generate_symbol_py.py
        ├── generate_test_torchscripts.py
        ├── generate_token.py
        ├── generate_torch_version.py
        ├── generate_ufunc_api.py
        ├── generate_umath.py
        ├── generate-word-cloud.py
        ├── generate-wrapper.py
        ├── Generating_permutations_with_python.py
        ├── generator.py
        ├── Generer_des_fausses_citations_latines_du_Roi_Loth.py
        ├── generic (2).py
        ├── generic_heap (2).py
        ├── generic_heap (3).py
        ├── generic_heap.py
        ├── genericpath.py
        ├── generic.py
        ├── generics.py
        ├── gen_fftw_ref.py
        ├── gen_inplace_or_view_type.py
        ├── gen_op.py
        ├── gen_op_registration_allowlist.py
        ├── gen-pre-packaged-apps.py
        ├── gen_proto_typestubs_helper.py
        ├── gen.py
        ├── genpy (2).py
        ├── gen_pyi.py
        ├── genpy.py
        ├── gen_python_functions.py
        ├── gen_selected_mobile_ops_header.py
        ├── genshi.py
        ├── gen_test_html.py
        ├── gen_test.py
        ├── GenTestScripts.py
        ├── gen_trace_type.py
        ├── gen_variable_factories.py
        ├── gen_variable_type.py
        ├── gen_version_header.py
        ├── gen_vulkan_glsl.py
        ├── gen_vulkan_spv.py
        ├── geoMean.py
        ├── geometric_mean.py
        ├── geometric_progression_builder.py
        ├── geometric.py
        ├── geometric_series.py
        ├── geo.py
        ├── german_stemmer.py
        ├── gesture_control.py
        ├── getargspec.py
        ├── getattr_static.py
        ├── getBindDn.py
        ├── get-gists (10).py
        ├── get-gists (11).py
        ├── get-gists (2).py
        ├── get-gists (3).py
        ├── get-gists (4).py
        ├── get-gists (5).py
        ├── get-gists (6).py
        ├── get-gists (7).py
        ├── get-gists (8).py
        ├── get-gists (9).py
        ├── get-gists.py
        ├── getitem.py
        ├── get-json-data-from-google-scholar.py
        ├── getJupyterVariableDataFrameInfo.py
        ├── getJupyterVariableDataFrameRows.py
        ├── getJupyterVariableShape.py
        ├── getlimits.py
        ├── getopt.py
        ├── getpass.py
        ├── get-pip (2).py
        ├── get-pip.py
        ├── get-remote-certificate.py
        └── GetSaveFileName.py

108 directories, 15003 files
