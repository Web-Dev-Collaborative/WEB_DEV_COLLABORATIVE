...
const program = new commander.Command(packageJson.name)
  ...
  .on('--help', () => {
    ...
    console.log(`      ${chalk.cyan('https://github.com/matmalkowski/create-react-resume/issues/new')}`)
    console.log()
  })
  .parse(process.argv)
...
if (typeof projectName === 'undefined') {
  ...
  console.log(`  ${chalk.cyan(program.name())} ${chalk.green('my-react-resume')}`)
  console.log()
...
const hiddenProgram = new commander.Command()
  .option(
    ...
    '(internal usage only, DO NOT RELY ON THIS) ' + 'use a non-standard application tarball'
  )
  .parse(process.argv)
...
function createResume(name, verbose, useNpm, useTypescript, tarball = tarballURL) {
  ...
  console.log(`Creating a new React resume in ${chalk.green(root)}.`)
  console.log()
  ...
  run(root, appName, verbose, tarball, useTypescript, useYarn)
}
...
async function run(root, appName, verbose, tarball, useTypescript, useYarn) {
  try {
    ...
    await install(root, useYarn, verbose, isOnline)
